...
class WindowsCheckWrapperTests(unittest.TestCase):
    ...
    def test_current_stack_mode_uses_isolated_global_ir_profile(self) -> None:
        ...
        self.assertIn('"-m" "tools.deployment.test_current_stack_deployment_verification"', self.check_content)
        self.assertIn(
    ...
    def test_forge_is_routed_through_checked_wrapper(self) -> None:
        ...
        self.assertIn(
            "forge script script/legacy/RehearseDeploymentSuite.s.sol:RehearseDeploymentSuite",
            self.check_content,
        )
        self.assertIn(
            "forge script script/legacy/RehearseDeployment.s.sol:RehearseDeployment", self.check_content
        )
        self.assertIn(
            "forge script script/legacy/RehearseAuctionCeremony.s.sol:RehearseAuctionCeremony",
            self.check_content,
        )
        self.assertIn(
            "forge script script/legacy/RehearseEmergencyRedeployment.s.sol:RehearseEmergencyRedeployment",
            self.check_content,
        )
    ...
    def test_selected_python_is_routed_through_checked_wrapper(self) -> None:
        ...
        self.assertIn('& $pythonPath @pythonArgs "-m" "tools.deployment.check_metadata_browser_sandbox"', self.check_content)
        self.assertIn('& $pythonPath @pythonArgs "-m" "tools.build.test_contract_size_budget"', self.check_content)
        self.assertIn('& $pythonPath @pythonArgs "-m" "tools.build.check_contract_size_budget"', self.check_content)
        self.assertIn(
        ...
        self.assertIn('& $pythonPath @pythonArgs "-m" "tools.deployment.test_deployment_rehearsal_gate"', self.check_content)
        self.assertIn('& $pythonPath @pythonArgs "-m" "tools.deployment.check_deployment_rehearsal_gate"', self.check_content)
