...
REQUIRED_HEADINGS = {
    DEFAULT_ARCHITECTURE: [(2, "System components"), (2, "Product Extension And Size-Budget Policy")],
    DEFAULT_THREAT_MODEL: [(2, "Threat Categories"), (2, "Actors And Trust Boundaries")],
}

REQUIRED_MATURITY_PHRASES = ['pre-audit', 'not production-ready']

REQUIRED_ARCHITECTURE_PHRASES = ['StreamCore', 'StreamMintManager', 'StreamFixedPriceSaleAdapter', 'StreamEnglishAuctionHouse', 'StreamGovernanceExecutor', 'StreamModuleRegistry', 'coordinatorAtMint', 'genesis']

REQUIRED_SIZE_POLICY_PHRASES = ['read adapters', 'linked', 'release-artifacts/contracts.json', 'ERC-165']

REQUIRED_THREAT_PHRASES = ['authorization and replay', 'reentrancy', 'scope', 'governance', 'external audit']

REQUIRED_COMMANDS = []

REQUIRED_LINK_TARGETS = ['SECURITY.md', 'docs/architecture.md', 'docs/threat-model.md', 'docs/status.md', 'docs/known-blockers.md', 'docs/release-readiness.md', 'release-artifacts/contracts.json']
