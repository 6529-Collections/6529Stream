#!/usr/bin/env python3
"""Negative and current-source checks for wallet signature flows documentation."""
...
from tools.docs import check_wallet_signature_flows as checker
...
class CurrentDocumentationTests(unittest.TestCase):
    def validate_changed(self, transform):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            checker.validate_wallet_signature_flows(root, path)

    def test_committed_document(self):
        checker.validate_wallet_signature_flows(ROOT, ROOT / checker.DEFAULT_DOC)
    ...
    def test_rejects_wrong_typed_data_integer_width(self):
        with self.assertRaisesRegex(ValueError, "fields differ"):
            self.validate_changed(lambda text: text.replace('{ name: "signerEpoch", type: "uint64" }', '{ name: "signerEpoch", type: "uint256" }'))
