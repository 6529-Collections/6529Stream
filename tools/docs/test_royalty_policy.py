...
""",
        ...
        elif relative == "test/regression/legacy/revenue/StreamRoyalty.t.sol":
            write_text(
                root / relative,
                """
...
""",
        ...
        elif relative == "test/unit/core/StreamCorePermanentTarget.t.sol":
            write_text(
                root / relative,
                """
...
""",
        ...
        elif relative == "test/unit/revenue/StreamRoyaltyResolver.t.sol":
            write_text(
                root / relative,
...
class RoyaltyPolicyTests(unittest.TestCase):
    ...
    def test_rejects_required_command_as_substring_only(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            text = minimal_royalty_policy().replace(
                "python -m tools.docs.check_royalty_policy",
                "python -m tools.docs.check_royalty_policy --help",
            )
            write_text(root / checker.DEFAULT_ROYALTY_POLICY, text)
    ...
    def test_rejects_test_constant_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_required_targets(root)
            test_file = root / "test" / "regression" / "legacy" / "revenue" / "StreamRoyalty.t.sol"
