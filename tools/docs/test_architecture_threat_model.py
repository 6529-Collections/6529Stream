...
def minimal_architecture_doc(runtime: int = 22390, margin: int = 2186) -> str:
    ...
    links = target_links()
    current = ", ".join(checker.REQUIRED_ARCHITECTURE_PHRASES + checker.REQUIRED_SIZE_POLICY_PHRASES)
    return f"""# Architecture

This pre-audit local baseline is not production-ready and not a security claim.
{current}. It covers StreamAdmins, StreamCore, StreamDrops, StreamAuctions,
...
def minimal_threat_model_doc() -> str:
    links = target_links()
    current = ", ".join(checker.REQUIRED_THREAT_PHRASES)
...
{current}. Authorization and replay, auction custody, pull-payment credits, randomizer lifecycle,
...
class ArchitectureThreatModelTests(unittest.TestCase):
    ...
    def test_rejects_missing_required_link(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_required_targets(root)
            text = minimal_architecture_doc().replace("- [SECURITY.md](../SECURITY.md)\n", "")
            write_text(root / checker.DEFAULT_ARCHITECTURE, text)
    ...
    def test_rejects_missing_size_policy_phrase(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_required_targets(root)
            text = minimal_architecture_doc().replace("ERC-165", "interface identity")
            write_text(root / checker.DEFAULT_ARCHITECTURE, text)
            write_text(root / checker.DEFAULT_THREAT_MODEL, minimal_threat_model_doc())
