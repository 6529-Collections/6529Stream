"""Fixed original scoped factory-policy V2 content reads; host owns source authentication.
...
class ScopedPolicyContentReads:
    def _content(self, statement):
        ...
        scope = wire.valid_scope(s[0],cid)
        head = self._one(address("router"), "scopedContentRootHead((uint8,uint256,uint256,bytes32))", "bytes32", (t.SCOPE,), (scope,))
        aggregate = self._one(address("router"), "scopedContentRootAggregate(uint256)", t.ROOT_AGGREGATE, ("uint256",), (cid,))
        require(head != ZERO, "scoped policy source empty scope root head")
        ...
        for log in logs:
            topics = log["topics"]
            require(log["address"] == address("router") and len(topics) == 4
            ...
            if topics[0] == wire.EVENTS["binding"]:
                ...
                require(version == 2 and topics[3] not in bindings, "policy source binding event identity")
                bindings[topics[3]] = (b, log)
                continue
            require(topics[0] == wire.EVENTS["root"], "scoped policy source root topic")
            version,r,historical_aggregate = decode(("uint16",t.ROOT_RECORD,t.ROOT_AGGREGATE),hex_bytes(log["data"]),maximum=8192)
            require(topics[2] == wire.scope_subject(chain,address("core"),r[0][0]), "scoped policy source root subject")
            digest = topics[3]
            require(version in (1, 2) and digest != ZERO and r[0][0][1] == cid, "policy source root event identity")
            stored = self._one(address("router"), "scopedContentRootRecord(bytes32)", t.ROOT_RECORD, ("bytes32",), (digest,), maximum=8192)
            binding = self._one(address("router"), "scopedPolicyContentRootBinding(bytes32)", t.ROOT_BINDING, ("bytes32",), (digest,))
            require(stored == r and ((version == 1 and binding[0] == ZERO) or (version == 2 and binding[0] == t.ROOT_PROFILE)),
                "policy source original root/event/binding differs")
            require(r[17] == uint(history["blockTimestamps"][str(quantity(log["blockNumber"]))]), "policy source root timestamp")
            roots.append({"recordHash": digest, "record": json_values(r), "binding": json_values(binding), "aggregate":json_values(historical_aggregate)})
        ...
        require(set(bindings) == {r["recordHash"] for r in roots if r["binding"][0] == t.ROOT_PROFILE}, "policy source binding event denominator")
        root_logs = {l["topics"][3]: l for l in logs if l["topics"][0] == wire.EVENTS["root"]}
        for row in roots:
            if row["binding"][0] != t.ROOT_PROFILE: continue
            binding, event = bindings[row["recordHash"]]; previous = root_logs[row["recordHash"]]
            require(json_values(binding) == row["binding"] and event["topics"][2] == previous["topics"][2] and event["transactionHash"] == previous["transactionHash"]
        ...
        require(binding[0] == t.ROOT_PROFILE and binding[1:5] == (address("outputManifest"), g["outputManifest"]["runtimeHash"],
            address("policyContent"), g["policyContent"]["runtimeHash"]), "policy source selected original V2 hosts")
        publication,receipt = self._read(address("policySnapshot"),"snapshotRecord(bytes32)",
            (preservation.SNAPSHOT_PUBLICATION,preservation.SNAPSHOT_RECEIPT),("bytes32",),(root[0][2],),maximum=8192)
        require(root[0][2] == s[7][1] and publication[0] == scope and receipt[0] == root[0][2]
            and receipt[3] == root[0][3] and receipt[5] == root[3] and receipt[7] == root[4],
            "scoped policy source original snapshot/root differs")
        manifest_key = publication[4]
        ...
        dependencies = self._one(address("sourceFactory"),"dependencies()",t.FACTORY_DEPENDENCIES)
        content = {"roots": {"selectedRootHash": s[7][0], "scopeHead": head, "collectionAggregate":json_values(aggregate), "history": roots},
            "checkpoint": {"id": m[0], "salt": salt, "plan": json_values(p), "outputs": json_values(outputs),
                "selectionPlan": json_values(selection), "selectionRows": json_values(rows), "factoryDependencies":json_values(dependencies)},
