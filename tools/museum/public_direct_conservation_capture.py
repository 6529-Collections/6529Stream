"""Read-only original DIRECT conservation receipts; item13 remains partial."""
import argparse
import os
...
MODE = "public_direct_conservation_capture"
PROFILE = "STREAM_MUSEUM_PUBLIC_DIRECT_CONSERVATION_CAPTURE_V1"
MISSING_JOINS = {
    ...
    "canonicalPacketContext": "A DIRECT canonical packet representation and the remaining original conservation prerequisites are not assembled.",
}
CLAIMS = {"readOnlyRpc": True, "consumerFixedFilters": True, "offlineReplayChecked": True,
    "originalSourceBytesRetained": True, "typedDirectReceiptHistoryRetained": True,
    "foreignReceiptDiscoveryRetained": True,
    ...
    "universalSettlementProjection": False, "mixedTargetReceiptFamiliesSupported": False,
    "supplementalDirectSupported": False,
    "authorizationPreimageRecovered": False, "allPaidRoutesCovered": False,
    "historicalExecutionReproved": False, "personhoodProven": False,
...
QUALIFICATION = (
    "Original typed DIRECT conservation receipt history for one collection at an externally pinned block. "
    "The full saved DIRECT bindings, original sale and floor receipt remain distinct from universal settlement "
    "candidates and results. Target universal or mixed histories fail closed; foreign receipt families remain "
    "retained in discovery. Authorization commitments do not recover unavailable signed preimages. "
    "Provider log completeness and canonical mapping remain trusted. Replay does not authenticate provenance "
    "or consensus, reexecute historical payments, prove personhood or establish coverage of every paid route. "
    "Item13 remains partial; no V4 compatibility, complete packet or actual-chain acceptance is claimed.")
PROFILE_BYTES = dumps({"name": PROFILE, "version": "1", "status": "prospective_unregistered_capture_profile",
    "source": "STREAM_MUSEUM_PUBLIC_DIRECT_CONSERVATION_SOURCE_V1", "inputPins": ["anchorHash", "sourceProfileHash"],
    "endpoint": "Explicit named process environment variable; never in retained evidence or remote error details.",
    "output": "Exact collection source triplet, frozen definitions, binding and catalogue, original first-sale/release/DIRECT receipt rows; complete offline reconstruction required.",
    "coverage": "All 19 acquisition requirements remain visible; item13 partial, all others unresolved. This capture is not a packet assembly.",
...
def _public(disclosure):
    require(disclosure == "public", "public DIRECT conservation requires public disclosure before reads")
...
def _anchor(raw, expected_hash, expected_profile):
    ...
    require(type(raw) is bytes and 0 < len(raw) <= 65536 and any(hex_bytes(expected_hash, 32))
        and keccak256(raw) == expected_hash, "public DIRECT conservation external anchor pin/bound differs")
    require(expected_profile == source.PROFILE_HASH, "public DIRECT conservation external source profile differs")
    return source.PublicDirectConservationSource
...
def _derive(snapshot):
    ...
    return {"direct-conservation/evidence.json": dumps(evidence),
        "direct-conservation/binding.json": dumps(snapshot["binding"]),
        "direct-conservation/catalogue.json": dumps(snapshot["catalogue"]),
        "direct-conservation/floor.json": dumps(snapshot["floor"])}
...
def _assemble(source, anchor_hash, source_profile_hash):
    ...
    require(source_profile_hash == module.PROFILE_HASH and keccak256(source.anchor_bytes) == anchor_hash,
        "public DIRECT conservation original capture pins differ")
    ...
    require(len(raw) <= MAX_MANIFEST, "public DIRECT conservation capture manifest bound")
...
def capture(anchor_bytes, anchor_hash, source_profile_hash, transport, *, disclosure):
    ...
    require(type(transport) is PublicRpcTransport, "public DIRECT conservation capture requires explicit read-only transport")
    ...
    require(verify(dict(result.files), result.manifest_hash).files == result.files,
        "public DIRECT conservation post-capture replay differs")
    return result
...
def replay(anchor_bytes, anchor_hash, source_profile_hash, transcript, transcript_hash, *, provenance, disclosure):
    ...
    _public(disclosure)
    constructor = _anchor(anchor_bytes, anchor_hash, source_profile_hash)
    require(provenance in ("trusted_rpc", "synthetic_fixture"), "public DIRECT conservation explicit provenance required")
...
def verify(files, expected_hash):
    ...
    require(keccak256(raw) == expected_hash, "public DIRECT conservation external manifest pin differs")
    ...
    require(dict(rebuilt.files) == files, "public DIRECT conservation reconstruction differs")
    return rebuilt
...
def _read_input(path, limit):
    ...
    require(stat.S_ISREG(state.st_mode) and not path.is_symlink()
        ...
        "public DIRECT conservation input must be a regular file")
    require(0 < state.st_size <= limit, "public DIRECT conservation input byte bound")
    with path.open("rb") as stream: raw = stream.read(limit + 1)
    require(0 < len(raw) <= limit, "public DIRECT conservation input byte bound")
    return raw
...
def main():
    ...
    if args.command in ("capture", "replay"):
        ...
        if args.command == "capture":
            require(re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]{0,127}", args.rpc_env) is not None,
                "public DIRECT conservation RPC variable name invalid")
        ...
        if args.command == "capture":
            ...
            require(endpoint is not None and endpoint != "", "public DIRECT conservation RPC variable unavailable")
            ...
            require(verify(dict(result.files), result.manifest_hash).files == result.files,
                "public DIRECT conservation post-replay reconstruction differs")
    ...
    print(dumps({"manifestHash": result.manifest_hash, "files": str(len(result.files)), "provenance": result.report["provenance"],
        ...
        "directSaleCount": str(len(result.report["floor"]["directSales"])),
