...
class ViewPreservationOutputReads:
    ...
    def _view_preservation_header(self):
        ...
        for role, kind, profile, getter, interface in (
            ...
            ('outputManifest', t.MANIFEST_CONFIG, t.MANIFEST_PROFILE, 'outputProfile()', t.MANIFEST_INTERFACE),
        ):
        ...
        require(plan[0][0] == 4 and plan[0][1] == uint(a['collectionId']) and plan[0][2] == 0 and plan[0][3] != ZERO
            and 0 < plan[5] <= t.MAX_ROWS and plan[6] == plan[5] and plan[8] != ZERO and plan[9] != ZERO,
    ...
    def _view_preservation_covered_carrier(self, saved):
    ...
    def _view_preservation_outputs(self, header, adoption, original_source):
        ...
        for index in range(mp[2]):
            ...
            parts.append({'recordHash': descriptor[0], 'record': json_values(part), 'descriptor': json_values(descriptor),
                **self._view_preservation_covered_carrier(part[1])})
        manifest.update(parts=parts, **self._view_preservation_covered_carrier(mp[1]))
        # The selected snapshot's original payload supplies this preimage.
        # Checkpoint.currentSource is a live gate, not a historical getter.
        cp['source'] = deepcopy(original_source)
