...
def verify_package(directory, expected_manifest_hash):
    ...
    if isinstance(value, dict) and value.get("mode") == "native_multiformat_package_v2":
        from .native_multiformat_package_v2 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "native_multiformat_package_v1":
        from .native_multiformat_package_v1 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "native_work_lido_package_v1":
        from .native_work_lido_package_v1 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "semantic_authoring_history_v2":
        from .semantic_authoring_history_v2 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "semantic_field_correspondence_v1":
        from .semantic_field_correspondence_v1 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "semantic_authoring_native_source_envelope_v1":
        from .semantic_authoring_native_sources_v1 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "semantic_authoring_package_v1":
        from .semantic_authoring_package_v1 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "canonical_dossier_evidence_links_v2":
        from .canonical_dossier_evidence_links_v2 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "canonical_dossier_script_links_v1":
        from .canonical_dossier_script_links_v1 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "canonical_object_dossier_v4_assembly":
        from .canonical_object_dossier_v4 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    ...
    if isinstance(value, dict) and value.get("mode") == "public_view_policy_output_capture_v2":
        ...
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_scoped_policy_finality_v9_assembly":
        from .acquisition_scoped_policy_finality_v9 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_scoped_policy_finality_capture_v2":
        from .public_scoped_policy_finality_capture_v2 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_governance_transactions_v1_assembly":
        from .acquisition_governance_transactions_v1 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_governance_transaction_capture":
        from .public_governance_transaction_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_policy_finality_v8_assembly":
        from .acquisition_policy_finality_v8 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)

    if isinstance(value, dict) and value.get("mode") == "public_policy_finality_capture_v2":
        from .public_policy_finality_capture_v2 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    ...
    if isinstance(value, dict) and value.get("mode") == "acquisition_finality_v6_assembly":
        ...
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_finality_capture":
        from .public_finality_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_title_v5_assembly":
        from .acquisition_title_v5 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_preservation_v5_assembly":
        from .acquisition_preservation_v5 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_media_master_capture":
        from .public_media_master_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_prospective_reference_capture":
        from .public_prospective_reference_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_attribution_v5_assembly":
        from .acquisition_attribution_v5 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_attribution_capture":
        from .public_attribution_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_packet_v5_assembly":
        from .acquisition_packet_v5 import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_direct_conservation_assembly":
        from .acquisition_direct_conservation import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_direct_personhood_assembly":
        from .acquisition_direct_personhood import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    ...
    if isinstance(value, dict) and value.get("mode") == "conservation_original_provider_binding":
        ...
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_conservation_provider_capture":
        from .public_conservation_provider_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "conservation_original_rights_assembly":
        from .conservation_rights import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_direct_conservation_capture":
        from .public_direct_conservation_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "acquisition_native_conservation_assembly":
        from .acquisition_conservation import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_conservation_floor_capture":
        from .public_conservation_floor_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_conservation_tier_capture":
        from .public_conservation_tier_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
    if isinstance(value, dict) and value.get("mode") == "public_condition_capture":
        from .public_condition_capture import verify
        from .bagit import read_tree
        return verify(read_tree(directory), expected_manifest_hash)
