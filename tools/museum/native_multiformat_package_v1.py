"""Replayed native V4 exports and exact four-format field correspondence."""
import argparse
...
from . import native_premis_v1 as premis
from . import native_iiif_v1 as iiif
...
NAME = 'STREAM_MUSEUM_NATIVE_MULTIFORMAT_PACKAGE_V1'
MODE = 'native_multiformat_package_v1'
...
QUALIFICATION = ('Four prospective local native adapters share one replayed V4 source. '
    ...
    'and checks of retained bytes remain separately attributed and never imply artwork '
    'media retrieval, historical preservation events, rights, current authority or '
...
PROFILE_BYTES = dumps({'name': NAME, 'version': '1', 'mode': MODE,
    ...
    'plan': 'Closed version=1, sourceManifestHash and adapters object containing exactly all four closed adapter plans.',
    'verification': 'Concrete original V4 replay, original field inventory, original-source adapters, local target ledger, exact complete-byte reconstruction using retained dependencies.',
...
def _plan(raw, digest, source_hash):
    ...
    require(type(value) is dict and set(value) == {'version', 'sourceManifestHash', 'adapters'}
        and value['version'] == '1' and value['sourceManifestHash'] == source_hash
