"""Read-only public conservation evidence capture; item13 remains partial."""
import argparse
import os
...
MODE = "public_conservation_capture"
PROFILE = "STREAM_MUSEUM_PUBLIC_CONSERVATION_CAPTURE_V1"
MISSING_PRODUCERS = {
    "tierDeclaration": "Authoritative conservation-tier declaration evidence is not supplied by this source.",
    "tierDefault": "Authoritative default-tier basis is not established by an empty conservation selector.",
    "saleFloor": "Required conservation sale-floor enforcement and its evidence remain unproven.",
}
CLAIMS = {"readOnlyRpc": True, "consumerFixedFilters": True, "offlineReplayChecked": True,
    "originalSourceBytesRetained": True, "originalArtistEstateLineagesSeparate": True,
...
QUALIFICATION = (
    "Public-history conservation selector evidence at an externally pinned block. Original collection/token "
    "Artist and estate lineages, source eligibility, locks, original records and observed preparations remain "
    "separate. Provider log completeness and canonical mapping are trusted. Replay does not authenticate "
    "provenance, consensus, historical signatures, interview delivery or legal truth. Item13 remains partial: "
    "tier declaration, default-tier basis and sale-floor evidence are missing; no canonical packet compatibility "
    "or complete acquisition packet is claimed. Empty selected heads never become a tier default or waiver.")
PROFILE_BYTES = dumps({"name": PROFILE, "version": "1", "status": "prospective_unregistered_capture_profile",
    "source": "STREAM_MUSEUM_PUBLIC_CONSERVATION_SOURCE_V1", "inputPins": ["anchorHash", "sourceProfileHash"],
    "endpoint": "Explicit named process environment variable; never in retained evidence or remote error details.",
    "output": "Exact source triplet, frozen definitions, full conservation scopes, original records and observed preparations; complete offline reconstruction required.",
    ...
    "missingProducers": MISSING_PRODUCERS, "claims": CLAIMS, "qualification": QUALIFICATION})
...
def _public(disclosure):
    require(disclosure == "public", "public conservation requires public disclosure before reads")
...
def _anchor(raw, expected_hash, expected_profile):
    ...
    require(type(raw) is bytes and 0 < len(raw) <= 65536 and any(hex_bytes(expected_hash, 32))
        and keccak256(raw) == expected_hash, "public conservation external anchor pin/bound differs")
    require(expected_profile == source.PROFILE_HASH, "public conservation external source profile differs")
    return source.PublicConservationSource


def _derive(snapshot):
    evidence = {key: snapshot[key] for key in ("identity", "binding", "scopes", "currentAssociation",
        "historyCoverage", "claims", "qualification", "remaining")}
    return {"conservation/evidence.json": dumps(evidence),
        "conservation/original-records.json": dumps(snapshot["records"]),
        "conservation/preparations.json": dumps(snapshot["preparations"])}
...
def _assemble(source, anchor_hash, source_profile_hash):
    ...
    require(source_profile_hash == module.PROFILE_HASH and keccak256(source.anchor_bytes) == anchor_hash,
        "public conservation original capture pins differ")
    ...
    items = [{"item": str(i), "name": title, "status": "partial" if i == 13 else "unresolved",
        ...
        "remaining": " ".join(MISSING_PRODUCERS.values()) if i == 13 else "Required evidence is not assembled by this capture."}
    ...
    report = {"profile": PROFILE, "provenance": source.provenance, "sourceProfileHash": source_profile_hash,
        ...
        "item13": {"status": "partial", "canonicalPacketCompatible": False, "missingProducers": MISSING_PRODUCERS},
    ...
    require(len(raw) <= MAX_MANIFEST, "public conservation capture manifest bound")
...
def capture(anchor_bytes, anchor_hash, source_profile_hash, transport, *, disclosure):
    ...
    require(type(transport) is PublicRpcTransport, "public conservation capture requires explicit read-only transport")
    ...
    require(verify(dict(result.files), result.manifest_hash).files == result.files,
        "public conservation post-capture replay differs")
    return result
...
def replay(anchor_bytes, anchor_hash, source_profile_hash, transcript, transcript_hash, *, provenance, disclosure):
    ...
    _public(disclosure)
    constructor = _anchor(anchor_bytes, anchor_hash, source_profile_hash)
    require(provenance in ("trusted_rpc", "synthetic_fixture"), "public conservation explicit provenance required")
...
def verify(files, expected_hash):
    ...
    require(keccak256(raw) == expected_hash, "public conservation external manifest pin differs")
    ...
    require(dict(rebuilt.files) == files, "public conservation reconstruction differs")
    return rebuilt
...
def _read_input(path, limit):
    ...
    require(stat.S_ISREG(state.st_mode) and not path.is_symlink()
        ...
        "public conservation input must be a regular file")
    require(0 < state.st_size <= limit, "public conservation input byte bound")
    with path.open("rb") as stream: raw = stream.read(limit + 1)
    require(0 < len(raw) <= limit, "public conservation input byte bound")
    return raw
    ...
    if args.command in ("capture", "replay"):
        ...
        if args.command == "capture":
            require(re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]{0,127}", args.rpc_env) is not None,
                "public conservation RPC variable name invalid")
        ...
        if args.command == "capture":
            ...
            require(endpoint is not None and endpoint != "", "public conservation RPC variable unavailable")
            ...
            require(verify(dict(result.files), result.manifest_hash).files == result.files,
                "public conservation post-replay reconstruction differs")
