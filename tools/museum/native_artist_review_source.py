...
class NativeArtistReviewSource:
    ...
    def _interpret(self, value, native, original, originals, positions):
        from .native_artist_review_profile import ASSERTION_PROFILE_SCHEMA_ID, ASSERTION_SCHEMA_BYTES
        ...
        require(len({assertion['id'] for assertion in value['assertions']}) == len(value['assertions']),
            'Artist review duplicate assertion identifier')
    ...
    def _capture(self):
        ...
        for native in artist['attestations']:
            ...
            if record[0] != schema_id('ARTIST_SEMANTIC_ASSERTION') or record[4] != schema_id(NAMES[1]) or record[2][2] != JCS_ID:
                continue
            try:
                value = _validate(ASSERTION_SCHEMA_BYTES, hex_bytes(original['payloadHex']))
            except MuseumError as exc:
                row.update(status='invalid', reasonCode='semantic_schema_invalid', diagnostic=str(exc)); continue
            if value['profileHash'] != self.profile.profile_hash: continue
            if not checked:
                self._registered_documents(); checked = True
            try:
                self._interpret(value, native, original, originals, positions)
            except MuseumError as exc:
