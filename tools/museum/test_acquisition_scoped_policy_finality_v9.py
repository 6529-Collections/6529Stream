"""Complete-shape V9 from twelve actual synthetic source replays."""
...
from ..metadata import acquisition_packet_v9 as v9
from . import acquisition_scoped_policy_finality_v9 as assembly
...
class PolicyFinalityV9AssemblyTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture = ScopedPolicyFinalityFixtureV2()
    ...
    def packet(self): return v9.validate(dict(self.result.files)[assembly.PACKET_PATH])
    ...
    def test_all_original_fields_files_and_captures_survive(self):
        ...
        self.assertEqual(set(old), set(new)); self.assertEqual(new["version"], 9)
        self.assertEqual(new["schema"], v9.PACKET)
    ...
    def test_native_policy_proof_original_citation_and_twelve_sources(self):
        ...
        self.assertEqual(packet["contentRootProof"]["kind"], "native_scoped_policy_token_content_proof_v2")
        ...
        self.assertEqual(len(fragment["definitions"]), 19)
    ...
    def test_burn_preserves_original_collection_membership(self):
        ...
        packet = v9.validate(dict(result.files)[assembly.PACKET_PATH])
        self.assertTrue(packet["sourceState"]["burned"])
        self.assertEqual(packet["ownershipProvenance"]["currentOwner"], v9.ZERO_ADDRESS)
    ...
    def test_pair_citation_and_unrelated_group_contradictions_reject(self):
        ...
        for mode in ("proof_kind", "proof_root", "citation", "entropy", "rights"):
            ...
            with self.subTest(mode=mode), self.assertRaises(MuseumError): v9.validate(dumps(packet))
