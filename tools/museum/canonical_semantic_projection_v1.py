...
The source inventory is verified by ``canonical_semantic_sources_v1`` before
this adapter is called.  This module binds a closed occurrence selection to
that complete inventory, emits one attributed statement per selected
occurrence, and retains every original and semantic leaf.  It never promotes
...
PROFILE = "STREAM_MUSEUM_CANONICAL_SEMANTIC_PROJECTION_V1"
SELECTION_PROFILE = "STREAM_MUSEUM_CANONICAL_SEMANTIC_SELECTION_V1"
RULE = "urn:6529stream:museum:canonical-semantic:v1:"
...
def _rule(name, source, target, cardinality, transformation, uncertainty,
    ...
    return {
        "id": RULE + name,
        "sourceProfile": "STREAM_MUSEUM_CANONICAL_SEMANTIC_SOURCES_V1",
...
CROSSWALK = {
    "name": "STREAM_MUSEUM_CANONICAL_SEMANTIC_CROSSWALK_V1",
    "version": "1",
    "rules": [
        ...
        _rule("complete-coverage", "/leaves/*", "coverage ledger",
            ...
            "Retain every leaf without claiming field-level CRM mapping; bind occurrence-level statement projection separately.",
...
PROFILE_BYTES = dumps({
    ...
    "rules": {
        "denominator": "Bind the complete admitted row and leaf inventory before applying selection.",
        ...
        "semantics": "All family fields remain exact typed sidecars; only the attributed statement and an optional payload carrier are projected.",
...
def _inventory(inventory):
    ...
    row_keys = {
        ...
        "interpretation", "authority", "currentness", "pointers", "catalog",
    }
...
def default_selection(inventory):
...
def historical_selection(inventory, occurrence_ids):
...
def _selection(inventory, rows, raw, expected_hash):
    ...
    } and value["profile"] == SELECTION_PROFILE and value["version"] == "1"
...
def _record_key(row):
    ...
    if selector["sourceId"] is not None:
...
def _project(inventory, selection_raw, selection_hash, model):
    ...
    for identifier in selection["selectedOccurrenceIds"]:
        ...
        emitted = []
        for role, resource in _resource(row):
            raw = dumps(resource)
            ...
            provenance.append({"entity": resource["id"], "occurrenceId": identifier,
                ...
                "sourcePointer": deepcopy(row["pointers"]["payload"] if role == "payload"
                    else (row["pointers"]["projection"] or row["pointers"]["payload"]
                        or row["pointers"]["original"])),
                "sourcePathBase": "input/",
                "rule": RULE + ("payload-carrier" if role == "payload" else "attributed-statement"),
                "qualification": QUALIFICATION})
        assertions.append({"occurrenceId": identifier, "family": row["family"],
            ...
            "semantic": deepcopy(row["semantic"]), "authority": deepcopy(row["authority"]),
    ...
    files[REPORT_PATH] = dumps({"profile": PROFILE, "profileHash": PROFILE_HASH,
        ...
        "recordIdentityRule": "chain/core/host/kind/recordHash/subjectId plus sourceId only when present",
