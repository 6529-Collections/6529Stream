"""Compose original native finality and token proof into a new complete-shape V7 export."""
import argparse
from copy import deepcopy
from pathlib import Path
import sys

from ..metadata import acquisition_packet_v7 as definition
from ..metadata import acquisition_scoped_static_finality_v1 as native
from . import acquisition_title_v5 as previous
from . import public_scoped_finality_capture as finality
...
MODE = "acquisition_finality_v7_assembly"
PROFILE = "STREAM_MUSEUM_ACQUISITION_FINALITY_V7_ASSEMBLY_V1"
ORIGINAL_MANIFEST = "inputs/title-v5-manifest.json"
FINALITY_PREFIX = "acquisition-scoped-finality/"
PACKET_PATH = "finality/acquisition-packet-v7.json"
...
QUALIFICATION = ("New V7 packet with all nineteen groups and paired original scoped STATIC finality/token-proof branches. "
    "The entire title V5 package and scoped capture remain byte-for-byte available. Twelve source captures share "
    "one token/collection state. Exact original scoped snapshots, complete membership and selection/output hash rows "
    "join full collection root history and original finalization. Supported original Executor inputs reconstruct "
    "metadata commitments; availability/unsupported cases remain partial. Historical Core facts, rendering preimages, "
    "signatures, role execution and runtime provenance remain separate. This bounded composition requires shared "
    "Metadata, schema Store/Registry and Artist identity. Provider completeness, source authenticity, consensus, "
    "historical execution and acquisition acceptance remain trust boundaries.")
PROFILE_BYTES = dumps({"name": PROFILE, "version": "1", "status": "prospective_unregistered_assembly_profile",
    ...
    "inputs": ["externally pinned title V5 assembly", "externally pinned original scoped STATIC finality capture"],
    ...
    "coverage": "Item3 remains partial: mathematical original scoped membership is checked, but historical Core/source preimages, authority execution and complete preservation remain unresolved.",
    "claims": CLAIMS, "qualification": QUALIFICATION})
PROFILE_HASH = keccak256(PROFILE_BYTES)


def _public(disclosure): require(disclosure == "public", "finality V7 requires public disclosure before reads")
...
def _reconcile(originals, capture_files, state):
    ...
    require(len(inputs) == 11, "finality V7 exact eleven previous sources required")
    ...
    for role, (anchor_raw, transcript_raw) in sorted(inputs.items()):
        ...
        require(common is None or own == common, "finality V7 common source state differs")
        common = own
        if "tokenId" in anchor: require(anchor["tokenId"] == state["tokenId"], "finality V7 source token differs")
        ...
        for address, digest in own_pins.items():
            require(address not in pins or pins[address] == digest, "finality V7 cross-source runtime differs")
            pins[address] = digest
        ...
        require(transcript["profile"] == own_rpc.PROFILE and transcript["version"] == own_rpc.VERSION,
            "finality V7 transcript profile differs")
        calls[role] = transcript["calls"]; size += len(anchor_raw) + len(transcript_raw); count += len(calls[role])
        require(size <= observations.MAX_INPUT_BYTES and count <= observations.MAX_ROWS, "finality V7 aggregate source bound")
    ...
    require(all(common[key] == state[key] for key in ("chainId", "core", "collectionId", "blockHash", "blockNumber"))
        and common["timestamp"] == state["examinedAt"], "finality V7 observed packet state differs")
...
def compose(packet_files, packet_hash, finality_files, finality_hash, *, disclosure):
    ...
    require(old.report["sourceProvenance"] == captured.report["provenance"], "finality V7 source provenance differs")
    packet = _load(originals, previous.PACKET_PATH)
    require(packet["metadataMode"] == "ONCHAIN" and packet["workClass"] == "script", "finality V7 unsupported supplied mode/work class")
    require(packet["sourceState"] == captured.report["sourceState"], "finality V7 captured token observation differs")
    reconciled = _reconcile(originals, captured_files, packet["sourceState"])
    fragment = native.validate(captured_files["scoped-finality/fragment.json"])
    original_packet = deepcopy(packet)
    packet["schema"], packet["version"] = definition.PACKET, 7
    packet["finality"] = {"kind": "native_scoped_static_finality", "fragment": fragment}
    ...
    require(all(packet[key] == value for key, value in original_packet.items() if key not in changed), "finality V7 unrelated field changed")
    items = deepcopy(old.report["items"])
    require([row["item"] for row in items] == [str(i) for i in range(1, 20)], "finality V7 exact nineteen requirements")
    item = items[2]; item["sourceCoverage"] = "partial"
    item["evidence"] += [PACKET_PATH, FINALITY_PREFIX + "scoped-finality/fragment.json", FINALITY_PREFIX + "scoped-finality/token-proof.json"]
    item["remaining"] = "Original scoped STATIC finality and complete ordered membership/output hash rows are checked. Original transaction reconstruction is retained with its availability limits; historical Core/render preimages, roles, signatures and execution remain unresolved."
    ...
    additions = {ORIGINAL_MANIFEST: originals["manifest.json"], "definitions/finality-v7-assembly-profile.json": PROFILE_BYTES,
        "definitions/acquisition-packet-v7.json": definition.PACKET_SCHEMA_BYTES, PACKET_PATH: packet_raw,
    ...
    require(not set(files).intersection(additions), "finality V7 retention path collision")
    ...
    require(len(manifest) <= MAX_MANIFEST, "finality V7 manifest bound")
...
def verify(files, expected_hash):
    ...
    require(any(hex_bytes(expected_hash, 32)) and keccak256(raw) == expected_hash, "finality V7 external manifest pin differs")
    ...
    require(type(manifest) is dict and set(manifest) == {"mode", "profile", "version", "profileHash", "inputs",
        ...
        and type(manifest["inputs"]) is dict and set(manifest["inputs"]) == {"packet", "finality"}, "finality V7 closed manifest differs")
    require(manifest["files"] == [base._ref(path, body) for path, body in sorted(files.items()) if path != "manifest.json"],
        "finality V7 file commitments differ")
    require(ORIGINAL_MANIFEST in files, "finality V7 original manifest missing")
    ...
    except (KeyError, TypeError) as exc: raise MuseumError("finality V7 original reconstruction failed") from exc
    require(dict(result.files) == files, "finality V7 reconstruction differs")
    return result
...
def main(argv=None):
    ...
    if args.command == "assemble":
        ...
        require(verify(result.files, result.manifest_hash).files == result.files, "finality V7 publication replay differs")
