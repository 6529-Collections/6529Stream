...
class PublicPolicyFinalityCaptureTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture = PolicyFinalityFixtureV2(count=3)
    ...
    def test_exact_original_triplet_and_seventeen_native_definitions(self):
        ...
        self.assertEqual(len({r['id'] for r in definitions}), 17)
        ...
        fragment = loads(files['policy-finality/fragment.json'], maximum=source.MAX_OUTPUT)
    ...
    def test_native_proof_and_preserved_partial_claims(self):
        proof = loads(dict(self.result.files)['policy-finality/token-proof.json'])
        self.assertEqual(proof['kind'], 'native_policy_token_content_proof_v2')
    ...
    def test_current_burn_does_not_rewrite_original_inventory_or_outputs(self):
        original = PolicyFinalityFixtureV2(count=1).policy_result()
        burned = PolicyFinalityFixtureV2(count=1,burned=True).policy_result()
    ...
    def test_coherently_rehashed_provider_dependency_cannot_change_shared_role(self):
        value = self.transcript(); graph = self.snapshot['graph']
        inventory = graph['renderCriticalInventory']['address']; provider = graph['provider']['address']
        def get(target, signature, kind):
            row = next(r for r in value['calls'] if r['method']=='eth_call' and
                r['params'][0]['to']==target and r['params'][0]['data']==calldata(signature))
            return decode((kind,),hex_bytes(row['result']))[0]
        d = get(inventory,'dependencies()',source.wire.INVENTORY_DEPENDENCIES)
        config = get(provider,'policyConfiguration()',source.wire.PROVIDER_CONFIG)
        targets,pins = list(d[0]),list(d[1])
        targets[0],pins[0] = graph['metadata']['address'],graph['metadata']['runtimeHash']
        changed = (tuple(targets),tuple(pins),*d[2:])
        digest = keccak256(encode((source.wire.INVENTORY_DEPENDENCIES,),(changed,)))
        config = (*config[:6],digest)
        profiles = source.neutral.from_json((source.wire.PROFILE,)*3,
            self.snapshot['bundle']['provider']['configuration']['profiles'])
        profiles = (*profiles[:2],(*profiles[2][:7],keccak256(encode((source.wire.PROVIDER_CONFIG,),(config,)))))
        context = (graph['core']['address'],graph['router']['address'],graph['router']['runtimeHash'],
            int(self.snapshot['source']['chainId']),config[3],graph['outputManifest']['address'],
            graph['outputManifest']['runtimeHash'],profiles)
        config_hash = source.neutral._hash('6529STREAM_FINALITY_SOURCE_CONFIGURATION_V1',
            ('uint256','address',source.wire.PROFILE_CONTEXT),(int(self.snapshot['source']['chainId']),provider,context))
        replacements = {(inventory,calldata('dependencies()')):encode((source.wire.INVENTORY_DEPENDENCIES,),(changed,)),
            (inventory,calldata('dependencyHash()')):encode(('bytes32',),(digest,)),
            (provider,calldata('policyConfiguration()')):encode((source.wire.PROVIDER_CONFIG,),(config,)),
            (provider,calldata('finalitySourceProfile(uint8)',('uint8',),(2,))):encode((source.wire.PROFILE,),(profiles[2],)),
            (provider,calldata('finalitySourceConfigurationHash()')):encode(('bytes32',),(config_hash,)),
            (graph['discovery']['address'],calldata('sourceConfigurationHash()')):encode(('bytes32',),(config_hash,))}
        for row in value['calls']:
            if row['method']=='eth_call':
                key = row['params'][0]['to'],row['params'][0]['data']
                if key in replacements: row['result'] = '0x'+replacements[key].hex()
        with self.assertRaisesRegex(MuseumError,'inventory dependency tuple'):
            self.replay_calls(value)
    ...
    def test_repinned_native_definition_and_derivatives_require_exact_rebuild(self):
        paths = ('definitions/native/'+source.wire.definitions()[0]['name']+'.json',
            'policy-finality/fragment.json','policy-finality/token-proof.json','source/snapshot.json','capture/report.json')
        for path in paths:
