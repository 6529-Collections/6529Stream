"""Read-only original COLLECTION policy V2 capture and exact offline replay."""
import argparse
import os
...
MODE = "public_policy_finality_capture_v2"
PROFILE = "STREAM_MUSEUM_PUBLIC_COLLECTION_POLICY_FINALITY_CAPTURE_V2"
...
QUALIFICATION = ("Original COLLECTION policy V2 finality, complete original inventory prefix and output/readiness rows, "
...
PROFILE_BYTES = dumps({"name": PROFILE, "version": "2", "status": "prospective_unregistered_capture_profile",
    "source": "STREAM_MUSEUM_PUBLIC_COLLECTION_POLICY_FINALITY_SOURCE_V2",
    ...
    "output": "Original source triplet, seventeen exact native definition documents, complete captured policy V2 evidence, transaction observations and supplied fragment/token proof.",
    "coverage": "Original COLLECTION policy V2 finality and complete original token inventory prefix; authority, terminal-admission preimages and Core/metadata facts remain partial.",
...
def _anchor(raw, expected_hash, expected_profile):
    ...
    return module.PublicPolicyFinalitySource


def _derive(snapshot, snapshot_bytes):
    from ..metadata import acquisition_policy_collection_finality_v2 as definition
    ...
    return {"policy-finality/fragment.json": dumps(fragment),
        "policy-finality/token-proof.json": dumps(definition.token_proof(fragment)),
        "definitions/policy-finality-fragment.json": definition.SCHEMA_BYTES}
...
def _assemble(source, anchor_hash, source_profile_hash):
    ...
    report = {"profile": PROFILE, "provenance": source.provenance, "sourceProfileHash": source_profile_hash,
        **{key: snapshot[key] for key in ("sourceState", "sourceReviewCommit", "identity", "historicalCoreFacts", "historyCoverage")},
        ...
        "fragmentPath": "policy-finality/fragment.json", "tokenProofPath": "policy-finality/token-proof.json",
