"""Compose original COLLECTION policy V2 finality into a complete-shape V8 export."""
import argparse
from copy import deepcopy
from pathlib import Path
import sys

from ..metadata import acquisition_packet_v8 as definition
from ..metadata import acquisition_policy_collection_finality_v2 as native
from . import acquisition_title_v5 as previous
from . import public_policy_finality_capture_v2 as finality
...
MODE = "acquisition_policy_finality_v8_assembly"
PROFILE = "STREAM_MUSEUM_ACQUISITION_POLICY_FINALITY_V8_ASSEMBLY_V1"
ORIGINAL_MANIFEST = "inputs/title-v5-manifest.json"
FINALITY_PREFIX = "acquisition-policy-finality/"
PACKET_PATH = "finality/acquisition-packet-v8.json"
...
QUALIFICATION = ("New V8 packet with all nineteen groups and paired original COLLECTION policy V2 finality/token-proof branches. "
    "The entire title V5 package and policy V2 capture remain byte-for-byte available. Twelve source captures share "
    "one token/collection state. Exact original policy, snapshot/reference records, complete original inventory prefix "
    "and selection/output/readiness rows join collection root history and finalization. Six STATIC component "
...
PROFILE_BYTES = dumps({"name": PROFILE, "version": "1", "status": "prospective_unregistered_assembly_profile",
    ...
    "inputs": ["externally pinned title V5 assembly", "externally pinned original COLLECTION policy V2 finality capture"],
    ...
    "coverage": "Item3 remains partial: mathematical original COLLECTION inventory membership is checked, but historical Core/metadata facts, terminal-admission preimages, authority execution and complete preservation remain unresolved.",
    "claims": CLAIMS, "qualification": QUALIFICATION})
PROFILE_HASH = keccak256(PROFILE_BYTES)


def _public(disclosure): require(disclosure == "public", "policy finality V8 requires public disclosure before reads")
...
def _reconcile(originals, capture_files, state):
    ...
    require(len(inputs) == 11, "policy finality V8 exact eleven previous sources required")
    ...
    for role, (anchor_raw, transcript_raw) in sorted(inputs.items()):
        ...
        require(common is None or own == common, "policy finality V8 common source state differs")
        common = own
        if "tokenId" in anchor: require(anchor["tokenId"] == state["tokenId"], "policy finality V8 source token differs")
        ...
        for address, digest in own_pins.items():
            require(address not in pins or pins[address] == digest, "policy finality V8 cross-source runtime differs")
            pins[address] = digest
        ...
        require(transcript["profile"] == own_rpc.PROFILE and transcript["version"] == own_rpc.VERSION,
            "policy finality V8 transcript profile differs")
        calls[role] = transcript["calls"]; size += len(anchor_raw) + len(transcript_raw); count += len(calls[role])
        require(size <= observations.MAX_INPUT_BYTES and count <= observations.MAX_ROWS, "policy finality V8 aggregate source bound")
    ...
    require(all(common[key] == state[key] for key in ("chainId", "core", "collectionId", "blockHash", "blockNumber"))
        and common["timestamp"] == state["examinedAt"], "policy finality V8 observed packet state differs")
...
def compose(packet_files, packet_hash, finality_files, finality_hash, *, disclosure):
    ...
    require(old.report["sourceProvenance"] == captured.report["provenance"], "policy finality V8 source provenance differs")
    packet = _load(originals, previous.PACKET_PATH)
    require(packet["metadataMode"] == "ONCHAIN" and packet["workClass"] == "script", "policy finality V8 unsupported supplied mode/work class")
    require(packet["sourceState"] == captured.report["sourceState"], "policy finality V8 captured token observation differs")
    reconciled = _reconcile(originals, captured_files, packet["sourceState"])
    fragment = native.validate(captured_files["policy-finality/fragment.json"])
    original_packet = deepcopy(packet)
    packet["schema"], packet["version"] = definition.PACKET, 8
    packet["finality"] = {"kind": "native_policy_collection_finality_v2", "fragment": fragment}
    ...
    require(all(packet[key] == value for key, value in original_packet.items() if key not in changed), "policy finality V8 unrelated field changed")
    items = deepcopy(old.report["items"])
    require([row["item"] for row in items] == [str(i) for i in range(1, 20)], "policy finality V8 exact nineteen requirements")
    item = items[2]; item["sourceCoverage"] = "partial"
    item["evidence"] += [PACKET_PATH, FINALITY_PREFIX + "policy-finality/fragment.json", FINALITY_PREFIX + "policy-finality/token-proof.json"]
    item["remaining"] = "Original COLLECTION policy V2 finality, complete original inventory/output/readiness rows and six STATIC component preimages are checked. Original transaction reconstruction retains its availability limits; Core/metadata facts, terminal admission preimages, full renderer execution, roles, signatures and execution remain unresolved."
    ...
    additions = {ORIGINAL_MANIFEST: originals["manifest.json"], "definitions/policy-finality-v8-assembly-profile.json": PROFILE_BYTES,
        "definitions/acquisition-packet-v8.json": definition.PACKET_SCHEMA_BYTES, PACKET_PATH: packet_raw,
    ...
    require(not set(files).intersection(additions), "policy finality V8 retention path collision")
    ...
    require(len(manifest) <= MAX_MANIFEST, "policy finality V8 manifest bound")
...
def verify(files, expected_hash):
    ...
    require(any(hex_bytes(expected_hash, 32)) and keccak256(raw) == expected_hash, "policy finality V8 external manifest pin differs")
    ...
    require(type(manifest) is dict and set(manifest) == {"mode", "profile", "version", "profileHash", "inputs",
        ...
        and type(manifest["inputs"]) is dict and set(manifest["inputs"]) == {"packet", "finality"}, "policy finality V8 closed manifest differs")
    require(manifest["files"] == [base._ref(path, body) for path, body in sorted(files.items()) if path != "manifest.json"],
        "policy finality V8 file commitments differ")
    require(ORIGINAL_MANIFEST in files, "policy finality V8 original manifest missing")
    ...
    except (KeyError, TypeError) as exc: raise MuseumError("policy finality V8 original reconstruction failed") from exc
    require(dict(result.files) == files, "policy finality V8 reconstruction differs")
    return result
...
def main(argv=None):
    ...
    if args.command == "assemble":
        ...
        require(verify(result.files, result.manifest_hash).files == result.files, "policy finality V8 publication replay differs")
