"""One synthetic RPC map for original adopted VIEW output evidence.
...
class ViewPolicyOutputFixtureV2(PublicHistoryFixture):
    ...
    def __init__(self, *, count=3, mode="disabled", burned=False, later_burn=False, later_adoption=False):
        ...
        from .test_view_policy_adoption_wire_v2 import supplied as adopted, install_adoption_reads
        from .test_view_policy_output_wire_v2 import supplied as output, install_output_reads
        super().__init__(end=20)
        self.anchor.update(chainId="31337", timestamp="120")
        ...
        self.context = {**self.anchor, "environment": "local_evm_fixture",
            "deploymentEvidenceHash": schema_id("synthetic e0b4 VIEW deployment artifact"),
        ...
        self.adoption_value = adopted(context=self.context, graph=self.graph,
            policy_binding=self.binding, adopted_at=103, later_head=later_adoption)
        ...
        self.output_value, _, _, _ = output(count, mode=mode, burned=burned,
            ...
            serving_configuration_hash=adoption_result["servingConfigurationHash"], coverage_timestamp=105)
        ...
        install_adoption_reads(self, self.adoption_value, self.context, self.graph)
    ...
    def _events(self):
        ...
        priorities = {kind: index for index, kind in enumerate((
            ...
            "view_manifest_advanced", "view_manifest_verified"))}
        ...
        def block(descriptor):
            ...
            if kind.startswith("view_checkpoint_"): return 4
            return 5
    ...
    def source(self):
        from .public_view_policy_output_source_v2 import PublicViewPolicyOutputSource
        return PublicViewPolicyOutputSource(dumps(self.view_anchor), self)

    def result(self):
        from .public_view_policy_output_source_v2 import MAX_OUTPUT
        return loads(self.source().snapshot(), maximum=MAX_OUTPUT)

    def capture(self):
        from . import public_view_policy_output_capture_v2 as capture
        from .public_view_policy_output_source_v2 import PROFILE_HASH
        source = self.source()
