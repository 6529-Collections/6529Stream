"""Historical scoped factory policy V2 reads for the additive public source.
...
"""
from . import scoped_policy_preservation_wire_v2 as wire
...
from .scoped_static_types import STATEMENT
...
class ScopedPolicyPreservationSourceReads:
    ...
    def _policy_payload(self, host, family, key, digest, size):
        ...
        # These scoped publishers expose the complete immutable payload, not a
        # chunk inventory getter. Native retain() partitions it into 8192 bytes.
        for offset in range(0, size, 8192):
            part = raw[offset:offset+8192]; chunk_hash = keccak256(part)
            pointer, length = self._read(self.a['store'], 'chunk(bytes32)',
                ('address','uint32'), ('bytes32',), (chunk_hash,))
            require(pointer != ZERO_ADDRESS and length == len(part), 'policy original Store chunk layout')
            require(self._chunk(chunk_hash) == part, 'policy original Store chunk bytes')
        return raw

    def _policy_source_set(self, source):
        a, graph = self.a, self.graph
        host, factory = a['entropySourceSet'], a['sourceFactory']
        e, scope, membership = source[9], source[0], source[1]
        ...
        require(self._one(factory,'scopedPolicyFactoryProfile()','bytes32') == schema_id('6529STREAM_SCOPED_ENTROPY_POLICY_SOURCE_FACTORY_V2'),
        ...
        require(source[6:9] == (factory,graph['sourceFactory']['runtimeHash'],
                    keccak256(encode((POLICY_DEPS,),(dependencies,)))), 'policy original snapshot factory commitments')
        for signature, expected, kind in (('sourceFactory()',factory,'address'),
                ('sourceFactoryCodeHash()',source[7],'bytes32'),('factoryDependenciesHash()',source[8],'bytes32')):
            require(self._one(a['policyContent'],signature,kind) == expected, 'policy checkpoint original factory binding')
        ...
        for role in ('entropySourceSet','sourceFactory'):
    ...
    def _preservation(self, statement, content_result):
        ...
        original_objects = []
        for digest in dict.fromkeys([groups['reference']['source'][6][1],
                                    *(sample[1][9][1] for sample in groups['reference']['source'][7])]):
        ...
        for coverage in [groups['reference']['source'][6],*(sample[1][9] for sample in groups['reference']['source'][7])]:
        ...
        subject = wire.scope_subject(scope,a)
        for family,role,pub_type,receipt_type in (
            ...
            name = 'ScopedPolicy'+family.title()
