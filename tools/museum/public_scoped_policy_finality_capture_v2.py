"""Read-only original scoped factory-policy V2 capture and exact offline replay."""
import argparse
import os
...
MODE = "public_scoped_policy_finality_capture_v2"
PROFILE = "STREAM_MUSEUM_PUBLIC_SCOPED_POLICY_FINALITY_CAPTURE_V2"
...
QUALIFICATION = ("Original scoped factory-policy V2 finality, complete original sealed scope membership and output/readiness rows, "
    "snapshot/reference/policy evidence, root lineage and six STATIC component preimages. Original Executor transaction "
    "inputs strengthen supported direct-call reconstruction while missing or unsupported inputs remain partial. "
    "Historical Core/metadata facts, terminal-admission preimages, complete renderer execution, authority and consensus "
    "remain unresolved. The capture is not a complete packet.")
PROFILE_BYTES = dumps({"name": PROFILE, "version": "2", "status": "prospective_unregistered_capture_profile",
    "source": "STREAM_MUSEUM_PUBLIC_SCOPED_POLICY_FINALITY_SOURCE_V2",
    ...
    "output": "Original source triplet, exact deduplicated native definition documents, complete captured policy V2 evidence, transaction observations and supplied fragment/token proof.",
    "coverage": "Original scoped factory-policy V2 finality and complete original sealed scope membership; authority, terminal-admission preimages and Core/metadata facts remain partial.",
...
def _public(disclosure):
    require(disclosure == "public", "public policy V2 finality requires public disclosure before reads")
...
def _anchor(raw, expected_hash, expected_profile):
    ...
    require(type(raw) is bytes and 0 < len(raw) <= module.MAX_ANCHOR
        ...
        "public policy V2 finality external anchor pin/bound differs")
    require(expected_profile == module.PROFILE_HASH, "public policy V2 finality external source profile differs")
    return module.PublicScopedPolicyFinalitySource


def _derive(snapshot, snapshot_bytes):
    from ..metadata import acquisition_scoped_policy_finality_v2 as definition
    ...
    return {"scoped-policy-finality/fragment.json": dumps(fragment),
        "scoped-policy-finality/token-proof.json": dumps(definition.token_proof(fragment)),
        "definitions/scoped-policy-finality-fragment.json": definition.SCHEMA_BYTES}
...
def _assemble(source, anchor_hash, source_profile_hash):
    ...
    require(source_profile_hash == module.PROFILE_HASH and keccak256(source.anchor_bytes) == anchor_hash,
        "public policy V2 finality original capture pins differ")
    ...
    report = {"profile": PROFILE, "provenance": source.provenance, "sourceProfileHash": source_profile_hash,
        **{key: snapshot[key] for key in ("sourceState", "sourceReviewCommit", "identity", "historicalCoreFacts", "historyCoverage")},
        ...
        "fragmentPath": "scoped-policy-finality/fragment.json", "tokenProofPath": "scoped-policy-finality/token-proof.json",
    ...
    raw = dumps({"mode": MODE, "profile": PROFILE, "version": "2", "profileHash": PROFILE_HASH,
        ...
        "claims": CLAIMS, "qualification": QUALIFICATION})
    require(len(raw) <= MAX_MANIFEST, "public policy V2 finality manifest bound")
...
def capture(anchor_bytes, anchor_hash, source_profile_hash, transport, *, disclosure):
    ...
    require(type(transport) is PublicRpcTransport, "public policy V2 finality capture requires explicit read-only transport")
    result = _assemble(constructor(anchor_bytes, transport, provenance="trusted_rpc"), anchor_hash, source_profile_hash)
    require(verify(dict(result.files), result.manifest_hash).files == result.files, "public policy V2 finality post-capture replay differs")
    return result


def replay(anchor_bytes, anchor_hash, source_profile_hash, transcript, transcript_hash, *, provenance, disclosure):
    _public(disclosure); constructor = _anchor(anchor_bytes, anchor_hash, source_profile_hash)
    require(provenance in ("trusted_rpc", "synthetic_fixture"), "public policy V2 finality explicit provenance required")
...
def verify(files, expected_hash):
    ...
    require(any(hex_bytes(expected_hash, 32)) and keccak256(raw) == expected_hash,
        "public policy V2 finality external manifest pin differs")
    ...
    require(dict(rebuilt.files) == files, "public policy V2 finality reconstruction differs")
    return rebuilt
...
def _read_input(path, limit):
    ...
    require(stat.S_ISREG(state.st_mode) and not path.is_symlink()
        ...
        "public policy V2 finality input must be a regular file")
    require(0 < state.st_size <= limit, "public policy V2 finality input byte bound")
    with path.open("rb") as stream: raw = stream.read(limit + 1)
    require(0 < len(raw) <= limit, "public policy V2 finality input byte bound")
    return raw
    ...
    if args.command in ("capture", "replay"):
        ...
        if args.command == "capture":
            require(re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]{0,127}", args.rpc_env) is not None,
                "public policy V2 finality RPC environment variable name")
            endpoint = os.environ.get(args.rpc_env)
            require(type(endpoint) is str and endpoint.startswith(("http://", "https://")), "public policy V2 finality RPC endpoint missing")
            ...
            require(keccak256(transcript) == args.transcript_hash, "public policy V2 finality transcript pin differs")
        ...
        require(verify(dict(result.files), result.manifest_hash).files == result.files,
            "public policy V2 finality pre-publication replay differs")
