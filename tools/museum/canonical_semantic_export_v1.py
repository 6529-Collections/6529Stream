"""Native semantic selection and offline expansion of a replayed canonical dossier.
...
NAME = 'STREAM_MUSEUM_CANONICAL_SEMANTIC_EXPORT_V1'
MODE = 'canonical_native_semantic_export_v1'
...
QUALIFICATION = (
    ...
    'Native WORK, original owner documentation and condition statements keep distinct '
    ...
    'registration, preserved execution tooling or institutional acceptance.')
...
def prepare_selection(files, source_hash, *, disclosure):
...
def _build(files, source_hash, selection_raw, selection_hash, *, disclosure, model_root):
    ...
    native_definitions = sources.definition_files()
    require(all(path.startswith('definitions/native-source/') for path in native_definitions)
    ...
    files['semantic/source-locations.json'] = dumps({
        ...
        'scope': 'Every source-file reference in the derived inventory and native projection provenance is relative to the unchanged original V3 dossier below input/. Export document references are package-relative.',
    ...
    report = {'profile': NAME, 'profileHash': PROFILE_HASH, 'version': '1',
    ...
    export = {'profile': NAME, 'profileHash': PROFILE_HASH, 'version': '1',
    ...
    envelope = dumps({'mode': MODE, 'profile': NAME, 'version': '1', 'profileHash': PROFILE_HASH,
...
def verify(files, expected_hash):
    try:
        ...
        require(type(value) is dict and set(value) == {'mode', 'profile', 'version', 'profileHash',
            ...
            'files', 'claims', 'qualification'} and value['mode'] == MODE and value['profile'] == NAME
            and value['version'] == '1' and value['profileHash'] == PROFILE_HASH
