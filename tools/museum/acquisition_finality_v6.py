"""Compose original native finality and token proof into a new complete-shape V6 export."""
import argparse
from copy import deepcopy
from pathlib import Path
import sys

from ..metadata import acquisition_packet_v6 as definition
from ..metadata import acquisition_native_finality_v1 as native
from . import acquisition_title_v5 as previous
from . import public_finality_capture as finality
...
MODE = "acquisition_finality_v6_assembly"
PROFILE = "STREAM_MUSEUM_ACQUISITION_FINALITY_V6_ASSEMBLY_V1"
ORIGINAL_MANIFEST = "inputs/title-v5-manifest.json"
FINALITY_PREFIX = "acquisition-finality/"
PACKET_PATH = "finality/acquisition-packet-v6.json"
CLAIMS = {"allNineteenGroupsValidated": True, "originalInputBytesPreserved": True,
    ...
    "batchCallMetadataVerified": False, "executionTransactionInputCaptured": False,
...
QUALIFICATION = ("New V6 packet with all nineteen groups and paired original native collection finality/token-proof branches. "
    "The entire title V5 package and original finality capture remain byte-for-byte available. Twelve source captures share "
    "one token/collection observation state. Original manifest, root lineage, complete inline-ONCHAIN leaf bytes and ordered "
    "Merkle membership are checked; root publication and Executor witnesses retain distinct authority. Historical Core facts "
    "remain hash-only and complete batch call metadata is unavailable. This bounded composition requires shared Metadata, "
    "schema Store/Registry and Artist Registry/artistId; recovered or mismatched graph composition is unsupported. Historical "
    "signer/generation/binding facts are never rewritten from current state. Provider completeness, runtime admission, source "
    "authenticity, consensus, historical execution and acquisition acceptance remain separate.")
PROFILE_BYTES = dumps({"name": PROFILE, "version": "1", "status": "prospective_unregistered_assembly_profile",
    ...
    "inputs": ["externally pinned title V5 assembly", "externally pinned original native finality capture"],
    ...
    "coverage": "Item3 remains partial despite complete mathematical native membership: historical Core facts preimage, full batch authority and externally admitted original execution remain unresolved.",
    "claims": CLAIMS, "qualification": QUALIFICATION})
PROFILE_HASH = keccak256(PROFILE_BYTES)


def _public(disclosure): require(disclosure == "public", "finality V6 requires public disclosure before reads")
...
def _reconcile(originals, capture_files, state):
    ...
    require(len(inputs) == 11, "finality V6 exact eleven previous sources required")
    ...
    for role, (anchor_raw, transcript_raw) in sorted(inputs.items()):
        ...
        require(common is None or own == common, "finality V6 common source state differs")
        common = own
        if "tokenId" in anchor: require(anchor["tokenId"] == state["tokenId"], "finality V6 source token differs")
        ...
        for address, digest in own_pins.items():
            require(address not in pins or pins[address] == digest, "finality V6 cross-source runtime differs")
            pins[address] = digest
        ...
        require(transcript["profile"] == observations.rpc.PROFILE and transcript["version"] == observations.rpc.VERSION,
            "finality V6 transcript profile differs")
        calls[role] = transcript["calls"]; size += len(anchor_raw) + len(transcript_raw); count += len(calls[role])
        require(size <= observations.MAX_INPUT_BYTES and count <= observations.MAX_ROWS, "finality V6 aggregate source bound")
    ...
    require(all(common[key] == state[key] for key in ("chainId", "core", "collectionId", "blockHash", "blockNumber"))
        and common["timestamp"] == state["examinedAt"], "finality V6 observed packet state differs")
...
def compose(packet_files, packet_hash, finality_files, finality_hash, *, disclosure):
    ...
    require(old.report["sourceProvenance"] == captured.report["provenance"], "finality V6 source provenance differs")
    packet = _load(originals, previous.PACKET_PATH)
    require(packet["metadataMode"] == "ONCHAIN" and packet["workClass"] == "script", "finality V6 unsupported supplied mode/work class")
    require(packet["sourceState"] == captured.report["sourceState"], "finality V6 captured token observation differs")
    reconciled = _reconcile(originals, captured_files, packet["sourceState"])
    fragment = native.validate(captured_files["native-finality/fragment.json"])
    original_packet = deepcopy(packet)
    packet["schema"], packet["version"] = definition.PACKET, 6
    packet["finality"] = {"kind": "native_collection_finality", "fragment": fragment}
    packet["contentRootProof"] = native.token_proof(fragment)
    digest = fragment["bundle"]["finality"]["record"][1]
    ...
    require(all(packet[key] == value for key, value in original_packet.items() if key not in changed), "finality V6 unrelated field changed")
    items = deepcopy(old.report["items"])
    require([row["item"] for row in items] == [str(i) for i in range(1, 20)], "finality V6 exact nineteen requirements")
    item = items[2]; item["sourceCoverage"] = "partial"
    item["evidence"] += [PACKET_PATH, FINALITY_PREFIX + "native-finality/fragment.json", FINALITY_PREFIX + "native-finality/token-proof.json"]
    item["remaining"] = "Native original finality and ordered token membership are checked. Historical Core facts remain hash-only; complete GovernanceCall metadata/action preimages and historical authority/execution remain unresolved."
    ...
    additions = {ORIGINAL_MANIFEST: originals["manifest.json"], "definitions/finality-v6-assembly-profile.json": PROFILE_BYTES,
        "definitions/acquisition-packet-v6.json": definition.PACKET_SCHEMA_BYTES, PACKET_PATH: packet_raw,
    ...
    require(not set(files).intersection(additions), "finality V6 retention path collision")
    ...
    require(len(manifest) <= MAX_MANIFEST, "finality V6 manifest bound")
...
def verify(files, expected_hash):
    ...
    require(any(hex_bytes(expected_hash, 32)) and keccak256(raw) == expected_hash, "finality V6 external manifest pin differs")
    ...
    require(type(manifest) is dict and set(manifest) == {"mode", "profile", "version", "profileHash", "inputs",
        ...
        and type(manifest["inputs"]) is dict and set(manifest["inputs"]) == {"packet", "finality"}, "finality V6 closed manifest differs")
    require(manifest["files"] == [base._ref(path, body) for path, body in sorted(files.items()) if path != "manifest.json"],
        "finality V6 file commitments differ")
    require(ORIGINAL_MANIFEST in files, "finality V6 original manifest missing")
    ...
    except (KeyError, TypeError) as exc: raise MuseumError("finality V6 original reconstruction failed") from exc
    require(dict(result.files) == files, "finality V6 reconstruction differs")
    return result
...
def main(argv=None):
    ...
    if args.command == "assemble":
        ...
        require(verify(result.files, result.manifest_hash).files == result.files, "finality V6 publication replay differs")
