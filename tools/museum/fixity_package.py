"""Offline derivative retaining an exact recorded package plus typed fixity evidence."""
...
from .recorded_fixity import (MAX_EVENTS, MAX_OBSERVATION_BYTES, PROFILE_BYTES,
    SCHEMAS, project_recorded_fixity)
from .recorded_projection import replay_source_bytes

MODE = "recorded_account_fixity_resource_package"
...
def build_fixity_package(directory, expected_manifest_hash, plan_bytes, *, plan_hash, profile_hash,
                         observations, disclosure):
    if disclosure != "public":
        raise MuseumError("fixity package requires explicit public classification of source and observations")
    ...
    if "premis_plan" not in manifest["pins"]:
        raise MuseumError("fixity package requires an explicit recorded file-only PREMIS plan")
    ...
    result = project_recorded_fixity(source, source_files["inputs/selection.json"], source_files["inputs/plan.json"],
        source_files["inputs/premis-plan.json"], plan_bytes,
        **{key + "_hash": pins[key] for key in ("selection", "plan", "premis_plan", "premis_profile")},
        fixity_plan_hash=plan_hash, fixity_profile_hash=profile_hash,
    ...
    files["inputs/fixity-plan.json"] = plan_bytes
    files["definitions/fixity-profile.json"] = PROFILE_BYTES
    ...
    files["premis-fixity/report.json"] = result.report
    files["premis-fixity/correspondence.json"] = result.correspondence
    files["premis-fixity/provenance.json"] = result.provenance
    if result.xml is not None: files["premis-fixity/premis.xml"] = result.xml
    return _assemble(directory, files, {"mode": MODE, "version": "1", "sourceManifestHash": original.manifest_hash,
        "fixityPlanHash": plan_hash, "fixityProfileHash": profile_hash, "disclosure": "public",
        "observationEvents": sorted(observations), "claims": CLAIMS})


def verify_fixity_package(directory, expected_manifest_hash):
    directory = Path(directory).resolve()
    raw, manifest, files = _read_package(directory, expected_manifest_hash)
    if (set(manifest) != {"mode", "version", "sourceManifestHash", "fixityPlanHash", "fixityProfileHash",
        ...
        raise MuseumError("unsupported fixity package manifest")
    ids = manifest["observationEvents"]
    for event_id in ids: hex_bytes(event_id, 32)
    if ids != sorted(set(ids)): raise MuseumError("fixity observation identifiers must be unique and ordered")
    try:
        observations = {event_id: files["observations/" + event_id[2:] + ".bin"] for event_id in ids}
        rebuilt = build_fixity_package(directory / "source", manifest["sourceManifestHash"], files["inputs/fixity-plan.json"],
            plan_hash=manifest["fixityPlanHash"], profile_hash=manifest["fixityProfileHash"],
            observations=observations, disclosure=manifest["disclosure"])
    except (KeyError, FileNotFoundError) as exc:
        raise MuseumError("fixity package reconstruction input missing") from exc
    if rebuilt.manifest != raw or dict(rebuilt.files) != files:
        raise MuseumError("fixity package semantic reconstruction differs")
    return rebuilt
...
def main():
    ...
    try:
        if args.command == "verify":
            result = verify_fixity_package(args.directory, args.manifest_hash)
        else:
            if args.disclosure != "public": raise MuseumError("restricted fixity export unsupported")
            if args.plan.stat().st_size > 524288: raise MuseumError("fixity plan byte bound")
            ...
            result = build_fixity_package(args.source, args.source_manifest_hash, args.plan.read_bytes(),
