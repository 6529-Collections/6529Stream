...
from tools.security import check_risk_register as checker
from tools.release import check_release_evidence_issue_links as issue_links_checker
...
LIVE_SIZE_MIRROR_REQUIREMENTS: dict[Path, tuple[str, ...]] = {
    ...
    Path("docs/reference/tooling/validation.md"): (
        "release-artifacts/latest/bytecode-release-proof.json",
...
RISK_DEFINITIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "checks": [
            "forge test --match-path test/unit/governance/StreamGovernanceExecutor.t.sol -vvv",
            "python -m tools.security.test_risk_register",
            "python -m tools.security.check_risk_register",
...
def _live_size_scan_text(relative_path: Path, text: str) -> str:
    ...
    if relative_path == Path("ops/AUTONOMOUS_RUN.md"):
        if text.startswith((AUTONOMOUS_DELIVERY_STATE_TITLE, "# Stream delivery state\n")):
            ...
            headings = (
                ...
                if text.startswith("# Stream delivery state\n")
                else AUTONOMOUS_DELIVERY_STATE_HEADINGS
            )
...
def build_register(
    ...
    *,
    ...
    return {
        ...
        "generated_by": f"tools/security/generate_risk_register.py:{GENERATOR_VERSION}",
