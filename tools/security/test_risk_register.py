...
def minimal_register(root: Path) -> dict[str, object]:
    ...
    seed_file(root, "tools/security/check_risk_register.py", "#!/usr/bin/env python3\n")
    risks = []
...
class RiskRegisterTests(unittest.TestCase):
    ...
    def test_reorganized_delivery_state_scans_after_last_heading(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_live_size_mirrors(root)
            path = root / "ops/AUTONOMOUS_RUN.md"
            required = "\n".join(generator.LIVE_SIZE_MIRROR_REQUIREMENTS[Path("ops/AUTONOMOUS_RUN.md")])
            text = "# Stream delivery state\n\n## Active work\n\n## Sepolia and release\n\n## Recovery and remaining scope\n" + required + "\n"
            write_text(path, text)
            generator.validate_live_size_mirrors(root)
            write_text(path, text + "Current Core is 19,000 bytes.\n")
            with self.assertRaisesRegex(generator.checker.RiskRegisterError, "owns a mutable"):
                generator.validate_live_size_mirrors(root)
    ...
    def test_generator_derives_slither_inventory_from_validated_baseline(self) -> None:
        ...
        self.assertIn("2 High and 28 Medium open", risk["residual_risk"])
