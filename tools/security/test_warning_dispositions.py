...
FIXTURE_DIR = Path(__file__).resolve().parents[2] / "test" / "fixtures" / "warning-dispositions"
...
class WarningDispositionTests(unittest.TestCase):
    ...
    def test_parses_real_forge_size_output_fixture(self) -> None:
        ...
        # This is an unmodified historical compiler capture. Its original source
        # names remain evidence; only the test's expected identities are projected
        # back through these exact relocations. The live checker accepts no aliases.
        historical_paths = {
            "smart-contracts/domains/mint/legacy/StreamMinter.sol":
                "smart-contracts/domains/mint/legacy/StreamMinter.sol".replace("/legacy/", "/"),
            "test/regression/legacy/helpers/LegacyStreamCore.sol":
                "test/regression/legacy/helpers/LegacyStreamCore.sol".replace("/regression/legacy/", "/"),
            **{
                "smart-contracts/integrations/randomizers/legacy/" + name + ".sol":
                "smart-contracts/integrations/randomizers/" + name + ".sol"
                for name in ("RandomizerNXT", "RandomizerRNG", "RandomizerVRF")
            },
        }
    ...
    def test_historical_capture_cannot_pass_current_warning_baseline(self) -> None:
        with self.assertRaisesRegex(checker.WarningDispositionError, "baseline drifted"):
            checker.validate_solc_warning_log(FORGE_SIZE_FIXTURE)
    ...
    def test_parses_windows_absolute_source_path(self) -> None:
        ...
        log_text = "\n".join(
            [
                ...
                "   --> C:\\repo\\6529Stream\\smart-contracts\\domains\\mint\\legacy\\StreamMinter.sol:298:5:",
                "    |",
        ...
        self.assertEqual(
            checker.parse_solc_warnings(log_text),
            {
                (
                    "2018",
                    "C:/repo/6529Stream/smart-contracts/domains/mint/legacy/StreamMinter.sol",
