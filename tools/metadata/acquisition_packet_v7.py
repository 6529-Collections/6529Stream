"""Additive supplied-data V7: original scoped STATIC finality and complete ordered token proof."""
import argparse
import copy
...
from . import acquisition_scoped_static_finality_v1 as finality
...
PACKET = "STREAM_ACQUISITION_PACKET_V7"
RIGHTS_SNAPSHOT_URI = v5.RIGHTS_SNAPSHOT_URI
QUALIFICATION = ("Prospective unregistered supplied-data acquisition packet V7. All nineteen required groups and "
    "earlier V6 branches remain. The paired scoped STATIC branch retains exact TOKEN/RELEASE/SEASON native "
    "finality, original snapshots, complete membership, selection and ordered output hash rows. Historical "
    "Core facts and rendering preimages remain hash-only. Supported original Executor inputs reconstruct "
    "metadata commitments without proving signatures, historical authority, execution, source authenticity "
    "or complete source coverage and acquisition acceptance.")
RULES = [
    "Packet version is numeric7; frozen V1 through V6 definitions remain byte-identical and all earlier branches retain their checks.",
    "native_scoped_static_finality and native_scoped_token_content_proof are paired with exact derived proof; ONCHAIN metadata mode and script work class are mandatory.",
    "TOKEN/RELEASE/SEASON use the original scoped STATIC snapshot and output-row manifest. COLLECTION, policy V2, VIEW and factory encodings cannot be relabeled into this branch.",
    "The selected historical root is latest for the same scope before original finalization; a later root head never replaces it.",
    "The target completed mint must precede original selection checkpoint creation. Burned tokens retain their original identity and membership; current lifecycle is separate.",
    "Original root/snapshot/finality receipts keep distinct authority and cannot be cast into generic records. Original governance inputs reconstruct supported direct Executor metadata only.",
    "All nineteen V5 group relationships remain version-local. Cross-fragment Core, Metadata, schema Store/Registry, Artist identity, runtime observations and event coordinates must agree.",
...
def definitions():
    d = copy.deepcopy(v6.definitions())
    d.update(v5._embed(finality, "scopedFinality__"))
    d["packet"]["properties"]["schema"] = {"const": PACKET}
    d["packet"]["properties"]["version"] = {"const": 7}
    d["previousFinalityV6"] = d["finality"]
    d["previousProofV6"] = d["proof"]
    d["finality"] = {"oneOf": [v1.ref("previousFinalityV6"), v1.closed({
        "kind": {"const": "native_scoped_static_finality"}, "fragment": v1.ref("scopedFinality__fragment")})]}
    d["proof"] = {"oneOf": [v1.ref("previousProofV6"), v1.ref("scopedFinality__tokenProof")]}
    return d
...
def schema_document_bytes():
    return dumps({"$schema": "https://json-schema.org/draft/2020-12/schema",
        ...
        "x-stream-original-packet": {"name": v6.PACKET, "hash": v6.PACKET_SCHEMA_HASH},
...
def _native(value):
    return value["finality"].get("kind") in ("native_scoped_static_finality", "native_collection_finality")


def _finality(value):
    native = value["finality"].get("kind") == "native_scoped_static_finality"
    proof_native = value["contentRootProof"].get("kind") == "native_scoped_token_content_proof"
    v1.need(native == proof_native, "V7 scoped finality/proof branches must be paired")
    if not native:
        v6._finality(value)
        return
    v1.need(value["metadataMode"] == "ONCHAIN" and value["workClass"] == "script",
        "V7 native finality requires ONCHAIN script")
    ...
    v1.need(value["citation"]["qualifier"] == {"kind": "fin", "hash": fragment["bundle"]["finality"]["record"][2]},
        "V7 native finality requires its original fin citation")
    ...
    v1.need(all(observed[k] == state[k] for k in ("chainId", "core", "collectionId", "tokenId", "blockHash", "blockNumber")),
        "V7 finality source identity differs")
    ...
    v1.need(graph["metadata"]["address"] == ctx["metadataHost"]
        ...
        and graph["artist"]["address"] == p["sourceBindings"]["currentRegistry"], "V7 shared native graph identities differ")
    ...
    v1.need(value["attribution"]["artistId"] == p["current"]["artistId"] == root[8] != ZERO,
        "V7 shared native Artist identity differs")
    v1.need(observed["timestamp"] == ctx["sourceBlockTimestamp"] == p["sourceState"]["timestamp"]
        ...
        and uint(observed["timestamp"], 64) <= uint(state["examinedAt"], 64), "V7 finality source time differs")
    v1.need(value["contentRootProof"] == finality.token_proof(fragment), "V7 native token proof differs")
    identity = fragment["identity"]
    v1.need(identity["collectionSerial"] == state["collectionSerial"] and identity["burned"] == state["burned"]
        and identity["lifecycle"] == ("3" if state["burned"] else "2"), "V7 native finality token lifecycle differs")
    ...
    v1.need(tuple(uint(mint[key]) for key in ("blockNumber", "logIndex"))
        ...
        "V7 target completed mint must precede native checkpoint")
    ...
    for record in v1._records_in(value):
        v1.need((record["host"], record["recordHash"]) not in native_keys,
            "V7 native finality/root cannot be relabeled as legacy authority")
...
def _joined_observations(value, fragment):
    ...
    def runtime(address, digest):
        v1.need(codes.setdefault(address, digest) == digest, "V7 supplied runtime commitments differ")
...
def validate(raw):
    try:
        ...
        raise MuseumError("invalid supplied acquisition packet V7") from exc
...
def main(argv=None):
    ...
    for path, raw in outputs().items():
        ...
        if args.check: v1.need(destination.is_file() and destination.read_bytes() == raw, "generated V7 bytes differ")
        else: destination.write_bytes(raw)
    print("V7 supplied packet definition matches; complete source coverage is not established.")
...
def _packet(value):
    ...
    if citation["qualifier"]["kind"] == "fin":
        if _native(value):
            need(citation["qualifier"]["hash"] == value["finality"]["fragment"]["bundle"]["finality"]["record"][2 if value["finality"]["kind"] == "native_scoped_static_finality" else 1], "native finality citation join")
        else:
