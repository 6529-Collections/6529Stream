"""Additive supplied-data V8: original COLLECTION policy V2 finality and token proof."""
import argparse
import copy
...
from . import acquisition_policy_collection_finality_v2 as finality
...
PACKET = "STREAM_ACQUISITION_PACKET_V8"
RIGHTS_SNAPSHOT_URI = v5.RIGHTS_SNAPSHOT_URI
QUALIFICATION = ("Prospective unregistered supplied-data acquisition packet V8. All nineteen required groups and "
    "earlier V7 branches remain. The paired COLLECTION policy V2 branch retains complete original membership, "
...
RULES = [
    "Packet version is numeric8; frozen V1 through V7 definitions remain byte-identical and all earlier branches retain their checks.",
    "native_policy_collection_finality_v2 and native_policy_token_content_proof_v2 are paired with exact derived proof; ONCHAIN metadata mode and script work class are mandatory.",
    "This branch supports original COLLECTION policy V2 only. Scoped policy, VIEW, V1 STATIC and factory encodings cannot be relabeled into it.",
    "The selected historical root is latest for the collection before original finalization; a later root head never replaces it.",
...
def definitions():
    d = copy.deepcopy(v7.definitions())
    d.update(v5._embed(finality, "policyFinality__"))
    d["packet"]["properties"]["schema"] = {"const": PACKET}
    d["packet"]["properties"]["version"] = {"const": 8}
    d["previousFinalityV7"] = d["finality"]
    d["previousProofV7"] = d["proof"]
    d["finality"] = {"oneOf": [v1.ref("previousFinalityV7"), v1.closed({
        "kind": {"const": "native_policy_collection_finality_v2"}, "fragment": v1.ref("policyFinality__fragment")})]}
    d["proof"] = {"oneOf": [v1.ref("previousProofV7"), v1.ref("policyFinality__tokenProof")]}
    return d
...
def schema_document_bytes():
    return dumps({"$schema": "https://json-schema.org/draft/2020-12/schema",
        ...
        "x-stream-original-packet": {"name": v7.PACKET, "hash": v7.PACKET_SCHEMA_HASH},
...
def _native(value):
    return value["finality"].get("kind") in ("native_policy_collection_finality_v2", "native_scoped_static_finality", "native_collection_finality")


def _finality(value):
    native = value["finality"].get("kind") == "native_policy_collection_finality_v2"
    proof_native = value["contentRootProof"].get("kind") == "native_policy_token_content_proof_v2"
    v1.need(native == proof_native, "V8 policy V2 finality/proof branches must be paired")
    if not native:
        v7._finality(value)
        return
    v1.need(value["metadataMode"] == "ONCHAIN" and value["workClass"] == "script",
        "V8 native finality requires ONCHAIN script")
    ...
    v1.need(value["citation"]["qualifier"] == {"kind": "fin", "hash": fragment["bundle"]["finality"]["record"][1]},
        "V8 native finality requires its original fin citation")
    ...
    v1.need(all(observed[k] == state[k] for k in ("chainId", "core", "collectionId", "tokenId", "blockHash", "blockNumber")),
        "V8 finality source identity differs")
    ...
    v1.need(graph["metadata"]["address"] == ctx["metadataHost"]
        ...
        and graph["artist"]["address"] == p["sourceBindings"]["currentRegistry"], "V8 shared native graph identities differ")
    ...
    v1.need(value["attribution"]["artistId"] == p["current"]["artistId"] == root[4] != ZERO,
        "V8 shared native Artist identity differs")
    v1.need(observed["timestamp"] == ctx["sourceBlockTimestamp"] == p["sourceState"]["timestamp"]
        ...
        and uint(observed["timestamp"], 64) <= uint(state["examinedAt"], 64), "V8 finality source time differs")
    v1.need(value["contentRootProof"] == finality.token_proof(fragment), "V8 native token proof differs")
    identity = fragment["identity"]
    v1.need(identity["collectionSerial"] == state["collectionSerial"] and identity["burned"] == state["burned"]
        and identity["lifecycle"] == ("3" if state["burned"] else "2"), "V8 native finality token lifecycle differs")
    ...
    v1.need(tuple(uint(mint[key]) for key in ("blockNumber", "logIndex"))
        ...
        "V8 target completed mint must precede native checkpoint")
    ...
    for record in v1._records_in(value):
        v1.need((record["host"], record["recordHash"]) not in native_keys,
            "V8 native finality/root cannot be relabeled as legacy authority")
    ...
    v1.need(readiness[0] == entropy["coordinatorAtMint"] and readiness[3] == entropy["status"]
        and readiness[9] == entropy["seed"], "V8 target original readiness differs from packet entropy")
...
def _joined_observations(value, fragment):
    ...
    def runtime(address, digest):
        v1.need(codes.setdefault(address, digest) == digest, "V8 supplied runtime commitments differ")
...
def validate(raw):
    try:
        ...
        raise MuseumError("invalid supplied acquisition packet V8") from exc
...
def main(argv=None):
    ...
    for path, raw in outputs().items():
        ...
        if args.check: v1.need(destination.is_file() and destination.read_bytes() == raw, "generated V8 bytes differ")
        else: destination.write_bytes(raw)
    print("V8 supplied packet definition matches; complete source coverage is not established.")
