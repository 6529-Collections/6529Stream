"""Additive supplied-data packet V6: native collection finality and ordered token proof."""
import argparse
import copy
...
from . import acquisition_native_finality_v1 as finality
...
PACKET = "STREAM_ACQUISITION_PACKET_V6"
RIGHTS_SNAPSHOT_URI = v5.RIGHTS_SNAPSHOT_URI
QUALIFICATION = ("Prospective unregistered supplied-data acquisition packet V6. All nineteen required "
    "groups and the frozen V5 native authority semantics remain. The additive native collection-finality "
    "branch retains its original receipt, typed manifest, component commitments, execution witness and "
    "scheduled finalizer calldata alongside the "
    "ordered ONCHAIN token-content proof. Hash-only historical Core facts remain hash-only. Internal "
    "consistency and Merkle membership do not authenticate supplied captures, reconstruct historical "
    "readiness, prove current liveness, or establish complete source coverage or acquisition acceptance.")
RULES = [
    "Packet version is numeric6. All nineteen original required groups remain; frozen V1–V5 files are unchanged.",
    "The native_collection_finality and native_token_content_proof branches are paired. Their complete native fragment and derived proof agree exactly; ONCHAIN metadata mode and script work class are mandatory.",
    "The leaf belongs to the original token while its authoritative root and finality scope belong to the collection. No generic record signer, schema or numeric authorityClass is fabricated.",
    "The fin citation names the original collection finality record hash. Current diagnostics and later Artist state do not replace the original finality or its hash-only Core-facts commitment.",
    "For the native branch, the original target-token completed mint Transfer must strictly precede checkpoint creation by block number and global log index; a checkpoint cannot include a token minted later.",
    "All unchanged V5 packet relationships run as version-local checks with direct reuse of its native personhood, conservation, reference and owner helpers. No placeholder legacy finality/proof projection or validator monkeypatch is used.",
    "Supplied native event coordinates, times and current runtime commitments reconcile across finality, personhood, DIRECT, conservation and owner evidence. Original component code commitments remain historical descriptors.",
    "This first paired native branch requires shared Metadata, schema Store/Registry and current Artist Registry/artistId across the retained fragments. Mismatched or recovered graph composition is unsupported; original finality Registry, signer, binding generation and binding hash are never rewritten from current state.",
]


def definitions():
    d = copy.deepcopy(v5.definitions())
    d.update(v5._embed(finality, "finality__"))
    d["packet"]["properties"]["schema"] = {"const": PACKET}
    d["packet"]["properties"]["version"] = {"const": 6}
    d["legacyFinalityV5"] = d["finality"]
    d["legacyProofV5"] = d["proof"]
    d["finality"] = {"oneOf": [v1.ref("legacyFinalityV5"), v1.closed({
        "kind": {"const": "native_collection_finality"}, "fragment": v1.ref("finality__fragment")})]}
    d["proof"] = {"oneOf": [v1.ref("legacyProofV5"), v1.ref("finality__tokenProof")]}
    return d
...
def schema_document_bytes():
    return dumps({"$schema": "https://json-schema.org/draft/2020-12/schema",
        ...
        "x-stream-original-packet": {"name": v5.PACKET, "hash": v5.PACKET_SCHEMA_HASH},
...
def _native(value):
    return value["finality"].get("kind") == "native_collection_finality"


def _finality(value):
    native = _native(value)
    proof_native = value["contentRootProof"].get("kind") == "native_token_content_proof"
    v1.need(native == proof_native, "V6 native finality/proof branches must be paired")
    if not native: return
    v1.need(value["metadataMode"] == "ONCHAIN" and value["workClass"] == "script",
        "V6 native finality requires ONCHAIN script")
    fragment = finality.validate(dumps(value["finality"]["fragment"]))
    v1.need(value["citation"]["qualifier"] == {"kind": "fin", "hash": fragment["bundle"]["finality"]["record"][1]},
        "V6 native finality requires its original fin citation")
    ...
    v1.need(all(observed[k] == state[k] for k in ("chainId", "core", "collectionId", "tokenId", "blockHash", "blockNumber")),
        "V6 finality source identity differs")
    ...
    v1.need(graph["metadata"]["address"] == ctx["metadataHost"]
        ...
        and graph["artist"]["address"] == p["sourceBindings"]["currentRegistry"], "V6 shared native graph identities differ")
    content = fragment["bundle"]["content"]
    root = next(row["record"] for row in content["rootHistory"] if row["recordHash"] == content["selectedRootHash"])
    v1.need(value["attribution"]["artistId"] == p["current"]["artistId"] == root[4] != ZERO,
        "V6 shared native Artist identity differs")
    v1.need(observed["timestamp"] == ctx["sourceBlockTimestamp"] == p["sourceState"]["timestamp"]
        ...
        and uint(observed["timestamp"], 64) <= uint(state["examinedAt"], 64), "V6 finality source time differs")
    v1.need(value["contentRootProof"] == finality.token_proof(fragment), "V6 native token proof differs")
    identity = fragment["identity"]
    v1.need(identity["collectionSerial"] == state["collectionSerial"] and identity["burned"] == state["burned"]
        and identity["lifecycle"] == ("3" if state["burned"] else "2"), "V6 native finality token lifecycle differs")
    mint = value["ownershipProvenance"]["transfers"][0]
    checkpoint = next(row["log"] for row in fragment["events"]
        if row["log"]["topics"][0] == finality.wire.EVENTS["checkpointStarted"])
    v1.need(tuple(uint(mint[key]) for key in ("blockNumber", "logIndex"))
        < tuple(quantity(checkpoint[key]) for key in ("blockNumber", "logIndex")),
        "V6 target completed mint must precede native checkpoint")
    ...
    for record in v1._records_in(value):
        v1.need((record["host"], record["recordHash"]) not in native_keys,
            "V6 native finality/root cannot be relabeled as legacy authority")
...
def _joined_observations(value, fragment):
    ...
    def runtime(address, digest):
        v1.need(codes.setdefault(address, digest) == digest, "V6 supplied runtime commitments differ")
...
def validate(raw):
    try:
        ...
        raise MuseumError("invalid supplied acquisition packet V6") from exc
...
def main(argv=None):
    ...
    for path, raw in outputs().items():
        ...
        if args.check: v1.need(destination.is_file() and destination.read_bytes() == raw, "generated V6 bytes differ")
        else: destination.write_bytes(raw)
    print("V6 supplied packet definition matches; complete source coverage is not established.")


# Version-local all-nineteen-group joins are copied from frozen V5 below. Only
# the fin citation hook differs; the native branch is independently validated.
...
def _packet(value):
    ...
    if citation["qualifier"]["kind"] == "fin":
        if _native(value):
            need(citation["qualifier"]["hash"] == value["finality"]["fragment"]["bundle"]["finality"]["record"][1], "native finality citation join")
        else:
