"""Closed supplied original COLLECTION policy V2 finality; authentication is external."""
import argparse
import copy
...
from tools.museum import policy_content_types_v2 as types
from tools.museum import policy_content_wire_v2 as content_wire
from tools.museum import policy_preservation_types_v2 as preservation
from tools.museum import policy_static_components_v2 as static
...
NAME = "STREAM_ACQUISITION_POLICY_COLLECTION_FINALITY_V2"
SOURCE_PROFILE = "STREAM_MUSEUM_PUBLIC_COLLECTION_POLICY_FINALITY_SOURCE_V2"
...
QUALIFICATION = ("Prospective unregistered supplied-data COLLECTION policy V2 finality. Complete original ordered "
    ...
    "Scoped policy V2, VIEW and other codecs are unsupported by this branch.")
RULES = [
    "Numeric version2 identifies this additive COLLECTION policy V2 fragment; frozen earlier definitions remain unchanged.",
    "Original provider roles six and seven retain their V1 meanings; V2 output bindings and the three fixed provider profiles are separate exact commitments.",
    "All original root predecessors and V1/V2 binding events are retained. The selected V2 root must be the latest collection root before original finalization; current head is a separate observation.",
    ...
    "The exact original policy snapshot/reference payloads and first/last sample commitments join retained policies, output rows, source identities, locks and seventeen native definitions.",
...
def definitions():
    ...
    d["finality"] = closed({"record": abi(wire.base.FINALITY_RECORD), "components": array(abi(wire.COMPONENT), 32, 1),
    ...
    d["rootRow"] = closed({"recordHash": h, "record": abi(types.ROOT_RECORD), "binding": abi(types.ROOT_BINDING)})
    d["roots"] = closed({"selectedRootHash": h, "rootHead": h, "history": array(ref("rootRow"), types.MAX_HISTORY, 1)})
    d["checkpoint"] = closed({"id": h, "salt": h, "plan": abi(types.CONTENT_PLAN),
        ...
        "selectionRows": array(abi(types.SELECTION_ROW), types.MAX_OUTPUTS, 1)})
    ...
    d["membership"] = closed({"facts": abi(preservation.MEMBERSHIP_FACTS), "inventoryState": abi(("uint256", "bytes32")),
        "tokens": array(u, types.MAX_OUTPUTS, 1), "identities": array(abi(("bool", "uint256", "uint256", "bool")), types.MAX_OUTPUTS, 1),
        "lifecycles": array(abi("uint8"), types.MAX_OUTPUTS, 1), "serialTokens": array(u, types.MAX_OUTPUTS, 1)})
    d["providerConfiguration"] = closed({**{k: abi(wire.PROVIDER_CONFIG) for k in ("originalConfiguration", "scopedConfiguration", "policyConfiguration")},
        "inventoryDependencies": abi(wire.INVENTORY_DEPENDENCIES), "profiles": abi((wire.PROFILE,) * 3), "configurationHash": h})
    ...
    d["bundle"] = closed({k: ref(k) for k in ("provider", "finality", "content", "snapshot", "reference", "membership", "staticComponents", "execution")})
    ...
    d["fragment"] = closed({"schema": {"const": NAME}, "version": {"const": 2}, "sourceRef": ref("sourceRef"),
        ...
        "events": array(ref("event"), MAX_EVENTS, 1), "definitions": array(ref("definition"), 17, 17),
    ...
    d["tokenProof"] = closed({"kind": {"const": "native_policy_token_content_proof_v2"}, "subjectId": h,
...
def token_proof(value):
    ...
    statement = wire.validate_finality(bundle["finality"], source, graph)["statement"]
    proof = content_wire.validate(bundle["content"], source, graph, statement)["targetProof"]
    return {"kind": "native_policy_token_content_proof_v2", "subjectId": subject_id("token", source["chainId"], source["core"],
        source["collectionId"], token_id=source["tokenId"]), "scope": ["0", source["collectionId"], "0", ZERO],
        **copy.deepcopy(proof)}


def native_record_keys(value):
    graph, bundle = value["graph"], value["bundle"]
    return {(graph["finality"]["address"], bundle["finality"]["record"][1])} | {
...
def event_observations(value):
    for row in value["events"]:
        ...
        yield publication, row["timestamp"], ("native_policy_finality_event", log["address"], tuple(log["topics"]), log["data"])
...
def runtime_observations(value):
    ...
    for row in bundle["snapshot"]["source"][8][5]: yield row[0], row[1]
...
def validate(raw):
    try:
        ...
        raise MuseumError("invalid supplied COLLECTION policy V2 finality fragment") from exc


def _source_profile_hash():
    from tools.museum import public_policy_finality_source_v2 as source
...
def main(argv=None):
    ...
    print("COLLECTION policy V2 supplied definition matches; source authentication remains separate.")
