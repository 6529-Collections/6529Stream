...
REQUIRED_CANONICAL_FIXTURE_PATHS = tuple(
    ...
    for path in (
        ...
        "tools/protocol/check_artist_owner_record_continuity.py",
        "tools/protocol/test_artist_owner_record_continuity.py",
        "docs/architecture/artist-owner-record-continuity-extension-v1.json",
        "docs/architecture/artist-owner-record-continuity-extension-v1.schema.json",
        "tools/protocol/check_artist_owner_record_continuity_extension.py",
        "tools/protocol/test_artist_owner_record_continuity_extension.py",
        ...
        "smart-contracts/interfaces/stream/parameters/IStreamGovernedParameterAuthority.sol",
...
class ReleaseArtifactVerifierTests(unittest.TestCase):
    ...
    def test_verifier_reviewed_trust_literals_are_exact(
        ...
        self.assertEqual(len(verifier.REVIEWED_RELEASE_TOOL_RUNTIME_CLOSURE), 32)
        ...
        self.assertIn(
            "test/unit/core/StreamCorePermanentTarget.t.sol",
            COMMITTED_COVERED_PATHS,
        )
        self.assertTrue(
    ...
    def test_closed_world_policy_rejects_unreviewed_surface_shapes(self) -> None:
        target = Path("tools/build/generate_bytecode_release_proof.py")
        mutations = {
            "unlisted-call": "\nPath.cwd()\n",
            "unlisted-member": "\nUNLISTED = Path.cwd\n",
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_release_bundle(root)
            runtime_root = root / "tools/release/generate_release_notes.py"
            write_bytes(
                runtime_root,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def capture_load(snapshot_root: Path, module_name: str):
                observed_roots.append(
                    (snapshot_root, (snapshot_root / "tools").is_dir())
                )
    ...
    def test_snapshot_checker_loader_uses_snapshot_dependency_and_restores_preloaded_module(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            checker_path = root / "tools/protocol/check_governed_parameter_inventory.py"
            write_text(
                checker_path,
                "from tools.protocol import check_governed_parameter_identifiers as identifier_checker\n"
            ...
            try:
                ...
                write_text(
                    checker_path,
                    "from tools.protocol import check_governed_parameter_identifiers\n"
    ...
    def test_verifier_rejects_required_trust_file_substituted_in_checksum_manifest(
        ...
        self.assert_release_tool_bundle_mutation_rejected(
            ...
            (
                "canonical line-ending checksum-index file-set mismatch.*"
                "SHA256SUMS-only=.*tools/build/generate_bytecode_release_proof.py"
            ),
        )
    ...
    def test_verifier_rejects_coordinated_transitive_source_deletion(
        ...
        target = "tools/docs/check_changelog.py"
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_release_bundle(root)
            latest = root / "release-artifacts/latest"
