#!/usr/bin/env python3
"""Validate the frozen historical migration proof, separately from the active layout."""
...
def source_receipt(repo_root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    # Never remap this proof onto today's files: the v1 receipt covers the original
    # migrated checkout, not later implementation work or the current source layout.
    missing = [move["new_path"] for move in manifest["moves"] if not (repo_root / move["new_path"]).is_file()]
    if missing:
        raise EquivalenceError(
            "historical source recomputation requires the original v1 migrated checkout; "
            "use --check-receipt for current checkouts; missing original targets: " + str(missing)
        )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    mode.add_argument(
        "--check-source",
        action="store_true",
        help="recompute the original v1 migrated checkout against its historical base; not a current-layout proof",
    )
