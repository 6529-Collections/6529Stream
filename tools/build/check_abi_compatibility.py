...
from tools.build import generate_release_artifacts as release_artifacts
...
def build_abi_surface(
    ...
    return {
        ...
        "generated_by": f"tools/build/check_abi_compatibility.py:{GENERATOR_VERSION}",
        "source": {
...
def compare_abi_surfaces(baseline: dict[str, Any], current: dict[str, Any]) -> dict[str, Any]:
    """Compare named ABI entries; source relocation does not rewrite frozen baselines.

    Artifact identity and exact current source keys are validated while building
    the current surface from its release receipt. They are provenance, not ABI.
    """
    incompatible: list[dict[str, Any]] = []
    additive: list[dict[str, Any]] = []
