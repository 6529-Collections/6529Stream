...
SNAPSHOT_CHECKER_MODULE_PATHS = {
    ...
    "check_record_family_authorization": Path(
        "tools/protocol/check_record_family_authorization.py"
    ),
}
SNAPSHOT_CHECKER_DEPENDENCY_PATHS = {
    "check_artist_semantic_owner_matrix": {},
    "check_governed_parameter_inventory": {
        "tools.protocol.check_governed_parameter_identifiers": Path(
            "tools/protocol/check_governed_parameter_identifiers.py"
        ),
    },
...
CANONICAL_COVERED_PATH_COUNT = 366
...
def _load_snapshot_checker(
    ...
    canonical_name = ".".join(relative_source.with_suffix("").parts)
    ...
    scripts_path = str(snapshot_root)
    original_path = list(sys.path)
    original_modules = set(sys.modules)
    # A cached parent package can redirect every child import through __path__.
    # Isolate the complete first-party namespace, not just the checker leaf.
    prior_dependencies = {
        name: value for name, value in sys.modules.items()
        if name == "tools" or name.startswith("tools.")
    }
    for dependency_name in prior_dependencies:
        sys.modules.pop(dependency_name, None)
    allowed_paths = dict(dependency_paths)
    allowed_paths[canonical_name] = relative_source
    for relative_path in (relative_source, *dependency_paths.values()):
        for parent in relative_path.parents:
            if parent.parts:
                allowed_paths[".".join(parent.parts)] = parent / "__init__.py"
    ...
    try:
        ...
        for dependency_name, dependency_relative_path in dependency_paths.items():
            ...
            if (
                ...
                or not dependency_resolved.is_relative_to(snapshot_root_resolved)
            ):
                raise ReleaseArtifactVerificationError(
        ...
        for loaded_name, expected_path in allowed_paths.items():
            loaded_module = sys.modules.get(loaded_name)
            if loaded_module is None:
                continue
            loaded_file = getattr(loaded_module, "__file__", None)
            if not isinstance(loaded_file, str) or Path(loaded_file).resolve(strict=True) != (snapshot_root / expected_path).resolve(strict=True):
                raise ReleaseArtifactVerificationError(
                    f"validated snapshot checker loaded a redirected package/module: {loaded_name}"
                )
        loaded_names = (set(sys.modules) - original_modules) | {
            name for name in sys.modules if name == "tools" or name.startswith("tools.")
        }
        for loaded_name in loaded_names:
            if loaded_name in allowed_paths:
                continue
            loaded_module = sys.modules.get(loaded_name)
            loaded_file = getattr(loaded_module, "__file__", None)
        ...
        for dependency_name in list(sys.modules):
            if dependency_name != "tools" and not dependency_name.startswith("tools."):
                continue
            sys.modules.pop(dependency_name, None)
