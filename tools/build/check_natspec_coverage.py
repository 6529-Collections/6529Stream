...
DEFAULT_BASELINE = Path("release-artifacts/natspec-coverage.json")
HISTORICAL_BASELINE = Path("release-artifacts/baselines/v0.1.0/natspec-coverage.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.write_baseline:
            if baseline.resolve() == (repo_root / HISTORICAL_BASELINE).resolve():
                raise NatSpecCoverageError("the v0.1.0 NatSpec snapshot is historical; write the active baseline instead")
            report = load_json(surface_report)
            gaps = coverage_gaps(repo_root, report)
