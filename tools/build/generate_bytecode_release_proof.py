...
def build_proof(
    ...
    return {
        ...
        "generated_by": f"tools/build/generate_bytecode_release_proof.py:{GENERATOR_VERSION}",
        "source": {
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not filecmp.cmp(expected_path, output_path, shallow=False):
            raise BytecodeReleaseProofError(
                ...
                "run `python -m tools.build.generate_bytecode_release_proof` and commit the regenerated file"
            )
