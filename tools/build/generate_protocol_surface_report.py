...
def build_report(
    ...
    return {
        ...
        "generated_by": f"tools/build/generate_protocol_surface_report.py:{GENERATOR_VERSION}",
        "source": {
...
def check_report(
    ...
    if not output_path.exists():
        print(
            f"{output_path} is missing; run `python -m tools.build.generate_protocol_surface_report` "
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not filecmp.cmp(generated, output_path, shallow=False):
            print(
                ...
                "`python -m tools.build.generate_protocol_surface_report` after `forge build`.",
                file=sys.stderr,
            )
            return 1
