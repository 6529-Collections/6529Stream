...
from tools.build import check_solidity_layout_equivalence as checker
...
class SolidityLayoutEquivalenceTests(unittest.TestCase):
                ...
                json.dumps(
                    {
                        ...
                        "generated_by": "tools/build/build_release_artifacts.py:5",
                        "targets": targets,
                    }
                ),
                encoding="utf-8",
            ...
            targets = [
                {
                    ...
                    "source": "smart-contracts/interfaces/stream/dependencies/IDependencyRegistry.sol",
    ...
    def test_source_recomputation_does_not_alias_current_targets_into_old_proof(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            current_path = root / "smart-contracts/interfaces/stream/core/IStreamCore.sol"
            current_path.parent.mkdir(parents=True)
            current_path.write_text("pragma solidity ^0.8.19; interface IStreamCore {}", encoding="utf-8")
            with mock.patch.object(checker, "git_show") as history:
                with self.assertRaisesRegex(checker.EquivalenceError, "original v1 migrated checkout"):
                    checker.source_receipt(root, self.manifest)
