...
class AbiCompatibilityTests(unittest.TestCase):
    ...
    def test_relocated_and_extracted_interface_preserves_frozen_baseline(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            self.write_contract(root)
            self.write_contract(root, "IExample")
            ...
            config["interfaces"][0]["source"] = "smart-contracts/interfaces/ExampleAggregate.sol"
            write_json(config_path, config)
            old_artifact = root / "out" / "IExample.sol" / "IExample.json"
            bundled_artifact = root / "out" / "ExampleAggregate.sol" / "IExample.json"
            bundled_artifact.parent.mkdir(parents=True)
            old_artifact.rename(bundled_artifact)
            baseline_path = self.write_baseline(root, config_path)
            frozen_baseline = baseline_path.read_bytes()

            config["production_contracts"][0]["source"] = "smart-contracts/domains/example/Example.sol"
            config["interfaces"][0]["source"] = "smart-contracts/interfaces/stream/core/IExample.sol"
            write_json(config_path, config)
            bundled_artifact.rename(old_artifact)
            current = checker.build_abi_surface(root, config_path, root / "out")
            report = checker.compare_abi_surfaces(checker.load_baseline(baseline_path), current)

            self.assertTrue(report["compatible"])
            self.assertEqual(report["incompatible_changes"], [])
            self.assertEqual(report["additive_changes"], [])
            self.assertEqual(baseline_path.read_bytes(), frozen_baseline)
            self.assertEqual(current["interfaces"]["IExample"]["source"], config["interfaces"][0]["source"])
            self.assertEqual(current["interfaces"]["IExample"]["artifact_path"], "out/IExample.sol/IExample.json")

    def test_relocation_requires_matching_release_receipt_source(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            self.write_contract(root)
            config_path = self.write_config(root)
            ...
            config["production_contracts"][0]["source"] = "smart-contracts/domains/example/Example.sol"
            write_json(config_path, config)
            receipt["source"]["config_sha256"] = checker.release_build.sha256_bytes(config_path.read_bytes())
            with self.assertRaisesRegex(checker.release_artifacts.ArtifactError, "target for"):
                checker.build_abi_surface(root, config_path, root / "out", receipt)
            receipt["targets"][0]["source"] = config["production_contracts"][0]["source"]
            surface = checker.build_abi_surface(root, config_path, root / "out", receipt)
            self.assertEqual(surface["contracts"]["Example"]["source"], receipt["targets"][0]["source"])
