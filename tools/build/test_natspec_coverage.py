...
class NatSpecCoverageTests(unittest.TestCase):
    ...
    def test_default_debt_record_is_separate_from_historical_snapshot(self) -> None:
        self.assertEqual(checker.DEFAULT_BASELINE, Path("release-artifacts/natspec-coverage.json"))
        self.assertNotEqual(checker.DEFAULT_BASELINE, checker.HISTORICAL_BASELINE)

    def test_writer_cannot_overwrite_historical_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            historical = root / checker.HISTORICAL_BASELINE
            write_text(historical, "frozen historical bytes\n")
            with redirect_stdout(StringIO()), redirect_stderr(StringIO()) as stderr:
                result = checker.main(["--repo-root", str(root), "--write-baseline", "--baseline", str(historical)])
            self.assertEqual(result, 1)
            self.assertIn("historical", stderr.getvalue())
            self.assertEqual(historical.read_bytes(), b"frozen historical bytes\n")
            self.assertFalse((root / checker.DEFAULT_BASELINE).exists())
