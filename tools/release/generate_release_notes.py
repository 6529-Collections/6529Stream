...
from tools.docs import check_changelog
...
def build_notes(
    ...
    notes = {
        ...
        "generated_by": f"tools/release/generate_release_notes.py:{GENERATOR_VERSION}",
        "source": {
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not filecmp.cmp(expected_json_path, json_path, shallow=False):
            raise ReleaseNotesError(
                ...
                "run `python -m tools.release.generate_release_notes`"
            )
