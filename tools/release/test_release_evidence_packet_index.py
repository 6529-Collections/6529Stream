...
def seed_all_templates(root: Path) -> None:
    ...
    write_text(
        root / generator.LIVE_DEPLOYMENT_MANIFEST_RETAINED_ARTIFACT_TEMPLATE,
        "Live deployment manifest retained artifact template.\n",
    )
    write_text(
...
class ReleaseEvidencePacketIndexTests(unittest.TestCase):
    ...
    def test_packet_rows_include_templates_commands_and_policy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("python -m tools.release.generate_release_evidence_packet_index --check", row["validation_commands"])
    ...
    def test_live_deployment_manifest_row_uses_canonical_retained_artifact(self) -> None:
        """Live deployment manifest rows point at the dedicated template."""
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_repo(root)
            ...
            row = next(
                ...
                for row in packet["rows"]
                if row["requirement_id"]
                == generator.LIVE_DEPLOYMENT_MANIFEST_REQUIREMENT_ID
            )

            self.assertEqual(
                row["retained_artifact_expectation"]["path"],
                generator.LIVE_DEPLOYMENT_MANIFEST_RETAINED_ARTIFACT_TEMPLATE.as_posix(),
            )
            self.assertEqual(
                row["retained_artifact_expectation"]["sha256"],
                checker.file_sha256(
