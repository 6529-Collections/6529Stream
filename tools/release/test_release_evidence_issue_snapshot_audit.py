...
def script_name(command: list[str]) -> str:
    ...
    if command[1] != "-m" or not command[2].startswith("tools.release."):
        raise AssertionError("audit children must use the release package")
    return command[2].rsplit(".", 1)[1] + ".py"
...
def run_success_and_write_snapshots(
    command: list[str], check: bool = False, cwd: Path | None = None
) -> subprocess.CompletedProcess:
    """Mock subprocess.run while materializing exporter snapshots."""
    del check
    if cwd != auditor.repo_root():
        raise AssertionError("audit children must run from the repository root")
...
class ReleaseEvidenceIssueSnapshotAuditTests(unittest.TestCase):
    ...
    def test_command_provenance_normalizes_python_and_repo_scripts(self) -> None:
        ...
        command = [
            sys.executable,
            "-m",
            "tools.release.export_release_evidence_issue_snapshot",
            "--output",
        ...
        self.assertIn(
            "python -m tools.release.export_release_evidence_issue_snapshot", provenance
        )
