...
from tools.release import check_release_evidence_live_audit_markdown as markdown_checker
from tools.release import check_release_evidence_live_audit_report as report_checker
...
def validation_commands(row: dict[str, Any]) -> list[str]:
    ...
    return [
        f"python -m tools.release.check_release_evidence_live_audit_report --report-json {report_json}",
        (
...
def check_outputs(
    ...
    if missing:
        ...
        print(
            "run `python -m tools.release.generate_release_evidence_live_audit_archive` "
