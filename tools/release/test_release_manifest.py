...
def minimal_risk_register(root: Path, source_path: Path, evidence_path: Path) -> dict[str, object]:
    ...
    write_text(root / "tools/security/check_risk_register.py", "#!/usr/bin/env python3\n")
    risks = []
    evidence = {
    ...
    for index, area in enumerate(
        ...
        if area == "audit_boundary":
            ...
            risk_ids = [
                generator.risk_register_checker.RECORD_FAMILY_AUTHORIZATION_RISK_ID,
            ...
            risks.append(
                {
                    ...
                    "evidence": [evidence],
                    "checks": ["python -m tools.security.check_risk_register"],
                    "tracking": [
...
def seed_release_tree(root: Path) -> dict[str, Path]:
    ...
    natspec_coverage = root / "release-artifacts" / "natspec-coverage.json"
    ...
    docs = [
        ...
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "interface-versioning.md",
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "frontend-reference-architecture.md",
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "integration-conformance-fixtures.md",
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "fixtures" / "integration-conformance-fixtures.json",
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "mobile-walletconnect.md",
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "electron-security-wallets.md",
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "operator-admin-ui.md",
        root / "docs" / "reference" / "legacy-stack" / "integrations" / "examples" / "react-viem.md",
    ]
...
class ReleaseManifestTests(unittest.TestCase):
    ...
    def test_record_family_authorization_records_run_semantic_validation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            paths = seed_release_tree(root)

            records = generator.record_family_authorization_records(root)
            expected_grant_map_schema = {
                **generator.file_record(
                    paths["record_family_grant_map_schema"],
                    root,
                ),
                "schema_id": generator.RECORD_FAMILY_AUTHORIZATION_GRANT_MAP_SCHEMA_ID,
        ...
        self.assertEqual(
            ...
            paths["record_family_inventory"].relative_to(root).as_posix(),
        )
        self.assertEqual(
            records["inventory_schema"],
            expected_inventory_schema,
        )
        self.assertEqual(records["evidence_schema"], expected_evidence_schema)
        self.assertEqual(
            records["evidence_template"]["path"],
            paths["record_family_evidence_template"].relative_to(root).as_posix(),
        )
        self.assertEqual(
            records["grant_map_schema"],
            expected_grant_map_schema,
        )

    def test_record_family_authorization_records_reject_inventory_schema_identity_drift(
        self,
    ) -> None:
        mutations = (
            ...
            with self.subTest(field=field):
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with self.assertRaisesRegex(
                        ...
                        generator.record_family_authorization_records(root)

    def test_record_family_authorization_records_reject_inventory_schema_version_drift(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
    ...
    def test_record_family_authorization_records_reject_evidence_schema_identity_drift(
        self,
    ) -> None:
        mutations = (
            ("$schema", "https://json-schema.org/draft/2019-09/schema", "JSON Schema"),
    ...
    def test_record_family_authorization_records_reject_evidence_schema_version_drift(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
    ...
    def test_record_family_authorization_records_reject_grant_schema_identity_drift(
        self,
    ) -> None:
        mutations = (
            ("$schema", "https://json-schema.org/draft/2019-09/schema", "JSON Schema"),
    ...
    def test_record_family_authorization_records_reject_grant_schema_version_drift(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
    ...
    def test_record_family_authorization_records_reject_semantic_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_release_tree(root)
            self.record_family_validator.side_effect = (
                generator.record_family_authorization_checker.RecordFamilyAuthorizationError(
                    "family_groups[0] normative source mismatch"
                )
            )
    ...
    def test_release_tool_call_policy_records_bind_validated_snapshot_bytes(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def mutate_after_snapshot(
                ...
                *,
                ...
                self.assertEqual(
                    policy_bytes,
                    paths["release_tool_call_policy"].read_bytes(),
                )
                self.assertEqual(
                    schema_bytes,
                    paths["release_tool_call_policy_schema"].read_bytes(),
                )
                write_json(
        ...
        self.assertEqual(
            records,
            {
                ...
                "schema": {
                    "path": (
                        generator.DEFAULT_RELEASE_TOOL_CALL_POLICY_SCHEMA.as_posix()
                    ),
                    ...
                    "document_schema_version": (
                        generator.RELEASE_TOOL_CALL_POLICY_SCHEMA
                    ),
                },
            },
        )
    ...
    def test_committed_manifest_binds_record_family_authorization_package(self) -> None:
        ...
        expected = {
            ...
            "evidence_template": generator.file_record(
                repo_root / generator.DEFAULT_RECORD_FAMILY_AUTHORIZATION_EVIDENCE_TEMPLATE,
                repo_root,
                schema_required=True,
            ),
        }
        ...
        self.assertEqual(
            manifest["release_artifacts"].get("record_family_authorization"),
            expected,
        )
    ...
    def test_committed_manifest_binds_release_tool_call_policy(self) -> None:
        ...
        expected = generator.release_tool_call_policy_records(repo_root)

        self.assertEqual(
            manifest["release_artifacts"].get("release_tool_call_policy"),
            expected,
        )
