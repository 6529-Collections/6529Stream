...
class ReleaseChecksumTests(unittest.TestCase):
    ...
    def test_release_tool_call_policy_has_exact_complete_static_surface(
        ...
        runtime_root = Path("tools/release/generate_release_notes.py")
        ...
        append_snapshot_bytes(
            ...
            b"'hidden', 'tools/docs/test_changelog_check.py')\n"
    ...
    def test_release_tool_call_policy_rejects_unlisted_static_surface(
        ...
        target_path = Path("tools/deployment/check_admin_ceremony_evidence.py")
        mutations = {
    ...
    def test_release_tool_call_policy_rejects_imported_binding_laundering(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            path = Path("tools/protocol/check_governed_parameter_inventory.py")
    ...
    def test_release_tool_call_policy_uses_one_immutable_source_snapshot(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            changed_source = root / "tools/deployment/check_admin_ceremony_evidence.py"
    ...
    def test_release_tool_call_policy_explicit_refresh_and_check_modes(
        ...
        with tempfile.TemporaryDirectory(dir=repo_root) as temp_dir:
            ...
            target = root / "tools/deployment/check_admin_ceremony_evidence.py"
    ...
    def test_canonical_build_rejects_transitive_runtime_substitution(
        ...
        repo_root = SCRIPT_PATH.parent.parent.parent
        removed = Path("tools/docs/check_changelog.py")
        covered = [
            candidate
            for candidate in generator.DEFAULT_COVERED_PATHS
            if candidate != removed
    ...
    def test_canonical_preflight_rejects_missing_or_symlinked_reviewed_runtime(
        ...
        target = Path("tools/docs/check_changelog.py")
    ...
    def test_canonical_build_rejects_broad_and_duplicate_coverage(self) -> None:
        ...
        with self.assertRaisesRegex(
            generator.ChecksumError,
            "duplicates=.*tools/security/generate_risk_register.py",
        ):
            generator.build_outputs(
                repo_root,
    ...
    def test_release_tool_snapshot_loader_source_is_exactly_bound(self) -> None:
        repo_root = SCRIPT_PATH.parent.parent.parent
        relative_path = Path("tools/build/verify_release_artifacts.py")
        self.assertEqual(
    ...
    def test_check_mode_rejects_mutated_release_tool_after_bundle_creation(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result, 1)
            self.assertIn(
                "hash mismatch for tools/build/generate_bytecode_release_proof.py",
                stderr.getvalue(),
            )
    ...
    def test_committed_checksums_cover_release_tool_trust_closure(self) -> None:
        ...
        self.assertEqual(len(manifest["source"]["covered_paths"]), 369)
    ...
    def test_default_covered_paths_include_evidence_artifacts(self) -> None:
        ...
        self.assertIn(Path("tools/release/generate_release_notes.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/build/verify_release_artifacts.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/build/build_release_artifacts.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/build/test_release_build_artifacts.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/development/test_windows_ci_wrapper.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("docs/first-30-minutes.md"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/docs/check_first_30_minutes.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/docs/test_first_30_minutes.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(
        ...
        self.assertIn(Path("tools/docs/check_issue_templates.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/docs/test_issue_templates.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/docs/check_pr_template.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/docs/test_pr_template.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/docs/check_markdown_links.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(Path("tools/docs/test_markdown_links.py"), generator.DEFAULT_COVERED_PATHS)
        self.assertIn(
    ...
    def test_committed_checksums_cover_artist_semantic_owner_packet(self) -> None:
        expected_paths = {
            ...
            Path(
                "smart-contracts/interfaces/stream/parameters/"
                "IStreamGovernedParameterAuthority.sol"
            ),
            Path(
                "smart-contracts/interfaces/stream/finality/"
                "IStreamArtworkFinalityRegistry.sol"
            ),
            Path("smart-contracts/domains/artist/StreamArtistArchiveV2.sol"),
            Path(
                "smart-contracts/interfaces/stream/artist/IStreamArtistArchiveV2.sol"
            ),
    ...
    def test_committed_checksums_cover_artist_record_event_correction_packet(
        self,
    ) -> None:
        expected_paths = {
            Path(
                "docs/architecture/"
                "artist-record-event-reconstruction-correction-v1.json"
            ),
            Path(
                "docs/architecture/"
                "artist-record-event-reconstruction-correction-v1.md"
            ),
            Path(
                "docs/architecture/"
                "artist-record-event-reconstruction-correction-v1.schema.json"
            ),
            Path(
                "docs/architecture/"
...
class PackageImportClosureTests(unittest.TestCase):
    def test_identical_leaf_names_in_two_packages_remain_distinct(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            files = {
                "tools/__init__.py": "",
                "tools/first/__init__.py": "",
                "tools/second/__init__.py": "",
                "tools/first/helper.py": "VALUE = 1\n",
                "tools/second/helper.py": "VALUE = 2\n",
                "tools/first/entry.py": "from tools.first import helper as a\nfrom tools.second import helper as b\n",
            }
            for relative, content in files.items():
                path = root / relative
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text(content, encoding="utf-8")
            actual = generator.release_tool_runtime_closure(root, (Path("tools/first/entry.py"),))
            self.assertEqual(set(actual), {Path(name) for name in files})

    def test_same_leaf_name_in_unreviewed_package_is_rejected(self):
        tree = ast.parse("from tools.build import check_governed_parameter_identifiers")
        with self.assertRaisesRegex(generator.ChecksumError, "unreviewed import"):
            generator._release_tool_call_policy_imports(tree, Path("tools/release/generate_release_manifest.py"), role="runtime")
