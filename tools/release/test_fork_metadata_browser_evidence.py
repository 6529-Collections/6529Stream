...
class ForkMetadataBrowserEvidenceTests(unittest.TestCase):
    ...
    def test_historical_commands_require_exact_reviewed_bytes(self) -> None:
        ...
        self.assertEqual(
            checker.hashlib.sha256(raw).hexdigest(),
            checker.HISTORICAL_REVIEWED_CAPTURE_SHA256,
        )
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "historical.md"
            path.write_bytes(raw)
            checker.validate_commands(path, path.read_text(encoding="utf-8"))
            with self.assertRaisesRegex(
                checker.ForkMetadataBrowserEvidenceError, "missing validation command"
            ):
                checker.validate_commands(path, raw.decode("utf-8") + "\n")
