...
from tools.release import check_non_local_release_evidence as non_local_checker
from tools.release import check_public_beta_evidence as evidence_checker
from tools.release import generate_public_beta_blocker_report as shared_report
...
PHASES = (
    {
        ...
        "blocker_report_check": "python -m tools.release.generate_public_beta_blocker_report --check",
    },
    {
...
PUBLIC_BETA_VERIFIED_DEPLOYED_ADDRESSES_REQUIREMENT_ID = (
    "verified_deployed_addresses"
)
PUBLIC_BETA_EXPLORER_VERIFICATION_STATUS_REQUIREMENT_ID = (
    "explorer_verification_status"
)
PUBLIC_BETA_VERIFIED_ADDRESSES_RETAINED_ARTIFACT_TEMPLATE = Path(
...
LIVE_DEPLOYMENT_MANIFEST_REQUIREMENT_ID = "live_deployment_manifest"
LIVE_DEPLOYMENT_MANIFEST_RETAINED_ARTIFACT_TEMPLATE = Path(
...
ROW_VALIDATION_COMMAND_OVERRIDES = {
    ...
    (PUBLIC_BETA_PHASE, PUBLIC_BETA_VERIFIED_DEPLOYED_ADDRESSES_REQUIREMENT_ID): (
        "python -m tools.release.test_public_beta_verified_addresses",
        "python -m tools.release.check_public_beta_verified_addresses",
    ),
    (PUBLIC_BETA_PHASE, PUBLIC_BETA_EXPLORER_VERIFICATION_STATUS_REQUIREMENT_ID): (
        "python -m tools.release.test_public_beta_verified_addresses",
        "python -m tools.release.check_public_beta_verified_addresses",
    ),
    ...
    (PRODUCTION_PHASE, LIVE_DEPLOYMENT_MANIFEST_REQUIREMENT_ID): (
...
def retained_artifact_expectation(
    ...
    if evidence_metadata is not None:
        ...
        retained_path = record["path"]
        retained_sha256 = record["sha256"]
    elif phase == PUBLIC_BETA_PHASE and requirement_id in {
        PUBLIC_BETA_VERIFIED_DEPLOYED_ADDRESSES_REQUIREMENT_ID,
        PUBLIC_BETA_EXPLORER_VERIFICATION_STATUS_REQUIREMENT_ID,
    }:
        record = file_record(
            resolve_repo_path(
                repo_root,
                PUBLIC_BETA_VERIFIED_ADDRESSES_RETAINED_ARTIFACT_TEMPLATE,
            ),
            repo_root,
        )
        retained_path = record["path"]
        ...
        phase == PRODUCTION_PHASE
        and requirement_id == LIVE_DEPLOYMENT_MANIFEST_REQUIREMENT_ID
    ):
        record = file_record(
            resolve_repo_path(
                repo_root,
                LIVE_DEPLOYMENT_MANIFEST_RETAINED_ARTIFACT_TEMPLATE,
            ),
            repo_root,
        )
        retained_path = record["path"]
...
def check_outputs(
    ...
    if missing:
        ...
        print(
            "run `python -m tools.release.generate_release_evidence_packet_index` "
