...
from tools.release import check_drop_authorization_signing_evidence as drop_signing_evidence_checker
from tools.deployment import check_admin_ceremony_evidence as admin_ceremony_checker
from tools.protocol import check_artist_semantic_owner_matrix as artist_semantic_owner_checker
from tools.protocol import check_governance_action_policy as governance_action_policy_checker
from tools.protocol import check_governed_parameter_inventory as governed_parameter_inventory_checker
from tools.release import check_non_local_release_evidence as non_local_evidence_checker
from tools.release import check_public_beta_evidence as public_beta_checker
...
from tools.release import check_release_signatures as release_signature_checker
from tools.release import check_signer_custody_readiness as signer_custody_checker
...
def default_natspec_coverage_path(protocol_versions: list[str]) -> Path:
    """Use active documentation debt; retain versioned historical snapshots unchanged."""
    default_gas_snapshot_path(protocol_versions)
    return Path("release-artifacts") / NATSPEC_COVERAGE_FILENAME
...
def checksum_bundle() -> dict[str, Any]:
    return {
        ...
        "generated_by": "tools/release/generate_release_checksums.py:1",
        "digest_policy": {
...
def build_manifest(
    ...
    return {
        ...
        "generated_by": f"tools/release/generate_release_manifest.py:{GENERATOR_VERSION}",
        "release": {
            "project": "6529Stream",
        ...
        "release_artifacts": {
            ...
            "dependency_provenance_attestation": file_record(
                release_artifacts_dir / DEPENDENCY_PROVENANCE_ATTESTATION_FILENAME,
                repo_root,
                schema_required=True,
            ),
...
def check_output(
    ...
    if not output_path.exists():
        ...
        print(
            "run `python -m tools.release.generate_release_manifest` and commit the regenerated file",
            file=sys.stderr,
        )
        return 1
