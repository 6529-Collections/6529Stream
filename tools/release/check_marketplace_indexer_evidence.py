...
# This reviewed retained transcript is immutable; future packets use current
# module commands and cannot inherit this exception after any byte changes.
HISTORICAL_REVIEWED_CAPTURE_SHA256 = (
    "db88ca51f51b18c6a0409116830ac93a60a46c7c15a094311d199819988268af"
)
...
def validate_commands(path: Path, text: str) -> None:
    ...
    raw = path.read_bytes()
    if (
        hashlib.sha256(raw).hexdigest() == HISTORICAL_REVIEWED_CAPTURE_SHA256
        and text.encode("utf-8") == raw
    ):
        return
