...
from tools.deployment import materialize_canonical_deployment_plan as materializer
...
class CanonicalDeploymentPlanTests(unittest.TestCase):
    ...
    def test_materializes_deterministically_with_exact_hashes(self) -> None:
        ...
        self.assertEqual(
            first["generated_by"],
            "tools/deployment/materialize_canonical_deployment_plan.py:4",
        )
    ...
    def test_repository_gates_run_real_candidate_after_canonical_build(
        ...
        check_targets = [
            ...
            if line.startswith("check:") and ":=" not in line
        ]
        self.assertIn(
        ...
        for token in (
            "tools.deployment.test_canonical_deployment_candidate",
            "tools.deployment.check_canonical_deployment_candidate",
            "--require-complete",
            "tools.deployment.test_materialize_canonical_deployment_plan",
            "tools.deployment.materialize_canonical_deployment_plan --candidate "
        ...
        write_at = shell.index(
            '"$python_bin" -m tools.deployment.materialize_canonical_deployment_plan '
            "--candidate",
            unit_at,
        )
        ...
        release_at = powershell.index(
            '"tools.build.build_release_artifacts" "--check"'
        )
        ...
        self.assertLess(
            block.index("test_canonical_deployment_candidate"),
            block.index("check_canonical_deployment_candidate"),
        )
        self.assertLess(
            block.index("check_canonical_deployment_candidate"),
        ...
        self.assertLess(
            block.index("test_materialize_canonical_deployment_plan"),
            block.index(
                "materialize_canonical_deployment_plan --candidate"
            ),
        )
        self.assertIn(
            "--output tmp/canonical-deployment-plan.json --check",
