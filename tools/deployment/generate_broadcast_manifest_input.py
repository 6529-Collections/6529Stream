...
def build_manifest_input(
    ...
    generated["generated_by"] = f"tools/deployment/generate_broadcast_manifest_input.py:{GENERATOR_VERSION}"
...
def check_manifest_input(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not filecmp.cmp(expected, output_path, shallow=False):
            ...
            print(
                "run `python -m tools.deployment.generate_broadcast_manifest_input` and commit the regenerated JSON",
                file=sys.stderr,
            )
            return 1
