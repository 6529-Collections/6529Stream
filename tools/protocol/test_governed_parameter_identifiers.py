...
from tools.protocol import check_governed_parameter_identifiers as checker
    ...
    def test_aggregate_and_release_wiring_runs_identifier_gate(self) -> None:
        for relative in (Path("scripts/check.sh"), Path("scripts/check.ps1")):
            with self.subTest(wrapper=relative.as_posix()):
                ...
                positions = [
                    text.find("tools.protocol.check_genesis_deployment_profile"),
                    text.find("tools.protocol.test_governed_parameter_identifiers"),
                    text.find("tools.protocol.check_governed_parameter_identifiers"),
                    text.find("tools.protocol.test_system_manifest_payload_vector"),
                ]
