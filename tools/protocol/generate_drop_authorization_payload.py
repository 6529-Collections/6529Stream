...
from tools.protocol import check_drop_authorization_fixtures as fixture_checker
...
def build_payload(document: dict[str, Any], document_id: str = "payload") -> dict[str, Any]:
    ...
    return {
        ...
        "generated_by": f"tools/protocol/generate_drop_authorization_payload.py:{GENERATOR_VERSION}",
        "description": description,
        "source": {
            "input_schema": INPUT_SCHEMA_VERSION,
            "generator": "tools/protocol/generate_drop_authorization_payload.py",
            "canonical_checker": "tools/protocol/check_drop_authorization_fixtures.py",
...
def write_or_check_output(payload: dict[str, Any], output_path: Path, check: bool) -> None:
    ...
    if check:
        ...
        if actual != expected:
            raise DropAuthorizationPayloadError(
                f"{output_path} is stale; rerun tools/protocol/generate_drop_authorization_payload.py"
            )
        return
