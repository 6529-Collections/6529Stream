...
from tools.protocol import check_artist_record_event_reconstruction_correction as checker
...
class ArtistRecordEventReconstructionCorrectionTests(unittest.TestCase):
    ...
    def test_aggregate_local_and_ci_wiring_is_exact(self) -> None:
        ...
        powershell_test = (
            ...
            '"-m" "tools.protocol.test_artist_record_event_reconstruction_correction"'
        )
        powershell_check = (
            '& $pythonPath @pythonArgs '
            '"-m" "tools.protocol.check_artist_record_event_reconstruction_correction"'
        )
        ...
        ci_gate = (
            ...
            "          set -o pipefail\n"
            "          mkdir -p ci-logs\n"
            "          python3 -m tools.protocol.test_artist_record_event_reconstruction_correction "
            "2>&1 | tee ci-logs/artist-record-event-reconstruction-correction-tests.log\n"
            "          python3 -m tools.protocol.check_artist_record_event_reconstruction_correction "
    ...
    def test_checker_has_no_git_subprocess_or_ref_dependency(self) -> None:
        source = (ROOT / "tools/protocol/check_artist_record_event_reconstruction_correction.py").read_text(
            encoding="utf-8"
        )
