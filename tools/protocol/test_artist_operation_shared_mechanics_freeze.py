...
CHECKER_PATH = REPO_ROOT / "tools/protocol/check_artist_operation_shared_mechanics_freeze.py"
...
class ArtistOperationSharedMechanicsFreezeTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        copied = {
            ...
            CHECKER.SCHEMA_PATH,
            CHECKER.MATRIX_PATH,
            *(Path(relative) for relative in CHECKER.EXPECTED_PRESENT_ARTIST_SOURCES),
            *(
                Path(relative)
    ...
    def test_missing_authority_is_reported_as_freeze_error(self) -> None:
        relative = Path("smart-contracts/interfaces/stream/artist/IStreamArtistArchiveV2.sol")
