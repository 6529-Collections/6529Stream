...
CORE_TEST_PATH = REPO_ROOT / "test/unit/core/StreamCorePermanentTarget.t.sol"
...
EXPECTED_SNAPSHOT_COMMAND = (
    ...
    "test/gas/StreamPostEntropyCompletionGas.t.sol --match-test "
    "testMeasureWorstCaseEoaPostCoordinatorTail --snap "
    "out/post-entropy-completion-gas.snap"
)
...
EXPECTED_LIMITATIONS = [
    'The numeric measurements are retained baseline snapshots and were not rerun during source reorganization; current source hashes and the scratch-output snapshot command are separate reproduction bindings.',
    ...
    "This generic completion measurement does not establish deployment-instance or artist/royalty integration evidence; those require their own current-stack tests and deployment readbacks.",
...
def _validate_source_bindings(evidence: dict[str, Any], repo_root: Path) -> None:
    ...
    expected_paths = [
        "test/gas/StreamPostEntropyCompletionGas.t.sol",
...
def _validate_measurement(evidence: dict[str, Any], repo_root: Path) -> int:
    ...
    expected_provenance = {
        "kind": "retained_baseline_snapshot",
        "retained_at_commit": "49fa5438f7ac905a979c54320e3aea43c11db3ed",
        "benchmark_rerun_during_source_reorganization": False,
        "refresh_scope": "current_source_bindings_and_reproduction_recipe_only"
    }
    if measurement.get("provenance") != expected_provenance:
        raise CompletionGasCheckError("retained measurement provenance drift")
