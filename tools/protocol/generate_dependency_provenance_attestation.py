...
def build_attestation(repo_root: Path, manifest_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_by": f"tools/protocol/generate_dependency_provenance_attestation.py:{GENERATOR_VERSION}",
        "release_status": {
            "project": "6529Stream",
...
def check_output(repo_root: Path, manifest_path: Path, output_path: Path) -> int:
    if not output_path.exists():
        ...
        print(
            "run `python -m tools.protocol.generate_dependency_provenance_attestation` and commit the regenerated file",
            file=sys.stderr,
        )
        return 1
