...
from tools.protocol import check_one_of_one_permanence_package as permanence_checker
...
def build_manifest(repo_root: Path, descriptor_dir: Path, output_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_by": f"tools/protocol/generate_one_of_one_permanence_manifest.py:{GENERATOR_VERSION}",
        "source": {
...
def check_output(repo_root: Path, descriptor_dir: Path, output_path: Path) -> int:
    ...
    if not output_path.exists():
        ...
        print(
            "run `python -m tools.protocol.generate_one_of_one_permanence_manifest` and commit the regenerated file",
            file=sys.stderr,
        )
        return 1
