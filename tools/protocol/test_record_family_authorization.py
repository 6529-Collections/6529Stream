...
from tools.protocol import check_record_family_authorization as checker
...
class RecordFamilyAuthorizationTests(unittest.TestCase):
    def _fixture(self) -> tuple[tempfile.TemporaryDirectory[str], Path]:
        ...
        required: set[Path] = {
            ...
            checker.HISTORICAL_SOURCE_CATALOG,
        ...
        for relative in sorted(required):
            relative = Path(checker.current_inventory_source_path(relative.as_posix()))
            target = root / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(ROOT / relative, target)
        return temporary, root

    def test_reorganization_retains_exact_old_source_catalog(self) -> None:
        temporary, root = self._fixture()
        with temporary:
            archive = root / checker.HISTORICAL_SOURCE_CATALOG
            self.assertEqual(_sha256(archive), checker.HISTORICAL_SOURCE_CATALOG_SHA256)
            self.assertEqual(_read(archive)["source_commit"], "f5c7164f37399ef758dbed2bb2e67fb73ee4fe75")
            self.assertEqual(_read(root / checker.DEFAULT_SOURCE_CATALOG)["source_commit"], checker.SOURCE_IMPLEMENTATION_COMMIT)
            archive.write_bytes(archive.read_bytes() + b"\n")
            with self.assertRaisesRegex(checker.RecordFamilyAuthorizationError, "historical source catalog byte identity changed"):
                checker.validate_package(root)

    def test_current_catalog_cannot_reuse_pre_reorganization_commit(self) -> None:
        temporary, root = self._fixture()
        with temporary:
            path = root / checker.DEFAULT_SOURCE_CATALOG
            catalog = _read(path)
            catalog["source_commit"] = _read(root / checker.HISTORICAL_SOURCE_CATALOG)["source_commit"]
            _write(path, catalog)
            with self.assertRaisesRegex(checker.RecordFamilyAuthorizationError, "source implementation commit mismatch"):
                checker.validate_package(root)

    def test_current_catalog_cannot_reuse_pre_reorganization_digest(self) -> None:
        temporary, root = self._fixture()
        with temporary:
            path = root / checker.DEFAULT_SOURCE_CATALOG
            catalog = _read(path)
            old = _read(root / checker.HISTORICAL_SOURCE_CATALOG)
            changed = next(i for i, row in enumerate(catalog["source_bindings"]) if row["sha256"] != old["source_bindings"][i]["sha256"])
            catalog["source_bindings"][changed]["sha256"] = old["source_bindings"][changed]["sha256"]
            _write(path, catalog)
            with self.assertRaisesRegex(checker.RecordFamilyAuthorizationError, "source binding paths/digests/order mismatch"):
                checker.validate_package(root)
        ...
        implementation_rows = [
            {
                ...
                "interface_path": "smart-contracts/interfaces/stream/metadata/IStreamCollectionMetadata.sol",
                "interface_sha256": _sha256(
                    root / "smart-contracts/interfaces/stream/metadata/IStreamCollectionMetadata.sol"
                ),
            ...
            },
            {
                ...
                "interface_path": "smart-contracts/interfaces/stream/preservation/IStreamPreservationRecords.sol",
                "interface_sha256": _sha256(
                    root / "smart-contracts/interfaces/stream/preservation/IStreamPreservationRecords.sol"
                ),
                "address": "0x" + "51" * 20,
                "record_family_registry": classifier["host_address"],
                "configuration_authority": classifier["configuration_authority"],
