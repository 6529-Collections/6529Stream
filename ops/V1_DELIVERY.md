...
The current source checkpoint is 2b333b74, including curated callers with 375 root
...
passes. Actual-token museum capture 1db47ad9 passes 62 root tests and independent
...
These paragraphs retain the capabilities and boundaries at each recorded commit.
The latest integration above supersedes their then-pending assignments.
...
| Artist builder | Recovery after completed rotation, then remaining historical, posthumous and supersession profiles |
| Snapshot builder | Actual retained snapshot publication, canonical manifests, current/locked reads and preservation joins |
...
| SALE-02 | Dutch schedule, clearing rebates and maximum-price excess credits conserve funds | Revenue; PAY-04 | Building: standard native Dutch and supplemental settlement have independently reviewed current composition. Clearing consumer `d807421e` has 59 reviewed domain cases, five fuzz properties and both compiler profiles; two actual-current Safe clearing cases pass independent review (`d9d7d46b`). An earlier full-current trace measures 8,755,856 gas in its first successful consumer call after prior setup/rejection/consent; it predates compressed storage and is not all-cold. Newer optimized functional captures do not establish current all-cold conformance to the 500,000 ceiling. Aggregate/storage optimization and shared mint/rights/settlement costs remain open; no cold gas or deployment acceptance |
