...
| Unit/integration tests | Tests cover admin guards, target-scoped function-admin permission regressions, domain-scoped pause controls, EIP-712/ERC-1271 drop authorization, auction custody and payment credits, fixed-price pull-payment credits, curator reward credits, current emergency-withdrawal boundaries, randomizer lifecycle/callback validation, request-level randomizer reserve lifecycle accounting, randomness pending/stale/failed/final metadata behavior, ERC-4906 metadata update signaling, raw-output hash storage, dependency-script encoding hashes, supply/replay/freeze invariants, auction-consistency invariants, reusable protocol state-machine smoke coverage, explicit local-initialization regressions, vendored utility-library regressions, and retained airdrop mint-accounting behavior; broader adversarial P1 tests are still missing | `scripts/check.ps1`; `forge test -vvv`; targeted ADV-001 command: `forge test --match-path test/StreamProtocolStateMachine.t.sol -vvv` | P0 regression and integration suite exists |
...
| Protocol state-machine smoke | A reusable harness composes fixed-price mint, auction bid/settlement, payment credits, pause/signer/cancel controls, randomness finalization, metadata mutation, and freeze into one deterministic cross-contract flow | `test/helpers/ProtocolStateMachine.sol`, `test/StreamProtocolStateMachine.t.sol` | Passing locally for ADV-001 issue #370: the harness deploys the local stack, mints a fixed-price drop, mints and settles an auction after an outbid, withdraws known fixed-price and auction credits, proves pause blocks drop execution and metadata mutation, rotates the drop signer, cancels a pending drop, verifies immediate-randomizer final metadata state, mutates token metadata, freezes the collection, and reasserts owed-balance/surplus coverage without production contract changes. Future ADV work should extend this helper with fuzzed adversarial sequences, stale-provider permutations, and fork-aware scenarios. | [`#370`](https://github.com/6529-Collections/6529Stream/issues/370) | Gate D/Gate F | TBD |
