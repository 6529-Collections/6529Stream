...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 23,781 bytes with 795 bytes of EIP-170 headroom, above the
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 23,781-byte
  runtime measurement leaves 795 bytes of headroom, so PRs with large
