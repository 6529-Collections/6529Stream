...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 23,661 bytes with 915 bytes of EIP-170 headroom, above the
  384-byte minimum release floor and 512-byte warning threshold. Large
  non-trivial Core work should still measure bytecode deltas or explicitly
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 23,661-byte
  runtime measurement leaves 915 bytes of headroom, so PRs with large
