...
| Event reconstructability from emitted logs | Representative drop, minter, auction, payment, and admin flows can be rebuilt by an indexer-style test harness using emitted logs plus documented read-after-event calls | `test/StreamEventReconstructability.t.sol`, `docs/integrations/events-and-indexing.md` | Active issue #440: add executable reconstructability coverage for the event/read model and document any remaining intentional read gaps as follow-up issues | [`#440`](https://github.com/6529-Collections/6529Stream/issues/440) | Gate D/Gate G | TBD |
