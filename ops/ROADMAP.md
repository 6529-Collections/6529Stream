...
- The current smoke baseline builds the repo, runs Foundry tests, checks
  ...
  runbook, checks no-secret drop authorization signing fixtures and unsigned
  payload-generator examples, and runs the local deployment and auction
...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  ceremonies, production signer custody/tooling and retained signing evidence,
  ...
  local signing fixtures and unsigned payload-generator examples document the
  fixed-price EOA, auction EOA, and ERC-1271 mock contract-signer paths.
  `StreamMinter` and
