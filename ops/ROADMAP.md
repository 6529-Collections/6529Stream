...
| Field | Value |
| --- | --- |
| Last verified | `2026-06-11 10:42 UTC` local checksum-bundle PR candidate validation; CI TBD |
...
| Release artifacts | Partial deterministic baseline: ABI checksums, bytecode checksums, interface IDs, event topic catalog, ABI compatibility baseline, local deployment manifest checksum, local address book, and signable release checksum bundle are generated from committed inputs; detached signatures, signed release tags, ABI change approval policy, production address books, and verified live addresses remain missing | `python scripts/generate_release_artifacts.py --check`, `python scripts/check_abi_compatibility.py --check`, `python scripts/generate_deployment_manifest.py --check`, `python scripts/generate_address_books.py --check`, `python scripts/generate_release_checksums.py --check`, `release-artifacts/latest/`, `release-artifacts/baselines/v0.1.0/abi-surface.json`, `deployments/schema/address-book.schema.json`, `deployments/examples/anvil-6529stream-v0.1.0-001.json`, and `deployments/address-books/anvil-6529stream-v0.1.0-001.json` | ABIs, manifests, checksums, and verified addresses published |
...
- Add ABI compatibility diff checks for every release. Initial production
  ...
  change approval/version policy remains future work.
