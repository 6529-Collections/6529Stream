...
- Continue [`P1-META-006`](https://github.com/6529-Collections/6529Stream/issues/51):
  ...
  attribute fragments that can break out of the enclosing attributes array.
- Add metadata schema and golden-file tests for `name`, `description`, `image`,
  `attributes`, and `animation_url`.
- Complete the remaining escaping work for generated HTML, JavaScript strings
  and arrays, URI policy, invalid UTF-8 policy, and untrusted token data.
...
| Metadata escaping and render safety | JSON, HTML, JavaScript, raw attributes, URI, and size-limit inputs are escaped, validated, or rejected | `test/StreamMetadataEscaping.t.sol` | Partial: on-chain JSON string fields are escaped for schema-v1 output, raw attribute fragments reject literal control characters, unterminated strings, unbalanced delimiters, and unquoted array/object breakout attempts, and decoded metadata is parser-checked for hostile collection/image strings. Remaining work: generated HTML/JavaScript escaping or rejection, semantic attribute schema validation or structured attributes, URI policy, invalid UTF-8 policy, numeric size limits, and render-sandbox tests. | [`P1-META-006`](https://github.com/6529-Collections/6529Stream/issues/51) | Gate D | TBD |
