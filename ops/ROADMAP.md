...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  EIP-170-deployable at 24,135 bytes with 441 bytes of headroom under the
  ...
  headroom sequence reached 21,792 bytes, then CON-012 added roughly 2,360 bytes
  ...
  24,135-byte build with a 17-byte net reduction while mandatory target hooks
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  non-trivial Core feature work is 512 bytes. The current 24,135-byte runtime
  measurement leaves 441 bytes of headroom, while the approved
