...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 23,159 bytes with 1,417 bytes of EIP-170 headroom after moving
  collection and dependency script assembly into the linked renderer library,
  above the 384-byte minimum release floor and 512-byte warning threshold.
  Large non-trivial Core work should still measure bytecode deltas or
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 23,159-byte
  runtime measurement leaves 1,417 bytes of headroom, so PRs with large
