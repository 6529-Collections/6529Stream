...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  after the current mint-manager hook slice: the IR-optimized runtime is
  EIP-170-deployable at 24,128 bytes with 448 bytes of headroom under the
  accepted development exception `CORE-SPEND-2026-06-24-001` for CON-012. The
  ...
  headroom sequence reached 21,792 bytes, then CON-012 added exactly 2,360 bytes
  of manager/prepared-mint hooks while the legacy Drops/Minter path remained
  live. The atomic operation-identity cutover retired that duplicate path and
  the Core lifetime operation-root replay map, producing a measured
  24,128-byte build with a 24-byte net reduction while mandatory target hooks
  remain open. The pre-genesis cutover pins one permanent target ABI, migrates
  satellites and callers before deleting legacy paths, and permits only
...
- A clean-main reviewer reassessment at commit
  ...
  size discipline.
  Current public beta is still blocked by the 28 Open first-party production
  ...
  issue #684 governed-parameter candidate host/evidence completion, retained live ceremony evidence,
...
28 Open first-party production Slither High/Medium rows plus the separately
...
for row-level review and disposition of 28 Open first-party production Slither
...
[issue #658](https://github.com/6529-Collections/6529Stream/issues/658) for the
28 Open first-party production Slither High/Medium findings and independent
...
34. [`P1-SIZE-001`](https://github.com/6529-Collections/6529Stream/issues/115) / P1/CODE+TEST+OPS: Recover sustainable `StreamCore` bytecode headroom or define an explicit size budget before adding further non-trivial Core feature work. The current measurement shows `StreamCore` at 24,128 runtime bytes with 448 bytes of EIP-170 headroom, 24 bytes smaller than the pre-cutover transitional baseline and above the interim 384-byte development floor but below the 512-byte warning threshold. The approved bytecode-spend baseline remains 22,184 bytes and `CORE-SPEND-2026-06-24-001` accepts the CON-012 development slice, but the normative production deployment gate requires 2,000 bytes and does not permit interim exceptions; [issue #654](https://github.com/6529-Collections/6529Stream/issues/654) tracks the blocking recovery.
...
- `ops/SLITHER_BASELINE.json` records the exact normalized first-party
  production set: 3 High and 25 Medium rows, all Open. A fast metadata/parity
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  non-trivial Core feature work is 512 bytes. The current 24,128-byte runtime
  measurement leaves 448 bytes of headroom, while the approved
  bytecode-spend baseline remains 22,184 bytes with 2,392 bytes of baseline
  EIP-170 margin and `CORE-SPEND-2026-06-24-001` accepts this development
  slice's spend. The governing production deployment gate requires 2,000 bytes
  of headroom and rejects interim exceptions; issue
  [`#654`](https://github.com/6529-Collections/6529Stream/issues/654) tracks the
  required recovery.
...
- Source commit: `8b7ce86396fa943ef332b6831e79a998237285a9`.
- Captured at: `2026-07-27T10:29:17Z`.
- Live gate: `python scripts/check_slither_baseline.py --run-slither`.
- Status: exact normalized drift is gated; all 28 first-party production rows
  remain Open under issue #658.
- Raw analyzed run: 3,131 findings across all impacts/scopes: 49 High, 794
  Medium, 1,251 Low, 997 Informational, and 40 Optimization. High/Medium scope
  totals are first-party production `3/25/28`, vendored `1/9/10`, test
  `45/753/798`, script `0/7/7`, and other `0/0/0`.
...
| Pending disposition | 22 | Open | Complete row-level review; no implicit acceptance or suppression |
...
| `StreamCore` bytecode headroom | Optimized failure paths keep the same guarded behavior, expose typed selectors, and production bytecode retains at least 2,000 bytes of EIP-170 headroom with no deployment-time exception | `test/StreamCoreCustomErrors.t.sol`, `test/StreamMetadataUtf8.t.sol`, `test/StreamRandomizerLifecycle.t.sol`, `test/StreamRoyalty.t.sol`, production size gate, `scripts/check_contract_size_budget.py`, `scripts/check_release_mode.py --phase production-release` | Passing ordinary deployability and interim development floor locally: selected admin, artist-signature, metadata-array length, final-supply timing, missing collection data, and unminted token metadata failures now use direct selector regressions; renderer-linked guard consolidation lets `StreamCore` enforce production UTF-8 for guarded metadata writes; lifecycle-aware stale/failed metadata state display remains covered; freeze metadata hash helpers now live in `StreamMetadataRenderer`; the old-randomizer lifecycle probe keeps unsupported-provider and failed-pending-probe migration behavior covered; unused inherited ERC-2981 storage/admin machinery was replaced with equivalent fixed royalty logic while preserving `royaltyInfo` and ERC-2981 interface support; off-chain URI/name formatting plus randomizer lifecycle probes now live in the linked renderer library; collection/dependency script assembly now lives in the linked renderer library while preserving `retrieveGenerativeScript`; field-specific metadata validation profiles now live in the linked renderer library while preserving public `StreamCore` size constants, custom-error selectors, and metadata output behavior; tokenURI/metadata-state dispatch helpers now live in the linked renderer library while preserving exact metadata output bytes and final-hash override behavior; CON-008 / issue #577 recovers 392 bytes with behavior-preserving repeated-storage caching plus invariant-bounded unchecked counter arithmetic; CON-012 adds manager-only mint hooks plus authoritative token identity reads under accepted development exception `CORE-SPEND-2026-06-24-001`; and the atomic operation-identity cutover retires the duplicate legacy path and Core lifetime root replay map for a measured 24-byte net reduction. The artifact-backed current build measures `StreamCore` at 24,128 runtime bytes with 448 bytes of EIP-170 headroom. That passes the interim 384-byte development floor but fails the normative 2,000-byte deployment requirement by 1,552 bytes; the approved baseline remains 22,184 bytes with 2,392 bytes of margin, and interim exceptions cannot satisfy production release mode. | [`P1-SIZE-001`](https://github.com/6529-Collections/6529Stream/issues/115), [`StreamCore UTF-8 size follow-up`](https://github.com/6529-Collections/6529Stream/issues/125), [`Metadata stale/failed state display`](https://github.com/6529-Collections/6529Stream/issues/130), [`StreamCore release-floor recovery`](https://github.com/6529-Collections/6529Stream/issues/132), [`CON-005`](https://github.com/6529-Collections/6529Stream/issues/414), [`#420`](https://github.com/6529-Collections/6529Stream/issues/420), [`#430`](https://github.com/6529-Collections/6529Stream/issues/430), [`#432`](https://github.com/6529-Collections/6529Stream/issues/432), [`#434`](https://github.com/6529-Collections/6529Stream/issues/434), [`#577`](https://github.com/6529-Collections/6529Stream/issues/577), [`#654`](https://github.com/6529-Collections/6529Stream/issues/654) | Gate D/Gate G | TBD |
