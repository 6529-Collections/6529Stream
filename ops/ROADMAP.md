...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 22,184 bytes with 2,392 bytes of EIP-170 headroom after moving
  collection/dependency script assembly, field-specific metadata validation
  profiles, and tokenURI dispatch helpers into the linked renderer library,
  above the 384-byte minimum release floor and 512-byte warning threshold. Large
...
34. [`P1-SIZE-001`](https://github.com/6529-Collections/6529Stream/issues/115) / P1/CODE+TEST+OPS: Recover sustainable `StreamCore` bytecode headroom or define an explicit size budget before adding further non-trivial Core feature work. The current rebased measurement shows `StreamCore` at 22,184 runtime bytes with 2,392 bytes of EIP-170 headroom, above the 384-byte minimum release floor and 512-byte warning threshold, so large non-trivial Core work still needs measured bytecode deltas or an explicit size-budget exception.
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 22,184-byte
  runtime measurement leaves 2,392 bytes of headroom, so PRs with large
