...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 22,390 bytes with 2,186 bytes of EIP-170 headroom after moving
  collection/dependency script assembly and field-specific metadata validation
  profiles into the linked renderer library, above the 384-byte minimum release
  floor and 512-byte warning threshold. Large non-trivial Core work should
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 22,390-byte
  runtime measurement leaves 2,186 bytes of headroom, so PRs with large
