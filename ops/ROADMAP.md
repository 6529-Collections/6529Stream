...
| Field | Value |
| --- | --- |
| Last verified | `2026-06-14 07:41 UTC` for current issue #314 first-party interface formatting-deferral retirement; local validation includes focused formatting checks, deployment/evidence/release artifact drift checks, ABI compatibility, and the full Windows `scripts\check.ps1` gate after fixing PR #316 CI deployment-manifest drift |
| OS tested | Windows full local validation for issue #314; Windows local validation for Queue Item 151 / PR #315; Linux GitHub Actions passed for latest merged baseline PR #315 run `27491250707` on final head `f038205ea0d1cfb591e8d78b5c9c93316e4466fd`, with merged baseline commit `e6cb5ce4029ee1f0eb2428b1fb8637041e6cba83` |
| Foundry version | `v1.7.1` |
| Solidity compiler version | `0.8.19` |
| Slither version | `0.11.5` |
| CI run | Latest merged baseline PR #315 final CI run `27491250707` passed on head `f038205ea0d1cfb591e8d78b5c9c93316e4466fd`; CodeRabbit status was success with no unresolved review threads after explicit review request comment `4700977903`, review-finished reply `4700978066`, merge-decision comment `4700999733`, and PR #315 squash-merged as `e6cb5ce4029ee1f0eb2428b1fb8637041e6cba83` |
...
| Formatting | Current issue #314 local validation retires first-party interface deferrals: 27 formatting-required files pass `forge fmt --check`, and the raw all-files diagnostic is allowed to fail only for 24 documented deferred legacy/provenance-sensitive files | `python scripts/test_solidity_formatting.py`, `python scripts/check_solidity_formatting.py`, `make fmt-check`; raw diagnostic: `forge fmt --check smart-contracts` | Scoped gate passes in CI; remaining deferred files retired through focused formatting/provenance PRs |
...
| Solidity formatting scoped gate | Formatting-required Solidity files stay formatted and the deferred raw all-files formatting baseline cannot grow silently | `scripts/test_solidity_formatting.py`, `scripts/check_solidity_formatting.py`, `make fmt-check`; raw diagnostic `forge fmt --check smart-contracts` | Passing locally for issue #314: `INextGenCore2.sol`, `IStreamDrops.sol`, and `IStreamMinter.sol` are formatted and removed from `DEFERRED_FORMATTING_FILES`; parser tests continue to cover Windows/POSIX diff headers, required-file filtering, exact deferred-baseline acceptance, unexpected new unformatted files, fixed deferred files without manifest updates, and missing baseline files; the checker now confirms 27 formatting-required files and 24 deferred legacy/provenance-sensitive raw `forge fmt` diffs. PR #312 introduced the gate, PR #315 recorded the latest merged baseline, and remaining deferred-file retirement will continue through focused provenance/formatting PRs. Local validation includes `make fmt-check`, deployment/evidence/release artifact drift checks, ABI compatibility, and full Windows `scripts\check.ps1` after fixing PR #316 CI deployment-manifest drift. | [`#311`](https://github.com/6529-Collections/6529Stream/issues/311), [`PR #312`](https://github.com/6529-Collections/6529Stream/pull/312), [`PR #315`](https://github.com/6529-Collections/6529Stream/pull/315), [`#314`](https://github.com/6529-Collections/6529Stream/issues/314) | Gate A/Gate G | TBD |
