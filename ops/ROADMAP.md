...
- The current smoke baseline builds the repo, runs Foundry tests, checks
  ...
  runbook, checks no-secret drop authorization signing fixtures, and runs the
  local deployment and auction ceremony rehearsals. It still does not prove
...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  local signing fixtures document the fixed-price EOA, auction EOA, and
  ERC-1271 mock contract-signer paths. `StreamMinter` and
