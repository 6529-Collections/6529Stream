...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  collection/dependency script assembly, field-specific metadata validation
  profiles, tokenURI dispatch helpers into the linked renderer library, and
  CON-008 measured micro-optimizations into Core. The approved bytecode-spend
  ceiling remains 22,184 bytes with 2,392 bytes of baseline margin. Current
  headroom remains above the 384-byte minimum release floor and 512-byte warning
  threshold. Large
  non-trivial Core work should
  still measure bytecode deltas or explicitly spend an approved size budget
  first.
...
34. [`P1-SIZE-001`](https://github.com/6529-Collections/6529Stream/issues/115) / P1/CODE+TEST+OPS: Recover sustainable `StreamCore` bytecode headroom or define an explicit size budget before adding further non-trivial Core feature work. The current CON-008 measurement shows `StreamCore` at 21,792 runtime bytes with 2,784 bytes of EIP-170 headroom, above the 384-byte minimum release floor and 512-byte warning threshold, while the approved bytecode-spend ceiling remains 22,184 bytes. Large non-trivial Core work still needs measured bytecode deltas or an explicit size-budget exception.
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 21,792-byte
  runtime measurement leaves 2,784 bytes of headroom, while the approved
  bytecode-spend ceiling remains 22,184 bytes with 2,392 bytes of baseline
  margin, so PRs with large
