...
| Field | Value |
| --- | --- |
| Last verified | `2026-07-24` at merged PR #686 / `b4af30a5`: canonical 53-target isolated release build, focused gas/time/executor/holder/profile/vector tests, exact retained and live Slither comparison, `make release-artifacts-verify` in 375.1 seconds, and the full Windows `scripts\check.ps1` gate in 2,107.4 seconds all passed. On current branch `codex/governed-parameter-inventory`, the 32-test inventory suite and ordinary 50-binding planning check plus the reconciled 19-test identifier suite/check pass; production completeness fails as intended while 129 facts remain unresolved, candidate completion stays categorically blocked pending #656 structured instance/linked-library reconciliation, and complete evidence stays blocked pending #684's candidate-bound measurement/reproduction/raise-chain model. Combined docs, generated-tail, and full-gate validation are pending before PR creation |
...
| CI run | PR #686 passed CI and CodeRabbit before squash merge `b4af30a58c22f79bafad241c6e4fab7a4a76063b`; current `codex/governed-parameter-inventory` branch CI is pending until PR creation |
