...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 24,157 bytes with 419 bytes of EIP-170 headroom after the
  accepted `CORE-SPEND-2026-06-24-001` exception for CON-012. The stricter
  approved bytecode-spend baseline remains 22,184 bytes with 2,392 bytes of
  baseline EIP-170 margin. Current headroom remains above the 384-byte minimum
  release floor but below the 512-byte warning threshold, so large non-trivial
  Core work should still measure bytecode deltas or explicitly spend an
  approved size budget first.
...
| Production size | Production deployable contracts pass the IR-optimized size gate; the P1-SIZE-001 recovery pass replaces selected legacy `StreamCore` string reverts with typed custom errors, rejects missing collection data before final-supply math, the Core UTF-8 pass moves reusable metadata guards into the linked renderer library while replacing inherited `_requireMinted` string reverts with `TokenNotMinted()`, the release-floor recovery moves freeze hash helpers into the renderer while preserving lifecycle migration behavior, the size-budget pass replaces unused inherited ERC-2981 machinery with equivalent fixed royalty logic, PR #421 moved off-chain URI/name formatting plus randomizer lifecycle probes into the linked renderer library, issue #430 moved collection and dependency script assembly into the linked renderer library, issue #432 moves field-specific metadata validation profiles into the linked renderer library, issue #434 moves tokenURI/metadata-state dispatch helpers into the linked renderer library, CON-008 recovers measured micro-optimization headroom, and CON-012 adds the manager-only mint hooks plus token identity reads under accepted bytecode-spend exception `CORE-SPEND-2026-06-24-001`; current `StreamCore` runtime is 24,157 bytes with 419 bytes of EIP-170 headroom, above the 384-byte minimum release floor but below the 512-byte warning threshold, while the approved spend baseline remains 22,184 bytes | `forge build --sizes --via-ir --skip test --skip script --force`; `python scripts/check_contract_size_budget.py` | Production size gate and budget checker pass in CI and deployment scripts use the same profile, with the agreed minimum `StreamCore` margin recovered or an explicit size-budget exception accepted before release |
...
34. [`P1-SIZE-001`](https://github.com/6529-Collections/6529Stream/issues/115) / P1/CODE+TEST+OPS: Recover sustainable `StreamCore` bytecode headroom or define an explicit size budget before adding further non-trivial Core feature work. The current CON-012 measurement shows `StreamCore` at 24,157 runtime bytes with 419 bytes of EIP-170 headroom, above the 384-byte minimum release floor but below the 512-byte warning threshold, while the approved bytecode-spend baseline remains 22,184 bytes and `CORE-SPEND-2026-06-24-001` accepts this slice's spend. Large non-trivial Core work still needs measured bytecode deltas or an explicit size-budget exception.
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 24,157-byte
  runtime measurement leaves 419 bytes of headroom, while the approved
  bytecode-spend baseline remains 22,184 bytes with 2,392 bytes of baseline
  EIP-170 margin and `CORE-SPEND-2026-06-24-001` accepts this slice's spend, so PRs with large
...
| 0008 Revenue splits and royalty resolver | Pre-launch design request | Proposed | `docs/adr/0008-revenue-splits-and-royalty-resolver.md` | Revenue split wallet and Core-native ERC-2981 implementation planning |
