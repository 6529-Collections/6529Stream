...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 24,172 bytes with 404 bytes of EIP-170 headroom after the
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 24,172-byte
  runtime measurement leaves 404 bytes of headroom, while the approved
