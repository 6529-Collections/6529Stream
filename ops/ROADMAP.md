...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  deployable at 21,780 bytes with 2,796 bytes of EIP-170 headroom after moving
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  ...
  future non-trivial Core feature work is 512 bytes. The current 21,780-byte
  runtime measurement leaves 2,796 bytes of headroom, while the approved
...
| MEV and timing attack model | Signed-payload submission semantics, fixed-price payer binding, inclusive drop deadlines, exact-end auction bids, strict post-end rejection, near-end extension, custody, and accounting survive bounded adversarial timing attempts | `test/StreamMEVTiming.t.sol`, `docs/threat-model.md` | Active issue #444: local branch adds focused tests for third-party free signed-payload submission without recipient theft, paid-drop payer binding without failed-attempt consumption, inclusive deadline behavior, expired-attempt preservation, exact-end auction bid acceptance/extension, late-bid rejection without custody/accounting mutation, and near-end outbid credit/extension behavior | [`#444`](https://github.com/6529-Collections/6529Stream/issues/444) | Gate D/Gate F | TBD |
