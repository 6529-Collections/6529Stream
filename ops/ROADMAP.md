...
- The current smoke baseline builds the repo, runs Foundry tests, checks
  ...
  browser sandbox, enforces the production-contract EIP-170 development gate,
...
- Known remaining production-readiness blockers include fork-level invariant
  ...
  coverage. The completed CON-002 minter read-model slice added additive
  ...
  production source. P0-INIT-001 fixed the then-known first-party production
  `uninitialized-local` rows; later-added surfaces now contribute 7 Open rows
  tracked under issue #658. P0-META-001 dependency script segment-safe
  ...
  The permanent `StreamCore` target in
  [`release-artifacts/latest/bytecode-release-proof.json`](../release-artifacts/latest/bytecode-release-proof.json)
  passes the non-waivable 2,000-byte deployment margin and restores the stricter
  22,184-byte objective under the production via-IR profile. Historical
  measurements are non-additive. The Core/mint critical path was:
  [#688](https://github.com/6529-Collections/6529Stream/issues/688) now
  implements the accepted ledger-owned batch root plus `N` token operation IDs
  and atomically removes the Core lifetime replay storage; issue
  [#672](https://github.com/6529-Collections/6529Stream/issues/672) now proves
  and enforces the actual post-entropy call boundary, full governed stipend,
  EOA completion reserve, and rollback in the permanent Core. The real
  restricted ERC-4906 single/batch refresh emitters required by #667 are also
  implemented and included in the runtime measurement. The recovery consumer
  probes the
...
- A clean-main reviewer reassessment at commit
  ...
  The artifact-backed Core headroom requirement under issue #654 is satisfied.
  Production remains additionally blocked by concrete #670 satellite
  implementation/security acceptance: ADR 0021's revenue architecture is
  Accepted but source-blocked, while ADR 0022's artist architecture remains
  Proposed. Issue #656 instance-aware genesis reconciliation, issue #684 governed-parameter
  candidate host/evidence completion, retained live ceremony evidence,
  production deployment manifests, production signatures, signed tags,
  explorer verification, live randomizer evidence, #656-bound Governance V2
  candidate deployment/rehearsal/review evidence, and post-audit remediation
  evidence.
...
| Field | Value |
| --- | --- |
| Last verified | `2026-07-24` at merged PR #686 / `b4af30a5`: canonical 53-target isolated release build, focused gas/time/executor/holder/profile/vector tests, exact retained and live Slither comparison, `make release-artifacts-verify` in 375.1 seconds, and the full Windows `scripts\check.ps1` gate in 2,107.4 seconds all passed. On PR #687, the 32-test inventory suite and ordinary 50-binding planning check plus the reconciled 19-test identifier suite/check pass; production completeness fails as intended while 129 facts remain unresolved, candidate completion stays categorically blocked pending #656 structured instance/linked-library reconciliation, and complete evidence stays blocked pending #684's candidate-bound measurement/reproduction/raise-chain model. Full local docs, generated-tail, release-artifact, and aggregate validation passed; the current CI and incremental CodeRabbit rerun follow a clean-checkout checksum correction and the first review's three minor documentation findings |
...
| CI run | PR #686 passed CI and CodeRabbit before squash merge `b4af30a58c22f79bafad241c6e4fab7a4a76063b`; PR #687 is open, and its CI plus incremental CodeRabbit review are rerunning after the clean-checkout checksum correction and first-pass review fixes |
...
| [`#46` Metadata schema and golden files](https://github.com/6529-Collections/6529Stream/issues/46) | Completed for current schema-v1 surface | PRs #81, #83, and #131; `docs/metadata.md`; pending/stale/failed/final fixtures in `test/fixtures/metadata/`; `test/regression/legacy/metadata/StreamMetadataGolden.t.sol` | Future metadata schema migrations should get new focused issues |
| [`#47` Collection freeze boundaries](https://github.com/6529-Collections/6529Stream/issues/47) | Completed for current `StreamCore` freeze boundary | PRs #84, #85, #86, and #131; `test/regression/legacy/metadata/StreamMetadataFreeze.t.sol`, `test/regression/legacy/dependencies/StreamDependencyRegistry.t.sol`, `test/regression/legacy/core/StreamCoreBurn.t.sol`; freeze manifest docs | Broader fork/invariant coverage belongs in future Gate D work, not this umbrella |
| [`#48` Dependency registry versioning](https://github.com/6529-Collections/6529Stream/issues/48) | Completed for versioning, immutability, provenance, collection pins, frozen-output stability, and production runbook coverage | PRs #85, #118, and #136; `test/regression/legacy/dependencies/StreamDependencyRegistry.t.sol`; dependency artifact manifest generator/tests; `docs/metadata.md` dependency sections; `docs/dependency-operations.md` | None |
...
| [`#124` Invalid UTF-8 production metadata inputs](https://github.com/6529-Collections/6529Stream/issues/124) | Completed | PRs #126 and #127; `test/regression/legacy/metadata/StreamMetadataUtf8.t.sol`; `docs/metadata.md` UTF-8 policy; release artifacts refreshed | None |
...
not be bypassed. Only checker-authorized public-beta external-evidence rows may
use active, explicit risk acceptance; external-audit evidence, the technical
Slither gate, production Core/genesis rules, and every production requirement
are non-waivable.
...
- Production drop launch requires Gates A through G complete and the strict
  production release-mode gate to pass.
- Audited release requires completed external-audit evidence; an audit waiver
  cannot satisfy public-beta or production release mode.
...
Status: In Progress.
Owner: TBD.
Blocking issue: [issue #658](https://github.com/6529-Collections/6529Stream/issues/658) for the
...
- Every first-party production Slither High/Medium row is remediated or has an
  issue-linked reviewed disposition; exact normalized drift CI is green.
...
`test/regression/legacy/protocol/StreamPaymentsInvariant.t.sol`; metadata golden and ERC-4906 event
baselines added in `test/regression/legacy/metadata/StreamMetadataGolden.t.sol` and
`test/regression/legacy/metadata/StreamMetadataEvents.t.sol`; focused local gas snapshot baseline added in
`test/gas/StreamGasSnapshot.t.sol` and
`release-artifacts/baselines/v0.1.0/gas-snapshot.snap`; local
supply/replay/freeze invariant baseline added in
`test/regression/legacy/protocol/StreamSupplyReplayFreezeInvariant.t.sol`; local auction-consistency
invariant baseline added in `test/regression/legacy/auctions/StreamAuctionInvariant.t.sol`; focused
request-level randomizer reserve lifecycle tests added in
`test/unit/entropy/StreamRandomizerPayments.t.sol`; reusable ADV-001 protocol
state-machine smoke coverage added in `test/regression/legacy/protocol/StreamProtocolStateMachine.t.sol`
...
and [issue #656](https://github.com/6529-Collections/6529Stream/issues/656) for
instance-aware genesis manifest, address-book, and verification reconciliation.
...
Status: In Progress. The local package index and scaffolding are complete;
...
`docs/threat-model.md`, `tools/docs/check_audit_package.py`, and
`tools/docs/check_architecture_threat_model.py` are merged from PRs #159 and #161;
...
- Static-analysis inventory is reproducible, and every first-party production
  High/Medium row is remediated or has an issue-linked reviewed disposition.
...
Status: In Progress.
Owner: TBD.
The artifact-backed Core headroom requirement under
[issue #654](https://github.com/6529-Collections/6529Stream/issues/654) is
satisfied. Blocking issues:
[issue #670](https://github.com/6529-Collections/6529Stream/issues/670) for
concrete satellite implementation/security acceptance,
...
for complete governed-parameter candidate hosts and evidence, and
[issue #658](https://github.com/6529-Collections/6529Stream/issues/658) for the
30 Open first-party production Slither High/Medium findings and independent
review of `RISK-GOV-003`. Issue #685 supplies the local closed-world
action/native-value implementation and catalog, but exact candidate binding and
deployment evidence remain blocked by #656.
...
- Release evidence tracker issue closure readiness is checked via
  `python -m tools.release.check_release_evidence_issue_closure`, with optional
...
- Public docs.
- CI link.
- `docs/release-readiness.md` and `python -m tools.release.check_release_readiness`.
...
34. [`P1-SIZE-001`](https://github.com/6529-Collections/6529Stream/issues/115) / P1/CODE+TEST+OPS: Sustainable `StreamCore` headroom is restored in the permanent target; the canonical proof passes the approved 22,184-byte objective. Future Core additions remain subject to the bytecode-spend policy and must preserve the deployment margin.
...
- Status: EOA and EIP-2098 typed authorization implemented in
  `smart-contracts/domains/mint/legacy/StreamDrops.sol`; ERC-1271 contract signer support is
...
- Blocks: Gate C.
- Dependencies: `P0-AUTH-ADR`.
- Status: Implemented in `smart-contracts/domains/mint/legacy/StreamDrops.sol` with target-state
  tests in `test/unit/mint/StreamDropsERC1271.t.sol`.
...
- `ops/SLITHER_BASELINE.json` records the exact normalized first-party
  ...
  preserves the Governance Executor's proposal-selected native-value authority
  after bounded assembly made it invisible to Slither.

Intended behavior:

- Every first-party production High/Medium finding is remediated with regression
  proof or receives an explicit, independently reviewable disposition. No row
  can become accepted or false positive merely because it matches the baseline.

Required code changes:

- Follow-up contract/test PRs are required for confirmed and actionable rows;
  issue #658 owns baseline-wide closure and issue linkage.
...
- Canonical machine-readable high/medium baseline in
  `ops/SLITHER_BASELINE.json` and its exact reviewer mirror in
  `ops/SLITHER_BASELINE.md`.
- Slither config remains free of finding suppressions.
...
- CI fails on added, removed, or changed normalized first-party production
  High/Medium findings; baseline equality never counts as acceptance.
...
- Emit phase update events. The CON-002 first slice covers
  ...
  as additive bridge events in issue #438 / PR #439.
...
- Keep the production-contract EIP-170 development gate in `make check`,
  Windows/Linux check scripts, and CI:
  `forge build --sizes --via-ir --skip test --skip script --force`. Keep the
  stricter 2,000-byte production deployment decision in opt-in release mode.
...
- Track `StreamCore` size as a release risk through the artifact-backed budget
  checker. The interim development floor is 384 bytes of EIP-170 runtime
  headroom under the IR-optimized profile; the warning threshold for future
  non-trivial Core feature work is 512 bytes. The permanent target's canonical
  proof is below the approved 22,184-byte objective and safely above the
  2,000-byte production minimum. Future changes remain subject to this budget
  without relying on the earlier development exception.
...
- Keep the exact normalized first-party production High/Medium drift gate in CI;
  added, removed, or semantically changed rows fail while row-level disposition
  remains Open. Baseline equality is inventory consistency, not acceptance.
...
- Keep `tools/docs/check_incident_response.py` and
  `tools/docs/test_incident_response.py` in the default local/CI gate.
...
- Keep the checked `CON-006` NatSpec coverage gate current:
  `tools/build/check_natspec_coverage.py` validates the release surface from
...
- Add changelog gate for release-impacting PRs. Initial `CHANGELOG.md`,
  `docs/release-policy.md`, `tools/docs/check_changelog.py`, and CI/local gate
  wiring exist.
...
- Add non-local release evidence metadata schema/checker. Initial
  ...
  `tools/release/check_non_local_release_evidence.py`, and focused tests validate the
...
- Every first-party production Slither High/Medium finding is remediated or has
  an issue-linked reviewed disposition, and the exact normalized drift gate is
  green.
...
- Non-local release evidence intake followed before any fork, testnet, live,
  ...
  pass `tools/release/check_non_local_release_evidence.py`.
- Public-beta evidence templates checked. Initial per-requirement templates
  ...
  validated by `tools/release/check_non_local_release_evidence.py`; they are starting
...
- Production-release evidence templates checked. Initial per-requirement
  ...
  by `tools/release/check_non_local_release_evidence.py`; they are starting points for
...
- Bounded assembly makes the Governance Executor's proposal-selected
  ...
  runtime and release catalog, but `RISK-GOV-003` stays High and open pending
  #656 exact candidate/deployment evidence and #658 independent review; the
  Governance V2 foundation is pre-audit and not production-ready.
...
| Classification | Count | Status | Required action |
| --- | ---: | --- | --- |
...
| External audit report retained artifact | Future external audit report evidence must retain a final report or public report reference, audited commit, audit scope, finding IDs and severities, remediation links, accepted-risk references, retest status, reviewer approval, and no-secret redaction fields | `tools/release/test_external_audit_report_evidence.py`, `tools/release/check_external_audit_report_evidence.py`, `release-artifacts/evidence/external-audit-report/external-audit-report-retained-artifact-template.md`, `release-artifacts/evidence/public-beta-templates/external-audit-report-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Merged in PR #354 / issue #353: a dedicated template and checker validate the external audit retained-artifact shape, packet/backlog/body-sync generators point issue #215 at the dedicated path, live issue #215 was updated from the generated body-sync payload, and `external_audit_report` remains `missing` until real reviewed evidence is retained. | [`#215`](https://github.com/6529-Collections/6529Stream/issues/215), [`#353`](https://github.com/6529-Collections/6529Stream/issues/353), [`PR #354`](https://github.com/6529-Collections/6529Stream/pull/354), [`#355`](https://github.com/6529-Collections/6529Stream/issues/355) | Gate F/Gate G | TBD |
| Testnet deployment rehearsal retained artifact | Future testnet deployment rehearsal evidence must retain Sepolia chain metadata, testnet block or transaction references, sanitized command transcript, sanitized Foundry broadcast, generated manifest, generated address book, explorer verification status, gas or invariant summary, reviewer approval, and no-secret redaction fields | `tools/deployment/test_testnet_deployment_rehearsal_evidence.py`, `tools/deployment/check_testnet_deployment_rehearsal_evidence.py`, `release-artifacts/evidence/testnet-deployment-rehearsal/testnet-deployment-rehearsal-retained-artifact-template.md`, `release-artifacts/evidence/public-beta-templates/testnet-deployment-rehearsal-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Template/checker support merged in PR #359 and EXT-018 hardening merged in PR #539 / issue #538. Future pending or reviewed Sepolia retained references must be repo-relative files, may include one optional declared `sha256:<64 lowercase hex>` digest, and fail on path escapes, missing files, stale hashes, duplicate hashes, trailing hash text, credentialed URLs, provider/API-token-shaped URLs, and bare 64-hex strings. Packet/backlog/body-sync generators continue to point issue #217 at the dedicated path, and `testnet_deployment_rehearsal` remains `missing` until real reviewed evidence is retained. | [`#217`](https://github.com/6529-Collections/6529Stream/issues/217), [`#357`](https://github.com/6529-Collections/6529Stream/issues/357), [`#538`](https://github.com/6529-Collections/6529Stream/issues/538), [`PR #539`](https://github.com/6529-Collections/6529Stream/pull/539) | Gate E/Gate G | TBD |
| Public-beta verified-addresses retained artifact | Future public-beta verified-addresses evidence must retain Sepolia address books, Sepolia deployment manifests, source verification inputs, explorer verification evidence, bytecode release proof, release manifest/checksum digests, address-book/deployment-manifest agreement, explorer-source verification status, runtime-bytecode proof agreement, constructor/link-library verification, common explorer/indexer links, reviewer approval, and no-secret redaction fields for `verified_deployed_addresses` and `explorer_verification_status` tracker rows | `tools/release/test_public_beta_verified_addresses.py`, `tools/release/check_public_beta_verified_addresses.py`, `release-artifacts/evidence/public-beta-verified-addresses/public-beta-verified-addresses-retained-artifact-template.md`, `release-artifacts/evidence/public-beta-templates/verified-deployed-addresses-template.json`, `release-artifacts/evidence/public-beta-templates/explorer-verification-status-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Merged in PR #551; issue #550 closed completed with a dedicated retained-artifact template/checker before future reviewed public-beta address-book or explorer evidence is retained. Tracker issues #221 and #222 remain `missing` until real reviewed evidence is linked from the shared public-beta evidence manifest. | [`#221`](https://github.com/6529-Collections/6529Stream/issues/221), [`#222`](https://github.com/6529-Collections/6529Stream/issues/222), [`#550`](https://github.com/6529-Collections/6529Stream/issues/550), [`PR #551`](https://github.com/6529-Collections/6529Stream/pull/551) | Gate E/Gate G | TBD |
| Live metadata browser retained artifact | Future live metadata browser evidence must retain deployed mainnet contract addresses, token/collection IDs, browser-sandbox command or CI job, generated tokenURI output or digest, browser summary JSON, console/page error summary, dependency request summary, screenshot or transcript, reviewer approval, and no-secret redaction fields | `tools/release/test_live_metadata_browser_evidence.py`, `tools/release/check_live_metadata_browser_evidence.py`, `release-artifacts/evidence/live-metadata-browser/live-metadata-browser-retained-artifact-template.md`, `release-artifacts/evidence/production-release-templates/live-metadata-browser-evidence-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Template/checker support merged in PR #474; EXT-019 hardening merged in PR #541 / issue #540 to require repo-relative retained paths, optional declared `sha256:` digest checks, missing-file failures, duplicate/trailing hash rejection, credentialed/provider URL rejection, bare 64-hex rejection, and stronger no-secret scans before future reviewed live metadata browser evidence is retained. EXT-038 / issue #607 centralizes the shared symlink-safe retained-path helper so live metadata-browser evidence rejects symlinked leaf files and files reached through symlinked directories with the same policy as the other release evidence checkers. Packet/backlog/body-sync generators continue to point issue #473 at the dedicated path, and `live_metadata_browser_evidence` remains `missing` until real reviewed evidence is retained. | [`#473`](https://github.com/6529-Collections/6529Stream/issues/473), [`#540`](https://github.com/6529-Collections/6529Stream/issues/540), [`PR #541`](https://github.com/6529-Collections/6529Stream/pull/541), [`#607`](https://github.com/6529-Collections/6529Stream/issues/607) | Gate F/Gate G | TBD |
| Live deployment manifest retained artifact | Future live deployment manifest evidence must retain production input provenance, live chain ID, deployment version, broadcast manifest input, generated live deployment manifest, generated live address book, source verification inputs, release manifest/checksum digest references, reviewer approval, and no-secret redaction fields | `tools/release/test_live_deployment_manifest_evidence.py`, `tools/release/check_live_deployment_manifest_evidence.py`, `release-artifacts/evidence/live-deployment-manifest/live-deployment-manifest-retained-artifact-template.md`, `release-artifacts/evidence/production-release-templates/live-deployment-manifest-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Template/checker support merged in PR #574 / issue #573 for future pending or reviewed `live_deployment_manifest` evidence. The checker requires repo-relative non-symlink UTF-8 retained files, optional declared `sha256:` drift checks, no-secret scans, live mainnet chain ID 1, matching deployment version, nonzero finalized contract addresses, bytecode hashes, constructor arguments, labeled release manifest/checksum digest references, duplicate contract-name rejection, and address-book agreement. EXT-034 / issue #575 retains the follow-up live issue body/closure audit after issue #227 was reopened and synced. Tracker issue #227 remains `missing` until real reviewed live deployment manifest evidence is retained. | [`#573`](https://github.com/6529-Collections/6529Stream/issues/573), [`PR #574`](https://github.com/6529-Collections/6529Stream/pull/574), [`#575`](https://github.com/6529-Collections/6529Stream/issues/575), [`#227`](https://github.com/6529-Collections/6529Stream/issues/227) | Gate F/Gate G | TBD |
| Live ceremony retained artifact | Future live ceremony evidence must retain production deployment context, governance participants, ownership transfer, role grants/revokes, signer setup, metadata/freeze ceremony, auction ceremony, emergency controls, dry-run mint and auction evidence, monitoring handoff, live deployment manifest, live address book, Safe or multisig export, explorer transaction bundle, post-state views, release manifest/checksum digests, reviewer approval, and no-secret redaction fields | `tools/deployment/test_live_ceremony_evidence.py`, `tools/deployment/check_live_ceremony_evidence.py`, `release-artifacts/evidence/live-ceremony/live-ceremony-retained-artifact-template.md`, `release-artifacts/evidence/production-release-templates/live-ceremony-evidence-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Template/checker support merged in PR #476; EXT-020 hardening merged in PR #543 / issue #542 to require repo-relative retained paths, optional declared `sha256:` digest checks, non-symlinked UTF-8 files, missing-file failures, duplicate/trailing hash rejection, credentialed/provider URL rejection, bare 64-hex rejection, and stronger no-secret scans before future reviewed live ceremony evidence is retained. Packet/backlog/body-sync generators continue to point issue #228 at the dedicated path, and `live_ceremony_evidence` remains `missing` until real reviewed evidence is retained. | [`#228`](https://github.com/6529-Collections/6529Stream/issues/228), [`#542`](https://github.com/6529-Collections/6529Stream/issues/542), [`PR #543`](https://github.com/6529-Collections/6529Stream/pull/543) | Gate F/Gate G | TBD |
| Live randomizer operations retained artifact | Future live randomizer operations evidence must retain production provider addresses, adapter addresses, provider epochs, funding status, reserve status, request health, callback validation, pending-request migration, stale/failed/retry evidence, pause policy, emergency withdrawal boundary, monitoring handoff, live deployment manifest, live address book, randomizer operations JSON, provider dashboard/export, explorer transaction bundle, release manifest/checksum digests, reviewer approval, and no-secret redaction fields | `tools/deployment/test_live_randomizer_operations_evidence.py`, `tools/deployment/check_live_randomizer_operations_evidence.py`, `tools/deployment/check_randomizer_operations.py`, `release-artifacts/evidence/live-randomizer-operations/live-randomizer-operations-retained-artifact-template.md`, `release-artifacts/evidence/production-release-templates/live-randomizer-operations-evidence-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Template/checker support merged in PR #477; EXT-021 hardening merged in PR #545 / issue #544 to require repo-relative retained paths, optional declared `sha256:` digest checks, non-symlinked UTF-8 files, missing-file failures, duplicate/trailing hash rejection, credentialed/provider URL rejection, bearer/CLI secret rejection, bare 64-hex rejection, and stronger no-secret scans before future reviewed live randomizer operations evidence is retained. Packet/backlog/body-sync generators continue to point issue #229 at the dedicated path, and `live_randomizer_operations_evidence` remains `missing` until real reviewed evidence is retained. | [`#229`](https://github.com/6529-Collections/6529Stream/issues/229), [`#544`](https://github.com/6529-Collections/6529Stream/issues/544), [`PR #545`](https://github.com/6529-Collections/6529Stream/pull/545) | Gate F/Gate G | TBD |
| Production verified-addresses retained artifact | Future production verified-addresses evidence must retain live address books, live deployment manifests, source verification inputs, explorer verification evidence, bytecode release proof, release manifest/checksum digests, address-book/deployment-manifest agreement, explorer-source verification status, runtime-bytecode proof agreement, constructor/link-library verification, common explorer/indexer links, reviewer approval, and no-secret redaction fields for `production_address_books` and `live_explorer_verification` tracker rows | `tools/release/test_production_verified_addresses.py`, `tools/release/check_production_verified_addresses.py`, `release-artifacts/evidence/production-verified-addresses/production-verified-addresses-retained-artifact-template.md`, `release-artifacts/evidence/production-release-templates/production-address-books-template.json`, `release-artifacts/evidence/production-release-templates/live-explorer-verification-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | Template/checker support merged in PR #471; EXT-022 hardening merged in PR #547 / issue #546 to require repo-relative retained paths, optional declared `sha256:` digest checks, non-symlinked UTF-8 files, missing-file failures, duplicate/trailing hash rejection, credentialed/provider URL rejection, bearer/CLI secret rejection, bare 64-hex rejection, and existing address-book/manifest/explorer/bytecode proof agreement checks before future reviewed production verified-addresses evidence is retained. Packet/backlog/body-sync generators continue to point issues #225 and #230 at the dedicated path, and `production_address_books` and `live_explorer_verification` remain `missing` until real reviewed evidence is retained. | [`#225`](https://github.com/6529-Collections/6529Stream/issues/225), [`#230`](https://github.com/6529-Collections/6529Stream/issues/230), [`#546`](https://github.com/6529-Collections/6529Stream/issues/546), [`PR #547`](https://github.com/6529-Collections/6529Stream/pull/547) | Gate F/Gate G | TBD |
| Production release-signing retained artifact | Future production release-signing evidence must retain release version/tag, release commit, signer fingerprint, signer custody and rotation policy, checksum bundle path and digest, detached checksum signature evidence, signed Git tag verification evidence, verification command outputs, release manifest/checksum digest references, reviewer approval, and no-secret redaction fields for `production_signatures` and `signed_git_tag` tracker rows | `tools/release/test_production_release_signing_evidence.py`, `tools/release/check_production_release_signing_evidence.py`, `tools/release/test_release_signatures.py`, `tools/release/check_release_signatures.py`, `tools/release/test_signed_release_tag.py`, `tools/release/check_signed_release_tag.py`, `release-artifacts/evidence/production-release-signing/production-release-signing-retained-artifact-template.md`, `release-artifacts/evidence/production-release-templates/production-signatures-template.json`, `release-artifacts/evidence/production-release-templates/signed-git-tag-template.json`, `release-artifacts/latest/release-evidence-packet-index.json`, `release-artifacts/latest/release-evidence-issue-backlog.json`, `release-artifacts/latest/release-evidence-issue-body-sync.json`, `docs/non-local-release-evidence.md` | EXT-023 merged in PR #549 / issue #548 to replace the generic retained-artifact placeholder with a dedicated template/checker before future reviewed production release-signing evidence is retained. Tracker issues #223 and #224 remain `missing` until real reviewed evidence is linked from the shared release evidence manifest. | [`#223`](https://github.com/6529-Collections/6529Stream/issues/223), [`#224`](https://github.com/6529-Collections/6529Stream/issues/224), [`#548`](https://github.com/6529-Collections/6529Stream/issues/548), [`PR #549`](https://github.com/6529-Collections/6529Stream/pull/549) | Gate F/Gate G | TBD |
| Fork deployment rehearsal retained artifact | Reviewed fork deployment rehearsal evidence must retain fork command output, fork block reference, sanitized broadcast, generated manifest, generated address book, verification status, gas or invariant summary, release digest references, and reviewer approval without secrets | `tools/deployment/test_fork_deployment_rehearsal_evidence.py`, `tools/deployment/check_fork_deployment_rehearsal_evidence.py`, `release-artifacts/evidence/fork-deployment-rehearsal/fork-deployment-rehearsal-retained-artifact-template.md`, `release-artifacts/evidence/fork-deployment-rehearsal/fork-deployment-rehearsal-evidence.json`, `deployments/broadcasts/fork-mainnet-6529stream-v0.1.0-001-run-latest.json`, `deployments/examples/fork-mainnet-6529stream-v0.1.0-001-broadcast.json`, `deployments/address-books/fork-mainnet-6529stream-v0.1.0-001-broadcast.json`, `release-artifacts/latest/public-beta-evidence.json`, `docs/non-local-release-evidence.md`, `tools/release/test_release_evidence_packet_index.py` | Merged in PR #350: reviewed mainnet-fork evidence captures fork block `25316366`, fork hash `0xb7c7a456e0f1246fa4ee52de6fca99cc16628ce1eafd85b65b0f3d22f3933ee7`, sanitized broadcast `sha256:554852de9ecdefb01dae1a7724c47d4379e876d2ba5ee06778d9fef78258a08f`, generated fork manifest `sha256:087ca9ab36fe291efbd29994b9331e55d9570f56bd8dec67cc8720e9a47fe736`, generated fork address book `sha256:34e0d4b267a4d8fb035d6971b14fd3ec03f4df15133af40039b0f19a3fac57d4`, reviewed non-local evidence metadata, CodeRabbit status success on PR #347/#349/#350, supporting CI runs `27503447725` and `27504228132`, and final PR #350 CI run `27505334136` passed Windows PowerShell wrapper job `81295431136` and Foundry smoke job `81295431140`. The `fork_deployment_rehearsal` public-beta row is `complete`, issue #216 left the active backlog/link/body-sync set and closed completed, and overall public-beta readiness remains `blocked` on the remaining missing evidence rows. | [`#325`](https://github.com/6529-Collections/6529Stream/issues/325), [`PR #326`](https://github.com/6529-Collections/6529Stream/pull/326), [`#327`](https://github.com/6529-Collections/6529Stream/issues/327), [`PR #328`](https://github.com/6529-Collections/6529Stream/pull/328), [`#329`](https://github.com/6529-Collections/6529Stream/issues/329), [`PR #330`](https://github.com/6529-Collections/6529Stream/pull/330), [`#333`](https://github.com/6529-Collections/6529Stream/issues/333), [`#216`](https://github.com/6529-Collections/6529Stream/issues/216), [`PR #347`](https://github.com/6529-Collections/6529Stream/pull/347), [`#348`](https://github.com/6529-Collections/6529Stream/issues/348), [`PR #349`](https://github.com/6529-Collections/6529Stream/pull/349), [`PR #350`](https://github.com/6529-Collections/6529Stream/pull/350), [`#351`](https://github.com/6529-Collections/6529Stream/issues/351) | Gate E/Gate G | TBD |
| Fork deployment release artifact coverage | The fork deployment retained artifact, sanitized broadcast, generated fork manifest, generated fork address book, non-local evidence envelope, public-beta evidence row, release manifest, checksum bundle, tooling docs, release-readiness docs, generated evidence tracker issue payloads, and retained live issue-body audit bundle stay covered by local gates and CI without overstating readiness claims | `Makefile`, `scripts/check.sh`, `scripts/check.ps1`, `.github/workflows/ci.yml`, `tools/deployment/generate_broadcast_manifest_input.py --template deployments/config/fork-mainnet-6529stream-v0.1.0-001.json --broadcast deployments/broadcasts/fork-mainnet-6529stream-v0.1.0-001-run-latest.json --output deployments/config/fork-mainnet-6529stream-v0.1.0-001-broadcast.json --manifest-output deployments/examples/fork-mainnet-6529stream-v0.1.0-001-broadcast.json --check`, `tools/deployment/generate_deployment_manifest.py --config deployments/config/fork-mainnet-6529stream-v0.1.0-001-broadcast.json --check`, `tools/deployment/generate_address_books.py --check`, `tools/release/generate_release_evidence_packet_index.py --check`, `tools/release/generate_release_evidence_issue_backlog.py --check`, `tools/release/generate_release_evidence_issue_body_sync.py --check`, `tools/release/check_release_evidence_issue_bodies.py`, `tools/release/check_release_evidence_issue_closure.py`, `tools/release/generate_release_evidence_live_audit_archive.py --archive-dir release-artifacts/evidence/live-audit-reports --check`, `tools/release/generate_release_manifest.py --check`, `tools/release/generate_release_checksums.py --check`, `docs/tooling.md`, `docs/release-readiness.md`, `CHANGELOG.md` | Merged in PR #350: fork retained evidence is reviewed, the public-beta blocker report moves the row to reviewed external evidence, the active evidence issue backlog and issue-link/body-sync artifacts drop #216 because its row is complete, release manifest/checksum coverage is refreshed, and CI run `27505334136` passed. Readiness is not overstated: public beta remains `blocked` because seven public-beta and nine production-release evidence rows are still missing. | [`#325`](https://github.com/6529-Collections/6529Stream/issues/325), [`PR #326`](https://github.com/6529-Collections/6529Stream/pull/326), [`#327`](https://github.com/6529-Collections/6529Stream/issues/327), [`PR #328`](https://github.com/6529-Collections/6529Stream/pull/328), [`#329`](https://github.com/6529-Collections/6529Stream/issues/329), [`PR #330`](https://github.com/6529-Collections/6529Stream/pull/330), [`#333`](https://github.com/6529-Collections/6529Stream/issues/333), [`#216`](https://github.com/6529-Collections/6529Stream/issues/216), [`PR #347`](https://github.com/6529-Collections/6529Stream/pull/347), [`#348`](https://github.com/6529-Collections/6529Stream/issues/348), [`PR #349`](https://github.com/6529-Collections/6529Stream/pull/349), [`PR #350`](https://github.com/6529-Collections/6529Stream/pull/350), [`#351`](https://github.com/6529-Collections/6529Stream/issues/351), [`P1-RELEASE-006`](https://github.com/6529-Collections/6529Stream/issues/105), [`P1-RELEASE-004`](https://github.com/6529-Collections/6529Stream/issues/101) | Gate E/Gate G | TBD |

Current reconciliation note: the completed PR #350 and PR #553 evidence remains
historical. The operation-identity cutover changes the retained deployment and
randomizer artifact set, so `fork_deployment_rehearsal` and
`fork_testnet_randomizer_operations_evidence` are currently `pending` in the
generated public-beta manifest and issue-sync artifacts until that changed set
receives review. Public beta remains blocked; no historical completion is being
used as current-set readiness evidence.
