...
| Production size | The canonical permanent `StreamCore` proof passes the non-waivable 2,000-byte production margin and approved 22,184-byte objective | `forge build --sizes --via-ir --skip test --skip script --force`; `python scripts/check_contract_size_budget.py`; `python scripts/check_release_mode.py --phase production-release`; `release-artifacts/latest/bytecode-release-proof.json` | Core size passes without an exception; candidate, audit, static-analysis, and live-evidence blockers remain independent |
