...
supersession (`91e223f5`) has 16; mint grace (`8c60b099`, catalogs `9dd5aeaa`)
has 21. Full37 construction (`24d3c0f1`) and activation helpers (`9ad53bd4`)
have 15 and 11 authored cases respectively. Native execution remains pending.
Scoped source snapshots (`17e299ba`) and original-consent CONTENT_ROOT joins
(`51be4289`) are source-integrated; retained output, VIEW and full finality remain.

The full publisher capture still passes five cases and fails four: its complete
preview consumes 35,360,587 gas and bounded publication exhausts memory gas.
Preparation-only passes do not close this failure. Manager/fallback runtime is
25,030/25,166 bytes at `9dd5aeaa`; a bounded extraction is underway. Existing
Artist/Router size blockers and the Estate helper's 189-byte excess also remain.
Original limits and historical failures are retained.

ART38 standing-conflict retention, later recovery histories, remaining Artist
client families and required preserved/archival joins are active builds. Held
proposals remain unapplied. Full latest-stack Safe/fuzz/gas/CI acceptance,
source freeze and matching new testnet evidence are still required.
...
| [mint.static](#mintstatic-evidence) Static caps, counter subjects and phase-policy grace windows | In progress | Partly tested | Partial | Static counters and immediate-predecessor grace now have the original consent-gated Manager producer (`8c60b099`) and exact delayed-governance catalogs (`9dd5aeaa`). Sixteen focused and five actual-Safe cases are authored. **Remaining:** Manager/fallback 25,030/25,166-byte runtime exceeds 24,576; bounded worker extraction, native grace/rollback and combined fuzz acceptance remain. |
...
| [ART38](#art38-evidence) C2PA credential/key-history authorship reconciliation | In progress | Partly tested | Not integrated | Isolated original-op24 credential history, explicit verifier/report reconciliation and optional renderer pairing are authored; focused offline and narrow codec evidence is recorded by the builder. **Remaining:** Complete, review and integrate the standing-conflict retention correction so stale report facts cannot hide an unresolved divergence, then historical Museum consumption, actual live provenance and current-graph acceptance. Existing full-output hashes stale when annotation bytes change; they are not silently reinterpreted. |
| [ART39](#art39-evidence) Complete Artist signing/recomputation client and measured ceremony rehearsal | In progress | Partly tested | Partial | Original principal3/16/20/21/54 and identity/delegation25–27 signing, state capture, exact receipts and Safe plans are source-integrated (`92a0a687`, `be596707`). Root719 client tests plus generation/build/strict types and exact ABI49 fixture pass. **Remaining:** Delegated-mode/current-grace callers, other original operation families and measured actual Safe ceremonies; simulated RPC is not native execution. |
