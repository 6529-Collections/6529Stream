...
focused passes and all 34 captured production products within deployment limits.
Full-byte preservation (`09fd273d`) has nine scoped passes. General attestations
(`9677f5bf`) have 86 root checks and 15 accepted-profile native passes. Their
large-payload successor (`2e0c3443`) is integrated with 64 affected root replay
checks and 25 passes from its own frozen native cohort (host runtime 23,179
...
seven native/ERC20 offer cases in `770ccea5`) cover all eight Manager
entrypoints with actual-Artist source recipes. Joined execution remains
pending. Full 37-role construction (`24d3c0f1`) includes genuine commerce,
clone and backup products plus 25 direct support entries; six candidate
and nine record cases are authored. ART25 rotation/recovery continuations
(`7a274c04`) add 27 authored cases. All 2,191 sources pass ABI/type/storage
checks at `17e299ba`. Complete activation, transitive STATIC analysis and
combined runtime acceptance remain open. Scoped STATIC source snapshots (`17e299ba`) are source-integrated;
complete content-root/finality joins remain. The real publisher run passes
five cases but fails four: complete preparation exceeds transaction gas.
Earlier passing inventory/environment probes do not close that failure.

Entropy's actual launch allowance passes registration/receiver rollback tests.
The new candidate's immutable floor still needs final cold-cost calibration.
Artist C2PA reconciliation, delegated policy/sale consent, remaining recovery
histories and full genesis activation are active feature work. Held proposals
remain unapplied. No complete-system acceptance, Museum conformance or new
testnet release is claimed.
...
| Item | Snapshot / state |
| --- | --- |
| Current integration source | `17e299ba0c8c843adbc163a755e1ddc70429a4a1`; see the [20 September batch](#latest-integration-checkpoint-20-september) for source-specific evidence and remaining joins |
...
| [mint.static](#mintstatic-evidence) Static caps, counter subjects and phase-policy grace windows | In progress | Partly tested | Partial | Static increment/cap engine and recipient, payer, executor, constant and context subjects; Ledger and consumers implement exact immediate-predecessor grace. **Remaining:** Audit found every Manager registration still passes zero grace. Mint now adds the missing original-consent-gated producer with an additive capability and delayed governance, then actual-current grace/Safe tests and final combined replay fuzzing. |
...
| [ART27](#art27-evidence) Dormancy-origin supersession and expanded adjudication histories | In progress | Not tested | Not integrated | Builder is implementing designated43 with original guardian supersession, including rotated/closed origins and complete current-cutoff APPEAL/ARBITER proof. **Remaining:** Source handoff and native acceptance; later43-to35-to-rotations-to35 and living35-to43-to35 require separate origin/epoch proofs. Class4 steward recovery remains ART26. |
...
| [ART38](#art38-evidence) C2PA credential/key-history authorship reconciliation | In progress | Tests written | Not integrated | Operative Artist credential enumeration and validation outcome consistent/divergent/unevaluated against binding and authority history. **Remaining:** Separate credential/personhood heads and exact original-op24 history reconstruction are authored and ABI-clean in the isolated Artist branch. The explicit verifier/report/current-identity consumer is being implemented; no integrated or native acceptance yet. |
...
| [ART42](#art42-evidence) Delegated mint-policy and sale-consent mode | In progress | Not tested | Not integrated | The revenue lead is implementing exact mode2 admission and delegated policy/sale records; the integrated source still has no mode2 producer; current policy/content/consent paths generally require ARTIST_SIGNED_POLICY mode1. **Remaining:** Implement exact mode2 admission, CAP_POLICY_CONSENT/CAP_SALE_CONSENT grants and current consumer joins; dispute delegation does not supply this mode. |
...
| [operator.full-genesis](#operatorfull-genesis-evidence) Deploy and activate the complete full-v1 system | In progress | Tests written | Partial | `24d3c0f1` constructs all37 distinct original roles with genuine commerce, locked clone singleton and distinct34/35 backups; 25 direct support entries and the complete supplied configuration are bound. Six candidate and nine record-product cases are authored. **Remaining:** Native construction, full transitive inventory, all delayed module/catalog/binding activation, actual provider/collection setup, restart/resume and complete gas/size acceptance. Earlier530-transaction rehearsal/productsActivated=false is not a launch. |
