...
| [MUSEUM-38](#museum-38-evidence) Complete media/history corpus and institutional conformance | In progress | Partly tested | Partial | All eight synthetic media/history scenarios now have retained original source packages and executable versioned semantic projections (`004984c37`), with 18 focused root tests passing and independent source review. Photo/two-print, written/AV interview, interactive, historical geography, conflicting documentation, independent accounts and offline revision examples preserve exact source/profile hashes; detached replay retains the model/crosswalk and rejects rehashed semantic tampering. V1/V2 golden packages remain reproducible. **Remaining:** Complete current-contract authenticated capture, required cross-format correspondence, adopted gate closure and named repository/practitioner evidence. Synthetic examples do not establish institutional integration. See [the V3 slice](../docs/museum-media-history-semantic-slice-v3.md). |
