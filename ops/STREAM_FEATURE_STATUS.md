...
Updated **21 September 2026 (UTC)**. This is the shared feature checklist for
...
Twenty-four of 29 preservation deployment-size repairs are integrated. The
final five-facade batch fits two and is repairing three residual overages.
Aggregate attestation workers have fourteen focused passes including 256 fuzz
runs at their exact producer source; full aggregate recovery is still being
built. All twenty MULTIPLE_BASE mocked client workflows pass. The next consent
client and complete owner-family museum semantics are in progress.

Activation11, caller-baseline6 and post-recording4 have scoped EVM passes.
Museum semantic34 and script/dependency58 root cases remain accepted. The fixed
ea4 test campaign is compiling and executing separately; its first Genesis33
capture timed out and is being split into original host contexts. Build counts
and isolated tests do not imply current combined-system acceptance.

[Current delivery status](V1_CURRENT_STATUS.md) gives exact source/evidence
boundaries. Remaining work is full feature composition, current graph/Safe,
stateful/fuzz, gas/size/CI, freeze and testnet. RC1 remains preserved.
...
| Item | Snapshot / state |
| --- | --- |
| Current integration source | `0a4739ecee01c49f337ad6fc147f07b08e96970c`; production `be7cfe0e`. Broader native/runtime campaign remains fixed at `ea4cf6b0`. |
...
| Latest broad cheap compilation evidence | ABI171:4,225 sources at `0a4739ec`, zero errors in50.594s, exact committed-source bridge; type/ABI/storage/method IDs only. |
