...
source-specific evidence. At `86074453`, all 2,417 Solidity sources pass
...
The actual-current terminal10 and three-version Safe9 recipes are source-built,
...
runtime-pending. Other held Artist/Router/Estate capacity issues remain.
...
is running with original limits. Prior native12 remains5PASS/4FAIL.

Remaining builds include complete entropy succession, recovery rewinds and
...
| Item | Snapshot / state |
| --- | --- |
| Current integration source | `860744532a7f1ae0a9d96e5fcd819519fcaa700b`; see the [20 September batch](#latest-integration-checkpoint-20-september) for bounded evidence and remaining joins |
...
| Latest broad cheap compilation evidence | 2,417-source ABI/type/storage pass at `86074453` in22.891 seconds (ABI78). Historical ABI52 and additive mode-2 ABI56 remain explicitly client-pinned; neither establishes whole-system runtime acceptance. |
...
| [ART02](#art02-evidence) Identity registration, documents, revision and personhood/deployment prerequisites | Built | Partly tested | Partial | Canonical identity ID, stored document/display name, revision chain, direct/contract-wallet signatures and original personhood/deployment attestation checks. **Remaining:** Native op24-selected typed personhood/notarization proof is incomplete: automatic review rejected its application and the corrected full inert proposal is being prepared. Existing opaque attestation checks do not implement that new linkage. Historical authority/import permutations are separate rows; institutional identity truth is intentionally not proved. |
...
| [entropy.fallback-continuity](#entropyfallback-continuity-evidence) Coordinator replacement, retained old reads and distinct safe-mode fallback | In progress | Partly tested | Not integrated | Frozen V2 successor permission, covered pending-request admission, original host provenance and configured ordinary backup helper are source-integratedcf57de5e with independent review. Eleven continuity and ten prior subject-identity cases pass on recorded sources. Actual-Core launch500000 registration/receiver rollback and low-budget atomic rejection pass 3/3 at6fb0d7c3. **Remaining:** Exact complete legacy/explicit policy import and ultimate-origin provider relay are being built; original frozen policy domains and future mints must survive pointer replacement. Calibrate the new candidate immutable floor from measured cold cost per EC-REGGAS, execute current/Safe cutover and complete genesis activation. The120000-floor success oracle failed on both original and new sources and was corrected. |
...
| [quality.capacity](#qualitycapacity-evidence) Full-system gas, deployment size and transaction capacity | In progress | Partly tested | Partial | Exact joined Artist repair `6d3e55df` fits all11 selected products; Registry24,018/Coordinator24,333/Identity24,179 runtime bytes. Commerce `128c8378` fits all52 captured production products; terminal consumers fit11 selected products. **Remaining:** Its native graph still has Router44,008 runtime/49,781 init; other held Attribution/Identity-deployment/Estate blockers remain. Native publication13 is running; isolated1,175,817 gas savings are not actual publication acceptance. Complete latest graph, cold gas, ceremonies and transaction limits remain. |
