...
source-specific evidence. At `bee91ae5`, all 2,282 Solidity sources pass
ABI/type/storage checks. Root passes 872 client cases and separate Museum
...
the latter introduces Identity/Coordinator size overruns at 25,254/24,698 bytes,
which are assigned for repair. Older Artist/Router/Estate blockers remain.
...
products; exact actual-publisher retry is running. Fourteen current gate/Safe
recipes and eight stateful campaign cases are integrated, with execution pending.
...
counter-read helpers have their own incomplete row. Explicit-version Museum
export/import (`bee91ae5`) passes eleven root cases with exact-byte restore and
full historical semantic replay; external institutional acceptance is separate.
...
| Item | Snapshot / state |
| --- | --- |
| Current integration source | `bee91ae5f0c07f3d8ec6a2bdc67fb06e47816edd`; see the [20 September batch](#latest-integration-checkpoint-20-september) for bounded evidence and remaining joins |
...
| Latest broad cheap compilation evidence | 2,282-source ABI/type/storage pass at `bee91ae5` in21.344 seconds. Historical ABI52 and additive mode-2 ABI56 remain explicitly client-pinned; neither establishes whole-system runtime acceptance. |
...
| [mint.reads](#mintreads-evidence) Advisory mint eligibility and counter read API | In progress | Partly tested | Partial | canMint (`dc0122af`) returns explicit-executor gate eligibility and complete projected counter diagnostics. All 21 focused Manager/Ledger/Registry/gate cases pass on exact `b7378eeb`, with typed Core/Artist/governance. Final selected Manager/fallback sizes fit at 24,359/24,476. **Remaining:** Original raw/derived counter values, proof-aware resolution/remaining reads and actual-current/Safe acceptance; those read helpers are being built. |
...
| [ART27](#art27-evidence) Dormancy-origin supersession and expanded adjudication histories | In progress | Tests written | Not integrated | First designated43 guardian supersession (`91e223f5`) uses the actual current vesting cutoff and original APPEAL/ARBITER/Archive rules; 16 cases are authored and source review is clear. Repeated43-to35 recovery is now source-integrated (`0c5de0d9`), with14 further authored cases preserving the original43 capability/epoch. Living35-to43 origin proof (`ed4d5572`) adds 17 source-reviewed actual-owner cases. Active-notice dismissal (`9ff19e55`,14 cases) and cancelled-notice histories (`ff60c308`,15 cases) are source-integrated and reviewed. Complete repeated-living history (`22e90a3b`) adds 18 reviewed cases. **Remaining:** Current-cause aborted-pending rotation joins, native acceptance and separate class4 ART26. |
...
| [ART34](#art34-evidence) Migration of multiple identities, collaborators and corrected binding generations | In progress | Tests written | Not integrated | Original-living generation-1 PRIMARY_ONLY multiple-Artist/multiple-collection hydration is source-integrated (`8cc47857`) with ten reviewed actual-owner/Safe cases. Complete per-Artist guards and seven-owner/Archive atomicity are preserved. **Remaining:** New Identity/Coordinator size overruns (25,254/24,698), native acceptance, collaborator graphs, corrected/rebound generations, attribution disputes and Platform correction histories. |
| [ART35](#art35-evidence) Migration of delegated, rotated, estate, dormancy and recovered authority | In progress | Not tested | Not integrated | Complete delegation/guardian/provisional/closure/vesting/supersession and original transition chains, with permanent nonce/revocation/veto guards. Original-living delegation/consent migration is actively being built and remains unintegrated. **Remaining:** Complete authority-transition profiles, source integration and runtime acceptance. No readiness from partial imports. |
...
| [ART39](#art39-evidence) Complete Artist signing/recomputation client and measured ceremony rehearsal | In progress | Partly tested | Partial | Original principal and identity/delegation callers, delegated14/16 (`b99b3781`) and mint-grace workflow (`9ac63a11`) and delegated economics15/royalty-freeze20 (`6b71507e`) are source-integrated. Mode-2 grace (`e5ec5fae`) adds exact ABI56 support. Root806 client tests, generation/build/strict types and exact ABI52/56 fixtures pass. **Remaining:** Original attestation24 and other operation families and measured actual Safe ceremonies; simulated RPC is not native execution. |
...
`9ff19e55` and `ff60c308` add original active-notice dismissal and cancelled-notice histories with 14 and 15 source-reviewed cases respectively. `22e90a3b` adds the complete repeated-living walker and 18 authored cases. Native acceptance and current-cause aborted-pending rotation joins remain pending.
...
`8cc47857` implements original-living multiple-Artist/multiple-collection hydration with ten source-reviewed current-owner/Safe cases. Selected Identity/Coordinator runtimes are over limit at 25,254/24,698 bytes; repair and native acceptance remain pending. Broader profiles in this row are still required.
