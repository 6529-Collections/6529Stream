...
the 22,184-byte / 2,392-byte-margin rebased headroom measurement. Adding
...
| `EXT-027` | Add metadata browser evidence capture outputs | E/F | In progress for issue #218 on branch `codex/metadata-browser-evidence-capture`; adds deterministic retained-output flags to the local metadata browser rehearsal checker so future fork/testnet metadata browser evidence can retain browser summary JSON, generated `tokenURI`, and a redacted transcript. This is preparation only and must not close #218 or mark `fork_testnet_metadata_browser_evidence` complete without reviewed deployed-contract fork/testnet evidence |
