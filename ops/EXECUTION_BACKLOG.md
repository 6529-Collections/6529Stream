...
| `EXT-028` | Add metadata browser evidence draft generator | E/F | Locally validated for issue #555 on branch `codex/metadata-browser-evidence-draft-generator`; converts retained capture outputs into a checker-compatible `pending_review` fork/testnet metadata-browser bundle, requires an explicit deployed-contract assertion, and must not close #218 or mark `fork_testnet_metadata_browser_evidence` complete without reviewed evidence |
