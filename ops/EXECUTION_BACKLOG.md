...
| `EXT-036` | Reject symlinked testnet deployment retained artifacts | E/G | Active PR #604 / issue #603 on branch `codex/testnet-retained-artifact-symlinks`; hardens the `testnet_deployment_rehearsal` retained-artifact checker so future pending/reviewed Sepolia transcripts, broadcasts, manifests, address books, and gas/invariant summaries cannot pass through symlinked files, while keeping issue #217 open until real reviewed testnet evidence exists |
