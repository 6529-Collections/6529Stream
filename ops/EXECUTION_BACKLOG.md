...
| `ADV-018` | Add auction no-bid/cancellation randomizer composition tests | D/F | Active PR #598 / issue #597 on branch `codex/auction-terminal-randomizer-composition`; extends `test/StreamAuctionRandomizerComposition.t.sol` to no-bid settlement, contract-poster pending no-bid claims, pre-bid auction cancellation, cancelled signed auction authorization rollback, rejected terminal repeats, zero-accounting boundaries, pending request bindings, migration blocking while pending, and later arRNG fulfillment without claiming live provider readiness |
