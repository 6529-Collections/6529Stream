...
| `EXT-025` | Retain reviewed fork ceremony evidence | E/F | In progress for issue #219 on branch `codex/fork-ceremony-evidence`; promotes the checked fork ceremony retained artifact from template-only preparation to reviewed fork evidence, adds Safe/admin and post-state retained artifacts, links the non-local evidence envelope from `public-beta-evidence.json`, and keeps public beta blocked on the remaining missing rows |
