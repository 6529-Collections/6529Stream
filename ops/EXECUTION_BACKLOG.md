...
| `ADV-018` | Add auction no-bid/cancellation randomizer composition tests | D/F | Active issue #597 on branch `codex/auction-terminal-randomizer-composition`; extends local auction/drop/arRNG composition coverage to no-bid and cancellation-like terminal behavior, proving terminal custody/claimant state, payment accounting, pending request bindings, and later arRNG fulfillment remain coherent without claiming live provider readiness |
