...
| `ADV-016` | Add signer lifecycle auction-randomizer composition tests | D/F | Active PR #594 / issue #593 on branch `codex/signer-auction-randomizer-composition`; extends the auction/drop/arRNG composition suite so signer-epoch invalidation, signer rotation, replay attempts, consumed-drop cancellation attempts, and drop-execution pause after auction creation cannot mutate existing auction custody, bid accounting, pending request bindings, settlement, or later arRNG fulfillment |
