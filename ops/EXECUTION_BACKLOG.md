...
| `ADV-017` | Add fixed-price randomizer/payment composition tests | D/F | Active PR #596 / issue #595 on branch `codex/fixed-price-randomizer-composition`; extends local composition coverage to paid fixed-price drops using real `StreamDrops -> StreamMinter -> StreamCore -> NextGenRandomizerRNG` paths, proving randomness-request pause rollback, signer lifecycle controls, fixed-price payment credits, withdrawals, request binding, and later arRNG fulfillment remain coherent without claiming live provider readiness |
