...
| `EXT-024` | Add public-beta verified-addresses retained artifact validation | E/G | In progress for issue #550 on branch `codex/public-beta-verified-addresses-evidence`; adds a dedicated retained-artifact template/checker for future reviewed `verified_deployed_addresses` and `explorer_verification_status` evidence without closing tracker issues #221 or #222 |
