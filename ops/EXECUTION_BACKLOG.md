...
| `EXT-029` | Add live release-evidence issue sync guard | E/G | Active issue #557 on branch `codex/release-issue-live-sync-guard`; add an authenticated GitHub issue snapshot fetcher plus live body/closure sync gate for linked release-evidence tracker issues without adding network access to default CI or changing readiness claims |
