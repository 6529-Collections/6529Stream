...
| --- | --- | --- | --- |
| `SHIP-001` | Working current-stack RC and Sepolia demonstration | Tested transactions and concrete deployment | Active issue #738 on branch `codex/current-stack-integration` |
...
Status: Foundation merged in PR #683 and issue #665 closed. PR #686 merged ADR
0017's raise-only gas/time target. The first #684 follow-up now provides an
exact, honestly incomplete governed-parameter inventory and production
completeness gate. The issue #685 implementation slice now adds the immutable
closed-world action/native-value catalog, schedule/execution enforcement,
machine policy, adversarial tests, monitoring contract, and release binding.
Concrete parameter host/evidence bindings (#684), exact candidate expansion
under #656, deployment, non-local rehearsal, and audit evidence remain open.
...
5. Issue #684 plus `RISK-GOV-003` and `RISK-GOV-004` remain open until those
   full cutover criteria are met; completing #665 and #685 implementation scope
...
| `EXT-039` | Centralize JSON release-evidence no-secret scanning | E/G | Issue #609 implementation in progress on `codex/centralize-json-no-secret`: one checksum-covered shared scanner now applies the strict union of secret-key and assignment-looking value patterns across public-beta, non-local, drop-authorization signing, and signer-custody evidence checkers while preserving checker-specific exception types; this does not add reviewed non-local evidence or change release readiness |
...
| `AUD-006` | Remediate or produce reviewed issue-linked dispositions for every open first-party production Slither High/Medium finding while preserving exact normalized drift CI | C/F/G | PR #662 merged the inventory and exact-drift gate; the current capture has 32 Open rows under issue #658: zero confirmed gaps, six design-review rows, and 26 pending dispositions. `RISK-GOV-003` separately preserves the High Governance Executor native-value authority that bounded assembly makes invisible to Slither; #658 plus the #656 candidate/evidence dependency remain blockers |
