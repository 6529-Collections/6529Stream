...
| `EXT-026` | Retain reviewed fork randomizer operations evidence | E/F | In progress for issue #220 on branch `codex/fork-randomizer-operations-evidence`; promotes the checked fork randomizer operations retained artifact from template-only preparation to reviewed fork evidence, adds redacted provider export, fork transaction bundle, post-state request views, schema-checked randomizer operations JSON, linked non-local evidence envelope, and public-beta evidence completion for `fork_testnet_randomizer_operations_evidence` while keeping public beta blocked on the remaining missing rows |
