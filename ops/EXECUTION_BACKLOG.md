...
| `EXT-023` | Add production release-signing retained artifact validation | F/G | In progress on issue #548 and branch `codex/production-release-signing-evidence-validation`; add a dedicated retained-artifact template/checker for future reviewed production checksum-signature and signed-Git-tag evidence without closing tracker issues #223 or #224 |
