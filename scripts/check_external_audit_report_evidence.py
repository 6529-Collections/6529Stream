#!/usr/bin/env python3
"""Validate retained external audit report evidence artifacts."""
...
REQUIRED_FIELDS = {
    ...
    "Retest status",
    "Final audit report or public report reference",
    "Audit scope statement",
    "Finding remediation tracker",
    "Retest or verification evidence",
...
class ExternalAuditReportEvidenceError(RuntimeError):
    """Raised when external audit report evidence is invalid."""
...
def read_text(path: Path) -> str:
    ...
    try:
        ...
        raise ExternalAuditReportEvidenceError(
            f"missing required file: {path}"
        ) from exc
    except UnicodeDecodeError as exc:
        raise ExternalAuditReportEvidenceError(f"{path} must be valid UTF-8") from exc
...
def validate_artifact(path: Path) -> None:
    """Validate one retained external audit report artifact."""
    text = read_text(path)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Validate retained external audit report evidence artifacts"
    )
    parser.add_argument(
        "--evidence",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    except ExternalAuditReportEvidenceError as exc:
        print(f"external audit report evidence check failed: {exc}", file=sys.stderr)
        return 1
    print("external audit report evidence is valid")
    return 0
