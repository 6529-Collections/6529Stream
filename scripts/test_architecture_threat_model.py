...
def seed_bytecode_proof(
    ...
    sizes = [(23781, 795)] if streamcore_sizes is None else streamcore_sizes
...
def seed_size_evidence_docs(root: Path, runtime: int = 23781, margin: int = 795) -> None:
    for relative in [
...
def minimal_architecture_doc(runtime: int = 23781, margin: int = 795) -> str:
...
class ArchitectureThreatModelTests(unittest.TestCase):
    ...
    def test_rejects_inconsistent_streamcore_size_proof(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            seed_required_targets(root)
            seed_bytecode_proof(root, [(23781, 795), (23661, 915)])
            write_text(root / checker.DEFAULT_ARCHITECTURE, minimal_architecture_doc())
            write_text(root / checker.DEFAULT_THREAT_MODEL, minimal_threat_model_doc())
