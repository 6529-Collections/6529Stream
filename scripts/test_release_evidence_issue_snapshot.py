...
class ReleaseEvidenceIssueSnapshotTests(unittest.TestCase):
    ...
    def test_exact_linked_issues_rejects_mismatched_issue_number(self) -> None:
        """Exact mode fails if GitHub returns a different issue number."""
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            issue_links = root / exporter.DEFAULT_ISSUE_LINKS
            output = root / "labels.json"
            issue_links.parent.mkdir(parents=True, exist_ok=True)
            issue_links.write_text(
                json.dumps({"links": [{"issue_number": 215}]}) + "\n",
                encoding="utf-8",
            )
            ...
            with patch.object(
                ...
                return_value=completed(
                    {"number": 999, "title": "Unexpected issue", "labels": []}
                ),
            ):
                stderr = StringIO()
        ...
        self.assertIn("while fetching #215", stderr.getvalue())
