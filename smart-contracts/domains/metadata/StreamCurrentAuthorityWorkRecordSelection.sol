...
import {
    StreamCurrentAuthorityWorkRecordContext
} from "../records/StreamCurrentAuthorityWorkRecordContext.sol";
import {
    IStreamRecordCurrentAuthority
} from "../../interfaces/stream/metadata/IStreamRecordCurrentAuthority.sol";

/// @notice Successor-aware original WORK selector with preserved selection and lock domains.
...
contract StreamCurrentAuthorityWorkRecordSelection is
    IStreamRecordCurrentAuthority,
    ...
    IStreamRecordSelectionLock
{
    ...
    constructor(address core_, address metadata_, address schemas_) {
        ...
        // Registration can follow deployment. Retain the initial tuple; current use resolves
        // authenticated succession afresh without modifying this original capture.
    ...
    function currentAuthorityProfile() external pure override returns (bytes32) {
        return keccak256("6529STREAM_CURRENT_AUTHORITY_WORK_SELECTION_V1");
    }
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == type(IStreamRecordCurrentAuthority).interfaceId
            || id == type(IStreamWorkRecordSelection).interfaceId
    ...
    function _context() private view returns (StreamWorkRecordContext.Dependencies memory d) {
        return StreamCurrentAuthorityWorkRecordContext.currentContext(
