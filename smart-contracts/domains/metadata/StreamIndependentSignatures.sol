...
library StreamIndependentSignatures {
        ...
        returns (bytes32 scheme)
    {
        if (signer == address(0) || signature.length > 4096) {
            revert A.InvalidIndependentSignature(signer);
        }
        if (msg.sender == signer && signature.length == 0) return keccak256("DIRECT");
        bool delegated;
        ...
        if (signer.code.length == 0 || delegated) {
            ...
            if (!delegated) revert A.InvalidIndependentSignature(signer);
        }
