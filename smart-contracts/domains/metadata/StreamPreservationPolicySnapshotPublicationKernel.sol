...
/// @notice Distinct current COLLECTION preservation snapshot; every original receipt remains unchanged.
...
abstract contract StreamPreservationPolicySnapshotPublicationKernel is I, StreamGasParameterHost {
    ...
    bytes32 public constant INVENTORY_GAS =
        keccak256("6529STREAM_GGP_POLICY_SNAPSHOT_INVENTORY_GAS");
    ...
    bytes32 private _family;

    constructor(
        S.Dependencies memory d,
        address executor,
        GasParameterConfig[3] memory configs,
        bytes32 family_
        ...
        Sources.bindings(dependencies(), family_);
    ...
    modifier guarded() {
        if (_entered) revert S.InvalidPolicySnapshot();
        _entered = true;
        _;
        _entered = false;
    }

    function preservationPolicySnapshotProfile() external pure virtual override returns (bytes32) {
        return keccak256("6529STREAM_PRESERVATION_POLICY_SNAPSHOT_V1");
    }
        ...
        returns (bytes32 hash)
    {
        ...
        hash = keccak256(
            abi.encode(
                SnapshotFamilies.recordDomain(_family, false),
                _fixed.chainId,
                address(this),
                core,
                metadataHost,
        ...
        r.chainHash = keccak256(
            abi.encode(
                SnapshotFamilies.chainDomain(_family, false),
                _fixed.chainId,
                address(this),
                core,
                p.scope,
        ...
        returns (S.Receipt memory r)
    {
        ...
        if (r.scopeSubject != subject || _head(subject) != hash || r.revision != revision) {
            revert S.PolicySnapshotLineage(hash, _head(subject));
        }
        ...
        returns (bytes32 actionScope, bytes32 oldState, bytes32 newState)
    {
        ...
        if (r.recordHash == 0) revert S.PolicySnapshotUnknown(0);
        if (_locks[r.scopeSubject].actionId != 0) revert S.PolicySnapshotLocked(r.scopeSubject);
        actionScope = keccak256(
            abi.encode(
                SnapshotFamilies.lockDomain(_family, false),
                _fixed.chainId,
                address(this),
                core,
                scope
    ...
    function lockSnapshot(StreamFinalityScope calldata scope) external override guarded {
        ...
        {
            revert S.PolicySnapshotAuthority(msg.sender);
        }
        ...
        emit PolicySnapshotLocked(
            SnapshotFamilies.version2(_family) ? 2 : 1, r.scopeSubject, _locks[r.scopeSubject]
        );
    }
    ...
    function _candidate(S.Publication memory p) private view {
        ...
        if (_locks[subject].actionId != 0) revert S.PolicySnapshotLocked(subject);
        if (
        ...
        if (_head(subject) != p.expectedHead || _history[subject].length != p.expectedRevision) {
            revert S.PolicySnapshotLineage(p.expectedHead, _head(subject));
        }
    ...
    {
        if (actor == address(0)) revert S.PolicySnapshotAuthority(actor);
        for (uint8 i; i < 2; ++i) {
            uint8 cls = i == 0 ? 7 : 8;
        ...
        revert S.PolicySnapshotAuthority(actor);
    }
    ...
    function _known(bytes32 hash) private view returns (S.Receipt memory r) {
        r = _receipts[hash];
        if (hash == 0 || r.recordHash != hash) revert S.PolicySnapshotUnknown(hash);
    }
}
