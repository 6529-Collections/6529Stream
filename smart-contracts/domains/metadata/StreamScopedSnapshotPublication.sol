...
import {
    IStreamScopedSnapshotPublication as I
...
import {
    StreamScopedSnapshotSourceReads as Sources
} from "../records/StreamScopedSnapshotSourceReads.sol";
import {
...
/// @notice Exact scope-bound source publication without reinterpreting a COLLECTION snapshot.
...
contract StreamScopedSnapshotPublication is I, StreamGasParameterHost {
    bytes32 public constant READ_GAS = keccak256("6529STREAM_GGP_SCOPED_SNAPSHOT_READ_GAS");
    bytes32 public constant SOURCE_GAS = keccak256("6529STREAM_GGP_SCOPED_SNAPSHOT_SOURCE_GAS");
    bytes32 public constant INVENTORY_GAS =
        keccak256("6529STREAM_GGP_SCOPED_SNAPSHOT_INVENTORY_GAS");
    ...
    modifier guarded() {
        if (_entered) revert S.InvalidScopedSnapshot();
        _entered = true;
        _;
        _entered = false;
    }
        ...
        returns (bytes32 hash)
    {
        ...
        hash = keccak256(
            abi.encode(
                keccak256("6529STREAM_SCOPED_SNAPSHOT_RECORD_V1"),
                _fixed.chainId,
                address(this),
                core,
                metadataHost,
        ...
        r.chainHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_SCOPED_SNAPSHOT_CHAIN_V1"),
                _fixed.chainId,
                address(this),
                core,
                p.scope,
        ...
        emit ScopedSnapshotPublished(1, r.scopeSubject, p.snapshotId, hash, p, r);
    }
        ...
        returns (S.Receipt memory r)
    {
        ...
        if (r.scopeSubject != subject || _head(subject) != hash || r.revision != revision) {
            revert S.ScopedSnapshotLineage(hash, _head(subject));
        }
        ...
        returns (bytes32 actionScope, bytes32 oldState, bytes32 newState)
    {
        ...
        if (r.recordHash == 0) revert S.ScopedSnapshotUnknown(0);
        if (_locks[r.scopeSubject].actionId != 0) revert S.ScopedSnapshotLocked(r.scopeSubject);
        actionScope = keccak256(
            abi.encode(
                keccak256("6529STREAM_SCOPED_SNAPSHOT_LOCK_V1"),
                _fixed.chainId,
                address(this),
                core,
                scope
    ...
    function lockSnapshot(StreamFinalityScope calldata scope) external override guarded {
        ...
        {
            revert S.ScopedSnapshotAuthority(msg.sender);
        }
        ...
        emit ScopedSnapshotLocked(1, r.scopeSubject, _locks[r.scopeSubject]);
    }
    ...
    function _candidate(S.Publication memory p) private view {
        ...
        if (_locks[subject].actionId != 0) revert S.ScopedSnapshotLocked(subject);
        if (
        ...
        if (_head(subject) != p.expectedHead || _history[subject].length != p.expectedRevision) {
            revert S.ScopedSnapshotLineage(p.expectedHead, _head(subject));
        }
    ...
    {
        if (actor == address(0)) revert S.ScopedSnapshotAuthority(actor);
        for (uint8 i; i < 2; ++i) {
            uint8 cls = i == 0 ? 7 : 8;
        ...
        revert S.ScopedSnapshotAuthority(actor);
    }
        ...
        returns (bytes32 hash, bytes memory canonical)
    {
        ...
        if (canonical.length > 524288) revert S.InvalidScopedSnapshot();
    }
    ...
    function _known(bytes32 hash) private view returns (S.Receipt memory r) {
        r = _receipts[hash];
        if (hash == 0 || r.recordHash != hash) revert S.ScopedSnapshotUnknown(hash);
    }
}
