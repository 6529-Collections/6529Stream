...
/// @notice Fixed counterpart of original RoutingV2 predicates with an explicit pinned Router.
/// @dev Only the supplemental host's immutable configuration is authoritative to its consumer.
library StreamViewCheckpointReadsV2 {
    function serve(
        address router,
        address core,
        uint256 token,
        ...
        // Exact original renderer input and output checks. The supplemental host supplies
        // only its constructor-owned cap; original Router FULL_VIEW_GAS stays unchanged.
        bytes memory raw = Read.bounded(renderer, input, 262208, rendererGas, false);
