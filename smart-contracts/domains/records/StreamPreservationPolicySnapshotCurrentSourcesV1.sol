...
import {
    StreamPreservationPolicySnapshotRootReadsV1 as RootReads
} from "./StreamPreservationPolicySnapshotRootReadsV1.sol";

/// @notice Single fixed current-source entry point; preserves read order and returns the complete typed source.
library StreamPreservationPolicySnapshotCurrentSourcesV1 {
        ...
        returns (S.Source memory f)
    {
        ...
        _entropy(d, p, f);
        (f.root, f.rootBinding) = RootReads.current(
            d, p, f.artist, f.outputs, f.entropy.inventoryHash, f.entropy.policyChainHash, family
        );
    }
