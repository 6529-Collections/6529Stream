...
library StreamArtistRecoveredMultipleGenerationConsentBaseImport {
    function install(
        ...
        // Original terms map retains its first record; every later binding association has
        // an independently authenticated native occurrence and replay key.
        for (uint256 i; i < b.economics.length; ++i) {
            Base.Economics memory row = b.economics[i];
            EH.Row memory r = row.item;
            bytes32 key = Association.key(
