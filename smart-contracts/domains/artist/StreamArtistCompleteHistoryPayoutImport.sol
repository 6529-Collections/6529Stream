...
/// @notice Full original per-principal Payout maps and artifact points in canonical Artist order.
/// @dev Each unchanged typed importer prechecks its complete principal before writing. A later
/// failure reverts the whole original operation60; no native receipt or owner commit is added here.
library StreamArtistCompleteHistoryPayoutImport {
    ...
    function importState(uint256[6] memory roots, AH.Query memory query, bytes memory raw) public {
        (M.State memory s, Payload.Payload memory payload,,) = Decode.collect(5, query, raw);
        if (s.rows.length != s.artists.length) revert RH.InvalidRecoveredHydrationProfile();
        if (s.artists.length == 0) {
            address source =
                payload.provenance.origins[payload.provenance.origins.length - 1].owners[5];
            Provenance.validateOwnerSource(payload.provenance, 5, source);
            EmptyOwners.empty(payload.provenance, 5);
        }
