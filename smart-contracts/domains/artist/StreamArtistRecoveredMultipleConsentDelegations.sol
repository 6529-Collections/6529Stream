...
/// The separate aggregate union proves the one global nonce cardinality; all original local
/// grant, revocation, nonce-bit consumption and grant-to-lane checks remain here.
...
library StreamArtistRecoveredMultipleConsentDelegations {
    ...
    function _nonces(IH.Bundle calldata b, RH.OwnerProvenance calldata p) private pure {
        if (p.eras.length == 0 || b.nonces.length > RH.MAX_NONCE_INDICES) {
