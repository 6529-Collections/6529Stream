...
import {
    StreamArtistPlatformCorrectionState as PlatformState
...
library StreamArtistRecoveredPlatformBindingValidation {
    ...
    function _correction(
        ...
        bytes memory originalCauseData = causeData(a.causeData);
        if (
    ...
    /// @dev Canonical supplemental interpretation only. The original approval still hashes
    /// its full wrapped causeData; the complete Platform validator authenticates every pin.
    function causeData(bytes memory raw) internal pure returns (bytes memory) {
        return PlatformState.tagged(raw) ? PlatformState.decode(raw).originalCauseData : raw;
    }
