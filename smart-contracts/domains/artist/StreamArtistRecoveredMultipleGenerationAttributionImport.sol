...
library StreamArtistRecoveredMultipleGenerationAttributionImport {
        ...
        returns (bool)
    {
        ...
        G.Inventory memory inventory = abi.decode(auxiliary, (G.Inventory));
        if (keccak256(auxiliary) != keccak256(abi.encode(inventory))) _invalid();
        A.AttributionBundle[] memory histories = new A.AttributionBundle[](scope.rows.length);
        ...
        for (uint256 k; k < scope.rows.length; ++k) {
            ...
            histories[k] = row.history;
            attested.rows[k] = abi.encode(row.records);
        }
        Clocks.Result memory clocks = Clocks.validateLocal(scope, payload.provenance, inventory);
        Revocations.validate(histories, scope, payload.provenance, inventory, clocks);
        Original.Bundle[] memory all =
            Validation.validate(attested, payload.provenance, inventory, clocks);
        RevocationImport.check(histories);
        for (uint256 k; k < all.length; ++k) {
            _empty(s, all[k]);
        }
        RevocationImport.install(histories);
        for (uint256 k; k < all.length; ++k) {
        ...
        for (uint256 i; i < payload.provenance.journal.length; ++i) {
            ...
            Credentials.note(
                registry, b.artistId, bindingHash, r.input.terms, r.record, r.statement, false
            );
            ...
            if (_credential(r.input.terms)) {
                heads[a] = _nextHead(heads[a], b, r, registry, bindingHash);
