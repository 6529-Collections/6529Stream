...
import {
    StreamArtistRecoveredMultipleAttestationOwners as Owners
} from "./StreamArtistRecoveredMultipleAttestationOwners.sol";
import {
...
import {
    StreamArtistRecoveredMultipleAttestationWitnesses as Witnesses
} from "./StreamArtistRecoveredMultipleAttestationWitnesses.sol";
import {
...
import {
    StreamArtistRecoveredMultipleAttestationComposition as Composition
} from "./StreamArtistRecoveredMultipleAttestationComposition.sol";
...
library StreamArtistRecoveredMultipleAttestationPreparation {
    struct Context {
    ...
    function encodeAdmitted(Context memory input) public view returns (bytes memory) {
        ...
        context.features = RH.MULTIPLE_ATTESTATIONS | RH.ATTESTATIONS;
        ...
        context.attestations = result.attestations;
        context.auxiliary = result.auxiliary;
        if (
            (context.features
                            & ~(RH.FIRST_GRAPH_FEATURES
                                ...
                                | RH.CONTENT_CONSENTS
                                | RH.ATTESTATIONS
                                | RH.MULTIPLE_ATTESTATIONS)) != 0
                || (context.features & RH.ATTESTATIONS) == 0
        ) revert T.UnsupportedProfile();
