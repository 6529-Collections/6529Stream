...
import { StreamArtistRecoveredPlatformTypes as H } from "./StreamArtistRecoveredPlatformTypes.sol";
import {
...
import {
    StreamArtistRecoveredHistoryRecordWrites as Records
} from "./StreamArtistRecoveredHistoryRecordWrites.sol";

library StreamArtistRecoveredHistoryRecordImport {
        ...
        returns (bool)
    {
        ...
        if ((h.requiredFeatures & RH.HISTORY_RECORDS) == 0) return false;
        if (p.nonces.length != 0) _invalid();
        if (
            (h.requiredFeatures & (RH.ATTESTATIONS | RH.DISPUTE_HISTORY))
                != (RH.ATTESTATIONS | RH.DISPUTE_HISTORY)
        ) _invalid();
        RH.Point[] memory points = Validation.validateEncoded(p.semanticState, q, p.provenance);
        bytes[5] memory members = Parts.members(p.semanticState);
        D.Bundle memory b = abi.decode(members[0], (D.Bundle));
        H.Platform memory platform = abi.decode(members[2], (H.Platform));
        if (
            (b.generations.length > 1 && (h.requiredFeatures & RH.BINDING_GENERATIONS) == 0)
                || (H.nativeCount(platform) != 0 && (h.requiredFeatures & RH.HISTORY_PLATFORM) == 0)
        ) _invalid();
        if (
        ...
        _empty(b);
        Platform.requireEmpty(s, platform);
        Records.requireEmptyEncoded(s, p.semanticState);
        ...
        Platform.applyState(s, platform);
        Records.applyEncoded(s, q, p.provenance, p.semanticState, points);
        return true;
    }
