...
library StreamArtistUnboundPlatformCodec {
    function selected(bytes memory outer, uint8 owner) public pure returns (bool) {
        ...
        return (h.requiredFeatures & U.FEATURE) != 0;
    }
    ...
    {
        validate(owner, s, p);
        return abi.encode(U.TAG, U.VERSION, owner, s);
    }
        ...
        returns (M.State memory s)
    {
        ...
        if (
            tag != U.TAG || version != U.VERSION || actual != owner
        ...
        returns (M.State memory s, Payload.Payload memory payload)
    {
        ...
        if (
            (h.requiredFeatures & U.FEATURE) == 0
                || (h.requiredFeatures & ~(RH.FIRST_GRAPH_FEATURES | U.FEATURE)) != 0
        ) _invalid();
    ...
    function anchorQuery(M.State memory s) public pure returns (AH.Query memory q) {
        ...
        q = AH.Query(
            ...
            first.bindingHash,
            first.policies,
            first.artistId == 0 ? first.records : s.artists[artist(s, first.artistId)].records
        );
    }
    ...
    function validate(uint8 owner, M.State memory s, RH.OwnerProvenance memory p) public pure {
        ...
        if (owner == 2 && s.artists.length == 0) count = 1;
        ...
        for (uint256 i; i < s.collections.length; ++i) {
            ...
            if (
                q.collectionId == 0 || q.policies.length > 128
                    || (i != 0 && q.collectionId <= s.collections[i - 1].collectionId)
            ) _invalid();
            if (q.artistId == 0) {
                if (q.bindingHash != 0 || q.policies.length != 0) _invalid();
                ++unbound;
            } else {
                if (q.bindingHash == 0) _invalid();
                artist(s, q.artistId);
            }
        ...
        if (policies > 128 || unbound == 0) _invalid();
        ...
        for (uint256 i; i < p.journal.length; ++i) {
            ...
            if (j.receipt.artistId == 0) {
                uint16 op = j.receipt.operation;
                if (
                    owner != 4 || j.receipt.collectionId == 0
                        || (op != 8 && op != 9 && op != 10 && op != 11 && op != 53)
                ) _invalid();
            } else {
