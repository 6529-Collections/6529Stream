...
/// @notice Local lane semantics followed by a bijection onto the unchanged global nonce inventory.
/// @dev No global checkpoint denominator is replaced with a per-Artist count.
library StreamArtistRecoveredMultipleIdentityNonces {
    function validateLocal(IH.Bundle calldata b) public pure {
        if (b.delegations.length != 0 || b.nonces.length > RH.MAX_NONCE_INDICES) _invalid();
    ...
    function ordered(M.State calldata s, RH.NonceInventory[] calldata inventory)
        public
        pure
        returns (IH.NonceLane[] memory all)
    {
