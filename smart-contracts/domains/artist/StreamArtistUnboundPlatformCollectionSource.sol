...
import {
    StreamArtistUnboundPlatformSource as Platform
} from "./StreamArtistUnboundPlatformSource.sol";
import {
...
import {
    IStreamArtistPlatformOwner as PlatformOwner
...
library StreamArtistUnboundPlatformCollectionSource {
    function collect(
        ...
        for (uint256 i; i < rows.length; ++i) {
            ...
            if (q.artistId == 0) {
                if (owner == 0) {
                    ...
                    if (
                        keccak256(abi.encode(Binding(source.owners[0]).binding(q.collectionId)))
                            != keccak256(abi.encode(zero))
                    ) _invalid();
                } else if (owner == 3) {
                    if (
                        Acceptance(source.owners[3]).acceptanceRecord(0) != 0
                            || Acceptance(source.owners[3]).acceptedAt(0) != 0
                    ) _invalid();
                } else if (owner == 4) {
                    (uint8 state, uint64 generation) =
                        Attribution(source.owners[4]).attributionState(q.collectionId);
                    if (state != 0 || generation != 0) _invalid();
                    P.Platform memory item = Platform.collect(source.owners[4], q, full);
                    Timeline.validate(item, p);
                    rows[i] = abi.encode(item);
                } else if (owner != 6) {
                    _invalid();
                }
                continue;
            ...
            if (owner == 0) {
                ...
                PW.State memory zero;
                if (
                    keccak256(
                            abi.encode(
                                PlatformOwner(source.owners[4]).platformWorksState(q.collectionId)
                            )
                        ) != keccak256(abi.encode(zero))
                ) _invalid();
                (uint8 state, uint64 generation) =
