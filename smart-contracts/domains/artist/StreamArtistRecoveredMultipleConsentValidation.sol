...
/// @notice Complete aggregate Consent validation at original full-owner journal coordinates.
...
library StreamArtistRecoveredMultipleConsentValidation {
    ...
    function validate(
        ...
        for (uint256 i; i < all.length; ++i) {
            ContentH.Bundle memory b = all[i];
        ...
        (bytes32[] memory surfaces, bytes32[] memory scopes) = _journal(all, p, 1);
        _eras(p);
    ...
    function _journal(ContentH.Bundle[] memory all, RH.OwnerProvenance memory p, uint64 generation)
        private
        pure
        returns (bytes32[] memory surfaces, bytes32[] memory scopes)
    {
        ...
        for (uint256 i; i < p.journal.length; ++i) {
            ...
            for (uint256 k; k < all.length; ++k) {
                if (
                    all[k].original.artistId == row.receipt.artistId
                        && all[k].original.collectionId == row.receipt.collectionId
                ) {
            ...
            if (selected == type(uint256).max) _invalid();
            ContentH.Bundle memory b = all[selected];
            if (
