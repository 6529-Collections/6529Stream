...
library StreamArtistPrimaryCollaboratorIdentityNative {
    function validate(IH.Bundle calldata b, RH.OwnerProvenance calldata p) public pure {
        ...
        for (uint256 i; i < p.journal.length; ++i) {
            ...
            // This profile separately proves original op6 against the complete Archive. Class4
            // and unsupported dispute effects still cannot be silently dropped.
            if (op == 44 || op == 45 || op == 46 || op == 47 || op == 49 || op == 50 || op == 59) {
                revert IH.InvalidRecoveredIdentity(j.receipt.recordHash);
            }
            ++counts[op];
        }
        if (
            counts[1] + counts[6] != 1 || counts[19] != b.sanctionGrants.length
                || counts[23] != b.findings.length || counts[25] != b.revisions.length
                || counts[26] != b.delegations.length || counts[28] != b.guardians.length
                || counts[29] != b.rotations.length
                || counts[33] + counts[48] != b.contests.length * 2 || counts[33] % 2 != 0
                || counts[48] % 2 != 0 || counts[35] != b.recoveries.length * 2
                || counts[36] != b.designations.length || counts[37] != b.directives.length
                || counts[38] != b.estates.length || counts[41] != b.notices.length
                || counts[51] != b.standingRecords.length || counts[58] != b.dismissals.length
        ...
        for (uint256 i; i < b.contests.length; ++i) {
            ...
            uint256 at = _native(
                p, b.artistId, b.contests[i].position, operation, b.contests[i].record.recordHash
            );
            if (i != 0) {
    ...
    function _contestOperation(RH.OwnerProvenance calldata p, bytes32 record)
        private
        pure
        returns (uint16 operation)
    {
        for (uint256 i; i < p.journal.length; ++i) {
            if (p.journal[i].receipt.recordHash != record) continue;
            uint16 op = p.journal[i].receipt.operation;
            if (operation != 0 || (op != 33 && op != 48)) {
                revert IH.InvalidRecoveredIdentity(record);
            }
            operation = op;
        }
