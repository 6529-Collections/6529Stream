...
/// @notice Original completion predicates over an interleaved global binding clock.
/// @dev The aggregate timeline proves the same collection proposal precedes this completion;
/// other collections may advance owner0 between them. All other original predicates are retained.
library StreamArtistRecoveredMultipleGenerationCompletionProof {
    function advance(
        ...
        if (
            generation.proposal.environmentHash != p.eras[era].originHash
                ...
                || e.after_[0].revision != e.before_[0].revision + 1
        ) _invalid();
