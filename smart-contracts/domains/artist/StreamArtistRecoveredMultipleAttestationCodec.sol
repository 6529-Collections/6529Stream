...
library StreamArtistRecoveredMultipleAttestationCodec {
    ...
    function selected(bytes memory outer, uint8 owner) public pure returns (bool) {
        ...
        return (h.requiredFeatures & RH.MULTIPLE_ATTESTATIONS) != 0;
    }
        ...
        returns (bytes memory)
    {
        validate(owner, s, p);
        if (owner == 4) _invalid();
        return abi.encode(SCHEMA, M.VERSION, owner, s, bytes(""));
    }
    ...
    function encodeAttribution(
        ...
        return abi.encode(SCHEMA, M.VERSION, uint8(4), s, auxiliary);
    }
        ...
        returns (M.State memory s, bytes memory auxiliary)
    {
        ...
        if (
            tag != SCHEMA || version != M.VERSION || actual != owner
                ...
                || (owner == 4 ? auxiliary.length == 0 : auxiliary.length != 0)
        ) _invalid();
        validate(owner, s, p);
    }
