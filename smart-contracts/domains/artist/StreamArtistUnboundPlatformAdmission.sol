...
import {
    StreamArtistUnboundPlatformSelectors as Selectors
} from "./StreamArtistUnboundPlatformSelectors.sol";
...
library StreamArtistUnboundPlatformAdmission {
    ...
    function _partition(OriginalAdmission.Certificate memory c, MH.Request memory selected)
        private
        view
    {
        ...
        for (uint256 i; i < c.collections.length; ++i) {
            ...
            if (selected.collections[i].artistId == 0) {
                T.Binding memory empty;
                if (keccak256(abi.encode(binding)) != keccak256(abi.encode(empty))) {
                    revert T.InvalidRecord();
                }
            } else if (
                binding.artistId != selected.collections[i].artistId || binding.bindingHash == 0
            ) {
                revert T.InvalidRecord();
            }
        ...
        for (uint8 owner; owner < 7; ++owner) {
            for (uint256 j; j < c.provenance.journals[owner].length; ++j) {
                ...
                if (row.collectionId != 0) {
                    ...
                    if (selected.collections[collection].artistId != row.artistId) {
                        revert T.InvalidRecord();
                    }
                    ++collectionCounts[collection];
                }
