...
import {
    StreamArtistRecoveredMultipleAttestationHeads as Heads
} from "./StreamArtistRecoveredMultipleAttestationHeads.sol";
import {
...
library StreamArtistRecoveredMultipleAttestationSource {
    function collect(
        address source,
        ...
        if (
            inputs.length != scope.collections.length || p.journal.length == 0
                || p.journal.length > 128
        ) _invalid();
        ...
        scope.rows = rows;
        Validation.validate(scope, p);
        Heads.requireMatches(source, scope, p, all);
    }
