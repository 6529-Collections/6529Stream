...
/// @notice Dispute-family bijection within the full shared Archive, including original46/48/49/50.
/// @dev Original source maps, immutable evidence metadata and complete seven-owner cutoffs are all retained.
library StreamArtistCompleteHistoryDisputeArchive {
    function validate(
        D.Bundle[] memory all,
        ...
        if (
            all.length != inventory.bindings.bindings.length
                || clocks.clocks.collections.length != all.length
                || inventory.archive.catalogues.length != p.eras.length
        ) _invalid();
        Catalogue.requireLocal(p, 4, inventory.archive.catalogues, inventory.archive.operations);
        uint256 expected;
        ...
        for (uint256 i; i < inventory.archive.operations.length; ++i) {
            ...
            if (row.operation < 44 || row.operation > 50) {
                if (row.operation != 61) continue;
            }
            uint256 era = A.era(p, row.originHash);
            H.Envelope memory e =
                Catalogue.read(p.origins[era], inventory.archive.catalogues[era], row.evidence);
            if (e.operation != row.operation || !MD.archivedOperation(e.operation)) _invalid();
            for (uint256 j; j < i; ++j) {
                H.OperationEvidence memory previous = inventory.archive.operations[j];
                if (
            ...
            (uint256 count, bool identityWrite) = _match(all, inventory, clocks, p, e, point);
            if (count != 1) _invalid();
            _snapshots(inventory.archive.catalogues[era], e, identityWrite);
            ++matched;
        }
    ...
    ) private pure returns (uint256 count, bool identityWrite) {
        for (uint256 k; k < all.length; ++k) {
            ...
            if (e.operation == 44 || e.operation == 45 || e.operation == 61) {
                ...
                for (uint256 i; i < b.resolutions.length; ++i) {
                    ...
                    resolution(r.record, b, inventory, clocks, p, point, k, e.payload);
                    ++count;
                }
            } else {
    ...
    /// @notice Original resolution payload leaf; the enclosing Archive proof authenticates its envelope.
    function resolution(
        ...
        if (
            terms.bindingGeneration == 0
                || terms.bindingGeneration
                    > inventory.bindings.bindings[collection].bindings.rows.length
                || clocks.clocks.collections[collection].attributionCompletions.length
                    != inventory.bindings.bindings[collection].bindings.rows.length
        ) _invalid();
        T.Binding memory saved =
        inventory.bindings.bindings[collection].bindings.rows[terms.bindingGeneration - 1].item;
        RH.Point memory completed = clocks.clocks
        .collections[collection].attributionCompletions[terms.bindingGeneration - 1];
        bool acceptedThen = saved.accepted && completed.environmentHash != 0
            && Clock.beforeOwner(p, 4, completed, point);
        if (binding_.accepted != acceptedThen) _invalid();
        // The Archive captured the original accepted flag. A pending resolution may precede
        // genuine acceptance later in the same generation; every other binding field is fixed.
        T.Binding memory immutableBinding = abi.decode(abi.encode(binding_), (T.Binding));
        immutableBinding.accepted = saved.accepted;
        if (
            keccak256(raw)
                ...
                || terms.collectionId != b.collectionId
                || binding_.generation != terms.bindingGeneration
                || keccak256(abi.encode(immutableBinding)) != keccak256(abi.encode(saved))
