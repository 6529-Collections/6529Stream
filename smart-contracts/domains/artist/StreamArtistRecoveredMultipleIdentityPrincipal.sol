...
library StreamArtistRecoveredMultipleIdentityPrincipal {
    ...
    /// @dev The aggregate nonce/registration validator already proved every authentic bundle agrees.
    function registration(uint256[17] memory roots, uint256 next) public {
        if (X.identity(roots).nextRegistrationNonce != 0) {
            revert IH.InvalidRecoveredIdentity(bytes32(0));
        }
        X.identity(roots).nextRegistrationNonce = next;
    }
