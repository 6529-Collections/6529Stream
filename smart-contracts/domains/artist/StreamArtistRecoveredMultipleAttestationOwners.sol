...
library StreamArtistRecoveredMultipleAttestationOwners {
    struct Context {
        ...
        returns (bytes memory)
    {
        ...
        for (uint8 i; i < 7; ++i) {
            ...
            if (i == 2) {
                ...
                scope.rows = x.attestations;
            } else if (i == 6) {
                scope.rows = x.consents;
            } else {
                scope.rows = Collections.collect(c.source, i, scope, c.provenance);
            }
            payload.semanticState = i == 4
                ? Codec.encodeAttribution(
                    Queries.project(scope, payload.provenance), payload.provenance, x.auxiliary
                )
                : Codec.encode(i, scope, payload.provenance);
