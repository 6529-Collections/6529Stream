...
/// @notice Original accepted-generation binding and correction predicates, without local clock assumptions.
/// @dev The aggregate caller supplies original source rows and proves the complete global
/// journal, replay keys, Archive proposal/completion points and every owner counter separately.
library StreamArtistRecoveredMultipleGenerationBindingLeaves {
    function row(
        G.Row memory r,
        AH.Query memory q,
        uint64 generation,
