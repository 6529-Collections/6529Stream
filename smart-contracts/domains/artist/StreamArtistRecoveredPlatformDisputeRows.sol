...
library StreamArtistRecoveredPlatformDisputeRows {
    function validate(
        ...
        _validate(b, q, p, true, Platform.nativeCount(platform));
    }
    ...
    function _validate(
        ...
        if (
            P.validateOwner(p, 4) != b.provenance || b.artistId != q.artistId
                ...
                || b.disputes.length + b.repudiations.length + platformCount != p.journal.length
        ) _invalid();
        ...
        for (uint256 i; i < p.journal.length; ++i) {
            ...
            if (Platform.nativeOperation(j.receipt.operation)) continue;
            if (
