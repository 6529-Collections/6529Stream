...
/// @notice Original op24 leaf predicates with an explicitly authenticated historical binding.
/// @dev Complete journal/generation/Identity joins belong to the enclosing fixed history validator.
/// This worker never writes state or reauthorizes a historical signer against current authority.
library StreamArtistRecoveredHistoryRecordRows {
    ...
    function validateRow(
        ...
        _row(q, origin, row, generation);
    }

    function validateSummary(
        AH.Query memory q,
        RH.OriginEnvironment memory origin,
        ReadinessH.AttestationRow memory row,
        Original.PersonhoodRow memory summary,
        uint64 generation
    ) public pure {
        _summary(q, origin, row, summary, generation);
    }
