...
/// @notice Exact original Binding maps across historical principals and latest partial/terminal heads.
/// @dev Decode authenticates the full source before this worker prechecks every target. Common
/// op60 alone installs provenance/replay and commits the owner; no producer receipt is replayed.
library StreamArtistCompleteHistoryBindingImport {
    struct Context {
        CB.Bundle[] rows;
    ...
    function applyState(
        ...
        Context memory c;
        (M.State memory scope,, CT.Inventory memory inventory,) = Decode.collect(0, anchor, outer);
        c.inventory = inventory;
        c.rows = inventory.bindings.bindings;
        ...
        for (uint256 k; k < c.rows.length; ++k) {
            ...
            uint256 id = b.bindings.collectionId;
        ...
        for (uint256 k; k < c.rows.length; ++k) {
            ...
            // An authentic unbound slot has no generations, so it touches no history,
            // terminal, terms, correction or collaborator key, including generation zero.
            for (uint256 i; i < b.bindings.rows.length; ++i) {
                uint64 g = uint64(i + 1);
                history[id][g] = b.bindings.rows[i].item;
                terms[id][g] = b.bindings.rows[i].terms;
                for (uint256 j; j < c.inventory.bindings.collaborators[k][i].length; ++j) {
                    collaborators[id][g].push(c.inventory.bindings.collaborators[k][i][j]);
                }
