...
/// @notice One original Archive catalogue for the complete binding, Platform and dispute families.
/// @dev Records preserve their original operation domains. Catalogue integrity grants no
/// authority: the typed proof must join all seven cutoffs, receipts, aliases and exact payloads.
library StreamArtistCompleteHistoryCatalogue {
        ...
        returns (PLH.Catalogue[] memory catalogues, H.OperationEvidence[] memory operations)
    {
        ...
        operations = new H.OperationEvidence[](H.MAX_CATALOGUE_ROWS);
        uint256 count;
        uint256 rows;
        uint256 bytesRead;
        for (uint256 era; era < p.origins.length; ++era) {
            ...
            for (uint256 i; i < c.count; ++i) {
                ...
                // An op2 may only write Acceptance until the remaining collaborators
                // accept. Ops5/6 never write Attribution. Select each original operation
                // by an owner that always commits it, not an inferred completion clock.
                uint8 owner = _clock(e.operation);
                uint64 revision = e.after_[owner].revision;
                if (
                    revision <= p.eras[era].lowerRevisions[owner]
                        || revision > p.eras[era].checkpoints[owner].ownerState.revision
                ) continue;
    ...
    function requireLocal(
        ...
        if (
            owner > 6 || expected.length != local.eras.length
        ...
        returns (PLH.Catalogue memory c)
    {
        ...
        c.configurationHash =
            IStreamPrimaryCollaboratorConfiguration(o.coordinator).configurationHash();
    ...
    function _evidence(
        ...
        if (
            e.version != 1 || e.configurationHash != c.configurationHash || !_operation(e.operation)
                || e.actor == address(0) || e.value == 0 || evidence.evidenceId != _id(o, e)
                || evidence.payloadHash != keccak256(raw)
        ) _invalid();
    ...
    function _operation(uint256 op) private pure returns (bool) {
        return (op >= 1 && op <= 13) || (op >= 44 && op <= 50) || op == 52 || op == 53 || op == 61;
    }

    function _clock(uint16 op) private pure returns (uint8) {
        if (op == 5) return 1;
        if (op == 6) return 2;
        if (op == 2 || op == 7) return 3;
        if (op == 1 || op == 3 || op == 4) return 0;
        if (op == 12 || op == 13 || op == 52) return 6;
        if ((op >= 8 && op <= 11) || (op >= 44 && op <= 50) || op == 53 || op == 61) return 4;
        _invalid();
        return 0;
    }
