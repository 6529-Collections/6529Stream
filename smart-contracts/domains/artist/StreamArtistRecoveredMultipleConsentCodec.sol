...
library StreamArtistRecoveredMultipleConsentCodec {
    bytes32 internal constant SCHEMA =
        keccak256("6529STREAM_ARTIST_RECOVERED_MULTIPLE_CONSENTS_V1");
    ...
    function selected(bytes memory outer, uint8 owner) public pure returns (bool) {
        ...
        return (h.requiredFeatures & RH.MULTIPLE_CONSENTS) != 0;
    }
    ...
    {
        validate(owner, s, p);
        return abi.encode(SCHEMA, M.VERSION, owner, s);
    }
        ...
        returns (M.State memory s, Payload.Payload memory payload)
    {
        ...
        if (
            (h.requiredFeatures & RH.MULTIPLE_CONSENTS) == 0
                || (h.requiredFeatures
