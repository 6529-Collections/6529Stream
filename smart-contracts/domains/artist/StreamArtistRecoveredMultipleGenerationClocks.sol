...
library StreamArtistRecoveredMultipleGenerationClocks {
    struct Result {
        G.Timeline[] collections;
        uint256[] counts;
    }
    ...
    ) public view returns (G.Inventory memory inventory, Result memory result) {
    ...
    function validateLocal(
        ...
        if (c.total <= scope.collections.length || inventory.operations.length != 2 * c.total) {
            _invalid();
        }
        for (uint256 i; i < inventory.operations.length; ++i) {
            H.OperationEvidence memory row = inventory.operations[i];
