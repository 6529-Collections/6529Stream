...
/// @notice One declared collection slice over complete authenticated owner4 provenance.
/// @dev Global membership, revision and replay bijections are checked by the new profile validator.
/// @dev The separate Archive timeline proves fields outside record hashes and op1/op2 mutations.
library StreamArtistUnboundPlatformNativeRows {
    struct Cursor {
        PW.State state;
        uint256 claims;
        uint256 contests;
        ...
        returns (D.Guard[] memory guards)
    {
        ...
        for (uint256 i; i < p.journal.length; ++i) {
            ...
            if (!P.nativeOperation(op) || j.receipt.collectionId != b.collectionId) continue;
            if (
