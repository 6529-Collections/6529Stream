...
/// @notice Explicit collection dependencies for the first recovered-authority graph.
/// @dev One unchanged accepted generation-one attribution and complete direct policy14 history.
...
library StreamArtistRecoveredCollectionHydration {
    ...
    function importAttribution(AS.State storage s, AH.Query memory q, bytes memory raw) public {
        (, Payload.Payload memory payload) = Payload.decode(raw, 4);
        (bytes32 tag, AttributionBundle memory b) =
            abi.decode(payload.semanticState, (bytes32, AttributionBundle));
        if (
            tag != ATTRIBUTION || keccak256(payload.semanticState) != keccak256(abi.encode(tag, b))
    ...
    function validateAttribution(
        ...
        if (
            b.provenance != RH.ownerProvenanceHash(p, 4) || b.artistId == 0
                ...
                || p.journal.length != 0 || p.aliases.length != 0
        ) revert RH.InvalidRecoveredHydrationProfile();
        for (uint256 i; i < p.eras.length; ++i) {
            if (
                p.eras[i].checkpoint.ownerState.revision != (i == 0 ? 2 : 1)
