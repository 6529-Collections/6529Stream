...
library StreamArtistPrimaryCollaboratorNonceImport {
    ...
    function _nonces(uint256[17] memory r, IH.NonceLane[] calldata rows) private {
        for (uint256 i; i < rows.length; ++i) {
            ...
            if (row.kind == 1) {
                ...
                if (row.key == 0 || uint256(row.key) > type(uint160).max) {
                    revert IH.InvalidRecoveredIdentity(row.key);
                }
                _nonce(X.collaboratorAccounts(r).available[address(uint160(uint256(row.key)))], row);
            } else if (row.kind == 4) {
