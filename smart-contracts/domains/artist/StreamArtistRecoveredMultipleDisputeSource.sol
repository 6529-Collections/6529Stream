...
library StreamArtistRecoveredMultipleDisputeSource {
    function collect(
        address source,
    ...
    function collect(
        ...
        for (uint256 k; k < all.length; ++k) {
            all[k] = _collect(
                source, scope.collections[k], p, inventory.bindings[k], inventory.generations[k]
            );
            PW.State memory empty;
            if (
                keccak256(
                        abi.encode(
                            Platform(source).platformWorksState(scope.collections[k].collectionId)
                        )
        ...
        for (uint256 k; k < all.length; ++k) {
            for (uint256 i; i < all[k].repudiations.length; ++i) {
                ...
                for (uint256 c; c < all.length; ++c) {
                    if (all[c].artistId != all[k].artistId) continue;
                ...
                if (Repudiation(source).repudiationCount(all[k].artistId, cohort) != count) {
                    _invalid();
                }
            }
        }
    ...
    function _collect(
        ...
        // The complete aggregate proof validates the full journal, guards and state timeline.
        _causes(b, bindings);
    }
    ...
    function _causes(D.Bundle memory b, CB.Bundle memory bindings) private pure {
        for (uint256 i; i + 1 < b.generations.length; ++i) {
            ...
            if (keccak256(expected) != keccak256(bindings.corrections[i + 1].approval.causeData)) {
                _invalid();
            }
        }
    }
