...
/// caller keeps revocation checks/install between these two original phases.
library StreamArtistRecoveredMultipleGenerationAttributionRecords {
    struct Context {
        M.State scope;
    ...
    function check(AS.State storage s, Original.Bundle[] memory all) public view {
    ...
    function install(AS.State storage s, Context memory c) public {
        ...
        T.AttestationRecord[] memory personhoodHeads = new T.AttestationRecord[](c.all.length);
        for (uint256 i; i < c.provenance.journal.length; ++i) {
            RH.JournalEntry memory entry = c.provenance.journal[i];
            ...
            bytes32 bindingHash = c.bindingHashes[k][r.record.generation - 1];
            ...
            address registry = _origin(c.provenance, entry.position.point.environmentHash).registry;
            Credentials.note(
        ...
        for (uint256 k; k < c.all.length; ++k) {
            if (
                Credentials.personhoodKey(c.all[k].collectionId, c.all[k].artistId)
