...
library StreamArtistRecoveredPlatformImport {
        ...
        returns (bool)
    {
        ...
        Validation.validateEncoded(p.semanticState, q, p.provenance);
        bytes[4] memory parts = Parts.members(p.semanticState);
        D.Bundle memory b = abi.decode(parts[0], (D.Bundle));
        H.Platform memory platform = abi.decode(parts[2], (H.Platform));
        if (
