...
/// @notice Original17/20/21 signature and delegated20 facts across saved binding generations.
...
library StreamArtistRecoveredMultipleGenerationConsentRows {
    struct IdentityRows {
        bytes32 artistId;
            ...
            if (op == 17) {
                ...
                if (
                    row.recordHash != native_.receipt.recordHash || row.artistId != q.artistId
                        || !_accepted(consent.bindings, row.bindingGeneration)
                ...
                if (
                    row.item.recordHash != native_.receipt.recordHash
                        ...
                        || !_accepted(consent.bindings, row.item.bindingGeneration)
                        || row.terms.collectionId != q.collectionId
                ) _invalid();
                if (row.grant != 0) {
    ...
    function _accepted(T.Binding[] memory bindings, uint64 generation) private pure returns (bool) {
        return generation != 0 && generation <= bindings.length
            && bindings[generation - 1].generation == generation
            && bindings[generation - 1].accepted;
    }
