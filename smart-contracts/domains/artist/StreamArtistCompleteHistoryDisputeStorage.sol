...
/// @dev Cohort counts retain each original record Artist, including former binding principals.
library StreamArtistCompleteHistoryDisputeStorage {
    ...
    function _install(D.Bundle memory b) private {
        ...
        for (uint256 i; i < b.repudiations.length; ++i) {
            ...
            if (row.terminal.phase == 1) {
                ++rs.counts[row.record.artistId][keccak256(abi.encode(row.record.authorityHead))];
            }
        }
        ...
        for (uint256 i; i < b.repudiations.length; ++i) {
            ...
            if (
                keccak256(abi.encode(rs.records[row.recordHash]))
                    ...
                    || rs.counts[row.artistId][keccak256(abi.encode(row.authorityHead))] != 0
            ) _invalid();
        }
    }
