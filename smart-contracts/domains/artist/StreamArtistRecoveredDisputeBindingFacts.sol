...
library StreamArtistRecoveredDisputeBindingFacts {
    struct IdentityRows {
        bytes32 artistId;
    ...
    function validateRows(
        ...
        if (
            identity.artistId != q.artistId || bindings.artistId != q.artistId
                ...
                || bindings.rows.length == 0 || bindings.rows.length > 128
        ...
        for (uint256 i; i < bindings.rows.length; ++i) {
            ...
            } else if (row.terminal.kind == 0) {
                // Complete Binding validation joins this pending row to the next exact
                // original class2 resolution cause. No fictitious acceptance signature.
                if (i + 1 == bindings.rows.length || row.terminal.recordHash != 0) _invalid();
            } else if (row.terminal.kind != 2 || row.terminal.recordHash != 0) {
                _invalid();
            }
        }
        if (accepted != p.journals[3].length || accepted == 0) _invalid();
