...
import {
    StreamArtistRecoveredMultipleDisputeTypes as G
...
library StreamArtistRecoveredMultipleDisputeCodec {
    ...
    function encode(
        ...
        validate(owner, s, p);
        if ((owner == 0 || owner == 3 || owner == 4)
                ? auxiliary.length == 0
                : auxiliary.length != 0) _invalid();
        ...
        returns (M.State memory s, Payload.Payload memory payload)
    {
        ...
        if (
            (h.requiredFeatures & (G.FEATURE | RH.DISPUTE_HISTORY))
                    != (G.FEATURE | RH.DISPUTE_HISTORY) || (h.requiredFeatures & ~G.ALLOWED) != 0
        ) _invalid();
