...
import { StreamArtistRecoveryRotationClosureRecords as Worker } from "./StreamArtistRecoveryRotationClosureRecords.sol";
...
library StreamArtistRecoveryRotationClosure {
    function proof(
        ...
        bytes32 closureProof = Worker._closure(rotations, resolutions, e, current, terminal);
        bytes32 vetoProof = Worker._lastVeto(rotations, resolutions, e, current, terminal);
    ...
    function beforeNext(
        ...
        return Worker._closure(rotations, resolutions, e, atNext, previous);
    }
    ...
    function resolvedBeforeNext(
        ...
        bytes32 proof = Worker._closure(rotations, resolutions, e, boundary, previous);
    ...
    function selectedBeforeNext(
        ...
        if (resolutions.causes[causeHash].facts.kind == 2) {
            return
                Worker._dismissed(rotations, resolutions, e, boundary, previous, selected, resolutionHash);
        }
        ...
        if (
            current.artistId == 0 || current.incumbent == address(0) || expected == 0
                ...
                || (r.terms.removePriorStanding
                        ...
                        : r.terms.expectedRetirementHash != 0)
                || r.recordHash != Worker._dismissalHash(e, r) || cause.causeHash == 0
                || cause.causeHash != r.terms.expectedCauseHash
                || cause.causeHash != Worker._causeHash(e, cause)
    ...
    function _selectedContestPending(
        ...
        e = Worker._recordEnvironment(e, c.terms.artistId, 33, c.recordHash);
        if (
    ...
    function _abortedCompromise(
        ...
        Worker._rotation(e, r, cause.facts.artistId, hash);
