...
library StreamNativePrimaryOfferSettlement {
    ...
    function callback(
        ...
        StreamNativePrimaryOfferSupport.requireSale(state, x, i.saleId);
        a.callbackConsumed = true;
        a.tokenId = facts.tokenId;
        e.operationRoot = facts.operationRoot;
        e.operationId = facts.operationId;
        result = IStreamPreparedNativeOfferSettlement(address(x.recorder))
        .settlePreparedNativeOffer{ value: i.amount }(
            facts, i
        );
        if (
        ...
        return abi.encode(IStreamPreparedNativeOfferSale.onPreparedNativeOfferMint.selector, result);
    }
    ...
    function requireReceipt(
        ...
        if (
            !a.callbackConsumed || !a.received || e.tokenId == 0 || e.tokenId != a.tokenId
                ...
                || !IStreamPreparedNativeOfferSettlement(address(x.recorder))
                    .preparedNativeOfferConsumed(address(this), a.purchase.purchaseId)
