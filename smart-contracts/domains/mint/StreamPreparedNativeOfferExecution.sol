...
import "../revenue/StreamPreparedNativeOfferValidation.sol";

/// @notice Atomic native primary offer preparation in Manager storage and original caller context.
library StreamPreparedNativeOfferExecution {
    function admit(
        StreamPreparedNativeContentExecution.State storage contentState,
        address registry,
        ...
        address recorder = StreamPreparedNativeOfferValidation.addressWord(
            msg.sender, "primarySaleSettlement()"
        );
        StreamPreparedNativeSettlementTypes.Intent memory i =
            StreamPreparedNativeOfferValidation.readIntent(msg.sender, recorder, hash);
        StreamPreparedNativeContentTypes.Facts memory c =
            StreamPreparedNativeOfferReads.requireBatch(registry, batch, gateData, hash, i);
        if (i.contentSelectionHash != c.contentLeaf) {
            revert IStreamPreparedNativeMint.InvalidPreparedNativeMint();
        }
        contentState.admissionHash = StreamPreparedNativeOfferHash.admissionHash(
            msg.sender, hash, StreamPreparedNativeOfferHash.readPurchase(msg.sender, hash, i), c
        );
        ...
        )
    {
        ...
        f.recorder = StreamPreparedNativeOfferValidation.addressWord(
            msg.sender, "primarySaleSettlement()"
        );
        f.recorderCodeHash =
            StreamPreparedNativeOfferValidation.word(msg.sender, "settlementCodeHash()");
        ...
        StreamPreparedNativeSettlementTypes.Intent memory intent =
            StreamPreparedNativeOfferValidation.readIntent(msg.sender, f.recorder, f.intentHash);
        StreamPreparedNativeSettlementAdmission.requireAdmission(
            op.registry, msg.sender, intent.saleId
        );
        StreamPreparedNativeOfferValidation.requireBindings(
            address(op.core), op.registry, address(this), msg.sender, f.recorder, f.recorderCodeHash
        );
        StreamPreparedNativeOfferValidation.requireIntentFields(f, intent);
        StreamPreparedNativeContentTypes.Facts memory content =
            StreamPreparedNativeOfferReads.requireBatch(
                op.registry, batch, gateData, op.intentHash, intent
            );
        ...
        if (
            contentState.admissionHash
                    != StreamPreparedNativeOfferHash.admissionHash(
                        msg.sender,
                        op.intentHash,
                        StreamPreparedNativeOfferHash.readPurchase(
                            msg.sender, op.intentHash, intent
                        ),
                        content
                    )
        ...
        if (
            contentState.admissionHash
                != StreamPreparedNativeOfferHash.admissionHash(
                    msg.sender,
                    op.intentHash,
                    StreamPreparedNativeOfferHash.readPurchase(msg.sender, op.intentHash, intent),
                    contentState.active
                )
        ) revert IStreamPreparedNativeMint.InvalidPreparedNativeMint();
        ...
        StreamPreparedNativeOfferValidation.requireActive(address(op.core), op.registry, f, intent);
        op.core
        ...
        StreamPreparedNativeOfferReads.requireActive(op.registry, f, intent);
        ...
        returns (StreamPrimarySettlementTypes.PrimarySettlementResult memory result)
    {
        bytes memory data = abi.encodeCall(
            IStreamPreparedNativeOfferSale.onPreparedNativeOfferMint, (facts)
        );
        ...
        if (!ok || size != 416) {
            revert IStreamPreparedNativeMint.PreparedNativeCallFailed(
                sale, IStreamPreparedNativeOfferSale.onPreparedNativeOfferMint.selector
            );
        }
        bytes4 magic;
        (magic, result) =
        ...
        if (
            magic != IStreamPreparedNativeOfferSale.onPreparedNativeOfferMint.selector
    ...
    function _requireResult(
        ...
        bytes memory raw = StreamPreparedNativeOfferValidation.read(
            f.recorder,
        ...
        raw = StreamPreparedNativeOfferValidation.read(
            f.recorder,
            abi.encodeCall(
        ...
        StreamPreparedNativeContentTypes.Facts memory content =
            IStreamPreparedNativeOfferMint(address(this)).activePreparedNativeOfferContent();
        if (
        ...
        StreamPreparedNativeOfferTypes.Purchase memory purchase =
            StreamPreparedNativeOfferHash.readPurchase(f.saleAdapter, f.intentHash, intent);
        raw = StreamPreparedNativeOfferValidation.read(
            f.recorder,
            abi.encodeCall(
                IStreamPreparedNativeOfferSettlement.preparedNativeOfferConsumed,
                (f.saleAdapter, purchase.purchaseId)
            ),
            32
        );
        if (abi.decode(raw, (uint256)) != 1) {
