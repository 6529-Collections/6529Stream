...
/// @notice Canonical dual-digest authorization for one ERC-20 primary offer mint.
...
/// when the claimed buyer signer differs from the buyer. Context.executor must come from the
/// authenticated payment-adapter execution, not the callback caller. This helper neither grants
/// token-pull authority nor validates payer intent, payment, replay storage or delegation.
library StreamERC20PrimaryOfferAuthorization {
    struct Context {
        address core;
        address manager;
    ...
    function validate(
        ...
        if (
            context.core == address(0) || context.manager == address(0)
                || context.executor == address(0) || context.signatureGas == 0
                || context.signatureGas > type(uint64).max || terms.saleId == 0
                || terms.collectionId == 0 || terms.phaseId == 0 || terms.buyer == address(0)
                || terms.buyer == address(this) || terms.asset == address(0) || terms.price == 0
        ...
        if (
            block.timestamp < terms.startsAt || block.timestamp > terms.deadline
                ...
                || authorization.payer != terms.buyer || authorization.executor != context.executor
                || authorization.asset != terms.asset || authorization.unitPrice != terms.price
        ...
        if (
            offer.chainId != block.chainid || offer.saleAdapter != address(this)
                ...
                || offer.buyer != terms.buyer || offer.asset != terms.asset
    ...
    function _requireBatch(
        ...
        if (
            batch.initialRecipients[0] != terms.buyer || batch.beneficiaries[0] != terms.buyer
