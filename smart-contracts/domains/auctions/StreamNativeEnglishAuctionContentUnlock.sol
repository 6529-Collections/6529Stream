...
library StreamNativeEnglishAuctionContentUnlock {
    function reasonHash(
        StreamNativeEnglishAuctionUnlock.Context memory x,
        ...
        if (reason == 1) {
            ...
            code(address(manager), x.managerHash);
            if (counterExhausted(x, a, contentId)) {
            ...
            if (
                incident(x.registry, address(this)) || incident(x.registry, x.recorder)
                    || (gate != address(0) && incident(x.registry, gate))
            ) {
    ...
    function counterExhausted(
        ...
        IStreamMintReads manager = IStreamMintReads(address(x.support.manager));
        IStreamNativeEnglishAuction.Configuration memory c = a.config;
        bytes32 context = StreamPreparedNativeContentHash.context(
            block.chainid, address(this), a.saleId, contentId
        );
