...
library StreamPlatformTokenCustodySettlement {
    ...
    function settle(
        ...
        ) = StreamPlatformTokenCustodyValidation.activation(x.base.resolver, address(this), f);
        (
        ...
        StreamPrimarySettlementTypes.PrimarySettlementResult memory r = IStreamPlatformTokenCustodySettlement(
            x.recorder
        )
        .settlePlatformTokenCustodyPrimarySale{ value: a.winner.amount }(
            id
        );
        ...
        StreamPrimarySettlementTypes.PrimarySettlementResult memory expected =
            StreamPrimarySettlementTypes.PrimarySettlementResult(
                StreamPlatformTokenCustodyHash.candidate(
                    x.recorder, address(this), f, activation, original, c, beneficiaryHash
                ),
                key,
                selected.profileId,
                selected.wallet,
        ...
        bytes32 factsHash = StreamPlatformTokenCustodyHash.facts(
            x.recorder, address(this), f, activation, original
        );
        if (
