...
/// @notice Shared unpaid acquisition worker through a fixed linked boundary.
/// @dev Delegatecall retains the original house, caller, storage and callback scope.
library StreamNativeEnglishAuctionCustodyRegistration {
    ...
    function registerAuction(
        ...
        StreamRefundWindowSupport.ArtistAssociation memory association =
            validate(s, support, c, auth, artwork, platformSignature, artistSignature, prepared);
        ...
        a.creationDigest = prepared ? preparedDigest(auth) : digest(auth);
        ...
        bytes32[] memory expectedOperations;
        if (prepared) {
            (expectedRoot, expectedOperations) = IStreamPreparedNativeMint(address(x.base.manager))
                .previewPreparedNativeMintOperation(batch, "");
        } else {
            (expectedRoot, expectedOperations) =
        ...
        if (prepared) {
            (tokens, root, operations) = x.base.manager.executePreparedMint(batch, "");
        } else {
            (tokens, root, operations) = x.base.manager.executeSingleStepMint(batch, "");
        }
        if (
    ...
    function validate(
        ...
        bytes32 hash = prepared ? preparedDigest(a) : digest(a);
