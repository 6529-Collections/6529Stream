...
library StreamPlatformCustodySettlement {
    ...
    function settle(
        ...
        StreamPrimarySettlementTypes.PrimarySettlementResult memory r = IStreamPlatformCustodyPrimarySettlement(
            x.recorder
        )
        .settlePlatformCustodyPrimarySale{ value: a.winner.amount }(
            id
        );
        ...
        StreamPrimarySettlementTypes.PrimarySettlementResult memory expected =
            StreamPrimarySettlementTypes.PrimarySettlementResult(
                StreamPlatformCustodyHash.candidate(
                    x.recorder, address(this), f, declaration, original, c, beneficiaryHash
                ),
                key,
                selected.profileId,
                selected.wallet,
        ...
        bytes32 factsHash =
            StreamPlatformCustodyHash.facts(x.recorder, address(this), f, declaration, original);
        if (
