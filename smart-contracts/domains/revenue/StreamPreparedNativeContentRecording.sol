...
import "./StreamPreparedNativeContentValidation.sol";
...
library StreamPreparedNativeContentRecording {
    struct Context {
        address core;
        bytes32 coreHash;
        address registry;
    ...
    function execute(
        ...
        StreamNativeSettlementTypes.SaleLifecycleBinding memory lifecycle =
            StreamPreparedNativeContentValidation.requireActive(x.core, x.registry, facts, intent);
        (
        ...
        StreamPreparedNativeContentTypes.Facts memory content =
            StreamPreparedNativeContentReads.requireActive(x.registry, facts, intent);
