...
import "./StreamPreparedNativeContentPurchaseValidation.sol";
...
library StreamPreparedNativeContentPurchaseRecording {
    struct Context {
        address core;
        bytes32 coreHash;
        address registry;
    ...
    event PreparedNativeContentRecorded(
        bytes32 indexed settlementKey,
    ...
    function execute(
        ...
        if (purchaseConsumed[facts.saleAdapter][purchase.purchaseId]) {
            revert PreparedNativeContentPurchaseAlreadySettled(
                facts.saleAdapter, purchase.purchaseId
            );
        }
        ...
        StreamPreparedNativeContentTypes.Facts memory content =
            StreamPreparedNativeContentPurchaseReads.requireActive(x.registry, facts, intent);
        ...
        emit PreparedNativeContentRecorded(key, contentHashes[key], content);
        emit PreparedNativeContentPurchaseRecorded(
            1, facts.saleAdapter, purchase.purchaseId, key, intent.saleId, intent.saleNonce
        );
    }
