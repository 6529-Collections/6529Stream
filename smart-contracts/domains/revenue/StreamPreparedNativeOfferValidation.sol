...
import "../mint/StreamPreparedNativeOfferReads.sol";
...
/// @notice Active offer admission joined to the original prepared Core, recorder and ledger facts.
/// @dev A zero selection is permitted only here and independently checked by OfferReads as the
/// explicit unselected, no-gate path; older prepared validation remains unchanged.
library StreamPreparedNativeOfferValidation {
        ...
        returns (StreamPreparedNativeSettlementTypes.Intent memory intent)
    {
        bytes memory raw = read(
            sale,
            abi.encodeCall(IStreamPreparedNativeOfferSale.activePreparedNativeOfferIntent, (hash)),
            576
        );
        intent = abi.decode(raw, (StreamPreparedNativeSettlementTypes.Intent));
        if (
            keccak256(raw) != keccak256(abi.encode(intent)) || hash == 0
                || StreamPreparedNativeOfferHash.intentHash(sale, recorder, intent) != hash
        ) {
            ...
            abi.decode(
                    read(
                        ...
                        abi.encodeWithSelector(
                            bytes4(0x01ffc9a7),
                            type(IStreamPreparedNativeOfferSettlement).interfaceId
                        ),
                        32
                    ),
                    (uint256)
    ...
    function requireIntentFields(
        ...
        if (
            intent.collectionId == 0 || intent.collectionId != facts.collectionId
                ...
                || intent.beneficiary != facts.beneficiary || intent.beneficiary != intent.payer
                || intent.amount == 0 || intent.primaryPolicyMode != 0
                || intent.originalPrimaryPolicyHash == 0 || intent.executionNonce == 0
                || intent.authorityMode != 1 || intent.saleAuthorizationDigest == 0
                || intent.saleExecutionHash == 0 || intent.mintCommitment != facts.mintCommitment
    ...
    function requireActive(
        ...
        StreamPreparedNativeOfferReads.requireActive(registry, facts, intent);
