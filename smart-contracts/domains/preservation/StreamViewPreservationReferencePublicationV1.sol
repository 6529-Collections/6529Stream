...
import {
    StreamViewPreservationReferenceSourceReadsV1 as Sources
} from "./StreamViewPreservationReferenceSourceReadsV1.sol";
import {
    StreamViewPreservationReferenceRecordsV1 as Records
} from "./StreamViewPreservationReferenceRecordsV1.sol";
import {
...
/// @notice Exact repeated admitted preservation reference observations for complete VIEW scopes.
/// @dev Separate original records and domains. This host neither grants Artist content authority
/// nor turns first/last samples into a complete render-critical archive. Only the distinct authenticated VIEW preservation source is accepted.
contract StreamViewPreservationReferencePublicationV1 is
    IStreamViewPreservationReferencePublicationV1,
    IStreamReferenceInventoryPreparation,
    IStreamReferenceEnvironmentPreparation,
    IStreamArtworkScopedFinalityComponent,
    ...
    StreamGasParameterHost
{
    bytes32 public constant READ_GAS =
        keccak256("6529STREAM_GGP_VIEW_PRESERVATION_REFERENCE_READ_GAS");
    bytes32 public constant SOURCE_GAS =
        keccak256("6529STREAM_GGP_VIEW_PRESERVATION_REFERENCE_SOURCE_GAS");
    bytes32 public constant SNAPSHOT_GAS =
        keccak256("6529STREAM_GGP_VIEW_PRESERVATION_REFERENCE_SNAPSHOT_GAS");
    bytes32 public constant ARCHIVE_GAS =
        keccak256("6529STREAM_GGP_VIEW_PRESERVATION_REFERENCE_ARCHIVE_GAS");
        ...
        StreamGasParameterHost(executor)
    {
        ...
        for (uint256 i; i < 4; ++i) {
            if (configs[i].failureClass != 1) revert T.InvalidViewPreservationReference();
        }
        _fixed = d;
        core = d.targets[0];
    ...
    modifier guarded() {
        if (_entered) revert T.InvalidViewPreservationReference();
        _entered = true;
        _;
        _entered = false;
    }
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == type(IERC165).interfaceId
            || id == type(IStreamViewPreservationReferencePublicationV1).interfaceId
        ...
        returns (bytes32 hash)
    {
        ...
        ) revert T.ViewPreservationReferenceAuthority(msg.sender);
        _receipts[hash] = r;
        _history[subject].push(hash);
        _ids[subject][p.observation.referenceId] = true;
        emit ViewPreservationReferencePublished(
        ...
        returns (T.Receipt memory r)
    {
        ...
        if (
            ...
            revert T.ViewPreservationReferenceLineage(hash, _head(subject));
        }
        ...
        returns (bytes32 actionScope, bytes32 oldHash, bytes32 newHash)
    {
        ...
        if (r.observation.recordHash == 0 || _locks[subject].actionId != 0) {
            revert T.ViewPreservationReferenceLocked(subject);
        }
        actionScope = keccak256(
            abi.encode(
                keccak256("6529STREAM_VIEW_PRESERVATION_REFERENCE_LOCK_SCOPE_V1"),
                deploymentChainId,
                address(this),
                core,
                scope,
    ...
    function lockReference(StreamFinalityScope calldata scope) external override guarded {
        ...
        emit ViewPreservationReferenceLocked(1, r.scopeSubject, value);
    }
        ...
        returns (StreamFinalityComponentState memory)
    {
        ...
        if (
            ...
            revert T.ViewPreservationReferenceLocked(r.scopeSubject);
        }
    ...
    function streamModuleVersion() public pure override returns (bytes32) {
        return keccak256("STREAM_VIEW_PRESERVATION_REFERENCE_RENDER_IMPLEMENTATION_V1");
    }
    ...
    function _authority(uint256 cid, address actor) private view returns (uint8 cls, uint64 rev) {
        if (actor == address(0)) revert T.ViewPreservationReferenceAuthority(actor);
        for (uint8 i; i < 2; ++i) {
            cls = i == 0 ? 3 : 8;
            bytes memory raw = Reads.read(
                metadataHost,
            ...
            if (keccak256(raw) != keccak256(abi.encode(enabled, rev))) {
                revert T.ViewPreservationReferenceDependency(metadataHost);
            }
            if (enabled && rev != 0) return (cls, rev);
        }
        revert T.ViewPreservationReferenceAuthority(actor);
    }
    ...
    function _candidate(T.Publication calldata p) private view returns (bytes32 subject) {
        ...
        if (_head(subject) != o.expectedHead || _history[subject].length != o.expectedRevision) {
            revert T.ViewPreservationReferenceLineage(o.expectedHead, _head(subject));
        }
        if (_locks[subject].actionId != 0) revert T.ViewPreservationReferenceLocked(subject);
    ...
    function _known(bytes32 hash) private view {
        if (hash == 0 || _receipts[hash].observation.recordHash != hash) {
            revert T.ViewPreservationReferenceUnknown(hash);
        }
    }
}
