...
import { StreamCurrentAuthorityScopedRenderCriticalStageGuard as StageGuard } from "./StreamCurrentAuthorityScopedRenderCriticalStageGuard.sol";

import { StreamCurrentAuthorityScopedInventoryLifecycle as Lifecycle } from "./StreamCurrentAuthorityScopedInventoryLifecycle.sol";
...
import {
    StreamCurrentAuthorityScopedRenderCriticalOriginalStages as Native
} from "./StreamCurrentAuthorityScopedRenderCriticalOriginalStages.sol";
import {
    StreamCurrentAuthorityScopedRenderCriticalOriginalStages as Reference
} from "./StreamCurrentAuthorityScopedRenderCriticalOriginalStages.sol";
import {
    StreamCurrentAuthorityScopedRenderCriticalDescriptionStages as Descriptions
} from "./StreamCurrentAuthorityScopedRenderCriticalDescriptionStages.sol";
import {
    StreamCurrentAuthorityScopedRenderCriticalRecordStages as Records
} from "./StreamCurrentAuthorityScopedRenderCriticalRecordStages.sol";
import {
    StreamCurrentAuthorityScopedRenderCriticalInterviewStage as Interviews
} from "./StreamCurrentAuthorityScopedRenderCriticalInterviewStage.sol";
import {
...
import {
    StreamCurrentAuthorityScopedRenderCriticalTokenStages as Tokens
} from "./StreamCurrentAuthorityScopedRenderCriticalTokenStages.sol";
...
contract StreamCurrentAuthorityScopedRenderCriticalInventory is
    IStreamMultiOriginScopedRenderCriticalInventory,
    IStreamCurrentAuthorityInventory
{
    ...
    constructor(S.Dependencies memory d, O.Dependencies memory od, D.Dependencies memory ad) {
        ...
        _dependencyHash = D.dependencyHash(D.SCOPED_INVENTORY_PROFILE, d, od, ad);
        OriginCalls.pin(od);
    ...
    function beginInventory(StreamFinalityScope calldata scope) external returns (bytes32 id) {
        return Lifecycle.beginInventory(_states, _config, _originDependencies, _dependencyHash, scope);
    }
    ...
    function appendReference(bytes32 id, uint64 maximum) external {
        Reference.appendReference(_states[id], id, maximum);
    }
    ...
    function appendRootAuthorization(
        ...
        StageGuard.stage(_states[id], id, 6);
    ...
    function sealInventory(bytes32 id) external returns (Scoped.Evidence memory evidence) {
        return Lifecycle.sealInventory(_states, id);
    }
    ...
    {
        return Lifecycle.requireCurrent(_states, _config, _originDependencies, _dependencyHash, scope);
    }

    function requireFullDefinitionBytes(bytes32 id) external view {
        Lifecycle.requireFullDefinitionBytes(_states, id);
    }
    ...
    function originProfile() external pure returns (bytes32) {
        return D.SCOPED_INVENTORY_PROFILE;
    }
    ...
    function appendOriginRuntime(bytes32 id) external {
        StageGuard.stage(_states[id], id, 8);
