...
/// @notice Fixed original op17 proof reader for scoped V2 inventory authorization.
/// @dev Complete original proof order and wire types are retained across the linked call.
library StreamScopedPolicyRenderCriticalRootAuthorizationReadV2 {
    struct Envelope {
        uint16 version;
    ...
    function contentItem(
        ...
        raw = IO.fixedRead(
            d.targets[4],
            abi.encodeCall(RootV2.scopedPolicyContentRootBinding, (c.rootRecordHash)),
            736,
            d.readGas
        );
        ...
        if (
            originalAggregate.revision == 0 || originalAggregate.transitionChain == 0
                || binding.profileId != RootSchemas.PROFILE || originalLegacyFamilyHash == 0
        ...
        Loaded memory loaded = _load(d, record.artistConsent, actor, 17);
