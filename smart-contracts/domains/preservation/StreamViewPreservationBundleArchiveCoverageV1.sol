...
contract StreamViewPreservationBundleArchiveCoverageV1 is
    IStreamViewPreservationBundleArchiveCoverageV1
{
    ...
    event ViewPreservationBundleRefreshAdvanced(
        uint16 schemaVersion,
        bytes32 indexed inventoryPlan,
    ...
    function supportsInterface(bytes4 id) external pure returns (bool) {
        return
            id == type(IStreamViewPreservationBundleArchiveCoverageV1).interfaceId
                || id == 0x01ffc9a7;
    }
    ...
    function beginCoverage(bytes32 id) external {
        ...
        bytes memory raw = IO.fixedRead(
            ...
            abi.encodeCall(
                IStreamViewPreservationRenderCriticalInventoryV1.inventoryEvidence, (id)
            ),
            736,
        ...
        emit ViewPreservationBundleCoverageStarted(1, id, e.renderCriticalEvidenceHash);
    }
    ...
    function _validateInventory(Scoped.Evidence memory full) private view {
        ...
        T.Evidence memory e = full.inventory;
        if (
            scope.scopeType != StreamFinalityScopeType.VIEW || scope.collectionId != e.collectionId
                || e.scopeSubject
        ...
        bytes memory raw = IO.fixedRead(
            _dependencies.targets[2],
            abi.encodeCall(IStreamViewPreservationRenderCriticalInventoryV1.dependencyHash, ()),
            32,
            _dependencies.readGas
        );
        ...
        if (
            IO.word(
                    _dependencies.targets[2],
                    abi.encodeCall(
                        IStreamViewPreservationRenderCriticalInventoryV1.inventoryProfile, ()
                    ),
                    _dependencies.readGas
                ) != Scoped.PROFILE
        ) revert T.InventorySourceChanged();
    ...
    function _append(
        ...
        p.evidenceChainHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_VIEW_PRESERVATION_BUNDLE_COVERED_ITEM_V1"),
                p.evidenceChainHash,
                id,
                p.itemCount,
                itemHash,
        ...
        emit ViewPreservationBundleItemAdmitted(1, id, p.itemCount, itemHash, a);
        p.itemCount += 1;
    ...
    function _finishSegment(bytes32 id, T.Segment memory s) private {
        ...
        emit ViewPreservationBundleCoverageCompleted(
            1, id, result.coverage.bundleCoverageHash, result
        );
    }
    ...
    function refreshNext(bytes32 id, uint64 expectedIndex) external {
        ...
        emit ViewPreservationBundleRefreshAdvanced(
    ...
    function _segment(bytes32 id, uint64 index) private view returns (T.Segment memory result) {
        ...
        bytes memory raw = IO.fixedRead(
            _dependencies.targets[2],
            abi.encodeCall(
                IStreamViewPreservationRenderCriticalInventoryV1.inventorySegment, (id, index)
            ),
            128,
            _dependencies.readGas
        );
    ...
    function _refreshId(bytes32 id, bytes32 environment) private view returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_VIEW_PRESERVATION_BUNDLE_REFRESH_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                id,
        ...
        returns (bytes32)
    {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_VIEW_PRESERVATION_BUNDLE_CURRENT_OBSERVATION_V1"),
                previous,
                index,
                itemHash,
                observation
