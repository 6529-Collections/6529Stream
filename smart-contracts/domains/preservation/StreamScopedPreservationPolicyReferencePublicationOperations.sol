...
import {
    StreamScopedPreservationPolicyReferenceRecordsV1 as Records
} from "./StreamScopedPreservationPolicyReferenceRecordsV1.sol";
...
library StreamScopedPreservationPolicyReferencePublicationOperations {
    event ScopedPolicyReferencePublished(
    ...
    bytes32 private constant READ_GAS =
        keccak256("6529STREAM_GGP_SCOPED_POLICY_REFERENCE_READ_GAS");
    bytes32 private constant SOURCE_GAS =
    ...
    bytes32 private constant ARCHIVE_GAS =
    ...
    function previewReference(
        ...
        bytes32 subject = _candidate(_fixed, _locks, _ids, _history, _publications, p);
    ...
    function publishReference(
        ...
        hash = keccak256(
            abi.encode(
                F.recordDomain(context._referenceFamily, true),
        ...
        r.observation.recordChainHash = keccak256(
            abi.encode(
                F.chainDomain(context._referenceFamily, true),
                context.deploymentChainId,
                address(this),
                context.core,
                subject,
        ...
        if (
            cls != r.observation.authorizationClass || rev != r.observation.grantRevision
                || _candidate(_fixed, _locks, _ids, _history, _publications, p) != subject
    ...
    function _candidate(
        ...
        _requireExactScope(_publications, _head(_history, subject), p.scope);
        ...
        if (
            ...
            revert T.ScopedPolicyReferenceLineage(o.expectedHead, _head(_history, subject));
        }
    ...
    function _receipt(
        ...
        r.observation.schemaHash = F.definition(context._referenceFamily, true).schemaHash;
        r.observation.profileHash = F.definition(context._referenceFamily, true).profileHash;
        r.observation.canonicalizationHash = F.definition(context._referenceFamily, true).canonHash;
    }
    ...
    function _authority(
        ...
        for (uint8 i; i < 2; ++i) {
            ...
            if (keccak256(raw) != keccak256(abi.encode(enabled, rev))) {
                revert T.ScopedPolicyReferenceDependency(context.metadataHost);
            }
