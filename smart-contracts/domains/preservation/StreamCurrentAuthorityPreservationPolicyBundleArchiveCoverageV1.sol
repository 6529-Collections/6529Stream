...
} from "../../interfaces/stream/preservation/IStreamPreservationPolicyRenderCriticalInventoryV1.sol";
import {
    IStreamRenderCriticalInventory
} from "../../interfaces/stream/preservation/IStreamRenderCriticalInventory.sol";
...
/// @notice Captured-authority archival admission for a preservation-policy collection inventory.
...
contract StreamCurrentAuthorityPreservationPolicyBundleArchiveCoverageV1 is
    ...
    IStreamCurrentAuthorityBundleArchiveCoverage
{
    ...
    bytes32 public constant INVENTORY_PROFILE = D.PRESERVATION_POLICY_INVENTORY_PROFILE;
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == 0x01ffc9a7 || id == type(IStreamBundleArchiveCoverage).interfaceId
            || id == type(IStreamCurrentAuthorityBundleArchiveCoverage).interfaceId;
    }

    function preservationPolicyBundleArchiveProfile() external pure returns (bytes32) {
        return PROFILE;
    }
    ...
    function _environment(bytes32 id) private view returns (bytes32) {
        ...
        _capability(
            inventory,
            type(IStreamCurrentAuthorityPreservationPolicyRenderCriticalInventoryV1).interfaceId,
            true
        );
        ...
        if (
            IO.word(
                    inventory,
                    abi.encodeCall(
                        IStreamPreservationPolicyRenderCriticalInventoryV1.preservationPolicyInventoryProfile,
                        ()
                    ),
    ...
    function _validateInventory(T.Evidence memory e) private view {
        if (e.sourceContextHash == 0 || e.segmentChainHash == 0) revert T.InventorySourceChanged();
        bytes32 dependency = IO.word(
            _dependencies.targets[2],
        ...
        e.renderCriticalEvidenceHash = expected;
    }
        ...
        external
    {
        ...
        (B.Admission memory a, bytes32 observation, bytes32 originHash) =
            Reads.admit(_dependencies, id, _inventories[id], item, proof);
        ...
        p.evidenceChainHash = keccak256(
            abi.encode(
                keccak256(
                    "6529STREAM_CURRENT_AUTHORITY_PRESERVATION_POLICY_BUNDLE_COVERED_ITEM_V1"
                ),
                p.evidenceChainHash,
                id,
                p.itemCount,
    ...
    function _finishSegment(bytes32 id, T.Segment memory s) private {
        ...
        result.bundleCoverageHash = keccak256(
            abi.encode(
                keccak256(
                    "6529STREAM_CURRENT_AUTHORITY_PRESERVATION_POLICY_BUNDLE_ARCHIVE_COVERAGE_V1"
                ),
                block.chainid,
                address(this),
                dependencyHash,
    ...
    function _refreshId(bytes32 id, bytes32 environment) private view returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_CURRENT_AUTHORITY_PRESERVATION_POLICY_BUNDLE_REFRESH_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                id,
        ...
        returns (bytes32)
    {
        return keccak256(
            abi.encode(
                keccak256(
                    "6529STREAM_CURRENT_AUTHORITY_PRESERVATION_POLICY_BUNDLE_CURRENT_OBSERVATION_V1"
                ),
                previous,
                index,
                itemHash,
