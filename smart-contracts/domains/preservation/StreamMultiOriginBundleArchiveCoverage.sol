...
/// @notice Additive archival admission for a sealed multi-origin collection inventory.
...
contract StreamMultiOriginBundleArchiveCoverage is IStreamBundleArchiveCoverage {
    bytes32 public constant PROFILE = O.COVERAGE_PROFILE;
    ...
    constructor(B.Dependencies memory d, O.Dependencies memory o, bytes32 inventoryProfile_) {
        ...
        if (
            inventoryProfile_ != O.INVENTORY_PROFILE
                && inventoryProfile_ != O.POLICY_INVENTORY_PROFILE
        ) revert O.InvalidArchiveOrigin();
        ...
        external
    {
        ...
        p.evidenceChainHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_BUNDLE_COVERED_ITEM_V1"),
                p.evidenceChainHash,
                id,
                p.itemCount,
                itemHash,
    ...
    function _finishSegment(bytes32 id, T.Segment memory s) private {
        ...
        result.bundleCoverageHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_BUNDLE_ARCHIVE_COVERAGE_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                PROFILE,
    ...
    function _refreshId(bytes32 id, bytes32 environment) private view returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_BUNDLE_REFRESH_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                id,
        ...
        returns (bytes32)
    {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_BUNDLE_CURRENT_OBSERVATION_V1"),
                previous,
                index,
                itemHash,
                observation
