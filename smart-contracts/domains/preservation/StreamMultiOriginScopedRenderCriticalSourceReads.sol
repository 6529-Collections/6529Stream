...
import {
    IStreamScopedReferencePublication as Reference
...
import {
    StreamScopedReferenceSourceReads as ScopedOriginal
...
import {
    StreamFinalityScopedReferenceReads as References
} from "../finality/StreamFinalityScopedReferenceReads.sol";
import {
...
library StreamMultiOriginScopedRenderCriticalSourceReads {
        ...
        returns (bytes32)
    {
        bytes memory raw = IO.read(
            d.targets[5],
            abi.encodeCall(
                IStreamScopedSnapshotPublication.snapshotRecord, (c.snapshot.recordHash)
            ),
            16384,
            d.sourceGas
        );
        (
            StreamScopedSnapshotTypes.Publication memory p,
            StreamScopedSnapshotTypes.Receipt memory receipt
        ) = abi.decode(
            raw, (StreamScopedSnapshotTypes.Publication, StreamScopedSnapshotTypes.Receipt)
        );
