...
contract StreamCurrentAuthorityScopedBundleArchiveCoverage is IStreamScopedBundleArchiveCoverage {
    ...
    constructor(
        ...
        if (
            inventoryProfile_ != D.SCOPED_INVENTORY_PROFILE
                && inventoryProfile_ != D.SCOPED_POLICY_INVENTORY_PROFILE
        ) revert O.InvalidArchiveOrigin();
        if (
        ...
        dependencyHash = keccak256(abi.encode(PROFILE, inventoryProfile_, d, o, a));
    }
        ...
        external
    {
        ...
        p.evidenceChainHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_CURRENT_AUTHORITY_SCOPED_BUNDLE_COVERED_ITEM_V1"),
                p.evidenceChainHash,
                id,
                p.itemCount,
                itemHash,
    ...
    function _finishSegment(bytes32 id, T.Segment memory s) private {
        ...
        result.coverage.bundleCoverageHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_CURRENT_AUTHORITY_SCOPED_BUNDLE_ARCHIVE_COVERAGE_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                PROFILE,
    ...
    function _refreshId(bytes32 id, bytes32 environment) private view returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_CURRENT_AUTHORITY_SCOPED_BUNDLE_REFRESH_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                id,
        ...
        returns (bytes32)
    {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_CURRENT_AUTHORITY_SCOPED_BUNDLE_CURRENT_OBSERVATION_V1"),
                previous,
                index,
                itemHash,
                observation
