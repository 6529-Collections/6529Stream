...
import "../../interfaces/stream/preservation/IStreamScopedRenderCriticalInventory.sol";
import "../../interfaces/stream/preservation/IStreamScopedBundleArchiveCoverage.sol";
import {
...
/// @notice Additive archival admission for an exact multi-origin TOKEN/RELEASE/SEASON inventory.
...
contract StreamMultiOriginScopedBundleArchiveCoverage is IStreamScopedBundleArchiveCoverage {
    bytes32 public constant PROFILE =
        keccak256("6529STREAM_MULTI_ORIGIN_SCOPED_BUNDLE_IMMUTABLE_STOP_AGGREGATE_V1");
    bytes32 public INVENTORY_PROFILE;
    ...
    event ScopedBundleCoverageCompleted(
        ...
        Scoped.BundleEvidence evidence
    );
    ...
    constructor(B.Dependencies memory d, O.Dependencies memory o, bytes32 inventoryProfile_) {
        ...
        if (
            inventoryProfile_ != O.SCOPED_INVENTORY_PROFILE
                && inventoryProfile_ != O.SCOPED_POLICY_INVENTORY_PROFILE
        ) revert O.InvalidArchiveOrigin();
        INVENTORY_PROFILE = inventoryProfile_;
        _dependencies = d;
        _originDependencies = o;
        dependencyHash = keccak256(abi.encode(PROFILE, inventoryProfile_, d, o));
    }
    ...
    function _environment(bytes32 id) private view returns (bytes32) {
        return Reads.environment(_dependencies, _originDependencies, INVENTORY_PROFILE, id);
    }
    ...
    function beginCoverage(bytes32 id) external {
        ...
        bytes memory raw = IO.fixedRead(
            ...
            abi.encodeCall(IStreamScopedRenderCriticalInventory.inventoryEvidence, (id)),
            736,
    ...
    function _validateInventory(Scoped.Evidence memory full) private view {
        ...
        bytes memory raw = IO.fixedRead(
            _dependencies.targets[2],
            abi.encodeCall(IStreamScopedRenderCriticalInventory.dependencyHash, ()),
            32,
            _dependencies.readGas
        );
        ...
        external
    {
        ...
        p.evidenceChainHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_SCOPED_BUNDLE_COVERED_ITEM_V1"),
                p.evidenceChainHash,
                id,
                p.itemCount,
                itemHash,
    ...
    function _finishSegment(bytes32 id, T.Segment memory s) private {
        ...
        p.complete = true;
        Scoped.BundleEvidence memory result = Scoped.BundleEvidence(
            full.scope,
        ...
        result.coverage.bundleCoverageHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_SCOPED_BUNDLE_ARCHIVE_COVERAGE_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                PROFILE,
    ...
    function requireCoverage(StreamFinalityScope calldata scope, bytes32 id, bytes32 expectedHash)
        ...
        view
        override
        returns (Scoped.BundleEvidence memory result)
    {
    ...
    function _segment(bytes32 id, uint64 index) private view returns (T.Segment memory result) {
        ...
        bytes memory raw = IO.fixedRead(
            _dependencies.targets[2],
            abi.encodeCall(IStreamScopedRenderCriticalInventory.inventorySegment, (id, index)),
            128,
            _dependencies.readGas
        );
    ...
    function _refreshId(bytes32 id, bytes32 environment) private view returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_SCOPED_BUNDLE_REFRESH_V1"),
                block.chainid,
                address(this),
                dependencyHash,
                id,
        ...
        returns (bytes32)
    {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_MULTI_ORIGIN_SCOPED_BUNDLE_CURRENT_OBSERVATION_V1"),
                previous,
                index,
                itemHash,
                observation
