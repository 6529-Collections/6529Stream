...
abstract contract StreamScopedPreservationPolicyReferencePublicationBase is
    IStreamScopedPreservationPolicyReferencePublicationV1,
    IStreamReferenceInventoryPreparation,
    IStreamReferenceEnvironmentPreparation,
    IStreamArtworkScopedFinalityComponent,
    ...
    StreamGasParameterHost
{
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == type(IERC165).interfaceId
            || id == type(IStreamScopedPreservationPolicyReferencePublicationV1).interfaceId
    ...
    function scopedPreservationPolicyReferenceProfile()
        external
        pure
        virtual
        override
        ...
        returns (bytes32 sourceHash, bytes memory canonical)
    {
        ...
        return Records.prepare(_fileInventories, dependencies(), p, r, false, _referenceFamily);
    }
        ...
        returns (bytes32 hash)
    {
        ...
        (r.observation.sourcesHash, canonical) =
            Records.prepare(_fileInventories, d, p, r, false, _referenceFamily);
        if (
        ...
        hash = keccak256(
            abi.encode(
                F.recordDomain(_referenceFamily, true),
                deploymentChainId,
                address(this),
                core,
                metadataHost,
        ...
        r.observation.recordChainHash = keccak256(
            abi.encode(
                F.chainDomain(_referenceFamily, true),
                deploymentChainId,
                address(this),
                core,
                subject,
        ...
        returns (bytes32 actionScope, bytes32 oldHash, bytes32 newHash)
    {
        ...
        actionScope = keccak256(
            abi.encode(
                F.lockDomain(_referenceFamily, true),
                deploymentChainId,
                address(this),
                core,
                scope,
    ...
    function lockReference(StreamFinalityScope calldata scope) external override guarded {
        ...
        emit ScopedPolicyReferenceLocked(F.isV2(_referenceFamily) ? 2 : 1, r.scopeSubject, value);
    }
        ...
        returns (StreamFinalityComponentState memory)
    {
        ...
        return StreamFinalityComponentState(
            ...
            streamModuleVersion(),
            F.definition(_referenceFamily, true).profileHash,
            keccak256(
                abi.encode(
                    F.componentDomain(_referenceFamily, true),
                    deploymentChainId,
                    address(this),
                    core,
                    scope,
        ...
        returns (T.Receipt memory r)
    {
        ...
        r.observation.schemaHash = F.definition(_referenceFamily, true).schemaHash;
        r.observation.profileHash = F.definition(_referenceFamily, true).profileHash;
        r.observation.canonicalizationHash = F.definition(_referenceFamily, true).canonHash;
    }
