...
/// @notice Fixed current-context worker for the original scoped inventory reader.
/// @dev No state, caller-selected callback or new policy. The original read/check/hash
/// order and caps remain intact; the complete memory context is returned to its caller.
library StreamScopedPreservationPolicyRenderCriticalCurrentReadsV1 {
    function read(S.Dependencies memory d, StreamFinalityScope memory scope)
        public
        view
