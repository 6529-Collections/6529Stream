...
import {
    StreamPolicyReferenceSourceReadsV2 as Sources
} from "./StreamPolicyReferenceSourceReadsV2.sol";
import { StreamPolicyReferenceRecordsV2 as Records } from "./StreamPolicyReferenceRecordsV2.sol";
import {
...
/// @notice Exact repeated reference observations for complete COLLECTION V2 policy/output scopes.
...
contract StreamPolicyReferencePublicationV2 is
    IStreamPolicyReferencePublicationV2,
    IStreamReferenceInventoryPreparation,
    IStreamReferenceEnvironmentPreparation,
    IStreamArtworkScopedFinalityComponent,
    ...
    StreamGasParameterHost
{
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == type(IERC165).interfaceId
            || id == type(IStreamPolicyReferencePublicationV2).interfaceId
        ...
        returns (bytes32 actionScope, bytes32 oldHash, bytes32 newHash)
    {
        ...
        actionScope = keccak256(
            abi.encode(
                keccak256("6529STREAM_POLICY_REFERENCE_LOCK_SCOPE_V2"),
                deploymentChainId,
                address(this),
                core,
                scope,
    ...
    function streamModuleVersion() public pure override returns (bytes32) {
        return keccak256("STREAM_POLICY_REFERENCE_RENDER_IMPLEMENTATION_V2");
    }
