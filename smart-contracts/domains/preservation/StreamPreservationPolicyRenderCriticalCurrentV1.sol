...
library StreamPreservationPolicyRenderCriticalCurrentV1 {
        ...
        returns (T.Evidence memory evidence)
    {
        ...
        if (p.nextToken != p.tokenCount || s.progress[id].phase != 0) {
        ...
        evidence.renderCriticalEvidenceHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_PRESERVATION_POLICY_RENDER_CRITICAL_EVIDENCE_V1"),
                block.chainid,
                address(this),
