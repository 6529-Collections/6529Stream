...
import {
    StreamScopedPreservationPolicyRenderCriticalOriginalReadsV1 as OriginalReads
} from "./StreamScopedPreservationPolicyRenderCriticalOriginalReadsV1.sol";
import {
...
library StreamScopedPreservationPolicyRenderCriticalTokenReadsV1 {
    // Retained original error ABI; these selectors are raised by the fixed row worker.
    ...
    function tokenItems(
        ...
        rows[6] = Items.bytesItem(
            ...
            sd.targets[6],
            c.snapshotSource.content.selectionId,
            index,
        ...
        rows[10] = Items.runtime(
            keccak256("ORIGINAL_TERMINAL_READINESS_RUNTIME"), o.readiness, record, token
        );
        rows[11] = o.output.entropy.terminal
            ? Items.bytesItem(
                T.Kind.NATIVE_BYTES,
                keccak256("ORIGINAL_TERMINAL_RENDER_ADMISSION"),
                o.readiness,
                record,
                token,
                o.terminalAdmission
            ...
            : Items.absent(
                keccak256("ORIGINAL_TERMINAL_RENDER_ADMISSION"), o.readiness, record, token
            );
    }
    ...
    function _original(
        ...
        return OriginalReads.read(d, c, sd, index, token);
    }
