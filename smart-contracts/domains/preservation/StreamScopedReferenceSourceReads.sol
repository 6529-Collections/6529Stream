...
import {
    StreamFinalityScopedSnapshotReads as SnapRead
} from "../finality/StreamFinalityScopedSnapshotReads.sol";
import {
    StreamFinalityRouterEvidence as Reads
} from "../finality/StreamFinalityRouterEvidence.sol";
import {
    StreamScopedReferenceSampleReads as Samples
} from "./StreamScopedReferenceSampleReads.sol";
import {
...
library StreamScopedReferenceSourceReads {
    ...
    function bindings(T.Dependencies memory d) public view returns (S.Dependencies memory source) {
        ...
        for (uint256 i; i < 7; ++i) {
            if (d.targets[i].code.length == 0 || d.targets[i].codehash != d.codeHashes[i]) {
                revert T.ScopedReferenceDependency(d.targets[i]);
            }
        }
        bytes memory raw =
        ...
        if (source.chainId != d.chainId) revert T.InvalidScopedReference();
        for (uint256 i; i < 5; ++i) {
            if (source.targets[i] != d.targets[i] || source.codeHashes[i] != d.codeHashes[i]) {
                revert T.ScopedReferenceDependency(d.targets[5]);
            }
        }
        if (
            abi.decode(
            ...
            revert T.ScopedReferenceDependency(d.targets[6]);
        }
    }
        ...
        returns (T.SourceFacts memory f)
    {
        ...
        if (p.observation.collectionId != p.scope.collectionId) revert T.InvalidScopedReference();
        ...
        returns (bytes32)
    {
        return keccak256(
            abi.encode(
                keccak256("6529STREAM_SCOPED_REFERENCE_SOURCES_V1"),
                d.chainId,
                address(this),
                d.targets,
                d.codeHashes,
    ...
    function _snapshot(
        ...
        if (
            domain != keccak256("6529STREAM_SCOPED_SNAPSHOT_PAYLOAD_V1") || chain != d.chainId
                || host != d.targets[5]
    ...
    function _root(T.Dependencies memory d, T.Publication memory p, T.SourceFacts memory f)
        private
        view
    {
    ...
    function _canonical(address target, bytes memory raw, bytes memory encoded) private pure {
        if (keccak256(raw) != keccak256(encoded)) revert T.ScopedReferenceDependency(target);
    }
}
