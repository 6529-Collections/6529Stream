...
library StreamMultiOriginScopedPolicyRenderCriticalStateV2 {
    struct DocumentPin {
        bytes32 id;
        bytes32 factsHash;
    }
    ...
    function idFor(bytes32 dependencyHash, Scoped.Context memory c, bytes32 lineageHash)
        internal
        view
        returns (bytes32)
    {
    ...
    function requireCurrent(State storage s, bytes32 id) internal view {
        ...
        if (
            s.plans[id].progress.collectionId == 0 || lineageHash != s.origins.lineage[id]
                || idFor(s.dependencyHash, current, lineageHash) != id
                || keccak256(abi.encode(current)) != s.plans[id].progress.sourceContextHash
    ...
    function append(State storage s, bytes32 id, T.Item[] memory rows, bytes32 witness) internal {
        ...
        bytes32 key = keccak256(
            abi.encode(keccak256("6529STREAM_SCOPED_POLICY_RENDER_CRITICAL_SEGMENT_V2"), id, index)
        );
        ...
        emit ScopedInventorySegmentRecorded(2, id, index, segment, rows);
    }
