...
/// @dev This sibling accepts only the explicitly typed scoped preservation inventory capability/profile.
...
contract StreamScopedPreservationPolicyBundleArchiveCoverageV1 is
    IStreamScopedPreservationPolicyBundleArchiveCoverageV1
{
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == type(IStreamScopedPreservationPolicyBundleArchiveCoverageV1).interfaceId
            || id == 0x01ffc9a7;
    }
    ...
    function beginCoverage(bytes32 id) external {
        ...
        bytes memory raw = IO.fixedRead(
            ...
            abi.encodeCall(
                IStreamScopedPreservationPolicyRenderCriticalInventoryV1.inventoryEvidence, (id)
            ),
            736,
    ...
    function _validateInventory(Scoped.Evidence memory full) private view {
        ...
        bytes memory raw = IO.fixedRead(
            _dependencies.targets[2],
            abi.encodeCall(
                IStreamScopedPreservationPolicyRenderCriticalInventoryV1.dependencyHash, ()
            ),
            32,
            _dependencies.readGas
        );
    ...
    function _segment(bytes32 id, uint64 index) private view returns (T.Segment memory result) {
        ...
        bytes memory raw = IO.fixedRead(
            _dependencies.targets[2],
            abi.encodeCall(
                IStreamScopedPreservationPolicyRenderCriticalInventoryV1.inventorySegment,
                (id, index)
            ),
            128,
            _dependencies.readGas
    ...
    /// archive bytes. Authenticate the preservation producer before decoding its full Evidence.
        ...
        if (
            IO.word(
                        ...
                        abi.encodeWithSelector(
                            bytes4(0x01ffc9a7),
                            type(IStreamScopedPreservationPolicyRenderCriticalInventoryV1)
                            .interfaceId
                        ),
                        _dependencies.readGas
                    ) != bytes32(uint256(1))
                || IO.word(
                        inventory,
                        abi.encodeCall(
                            IStreamScopedPreservationPolicyRenderCriticalInventoryV1.scopedPreservationPolicyInventoryProfile,
                            ()
                        ),
                        _dependencies.readGas
                    ) != Scoped.PROFILE
