...
library StreamScopedReferenceRecords {
    function prepare(
        ...
        canonical = abi.encode(
            keccak256("6529STREAM_SCOPED_REFERENCE_PAYLOAD_V1"),
            d.chainId,
            address(this),
            p,
            receipt,
        ...
        if (canonical.length == 0 || canonical.length > 524288) revert T.InvalidScopedReference();
    }
    ...
    function source(Bytes.Manifest storage payload) public view returns (bytes memory) {
        ...
        if (
            domain != keccak256("6529STREAM_SCOPED_REFERENCE_PAYLOAD_V1")
                || keccak256(raw)
        ...
        returns (T.Publication memory p)
    {
        ...
        if (keccak256(raw) != keccak256(abi.encode(p))) revert T.InvalidScopedReference();
    }
