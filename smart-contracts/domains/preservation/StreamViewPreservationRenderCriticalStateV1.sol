...
library StreamViewPreservationRenderCriticalStateV1 {
    struct DocumentPin {
        bytes32 id;
        bytes32 factsHash;
    }
    ...
    function append(State storage s, bytes32 id, T.Item[] memory rows, bytes32 witness) internal {
        ...
        bytes32 key = keccak256(
            abi.encode(
                keccak256("6529STREAM_VIEW_PRESERVATION_RENDER_CRITICAL_SEGMENT_V1"), id, index
            )
        );
        ...
        emit ViewPreservationInventorySegmentRecorded(1, id, index, segment, rows);
    }
