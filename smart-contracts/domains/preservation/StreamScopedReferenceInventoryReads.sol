...
import {
    StreamScopedReferenceTypes as SR
...
import {
    StreamScopedReferenceDefinitions as Definitions
...
library StreamScopedReferenceInventoryReads {
    ...
    function appendReference(State.State storage state, bytes32 id, uint64 maximum) public {
        ...
        (T.Item[] memory rows, uint64 total) =
            items(state.dependencies, state.contexts[id], plan_.referenceCursor, maximum);
        ...
        for (uint256 i; i < length; ++i) {
            ...
            if (at == 0) {
                ...
                result[i] = Items.bytesItem(
                    ...
                    keccak256("SCOPED_REFERENCE_MANIFEST"),
                    d.targets[6],
                    receipt.recordHash,
                    0,
                    payload
