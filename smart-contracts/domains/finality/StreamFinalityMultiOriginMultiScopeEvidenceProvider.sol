...
import "./StreamFinalityMultiOriginNativeEvidenceProvider.sol";
...
import {
    StreamFinalityMultiOriginScopedProviderOperations
} from "./StreamFinalityMultiOriginScopedProviderOperations.sol";
...
/// @notice Multi-origin COLLECTION and distinct TOKEN/RELEASE/SEASON evidence at one selected host.
...
/// COLLECTION policy V2 and selected/adopted VIEW remain separate required profile joins.
contract StreamFinalityMultiOriginMultiScopeEvidenceProvider is
    StreamFinalityMultiOriginNativeEvidenceProvider,
    ...
    IStreamScopedContentRootEvidenceBinding
{
    ...
    ) StreamFinalityMultiOriginNativeEvidenceProvider(original) {
        if (
        ...
        returns (bytes memory)
    {
        ...
        _requireScoped(scope);
        return StreamFinalityMultiOriginScopedProviderOperations.manifest(_scoped, scope);
    }
        ...
        returns (StreamFinalityScopeInputs memory, bytes32, bytes32)
    {
        ...
        _requireScoped(scope);
        return
            StreamFinalityMultiOriginScopedProviderOperations.inputs(_scoped, scope, manifestHash);
    }
        ...
        returns (IStreamFinalitySanctionReview.ReviewFacts memory)
    {
        ...
        _requireScoped(scope);
        return
            StreamFinalityMultiOriginScopedProviderOperations.review(_scoped, scope, manifestHash);
    }
    ...
    function requirePreparedFinalityScopeInputs(
        ...
        (StreamFinalityScopeInputs memory inputs_, bytes32 schema, bytes32 canon,) = StreamFinalityMultiOriginScopedProviderOperations.prepared(
        ...
        )
    {
        ...
        // Fixed worker retains Registry/runtime admission before scope/source validation.
        return StreamFinalityMultiOriginScopedProviderOperations.prepared(
