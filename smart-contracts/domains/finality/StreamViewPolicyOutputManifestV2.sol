...
import {
    IStreamViewPolicyOutputManifestV2 as I,
    T
} from "../../interfaces/stream/finality/IStreamViewPolicyOutputManifestV2.sol";
import {
    StreamViewPolicyCheckpointTypesV2 as C
} from "../../interfaces/stream/finality/StreamViewPolicyCheckpointTypesV2.sol";
import { StreamViewPolicyManifestReadsV2 as Reads } from "./StreamViewPolicyManifestReadsV2.sol";
import {
    StreamViewPolicyManifestEncodingV2 as Encoding
} from "./StreamViewPolicyManifestEncodingV2.sol";
import {
    StreamViewPolicyOutputSchemasV2 as Definitions
} from "./StreamViewPolicyOutputSchemasV2.sol";
...
contract StreamViewPolicyOutputManifestV2 is I, IERC165 {
    ...
    bytes32 private constant PART = keccak256("6529STREAM_VIEW_OUTPUT_PART_VERIFIED_V2");
    bytes32 private constant PLAN = keccak256("6529STREAM_VIEW_OUTPUT_MANIFEST_PLAN_V2");
    bytes32 private constant CHAIN = keccak256("6529STREAM_VIEW_OUTPUT_PART_CHAIN_V2");
    bytes32 private constant RECORD = keccak256("6529STREAM_VIEW_OUTPUT_MANIFEST_VERIFIED_V2");
    ...
    function preparePart(
        ...
        uint16 count = uint16(
            h.tokenCount - first > T.PART_ROWS ? T.PART_ROWS : h.tokenCount - first
        );
        ...
        if (expected.length != 640 + uint256(count) * 992) revert T.InvalidViewManifest();
    ...
    function verifyNextPart(bytes32 key, bytes32 partKey) external returns (bytes32 recordHash) {
        ...
        if (_hash(raw, 640 + uint256(p.nextPart) * 288, 288) != keccak256(abi.encode(d))) {
            revert T.ViewManifestOrder(p.nextPart);
        }
    ...
    {
        if (offset > raw.length || length > raw.length - offset) revert T.InvalidViewManifest();
