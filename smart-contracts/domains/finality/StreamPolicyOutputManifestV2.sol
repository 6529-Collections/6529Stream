...
import "../../interfaces/stream/finality/IStreamPolicyOutputManifestV2.sol";
import "../../interfaces/stream/finality/IStreamPolicyContentCheckpointV2.sol";
...
contract StreamPolicyOutputManifestV2 is
    IStreamPolicyOutputManifestV2,
    ...
    IERC165
{
    ...
    constructor(
        ...
        if (
            abi.decode(
                        _read(
                            checkpoint_,
                            abi.encodeCall(IStreamPolicyContentCheckpointV2.core, ()),
                            32
                        ),
                        (address)
                    ) != core_
                ...
                || !abi.decode(
                    _read(
                        ...
                        abi.encodeCall(
                            ...
                            (type(IStreamPolicyContentCheckpointV2).interfaceId)
                        ),
                        32
                    ),
                    (bool)
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == type(IERC165).interfaceId || id == type(IStreamGasParameterHost).interfaceId
            || id == type(IStreamPolicyOutputManifestV2).interfaceId;
    }
    ...
    function beginManifest(
        ...
        _pins();
        _documents();
        IStreamPolicyContentCheckpointV2.Plan memory p = _checkpoint(checkpointHash);
        ...
        Manifest memory m = Manifest(
            ...
            abi.decode(
                _read(
                    contentCheckpoint,
                    abi.encodeCall(IStreamPolicyContentCheckpointV2.entropySourceSet, ()),
                    32
                ),
                (address)
            ),
        ...
        planHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_POLICY_OUTPUT_MANIFEST_PLAN_V2"),
                deploymentChainId,
                address(this),
                core,
                contentCheckpoint,
        ...
        returns (bytes32 recordHash)
    {
        ...
        for (uint256 i; i < count; ++i) {
            bytes memory actual = _read(
                contentCheckpoint,
                abi.encodeCall(
                    IStreamPolicyContentCheckpointV2.outputAt,
        ...
        if (p.nextIndex == p.manifest.tokenCount) {
            ...
            recordHash = keccak256(
                abi.encode(keccak256("6529STREAM_POLICY_OUTPUT_MANIFEST_VERIFIED_V2"), planHash)
            );
    ...
    function _current(Manifest memory m) private view {
        _documents();
        IStreamPolicyContentCheckpointV2.Plan memory p = _checkpoint(m.checkpointHash);
    ...
    function _checkpoint(bytes32 hash)
        private
        view
        returns (IStreamPolicyContentCheckpointV2.Plan memory)
    {
        return abi.decode(
            _read(
                contentCheckpoint,
                abi.encodeCall(IStreamPolicyContentCheckpointV2.requireCurrentCheckpoint, (hash)),
                416
            ),
            (IStreamPolicyContentCheckpointV2.Plan)
        );
    }
