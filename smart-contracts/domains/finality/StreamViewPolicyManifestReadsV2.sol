...
import {
    IStreamViewPolicyContentCheckpointV2 as Checkpoint
...
library StreamViewPolicyManifestReadsV2 {
    ...
    function header(T.Configuration memory c, bytes32 id) public view returns (T.Header memory h) {
        ...
        if (
            id == 0 || keccak256(raw) != keccak256(abi.encode(p))
                ...
                || p.adoptionRecord == 0 || p.sourceContextHash == 0 || p.membershipHash == 0
                || p.policyChainHash == 0
        ) revert T.InvalidViewManifest();
        h = T.Header(
            id,
    ...
    function documents(T.Configuration memory c) public view {
        bytes32[4] memory ids =
            [Definitions.PART, Definitions.PART_CANON, Definitions.INDEX, Definitions.INDEX_CANON];
        for (uint256 i; i < 4; ++i) {
            bytes memory raw =
