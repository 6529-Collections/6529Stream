...
import {
    StreamFinalityScopedPolicyDiscoveryFactoryReadsV2 as PublicationFactoryReads
} from "./StreamFinalityScopedPolicyDiscoveryFactoryReadsV2.sol";
...
import {
    StreamFinalityPolicyDiscoveryFactoryReadsV2 as CollectionFactoryReads
} from "./StreamFinalityPolicyDiscoveryFactoryReadsV2.sol";
import {
    StreamPolicySnapshotDefinitionsV2 as CollectionPolicyDefinitions
} from "../records/StreamPolicySnapshotDefinitionsV2.sol";

/// @notice Two static profiles and genuine fixed-factory COLLECTION/scoped policy graphs.
...
contract StreamFinalityFullPolicyDiscoveryV2 is
    IERC165,
    ...
    IStreamFinalityEvidenceDiscoveryBinding
{
        ...
        returns (StreamFinalityCurrentComponentRoute memory route)
    {
        ...
        if (index == 8) _pinReference(scope, target);
        else _pin(target);
    ...
    function _initializeProfiles(bytes32 expected) private {
        ...
        if (
            abi.decode(
                    _read(
                        c.provider,
                        abi.encodeCall(Catalogue.factorySourceProfile, ()),
                        32,
                        c.readGas
                    ),
                    (bytes32)
