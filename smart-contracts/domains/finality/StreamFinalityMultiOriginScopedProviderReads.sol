...
import {
    StreamFinalityMultiOriginConfiguration as OriginConfiguration
...
/// @notice Additive multi-origin TOKEN/RELEASE/SEASON inputs from the fixed scoped inventory.
...
library StreamFinalityMultiOriginScopedProviderReads {
    ...
    function requirePins(Scoped.Config memory c) public view {
        if (
    ...
    function currentComponents(Scoped.Config memory c, StreamFinalityScope memory scope)
        public
        view
    ...
    function manifestDependencies(Scoped.Config memory c)
        public
        pure
    ...
    function _inventoryConfiguration(Scoped.Config memory c) private view {
    ...
    function _snapshotConfiguration(Scoped.Config memory c) private view {
        bytes memory raw = _read(
    ...
    function _core(
        ...
        StreamCoreFinalityScopeQuery memory
            query =
    ...
    function _read(Scoped.Config memory c, uint256 i, bytes memory input, uint256 size, uint256 cap)
        private
        view
    ...
    function _word(Scoped.Config memory c, uint256 i, bytes memory input)
        private
        view
        returns (bytes32)
    {
    ...
    function _address(Scoped.Config memory c, uint256 from, string memory selector, uint256 to)
        private
        view
    {
        if (
    ...
    function _self(Scoped.Config memory c, uint256 from, string memory selector) private view {
        if (
            _word(c, from, abi.encodeWithSignature(selector))
