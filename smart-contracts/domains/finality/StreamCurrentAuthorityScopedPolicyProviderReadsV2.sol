...
import {
    StreamFinalityScopedPolicyProviderReadsV2 as Original
...
/// @notice Current-authority scoped-policy TOKEN/RELEASE/SEASON inputs from the complete authenticated scoped inventory.
...
library StreamCurrentAuthorityScopedPolicyProviderReadsV2 {
    ...
    function requirePins(Original.Config memory c) public view {
        ...
        _profile(
            c,
            18,
            type(IStreamMultiOriginScopedPolicyRenderCriticalInventoryV2).interfaceId,
        ...
        // The generic scoped bundle is admitted by its exact current-authority profile,
        // inventory profile and complete constructor hash in AuthorityConfiguration.read.
        _address(c, 6, "core()", 0);
    ...
    function statement(
        ...
        bytes memory raw = _read(
            ...
            18,
            abi.encodeCall(
                IStreamMultiOriginScopedPolicyRenderCriticalInventoryV2.requireCurrent, (scope)
            ),
            736,
            c.sourceGas
        );
    ...
    function _coverage(
        ...
        CoverageTypes.BundleEvidence memory scoped = abi.decode(raw, (CoverageTypes.BundleEvidence));
