...
import "./StreamFinalityViewPreservationInputSchemasV1.sol";
...
import "../../interfaces/stream/finality/StreamFinalityViewPreservationInputTypesV1.sol";
...
library StreamFinalityViewPreservationInputReadsV1 {
    struct Dependencies {
    ...
    function encode(
        ...
        payload = abi.encode(
            StreamFinalityViewPreservationInputSchemasV1.SCHEMA_ID,
            StreamFinalityViewPreservationInputSchemasV1.CANON_ID,
            d.chainId,
            d.targets[0],
            d.targets[1],
            d.targets[4],
    ...
    function requireCurrent(
        Dependencies memory d,
        StreamFinalityViewPreservationInputTypesV1.Statement memory expected,
        bytes32 contentHash
        ...
        schemaId = StreamFinalityViewPreservationInputSchemasV1.SCHEMA_ID;
        canonicalizationId = StreamFinalityViewPreservationInputSchemasV1.CANON_ID;
    ...
    function _definition(Dependencies memory d, bytes32 id, IStreamSchemaRegistry.DocumentKind kind)
        ...
        view
    {
        bytes memory expected = StreamFinalityViewPreservationInputSchemasV1.document(id);
        bytes memory raw = StreamFinalityRouterEvidence.read(
            d.targets[2],
    ...
    function _shape(StreamFinalityViewPreservationInputTypesV1.Statement memory s) private pure {
        if (
            s.scope.scopeType != StreamFinalityScopeType.VIEW || s.scope.collectionId == 0
                ...
                || s.entropy.sourceSet == address(0) || s.entropy.sourceSetCodeHash == 0
                ...
                || s.completeBindingRecordHash == 0 || s.adoptionRecordHash == 0
                || s.membershipHash == 0 || s.rootBindingHash == 0 || s.viewId == 0
                || s.adoptionProfile != keccak256("6529STREAM_STATIC_ADOPTED_POLICY_VIEW_V2")
                || s.outputProfile != keccak256("6529STREAM_ADOPTED_POLICY_VIEW_PRESERVATION_V1")
