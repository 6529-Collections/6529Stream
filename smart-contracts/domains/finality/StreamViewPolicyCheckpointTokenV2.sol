...
library StreamViewPolicyCheckpointTokenV2 {
        ...
        returns (T.Output memory o, bytes memory json, bytes memory html)
    {
        ...
        bool exists;
        uint256 cid;
        (exists, cid, o.collectionSerial, o.burned) = abi.decode(
            raw, (bool, uint256, uint256, bool)
        );
        if (
        ...
        for (uint256 i; i < s.policy.policyCount; ++i) {
            ...
            if (keccak256(raw) != keccak256(abi.encode(candidate))) revert T.InvalidViewCheckpoint();
        ...
        returns (bytes memory out)
    {
        ...
        if (o.burned) {
            raw = Read.bounded(
                c.serving,
                abi.encodeCall(Serving.historicalOutput, (s.adoption.recordHash, o.tokenId, mode)),
                262336,
                c.servingGas,
                false
            );
            ...
            raw = Read.bounded(
                c.serving,
                abi.encodeCall(Serving.currentOutput, (s.adoption.input.scope, o.tokenId, mode)),
                262240,
                c.servingGas,
                false
            );
