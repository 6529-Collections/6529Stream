...
import {
    IStreamScopedPreservationPolicyPublicationFactoryV1 as Factory
...
import {
    StreamScopedPreservationPolicyPublicationGraphReadsV1 as GraphReads
...
/// @notice Fixed publication-factory joins for the additive scoped-policy discovery sibling.
/// @dev Constructor validation never asks for a scope, graph, publication or component state.
library StreamFinalityScopedPreservationPolicyDiscoveryFactoryReadsV1 {
        ...
        returns (address entropyFactory, bytes32 entropyCodeHash)
    {
        ...
        if (
            _word(
                        b.factory,
                        abi.encodeCall(
                            Factory.scopedPreservationPolicyPublicationFactoryProfile, ()
                        ),
                        c.readGas
                    ) != Graph.PROFILE
