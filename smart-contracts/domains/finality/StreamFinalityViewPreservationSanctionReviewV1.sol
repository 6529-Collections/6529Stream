...
import "./StreamFinalityViewPreservationInputReadsV1.sol";
import {
    StreamFinalityViewPreservationReferenceReadsV1 as ReferenceReads
} from "./StreamFinalityViewPreservationReferenceReadsV1.sol";
import {
...
import {
    StreamViewPreservationReferenceTypesV1 as R2
...
import { StreamFinalityViewMediaReviewV1 as Media } from "./StreamFinalityViewMediaReviewV1.sol";
import {
    StreamFinalityViewSanctionProfileV1 as ViewProfile
} from "./StreamFinalityViewSanctionProfileV1.sol";

/// @notice Complete ordered actual VIEW media and original PNG content hashes.
...
library StreamFinalityViewPreservationSanctionReviewV1 {
    ...
    function review(
        ...
        StreamFinalityNativeProviderReads.Config memory original =
            abi.decode(abi.encode(c), (StreamFinalityNativeProviderReads.Config));
        c = Configuration.resolve(original).effective;
        if (
            c.chainId != block.chainid || s.scope.scopeType != StreamFinalityScopeType.VIEW
        ...
        bytes32[] memory media = Media.hashes(original, s);
        if (media.length != 0) {
            ViewProfile.requireCurrent(
        ...
        result.profile = media.length != 0 ? 3 : count == 1 ? 1 : 2;
        result.contentRoot = s.contentRoot;
        // Every actual VIEW media occurrence is separately authenticated against its full archive object.
        result.mediaContentHashes = media;
