...
import {
    IStreamPreservationPolicyRenderCriticalInventoryV1 as Inventory
...
library StreamFinalityFactoryPreservationPolicyProviderReadsV1 {
    // The host supplies these output pins only from its fixed factory's exact current graph.
    // No global output getter or caller-selected source is accepted by the provider.
    ...
    function requirePins(Native.Config memory c, address manifest, bytes32 manifestCode)
        public
        view
    {
        if (
        ...
        if (
            _word(c, 18, abi.encodeCall(Inventory.preservationPolicyInventoryProfile, ()))
        ...
        _snapshotConfiguration(c, manifest, manifestCode);
    ...
    function statement(
        ...
        address manifest,
        bytes32 manifestCode
    )
        public
        view
        returns (StreamFinalityPreservationPolicyInputManifestTypesV1.Statement memory s)
    {
        requirePins(c, manifest, manifestCode);
        if (
        ...
        _coverage(c, e, s);
        _headers(c, e, s, manifest, manifestCode);
        _core(c, s);
    }
    ...
    function manifestDependencies(Native.Config memory c)
        public
        pure
        returns (StreamFinalityPreservationPolicyInputManifestReadsV1.Dependencies memory d)
    {
    ...
    function _entropy(
        ...
        Snapshot.Dependencies memory sd = _snapshotConfiguration(c, manifest, manifestCode);
        bytes memory raw = _read(
            c,
            2,
            abi.encodeCall(PreservationRoot.preservationPolicyContentRootBinding, (root)),
            608,
            c.readGas
        );
        ...
        // The factory derives membership with its own inventory budget; this is an
        // aggregate source read, not a scalar getter despite its 32-byte result.
