...
import "./StreamFinalityLineagePreparation.sol";
import {
...
import {
    IStreamArtistCurrentAuthorityResolver as AuthorityResolver
} from "../../interfaces/stream/preservation/IStreamArtistCurrentAuthorityResolver.sol";
import {
    IStreamArtistCurrentFinality as ArtistCurrentFinality
...
/// @notice Original Finality anchor with separately authenticated current Artist authority.
...
contract StreamLineageArtworkFinalityRegistry is
    ...
    IStreamFinalityCurrentAuthority
{
    ...
    address public immutable override currentAuthorityResolver;
    bytes32 public immutable override currentAuthorityResolverCodeHash;
    bytes32 private immutable _authorityAnchorsHash;
        ...
        )
    {
        ...
        AuthorityTypes.Anchors memory anchors_ = _resolverAnchors(resolver_);
        address router_ = _readExactAddress(provider, bytes4(keccak256("metadataRouter()")));
        if (
            anchors_.targets[0] != coreReads_ || anchors_.codeHashes[0] != coreReads_.codehash
                || anchors_.targets[1] != metadataReads_
                || anchors_.codeHashes[1] != metadataReads_.codehash
                || anchors_.targets[2] != router_ || anchors_.codeHashes[2] != router_.codehash
                || anchors_.targets[3] != sanctionReads_
                || anchors_.codeHashes[3] != sanctionReads_.codehash
                || anchors_.targets[4] != provider || anchors_.codeHashes[4] != provider.codehash
                || anchors_.finalityRegistry != address(this) || anchors_.chainId != block.chainid
        ) revert AuthorityTypes.InvalidCurrentAuthority();
        currentAuthorityResolver = resolver_;
        currentAuthorityResolverCodeHash = resolver_.codehash;
        _authorityAnchorsHash = keccak256(abi.encode(anchors_));
    }

    function currentAuthorityProfile() external pure override returns (bytes32) {
        return AuthorityTypes.PROFILE;
    }

    /// @notice A current route is distinct from this host's original sanctionReads anchor.
    /// @dev The resolver never calls this function. The Coordinator calls that resolver
    /// directly, so exact reciprocity cannot recurse through preparation or discovery.
    function currentArtistAuthority(uint256 collectionId)
        public
        view
        override
        returns (AuthorityTypes.Route memory route)
    {
        if (
            collectionId == 0 || currentAuthorityResolver.code.length == 0
                || currentAuthorityResolver.codehash != currentAuthorityResolverCodeHash
        ) revert AuthorityTypes.CurrentAuthorityChanged();
        AuthorityTypes.Anchors memory a = _resolverAnchors(currentAuthorityResolver);
        if (keccak256(abi.encode(a)) != _authorityAnchorsHash) {
            revert AuthorityTypes.CurrentAuthorityChanged();
        }
        bytes memory raw = _authorityRead(
            currentAuthorityResolver,
            abi.encodeCall(AuthorityResolver.currentFinalityRoute, (collectionId)),
            320
        );
        route = abi.decode(raw, (AuthorityTypes.Route));
        if (
            keccak256(raw) != keccak256(abi.encode(route))
                || route.finalityRegistry != address(this)
                || route.finalityCodeHash != address(this).codehash
                || route.provider != scopeEvidenceProvider
                || route.providerCodeHash != _providerCodeHash || route.registry.code.length == 0
                || route.registryCodeHash == 0 || route.registry.codehash != route.registryCodeHash
                || route.coordinator.code.length == 0 || route.coordinatorCodeHash == 0
                || route.coordinator.codehash != route.coordinatorCodeHash
                || route.selectionHash == 0 || route.presentationHash == 0
        ) revert AuthorityTypes.CurrentAuthorityChanged();
        if (
            !_readExactInterfaceSupport(route.coordinator, type(IERC165).interfaceId)
                || !_readExactInterfaceSupport(
                    route.coordinator, type(ArtistCurrentFinality).interfaceId
                ) || _readExactInterfaceSupport(route.coordinator, 0xffffffff)
        ) revert AuthorityTypes.CurrentAuthorityChanged();
        bytes memory reciprocal = _authorityRead(
            route.coordinator,
            abi.encodeCall(ArtistCurrentFinality.currentFinalityRoute, (collectionId)),
            320
        );
        if (keccak256(reciprocal) != keccak256(raw)) {
            revert AuthorityTypes.CurrentAuthorityChanged();
        }
    }

    function _resolverAnchors(address resolver_)
        private
        view
        returns (AuthorityTypes.Anchors memory a)
    {
        if (
            resolver_.code.length == 0
                || !_readExactInterfaceSupport(resolver_, type(IERC165).interfaceId)
                || !_readExactInterfaceSupport(resolver_, type(AuthorityResolver).interfaceId)
                || _readExactInterfaceSupport(resolver_, 0xffffffff)
                || abi.decode(
                        _authorityRead(
                            resolver_,
                            abi.encodeCall(AuthorityResolver.currentAuthorityProfile, ()),
                            32
                        ),
                        (bytes32)
        ...
        bytes memory raw =
            _authorityRead(resolver_, abi.encodeCall(AuthorityResolver.anchors, ()), 416);
        a = abi.decode(raw, (AuthorityTypes.Anchors));
        if (
            keccak256(raw) != keccak256(abi.encode(a)) || a.chainId != block.chainid
                || a.finalityRegistry != address(this) || a.readGas < 50000
                || a.readGas > type(uint64).max
        ) revert AuthorityTypes.InvalidCurrentAuthority();
        for (uint256 i; i < 5; ++i) {
            if (
                a.targets[i].code.length == 0 || a.codeHashes[i] == 0
                    || a.targets[i].codehash != a.codeHashes[i]
            ) revert AuthorityTypes.CurrentAuthorityChanged();
        }
    }

    function _authorityRead(address target, bytes memory input, uint256 size)
        private
        view
        returns (bytes memory raw)
    {
        bool ok;
        (ok,, raw) = _readExactStatic(target, input, size);
        if (!ok) revert AuthorityTypes.CurrentAuthorityChanged();
    }
    ...
    function streamModuleVersion() public pure override returns (bytes32) {
        return keccak256("6529stream.lineage-artwork-finality.v1");
    }
    ...
    function supportsInterface(bytes4 id) public view override returns (bool) {
        return id == type(IStreamArtworkFinalityRegistry).interfaceId
            ...
            || id == type(IStreamFinalitySanctionArchive).interfaceId
            || id == type(IStreamFinalityCurrentAuthority).interfaceId
    ...
    /// @dev The guardian is re-resolved through the authority at veto time â€” a role reference,
    ...
    function _finalize(
        ...
        bytes32 evidenceHash =
            _prepareArchive(scope.collectionId, components, execution, proof, hasArchive);
    ...
    function finalityExecutionContextWithArchive(
        ...
        (, execution) = _prepareExecution(scope, components, expectedFinalityRecordHash, manifest);
        _prepareArchive(scope.collectionId, components, execution, proof, true);
    }
    ...
    function _prepareArchive(
        ...
        evidenceHash = StreamFinalitySanctionArchive.requireProof(
            StreamFinalitySanctionArchive.Pins(
                address(coreReads),
                currentArtistAuthority(collectionId).registry,
                artifactCoverage,
                _artifactCodeHash,
                _componentReadGas()
            ),
    ...
    function prepareSanction(
        ...
        return StreamFinalityLineagePreparation.prepareSanction(
        ...
        )
    {
        ...
        _requireComponentListWellFormed(nonSanctionComponents);
        _requireManifestValid(manifest);
        return StreamFinalityLineagePreparation.prepareSanctionWithReview(
            _preparationDependencies(), scope, nonSanctionComponents, manifest
        );
    }
        ...
        )
    {
        ...
        _requireManifestValid(manifest);
        return StreamFinalityLineagePreparation.prepare(
