...
import {
    StreamViewAdoptionDocumentsV2 as Documents
} from "../metadata/StreamViewAdoptionDocumentsV2.sol";
import { StreamViewPolicyTypesV2 as T } from "../metadata/StreamViewPolicyTypesV2.sol";
import {
    IStreamViewAdoptionPolicyRouterV2 as ProfileRouter
} from "../../interfaces/stream/metadata/IStreamViewAdoptionPolicyRouterV2.sol";
import {
    IStreamViewRendererV2 as PolicyRenderer
...
/// @notice Fixed current-source interpreter for the closed adopted policy VIEW V2 profile.
...
library StreamFinalityViewPolicySourceReadsV2 {
    struct Dependencies {
    ...
    function _record(Dependencies memory d, bytes32 key) private view returns (V.Record memory r) {
        if (
            bytes32(
                    Read.word(
                        d.targets[1],
                        abi.encodeCall(ProfileRouter.viewAdoptionProfile, (key)),
                        d.readGas
                    )
                ) != T.PROFILE
        ) revert V.InvalidViewAdoption();
        ...
        raw = Read.read(
            r.source.renderer.renderer,
            abi.encodeCall(PolicyRenderer.policyViewBinding, ()),
            736,
            d.readGas
        );
        T.Binding memory binding = abi.decode(raw, (T.Binding));
        if (
            keccak256(raw) != keccak256(abi.encode(binding)) || binding.chainId != d.chainId
                || binding.core != d.targets[0] || binding.coreCodeHash != d.codeHashes[0]
