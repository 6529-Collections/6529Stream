...
library StreamPreservationPolicyContentObservationV1 {
        ...
        )
    {
        (value.preservation, value.preservationAdmission) = Admission.observe(
            _gasParameters,
            Admission.Context(context.core, context.metadataRouter, context.preservationProfile),
            row,
            producer
        );
        P.Admission memory admission = value.preservationAdmission;
        imageURI = Admission.sourceImage(_gasParameters, p, row, context.metadataRouter);
