...
import "../../interfaces/stream/finality/IStreamStaticOutputManifest.sol";
import "../../interfaces/stream/finality/IStreamStaticContentCheckpoint.sol";
...
import { StreamStaticOutputSchemas as Definitions } from "./StreamStaticOutputSchemas.sol";
...
contract StreamStaticOutputManifest is
    IStreamStaticOutputManifest,
    ...
    IERC165
{
    ...
    uint256 private constant _HEADER_BYTES = 480;
    uint256 private constant _LEAF_BYTES = 288;
    ...
    constructor(
        ...
        if (
            abi.decode(
                        _read(
                            checkpoint_, abi.encodeCall(IStreamStaticContentCheckpoint.core, ()), 32
                        ),
                        (address)
                    ) != core_
                || abi.decode(
    ...
    function supportsInterface(bytes4 id) external pure override returns (bool) {
        return id == type(IERC165).interfaceId || id == type(IStreamGasParameterHost).interfaceId
            || id == type(IStreamStaticOutputManifest).interfaceId;
    }
    ...
    function beginManifest(
        ...
        _pins();
        _documents();
        IStreamStaticContentCheckpoint.Plan memory p = _checkpoint(checkpointHash);
        ...
        // Four-word scope is inlined; fourteen head words precede the exact array count.
        ...
        planHash = keccak256(
            abi.encode(
                keccak256("6529STREAM_STATIC_OUTPUT_MANIFEST_PLAN_V1"),
                deploymentChainId,
                address(this),
                core,
                contentCheckpoint,
        ...
        if (_plans[planHash].manifest.tokenCount == 0) {
            ...
            emit OutputManifestStarted(1, planHash, m);
        }
    }
        ...
        returns (bytes32 recordHash)
    {
        ...
        for (uint256 i; i < count; ++i) {
            bytes memory actual = _read(
                contentCheckpoint,
                abi.encodeCall(
                    IStreamStaticContentCheckpoint.outputAt,
            ...
            assembly ("memory-safe") {
                supplied := keccak256(add(add(batch, 32), mul(i, 288)), 288)
            }
            if (supplied != keccak256(actual)) revert OutputManifestMismatch(uint256(first) + i);
        }
        p.nextIndex = first + uint64(count);
        emit OutputManifestAdvanced(1, planHash, first, p.nextIndex);
        if (p.nextIndex == p.manifest.tokenCount) {
            _current(p.manifest);
            recordHash = keccak256(
                abi.encode(keccak256("6529STREAM_STATIC_OUTPUT_MANIFEST_VERIFIED_V1"), planHash)
            );
            p.recordHash = recordHash;
            _recordPlans[recordHash] = planHash;
            emit OutputManifestVerified(1, recordHash, planHash, p.manifest);
        }
    }
    ...
    function _current(Manifest memory m) private view {
        _documents();
        IStreamStaticContentCheckpoint.Plan memory p = _checkpoint(m.checkpointHash);
    ...
    function _checkpoint(bytes32 hash)
        private
        view
        returns (IStreamStaticContentCheckpoint.Plan memory)
    {
        return abi.decode(
            _read(
                contentCheckpoint,
                abi.encodeCall(IStreamStaticContentCheckpoint.requireCurrentCheckpoint, (hash)),
                352
            ),
            (IStreamStaticContentCheckpoint.Plan)
        );
    }
