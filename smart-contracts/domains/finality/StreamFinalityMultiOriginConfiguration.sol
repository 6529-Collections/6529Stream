...
library StreamFinalityMultiOriginConfiguration {
    ...
    function read(
        ...
        bytes32 coverageProfile = _coverageProfile(inventoryProfile);
        if (
        ...
        if (
            od.profile != O.PROFILE || od.originGas < 50000 || od.originGas > type(uint64).max
                || _word(inventory, abi.encodeCall(Inventory.originProfile, ()), readGas)
                    != inventoryProfile
                || O.inventoryDependencyHash(inventoryProfile, d, od) != dependencyHash
        ...
        _bundle(targets, hashes, chainId, readGas, d, od, inventoryProfile, coverageProfile);
    }
        ...
        if (
            keccak256(abi.encode(bo)) != keccak256(abi.encode(od))
                ...
                || _word(bundle, abi.encodeWithSignature("dependencyHash()"), readGas)
                    != keccak256(abi.encode(coverageProfile, inventoryProfile, b, od))
        ) {
            revert InvalidFinalityArchiveConfiguration(bundle);
        }
    }

    function _coverageProfile(bytes32 inventoryProfile) private pure returns (bytes32) {
        if (
            inventoryProfile == O.INVENTORY_PROFILE
                || inventoryProfile == O.POLICY_INVENTORY_PROFILE
        ) return O.COVERAGE_PROFILE;
        if (
            inventoryProfile == O.SCOPED_INVENTORY_PROFILE
                || inventoryProfile == O.SCOPED_POLICY_INVENTORY_PROFILE
        ) {
            return keccak256("6529STREAM_MULTI_ORIGIN_SCOPED_BUNDLE_IMMUTABLE_STOP_AGGREGATE_V1");
        }
        revert InvalidFinalityArchiveConfiguration(address(0));
    }
