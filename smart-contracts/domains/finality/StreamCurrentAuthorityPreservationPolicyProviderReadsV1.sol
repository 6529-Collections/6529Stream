...
library StreamCurrentAuthorityPreservationPolicyProviderReadsV1 {
        ...
        view
    {
        ...
        if (
            _word(c, 18, abi.encodeWithSignature("preservationPolicyInventoryProfile()"))
                    != CurrentInventoryTypes.PRESERVATION_POLICY_INVENTORY_PROFILE
        ...
        if (
            _word(c, 18, abi.encodeCall(IERC165.supportsInterface, (type(Inventory).interfaceId)))
                != bytes32(uint256(1))
        ) revert NativeProviderDependency(c.targets[18]);
