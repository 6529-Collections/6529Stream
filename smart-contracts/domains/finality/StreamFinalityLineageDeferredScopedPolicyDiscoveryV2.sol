...
/// @dev Original Artist/Finality pins remain fixed; only the authenticated sanction component follows succession.
/// @dev Native/scoped profiles retain strict constructor pins; strict prior Discovery siblings are unchanged.
...
contract StreamFinalityLineageDeferredScopedPolicyDiscoveryV2 is
    IERC165,
    ...
    IStreamFinalityEvidenceDiscoveryBinding
{
    ...
    Profiles.Profile[2] private _profiles;
    Deferred.Capability private _deferredCapability;
    PublicationBinding.FactoryBinding private _publicationBinding;
    address private _scopedPolicyEntropyFactory;
    bytes32 private _scopedPolicyEntropyCodeHash;
    ...
    constructor(
        ...
        PublicationBinding.FactoryBinding memory b
    ) {
        if (
        ...
        if (
            b.graphGas > type(uint32).max || c.componentGas <= b.graphGas + b.graphGas / 63 + 100000
        ) revert DiscoveryConfiguration(b.factory);
        (_scopedPolicyEntropyFactory, _scopedPolicyEntropyCodeHash) =
            PublicationFactoryReads.validate(c, b);
        _publicationBinding = b;
        _initializeCapability(b);
        _admitExact(_scopedPolicyEntropyFactory, _scopedPolicyEntropyCodeHash);
    ...
    {
        AuthorityTypes.Route memory authority = _current(scope, _configuration);
        ...
        for (uint256 i; i < order.length; ++i) {
            routes[i] = _identity(scope, order[i], authority);
        }
    }
    ...
    function _identity(
        ...
        uint8 index,
        AuthorityTypes.Route memory authority
    ) private view returns (StreamFinalityCurrentComponentRoute memory route) {
        ...
        if (index < 7) {
            ...
            target = index == 8 ? c.referenceRender : authority.registry;
            if (
                index == 9
                    && abi.decode(
                            _read(
        ...
        if (index == 9) _currentPin(authority);
        else if (index == 8) _pinReference(scope, target);
        else _pin(target);
        ...
        returns (StreamFinalityComponentExpectation[] memory entries)
    {
        ...
        for (uint256 i; i < order.length; ++i) {
            entries[i] = _route(scope, order[i], authority);
        }
    }
    ...
    function _route(
        ...
        uint8 index,
        AuthorityTypes.Route memory authority
    ) private view returns (StreamFinalityComponentExpectation memory entry) {
        StreamFinalityDiscoveryTypes.Configuration memory c = _scopeConfiguration(scope);
        bytes32 family = _family(index);
        if (index < 7) {
        ...
        return _componentState(authority.registry, family, scope);
    }
    ...
    function _current(
        ...
        _pin(c.provider);
        _pin(c.membership);
        _pinEntropy(scope, c.entropyFactory);
    ...
    function _initializeCapability(PublicationBinding.FactoryBinding memory b) private {
        _supports(scopeEvidenceProvider, type(DeferredBinding).interfaceId);
        if (
            abi.decode(
                    _read(
        ...
        Deferred.Capability memory cap = abi.decode(raw, (Deferred.Capability));
        if (
            keccak256(raw) != keccak256(abi.encode(cap)) || cap.capabilityHash == 0
                || Deferred.hashCapability(deploymentChainId, scopeEvidenceProvider, cap)
                    != cap.capabilityHash || cap.originalHash != _profiles[0].configurationHash
                || cap.scopedHash != _profiles[1].configurationHash
                || cap.graphHash != b.configurationHash || cap.authority.code.length == 0
                || cap.authorityCodeHash == 0 || cap.authority.codehash != cap.authorityCodeHash
        ...
        _address(metadataHost, "governanceAuthority()", cap.authority);
        if (
            abi.decode(
                    _read(
                        metadataHost,
                    ...
                    (bytes32)
                ) != cap.authorityCodeHash
        ) revert DiscoveryConfiguration(metadataHost);
    ...
    /// receipt can supply a reference outside the original two constructor-pinned profiles.
    function _pinReference(StreamFinalityScope memory scope, address target) private view {
        ...
        if (
            (p.profileHash != ScopedPolicyDefinitions.PROFILE_HASH
                    && p.profileHash != ProfileReads.profileHash(2)) || target != p.referenceRender
                || p.referenceRenderCodeHash == 0 || target.code.length == 0
                || target.codehash != p.referenceRenderCodeHash
        ) {
    ...
    function _pinEntropy(StreamFinalityScope memory scope, address target) private view {
        ...
        Profiles.Profile memory p = _selectedProfile(scope);
        if (p.profileHash != ProfileReads.profileHash(2) || target != p.entropyFactory) {
            revert DiscoveryDependency(target);
        }
    ...
    {
        if (family == StreamFinalityDomains.COMPONENT_REFERENCE_RENDER) {
            _pinReference(scope, target);
        } else {
            _pin(target);
        }
        return _componentState(target, family, scope);
    }
        ...
        returns (StreamFinalityComponentExpectation memory)
    {
        ...
        return _route(scope, order[index], authority);
    }
