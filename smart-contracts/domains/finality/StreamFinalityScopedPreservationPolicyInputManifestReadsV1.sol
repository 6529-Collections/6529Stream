...
import "./StreamFinalityScopedPreservationPolicyInputManifestSchemasV1.sol";
import "./StreamFinalityRouterEvidence.sol";
import "../../interfaces/stream/finality/StreamFinalityScopedPreservationPolicyInputManifestTypesV1.sol";
...
library StreamFinalityScopedPreservationPolicyInputManifestReadsV1 {
    struct Dependencies {
    ...
    function encode(
        ...
        payload = abi.encode(
            StreamFinalityScopedPreservationPolicyInputManifestSchemasV1.SCHEMA_ID,
            StreamFinalityScopedPreservationPolicyInputManifestSchemasV1.CANON_ID,
            d.chainId,
            d.targets[0],
            d.targets[1],
            d.targets[4],
    ...
    function requireCurrent(
        Dependencies memory d,
        StreamFinalityScopedPreservationPolicyInputManifestTypesV1.Statement memory expected,
        bytes32 contentHash
        ...
        schemaId = StreamFinalityScopedPreservationPolicyInputManifestSchemasV1.SCHEMA_ID;
        canonicalizationId = StreamFinalityScopedPreservationPolicyInputManifestSchemasV1.CANON_ID;
    ...
    {
        bytes memory expected =
            StreamFinalityScopedPreservationPolicyInputManifestSchemasV1.document(id);
        bytes memory raw = StreamFinalityRouterEvidence.read(
            d.targets[2],
    ...
    function _shape(StreamFinalityScopedPreservationPolicyInputManifestTypesV1.Statement memory s)
        private
        pure
    {
        if (
            s.scope.collectionId == 0
                || (s.scope.scopeType == StreamFinalityScopeType.TOKEN
                        ? s.scope.tokenId == 0 || s.scope.scopeId != 0 || s.leafCount != 1
