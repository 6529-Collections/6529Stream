...
import {
    StreamScopedPolicyReferenceTypesV2 as Reference
...
library StreamCurrentAuthorityScopedPolicyPublicationGraphReadsV2 {
    function bindings(
        T.Recipe memory r,
        O.Dependencies memory origin,
        D.Dependencies memory authority,
        bool operative
    ) public view returns (bytes32 dependenciesHash) {
        if (r.inventory.chainId != block.chainid) {
            revert T.PublicationRecipeInvalid();
        }
        capabilities(r, origin, authority, operative);
        for (uint256 i; i < 4; ++i) {
    ...
    function current(
    ...
    ) public view returns (T.Graph memory g) {
        if (
        ...
        if (bindings(r, origin, authority, true) != dependenciesHash) {
    ...
    function children(
        ...
        capabilities(r, origin, authority, true);
        ...
        if (
            g.preparedChildren > 5
                && _word(g.children[5], "dependencyHash()", budget)
                    != D.dependencyHash(
                        D.SCOPED_POLICY_INVENTORY_PROFILE, Recipe.inventory(r, g), origin, authority
                    )
        ) revert T.PublicationGraphChanged(g.graphId);
        if (
            g.preparedChildren > 6
                && _word(g.children[6], "dependencyHash()", budget)
                    != keccak256(
                        abi.encode(
                            keccak256(
                                "6529STREAM_CURRENT_AUTHORITY_SCOPED_BUNDLE_IMMUTABLE_STOP_AGGREGATE_V1"
                            ),
    ...
    /// @dev Construction may precede the immutable resolver/provider/Finality cycle. A live
    /// resolver is always checked, and every operative use requires it. This reads fixed
    /// configuration only; neither construction nor graph preparation calls currentSelection.
    function capabilities(
        ...
        OriginCalls.pin(origin);
        if (
            authority.resolver == address(0) || authority.resolverCodeHash == 0
                || authority.resolverGas < 50000 || authority.resolverGas > type(uint64).max
        ) {
            revert T.PublicationRecipeInvalid();
        }
        if (!operative && authority.resolver.code.length == 0) return;
        _pin(authority.resolver, authority.resolverCodeHash);
        uint256 cap = authority.resolverGas;
        if (
            _support(authority.resolver, type(IERC165).interfaceId, cap) != 1
                || _support(authority.resolver, type(Resolver).interfaceId, cap) != 1
                || _support(authority.resolver, 0xffffffff, cap) != 0
                || _word(authority.resolver, "currentAuthorityProfile()", cap) != C.PROFILE
        ) {
            revert T.PublicationGraphDependency(authority.resolver);
        }
        bytes memory raw =
            Reads.read(authority.resolver, abi.encodeCall(Resolver.anchors, ()), 416, cap);
        C.Anchors memory a = abi.decode(raw, (C.Anchors));
        if (
            keccak256(raw) != keccak256(abi.encode(a)) || a.chainId != r.inventory.chainId
                || a.readGas < 50000 || a.readGas > type(uint64).max
                || a.targets[0] != r.inventory.targets[0]
                || a.codeHashes[0] != r.inventory.codeHashes[0]
                || a.targets[1] != r.inventory.targets[1]
                || a.codeHashes[1] != r.inventory.codeHashes[1]
                || a.targets[2] != r.inventory.targets[4]
                || a.codeHashes[2] != r.inventory.codeHashes[4]
                || a.targets[3] != r.inventory.artistTargets[0]
                || a.codeHashes[3] != r.inventory.artistCodeHashes[0] || a.targets[4] == address(0)
                || a.codeHashes[4] == 0 || a.finalityRegistry == address(0)
        ) {
            revert T.PublicationGraphDependency(authority.resolver);
        }
        for (uint256 i; i < 5; ++i) {
            _pin(a.targets[i], a.codeHashes[i]);
        }
        if (operative && a.finalityRegistry.code.length == 0) {
            revert T.PublicationGraphDependency(a.finalityRegistry);
        }
    }
