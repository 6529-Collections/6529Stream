...
import { StreamFinalityPreparation as Original } from "./StreamFinalityPreparation.sol";
import {
...
library StreamFinalityLineagePreparation {
    ...
    function prepareSanction(
        Original.Dependencies memory deps,
    ...
    {
        deps.sanctionReads = _requireCurrentBindings(deps, scope.collectionId);
        {
    ...
    function prepare(
        ...
        public
        view
        returns (Original.Prepared memory ctx, StreamFinalityExecutionContext memory execution)
    {
    ...
    function _requireCurrentBindings(Original.Dependencies memory deps, uint256 collectionId)
        private
        view
        returns (IStreamFinalitySanctionReads)
    {
        ...
        AuthorityTypes.Route memory route = abi.decode(raw, (AuthorityTypes.Route));
        if (
            keccak256(raw) != keccak256(abi.encode(route))
                ...
                || route.registry != _selected(deps, keccak256("ARTIST_REGISTRY"))
                ...
                || abi.decode(
                        _requiredRead(
                            ...
                            address(this),
                            abi.encodeCall(
                                IStreamFinalityCurrentAuthority.currentAuthorityProfile, ()
                            ),
                            32
                        ),
                        (bytes32)
                    ) != AuthorityTypes.PROFILE
        ) revert FinalityCurrentBindingInvalid(route.registry);
        return IStreamFinalitySanctionReads(route.registry);
    }

    function _selected(Original.Dependencies memory deps, bytes32 kind)
        private
        view
    ...
    function _readExactAddress(Original.Dependencies memory deps, address adapter, bytes4 selector)
        private
        view
    ...
    function _verifyCoreGatesAndFacts(
        Original.Dependencies memory deps,
        StreamFinalityScope memory scope
    ) private view returns (bytes32 factsHash, uint256 expectedLeafCount, bool exactLeafCount) {
    ...
    function _componentCallData(Original.Dependencies memory deps, StreamFinalityScope memory scope)
        private
        pure
    ...
    function _adapterScope(Original.Dependencies memory deps, StreamFinalityScope memory scope)
        private
        pure
    ...
    function _scopeKey(Original.Dependencies memory deps, StreamFinalityScope memory scope)
        private
        pure
        returns (bytes32)
    {
