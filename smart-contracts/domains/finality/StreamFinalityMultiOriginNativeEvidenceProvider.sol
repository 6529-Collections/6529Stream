...
import "./StreamFinalityMultiOriginNativeSanctionReview.sol";
import "./StreamFinalityMultiOriginNativeProviderReads.sol";
...
contract StreamFinalityMultiOriginNativeEvidenceProvider is StreamFinalityRouterEvidenceProvider {
        ...
        )
    {
        if (
            ...
            revert StreamFinalityMultiOriginNativeProviderReads.NativeProviderConfiguration();
        }
        for (uint256 i; i < 22; ++i) {
            ...
            if (
                ...
                revert StreamFinalityMultiOriginNativeProviderReads.NativeProviderDependency(c.targets[
                        i
                    ]);
            }
        }
        ...
        returns (bytes memory)
    {
        ...
        StreamFinalityInputManifestTypes.Statement memory s =
            StreamFinalityMultiOriginNativeProviderReads.statement(
                c, scope, StreamFinalityMultiOriginNativeProviderReads.currentComponents(c, scope)
            );
        return StreamFinalityInputManifestReads.encode(
            StreamFinalityMultiOriginNativeProviderReads.manifestDependencies(c), s
        );
    }
        ...
        returns (StreamFinalityScopeInputs memory, bytes32, bytes32)
    {
        ...
        return _admit(
            ...
            scope,
            manifestHash,
            StreamFinalityMultiOriginNativeProviderReads.currentComponents(c, scope)
        );
    }
        ...
        returns (IStreamFinalitySanctionReview.ReviewFacts memory)
    {
        ...
        StreamFinalityInputManifestReads.requireCurrent(
            StreamFinalityMultiOriginNativeProviderReads.manifestDependencies(c), s, manifestHash
        );
        return StreamFinalityMultiOriginNativeSanctionReview.review(c, s);
    }
    ...
    function requirePreparedFinalityScopeInputs(
        ...
        return _admit(
            ...
            scope,
            manifestHash,
            StreamFinalityMultiOriginNativeProviderReads.independentComponents(components)
        );
    }
        ...
        )
    {
        ...
        inputs = s.inputs;
        review = StreamFinalityMultiOriginNativeSanctionReview.review(c, s);
    }
    ...
    function _requirePreparedCandidate(
        ...
        if (!StreamFinalityRouteReads.verifyIfSupported(
            ...
            revert StreamFinalityMultiOriginNativeProviderReads.NativeProviderSource();
        }
    }
    ...
    function _admit(
        ...
        StreamFinalityInputManifestTypes.Statement memory s =
            StreamFinalityMultiOriginNativeProviderReads.statement(c, scope, components);
        (schema, canon) = StreamFinalityInputManifestReads.requireCurrent(
            StreamFinalityMultiOriginNativeProviderReads.manifestDependencies(c), s, manifestHash
        );
    ...
    function _candidate(
        ...
        StreamFinalityMultiOriginNativeProviderReads.requirePins(c);
        if (
