...
/// @notice Additive multi-origin collection inputs from the fixed authenticated inventory.
...
library StreamFinalityMultiOriginNativeProviderReads {
    ...
    function _snapshotConfiguration(Native.Config memory c) private view {
        bytes memory raw = _read(
    ...
    function _core(Native.Config memory c, StreamFinalityInputManifestTypes.Statement memory s)
        private
        view
    {
        bytes memory raw = _read(
