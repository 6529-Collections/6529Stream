...
import "../interfaces/IDependencyRegistry.sol";
...
contract StreamCore is ERC721Enumerable, ERC2981, Ownable {
    ...
    function setCollectionData(uint256 _collectionID, address _collectionArtistAddress, uint256 _maxCollectionPurchases, uint256 _collectionTotalSupply, uint _setFinalSupplyTimeAfterMint) public FunctionAdminRequired(this.changeMetadataView.selector) {
    ...
    function freezeCollection(uint256 _collectionID) public FunctionAdminRequired(this.freezeCollection.selector) {
        require(block.timestamp > IStreamMinter(minterContract).getEndTime(_collectionID) && IStreamMinter(minterContract).getEndTime(_collectionID) != 0 && wereDataAdded[_collectionID] == true);
    ...
    function setFinalSupply(uint256 _collectionID) public FunctionAdminRequired(this.setFinalSupply.selector) {
        require (block.timestamp > IStreamMinter(minterContract).getEndTime(_collectionID) + collectionAdditionalData[_collectionID].setFinalSupplyTimeAfterMint, "Time has not passed");
