...
} from "./StreamCurrentFullPreservationPolicyCreation.sol";
...
import {
    StreamPreservationPolicyPublicationGraphTypesV1
} from "../../smart-contracts/interfaces/stream/finality/StreamPreservationPolicyPublicationGraphTypesV1.sol";
import {
    StreamScopedPreservationPolicyPublicationGraphTypesV1
...
/// @notice Genuine full-preservation-policy companions for the current graph's reserved CREATE topology.
...
abstract contract StreamCurrentFullPreservationPolicyGraph is StreamCurrentFinalityGraph {
    mapping(PreservationCreation.Kind => bool) private preservationVerifiedTemplates;

    /// @dev Separate literal creation identities leave both original creation rosters unchanged.
    ...
    {
        return PreservationCreation.creation(kind);
    }

    function _preservationAssemblyCreate(PreservationCreation.Kind kind, bytes memory args)
        internal
        returns (address product)
    {
        bytes memory creation = _preservationGraphCreation(kind);
        if (!preservationVerifiedTemplates[kind]) {
            _linkRuntime(graphVm.readFile(_artifact(PreservationCreation.name(kind))), creation);
            preservationVerifiedTemplates[kind] = true;
        }
        bytes memory initcode = bytes.concat(creation, args);
        require(initcode.length <= 49152, "actual preservation product initcode fits");
    ...
    StreamPreservationPolicyPublicationFactoryV1 internal fullPolicyCollectionFactory;
    StreamScopedPreservationPolicyPublicationFactoryV1 internal fullPolicyScopedFactory;
    ...
    struct FullPolicyCompanionState {
        ...
        IStreamPreservationPolicyPublicationGraphBindingV1.CollectionFactoryBinding
            collectionBinding;
        IStreamScopedPreservationPolicyPublicationEvidenceBindingV1.FactoryBinding scopedBinding;
    ...
    function _fullPolicyCompanionState() internal view returns (FullPolicyCompanionState memory s) {
        ...
        for (uint256 i; i < 11; ++i) {
            require(s.products[i].code.length != 0, "actual full-preservation-policy companion");
        ...
        s.collectionBinding = IStreamPreservationPolicyPublicationGraphBindingV1(s.provider)
            .collectionPreservationPolicyPublicationBinding();
        s.scopedBinding = IStreamScopedPreservationPolicyPublicationEvidenceBindingV1(s.provider)
            .scopedPreservationPolicyPublicationBinding();
        IStreamPreservationPolicyPublicationGraphBindingV1.CollectionFactoryBinding memory
            collection = _fullPolicyCollectionBinding();
        IStreamScopedPreservationPolicyPublicationEvidenceBindingV1.FactoryBinding memory scoped =
        ...
        require(
            collection.recipeHash
                    == keccak256(
                        abi.encode(
                            StreamPreservationPolicyPublicationGraphTypesV1.PROFILE,
                            block.chainid,
                ...
                && scoped.recipeHash
                    == keccak256(
                        abi.encode(
                            StreamScopedPreservationPolicyPublicationGraphTypesV1.PROFILE,
                            block.chainid,
        ...
        s.sourceConfigurationHash = IStreamFinalityPreservationFactoryProfileSourcesV1(s.provider)
            .finalitySourceConfigurationHash();
        require(
            s.sourceConfigurationHash != 0
                && StreamFinalityFullPreservationPolicyDiscoveryV1(s.discovery)
                        .sourceConfigurationHash() == s.sourceConfigurationHash
                && StreamFinalityFullPreservationPolicyDiscoveryV1(s.discovery)
                    .scopeEvidenceProvider() == s.provider,
        ...
        view
    {
        require(
            ...
            "unchanged full-preservation-policy companion checkpoint"
        );
    }
    ...
    function _assemblyProviderCreation() internal view virtual override returns (bytes memory) {
        return _preservationGraphCreation(
            PreservationCreation.Kind.StreamFinalityFullPreservationPolicyEvidenceProviderV1
        );
    }
    ...
    function _assemblyDiscoveryCreation() internal view virtual override returns (bytes memory) {
        return _preservationGraphCreation(
            PreservationCreation.Kind.StreamFinalityFullPreservationPolicyDiscoveryV1
        );
    }
        ...
        returns (bytes memory)
    {
        ...
        return abi.encode(
            d,
            IStreamFinalityPreservationFactoryProfileSourcesV1(provider)
                .finalitySourceConfigurationHash(),
            IStreamPreservationPolicyPublicationGraphBindingV1(provider)
                .collectionPreservationPolicyPublicationBinding(),
            IStreamScopedPreservationPolicyPublicationEvidenceBindingV1(provider)
                .scopedPreservationPolicyPublicationBinding()
        );
    }
    ...
    function _prepareAssemblyProviderCompanions() internal virtual override {
        require(
            address(fullPolicySelection) == address(0), "fresh full-preservation-policy companions"
        );
        ...
        fullPolicyCollectionFactory = StreamPreservationPolicyPublicationFactoryV1(
            _preservationAssemblyCreate(
                PreservationCreation.Kind.StreamPreservationPolicyPublicationFactoryV1,
                abi.encode(_fullPolicyCollectionRecipe())
            )
        );
        fullPolicyScopedFactory = StreamScopedPreservationPolicyPublicationFactoryV1(
            _preservationAssemblyCreate(
                PreservationCreation.Kind.StreamScopedPreservationPolicyPublicationFactoryV1,
    ...
    function _fullPolicyScopedRecipe()
        internal
        view
        returns (StreamScopedPreservationPolicyPublicationGraphTypesV1.Recipe memory r)
    {
        StreamPreservationPolicyPublicationGraphTypesV1.Recipe memory c =
            _fullPolicyCollectionRecipe();
    ...
    function _fullPolicyCollectionBinding()
        internal
        view
        returns (IStreamPreservationPolicyPublicationGraphBindingV1
                    .CollectionFactoryBinding memory b)
    {
    ...
    function _fullPolicyScopedBinding()
        internal
        view
        returns (IStreamScopedPreservationPolicyPublicationEvidenceBindingV1
                    .FactoryBinding memory b)
    {
