// Exact ABI157 witness for the genuine VIEW inventory and Bundle consumer clients.
...
const roots = ["StreamViewPreservationRenderCriticalInventoryV1", "StreamViewPreservationBundleArchiveCoverageV1"];
...
export function currentViewPreservationConsumersV1Fixture(inputBytes, outputBytes, bridgeBytes) {
  ...
  if (input.language !== "Solidity" || Object.keys(input.sources ?? {}).length !== 4059
    || output.errors?.some(error => error.severity === "error")) throw Error("Invalid VIEW consumer ABI157 capture");
  ...
  // Select every compiler declaration already in both consumer hosts' imported closure.
  ...
  const documents = [
    ...
    "docs/integrations/view-preservation-inventory-v1.md",
    "docs/integrations/view-locator-correspondence-v1.md",
  ...
  return {
    schemaVersion: 1,
    profile: "current-view-preservation-consumers-v1",
    ...
    sourceBinding: "Every compiler input literal matches its frozen Git blob byte-for-byte; this verifier performs no line-ending normalization. All selected ABIs are complete compiler output, with the complete union of both original VIEW consumer hosts' imported source closures. Interpretation documents come from the same source commit.",
    qualification: "Exact ABI157 source/ABI witness for the genuine retrieval-enabled VIEW inventory and Bundle hosts at a2973d36, retaining their complete concrete ABIs and imported source closure. Ordinary consumer writes and reads remain distinct from nominal library selectors, token inventory profiles and the retrieval producer. Historical fixtures remain unchanged. This fixture does not establish actual native/Safe execution, runtime provenance, rollback, gas/capacity, release or deployment acceptance.",
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!input || !output || !bridge || mode !== undefined && mode !== "--check") {
    throw Error("Usage: generate-current-view-preservation-consumers-v1-fixture.mjs INPUT OUTPUT COMMITTED_SOURCE_BRIDGE [--check]");
  }
  const rendered = JSON.stringify(currentViewPreservationConsumersV1Fixture(await readFile(input), await readFile(output), await readFile(bridge)), null, 2) + "\n";
  const target = new URL("../test/fixtures/current-view-preservation-consumers-v1-abi.json", import.meta.url);
  if (mode === "--check") {
    if (await readFile(target, "utf8") !== rendered) throw Error("Stale current VIEW preservation consumer V1 fixture");
    process.stdout.write("Current VIEW preservation consumer V1 fixture matches\n");
  } else {
    await writeFile(target, rendered, "utf8");
    process.stdout.write("Current VIEW preservation consumer V1 fixture written\n");
  }
}
