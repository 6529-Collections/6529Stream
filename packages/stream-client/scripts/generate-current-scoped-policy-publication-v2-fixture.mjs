...
export function scopedPolicyPublicationV2Fixture(inputBytes, outputBytes, bridgeBytes) {
  ...
  if (input.language !== "Solidity" || Object.keys(input.sources ?? {}).length !== 3262
    || output.errors?.some(error => error.severity === "error")) throw Error("Invalid scoped policy publication V2 capture");
  ...
  return {
    schemaVersion: 1,
    profile: "scoped-policy-publication-v2",
    ...
    qualification: "Source/ABI and mocked client evidence for actual scoped full-policy checkpoint, covered-output and root-free snapshot publication. This explicit profile admits TOKEN, RELEASE and SEASON using a genuine complete publication graph. Original complete STATIC Selection and artifact coverage are prerequisites; these client calls do not create their admission evidence. Exact nonpayable checkpoint and output calls retain original retry/progress semantics. Snapshot publication requires independently current SNAPSHOT and IDENTITY grants and is not idempotent. A published root-free snapshot grants no CONTENT_ROOT Artist consent, governance lock or finality authority. Original Artist operation-17 root adoption, reference/inventory/archive/finality ceremonies and COLLECTION/VIEW profiles remain separate. Native/Safe execution, deployed-runtime provenance, whole-call rollback, gas/capacity and release acceptance remain separately qualified.",
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!input || !output || !bridge || mode !== undefined && mode !== "--check") {
    throw Error("Usage: generate-current-scoped-policy-publication-v2-fixture.mjs INPUT OUTPUT COMMITTED_SOURCE_BRIDGE [--check]");
  }
  const rendered = JSON.stringify(scopedPolicyPublicationV2Fixture(await readFile(input), await readFile(output), await readFile(bridge)), null, 2) + "\n";
  const target = new URL("../test/fixtures/current-scoped-policy-publication-v2-abi.json", import.meta.url);
  if (mode === "--check") {
    if (await readFile(target, "utf8") !== rendered) throw Error("Stale scoped policy publication V2 fixture");
    process.stdout.write("Scoped policy publication V2 fixture matches\n");
  } else {
    await writeFile(target, rendered, "utf8");
    process.stdout.write("Scoped policy publication V2 fixture written\n");
  }
}
