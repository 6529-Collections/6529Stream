...
export function scopedPolicyReferenceV2Fixture(inputBytes, outputBytes, bridgeBytes) {
  ...
  if (input.language !== "Solidity" || Object.keys(input.sources ?? {}).length !== 3262
    || output.errors?.some(error => error.severity === "error")) throw Error("Invalid scoped policy reference V2 capture");
  ...
  return {
    schemaVersion: 1,
    profile: "scoped-policy-reference-v2",
    ...
    qualification: "Source/ABI and mocked client evidence for original scoped full-policy reference preparation and publication. This explicit ABI129 profile admits TOKEN, RELEASE and SEASON on the original seven-dependency reference host. Environment and file-inventory preparations are permissionless; publication requires original CURATOR collection/global grants and genuine current snapshot, exact V2 root, current STATIC samples and external ZIP/PNG archive coverage. Canonical payload and submitted publication chunks must already exist in the fixed Store. Current external coverage preserves the original receipt pair and may use later valid fixity. Historical records retain immutable commitments separately from current admission. Governance locks, render-critical inventory, complete bundle coverage, finality, COLLECTION/VIEW and the newer preservation architecture remain separate. Pure environment/inventory codecs retain their original interpretation. Native/Safe execution, deployed-runtime provenance, whole-call rollback, gas/capacity and release acceptance remain separately qualified.",
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!input || !output || !bridge || mode !== undefined && mode !== "--check") {
    throw Error("Usage: generate-current-scoped-policy-reference-v2-fixture.mjs INPUT OUTPUT COMMITTED_SOURCE_BRIDGE [--check]");
  }
  const rendered = JSON.stringify(scopedPolicyReferenceV2Fixture(await readFile(input), await readFile(output), await readFile(bridge)), null, 2) + "\n";
  const target = new URL("../test/fixtures/current-scoped-policy-reference-v2-abi.json", import.meta.url);
  if (mode === "--check") {
    if (await readFile(target, "utf8") !== rendered) throw Error("Stale scoped policy reference V2 fixture");
    process.stdout.write("Scoped policy reference V2 fixture matches\n");
  } else {
    await writeFile(target, rendered, "utf8");
    process.stdout.write("Scoped policy reference V2 fixture written\n");
  }
}
