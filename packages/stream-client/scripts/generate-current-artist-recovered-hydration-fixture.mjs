...
const capture = {
  capture: "parallel-feature-batch104-20260920", compilerVersion: "0.8.19",
  sourceCommit: "d56e13ffc8664b322a3a208f21d308918ed47072",
  sourceTree: "3425349f0e485ac26c5e9fccc8121b240f741739",
  sourceCount: 2796, literalBytes: 32_989_587,
  inputSha256: "1aeb2801cab6890878e1e1245982f44f0cc8b13a3496c471c3e7bcb6256383cb",
  outputSha256: "a7ae072ebc84e6b8b958acb7bda5a317780ad64e053045b399555473f546a9ff",
};
...
export function artistRecoveredHydrationFixture(inputBytes, outputBytes) {
  ...
  return {
    schemaVersion: 1, profile: "artist-recovered-authority-hydration-v1", ...capture,
    sourceBinding: "Every literal compiler input byte is matched to its frozen Git blob by this generator.",
    qualification: "Source/ABI and bounded client evidence only; actual contract/Safe execution, whole graph, gas and release acceptance remain separate. Future content/freeze composition is excluded.",
    librarySelectorEvidence: {
      selection: prepared, outputSha256: capture.outputSha256, methodIdentifiers: nominalIdentifiers,
      qualification: "Original compiler evm.methodIdentifiers from the same authenticated ABI104 output. Public library selectors retain nominal struct names; ordinary tuple selectors do not identify these calls. No bytecode/runtime acceptance is inferred.",
    },
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!input || !output || mode !== undefined && mode !== "--check") throw Error("Usage: generate-current-artist-recovered-hydration-fixture.mjs INPUT OUTPUT [--check]");
  const result = JSON.stringify(artistRecoveredHydrationFixture(await readFile(input), await readFile(output)), null, 2) + "\n";
  const target = new URL("../test/fixtures/current-artist-recovered-hydration-abi.json", import.meta.url);
  if (mode === "--check") {
    if (await readFile(target, "utf8") !== result) throw Error("Recovered-hydration fixture differs");
  } else await writeFile(target, result, "utf8");
  console.log(`Recovered-hydration fixture ${mode === "--check" ? "matches" : "written"}`);
}
