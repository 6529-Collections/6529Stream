// Project exact retained entropy-policy and unchanged governance captures; never compile.
import { createHash } from "node:crypto";
import { readFile, writeFile } from "node:fs/promises";
import { posix, resolve } from "node:path";
import { fileURLToPath } from "node:url";

const SOURCE_COMMIT = "d7fb42128cdef9dd9716d1f955ee3aba610f085b";
const INPUT_SHA = "31585c207bf1a77e652c9bd2f463dc2375a56a50cf08ad6db67e461ef47a7027";
const OUTPUT_SHA = "9ef754a4cdfc8f80d02e646ee39e6229f01639d9558608ca90c6c6df144389c6";
...
export function entropyCollectionPolicyFixture(inputBytes, outputBytes, governanceInputBytes, governanceOutputBytes) {
  if (sha(inputBytes) !== INPUT_SHA || sha(outputBytes) !== OUTPUT_SHA) throw Error("Expected exact frozen entropy-policy capture");
  ...
  for (const [key, count] of [["policy", 1034], ["governance", 2309]]) {
  ...
  return {
    schemaVersion: 1, capture: "entropy-collection-policy-abi-final-1", compilerVersion: "0.8.19", sourceCommit: SOURCE_COMMIT,
    sourceCount: 1034, inputSha256: INPUT_SHA, outputSha256: OUTPUT_SHA,
    governanceWitness: { capture: "parallel-feature-batch65-20260920", sourceCommit: "ea92b7d41ae9e9eede2a00f2c12b70543f125307", sourceCount: 2309,
      ...
      qualification: "The policy capture omits concrete Executor/Bootstrap. Their complete selected 35-source ABI65 closure independently matches d7fb4212 byte-for-byte. Only these unchanged governance products are projected; no compilation or global fixture refresh.",
      sourceHashes: Object.fromEntries(Object.entries(governanceSourceHashes).sort(([a], [b]) => a.localeCompare(b))) },
    sourceBinding: "All 1034 policy compiler literals and the selected supplemental 35-source governance closure independently verified byte-for-byte against d7fb42128cdef9dd9716d1f955ee3aba610f085b.",
    qualification: "Source/ABI and client evidence only. Original Artist17, class1 configuration, class2 freeze and legacy ASYNC remain separate; INSTANT is unsupported. No native current-stack, actual Safe, operational gas, finality consumer or release acceptance is established.",
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!input || !output || !governanceInput || !governanceOutput || (mode !== undefined && mode !== "--check")) throw Error("Usage: generate-current-entropy-collection-policy-fixture.mjs INPUT OUTPUT GOVERNANCE_INPUT GOVERNANCE_OUTPUT [--check]");
  const rendered = JSON.stringify(entropyCollectionPolicyFixture(await readFile(input), await readFile(output), await readFile(governanceInput), await readFile(governanceOutput)), null, 2) + "\n";
  const target = new URL("../test/fixtures/current-entropy-collection-policy-abi.json", import.meta.url);
  if (mode === "--check") {
    if (await readFile(target, "utf8") !== rendered) throw Error("Stale entropy collection policy fixture");
