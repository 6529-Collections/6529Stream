// Project the original Mint policy/grace and governance ABI from retained ABI52; never compile.
...
export function mintPolicyGraceFixture(inputBytes, outputBytes) {
  if (sha(inputBytes) !== INPUT_SHA || sha(outputBytes) !== OUTPUT_SHA) throw Error("Expected exact frozen Mint grace ABI52 capture");
  ...
  return {
    ...
    sourceBinding: "All 2212 literal input sources independently verified byte-for-byte against this Git commit. Closure hashes and original policy/governance source texts follow.",
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!input || !output || (mode !== undefined && mode !== "--check")) throw Error("Usage: generate-current-mint-policy-grace-fixture.mjs INPUT OUTPUT [--check]");
  const rendered = JSON.stringify(mintPolicyGraceFixture(await readFile(input), await readFile(output)), null, 2) + "\n";
  const target = new URL("../test/fixtures/current-mint-policy-grace-abi.json", import.meta.url);
  if (mode === "--check") {
