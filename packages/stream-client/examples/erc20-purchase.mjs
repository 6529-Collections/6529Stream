...
export async function purchaseERC20(client, { submitter, payer, platform, artist }, authorization, intent, tokenData, { onSubmitted = () => {} } = {}) {
  ...
  if (record.saleNonce === 0n || record.cancelled || !sameHash(record.configHash, authorization.saleConfigHash)) throw Error("Sale configuration is unavailable or changed");
  ...
  if (!sameHash(policy.policyHash, record.config.expectedPrimaryPolicyHash)) throw Error("Primary policy changed");
  if (getAddress(intent.payer) !== getAddress(authorization.payer) || getAddress(intent.asset) !== getAddress(record.config.asset) || intent.maxAmount < record.config.price || !sameHash(intent.saleRef, authorization.saleId) || !sameHash(intent.expectedPrimaryPolicyHash, policy.policyHash)) throw Error("Payer intent differs from this sale");
