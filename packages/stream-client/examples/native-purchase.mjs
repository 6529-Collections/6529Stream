...
export async function purchaseNative(client, { payer, platform, artist }, authorization, tokenData, { onSubmitted = () => {} } = {}) {
  ...
  if (settled.args.authorizationDigest !== payload.digest || settled.args.profileId !== authorization.profileId || settled.args.amount !== authorization.price) throw Error("Receipt differs from signed terms");
