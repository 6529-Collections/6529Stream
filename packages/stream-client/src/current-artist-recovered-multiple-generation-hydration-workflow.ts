/** MULTIPLE_GENERATIONS operation60 adapter. Original Registry simulation remains semantic admission. */
...
import * as multiple from "./current-artist-recovered-multiple-generation-hydration.js";
import { ARTIST_ATTRIBUTION_HEAD_TUPLE, ARTIST_ATTRIBUTION_RECORD_TUPLE, ARTIST_ATTRIBUTION_RESOLUTION_TUPLE, ARTIST_ATTRIBUTION_GOVERNANCE_WITNESS_TUPLE } from "./current-artist-attribution.js";
...
export type {
  ArtistRecoveredHydrationCodePin as ArtistRecoveredMultipleGenerationHydrationCodePin,
  ArtistRecoveredHydrationSuitePins as ArtistRecoveredMultipleGenerationHydrationSuitePins,
  ArtistRecoveredHydrationDeployment as ArtistRecoveredMultipleGenerationHydrationDeployment,
  ArtistRecoveredHydrationReader as ArtistRecoveredMultipleGenerationHydrationReader,
  ArtistRecoveredHydrationReceiptReader as ArtistRecoveredMultipleGenerationHydrationReceiptReader,
  ArtistRecoveredHydrationObservation as ArtistRecoveredMultipleGenerationHydrationObservation,
  ArtistRecoveredHydrationOwnerObservation as ArtistRecoveredMultipleGenerationHydrationOwnerObservation,
} from "./internal/artist-recovered-hydration-workflow.js";
export type ArtistRecoveredMultipleGenerationHydrationCapture = ArtistRecoveredHydrationCapture<multiple.ArtistRecoveredMultipleGenerationHydrationCall>;
export type ArtistRecoveredMultipleGenerationHydrationSimulation = ArtistRecoveredHydrationSimulation<multiple.ArtistRecoveredMultipleGenerationHydrationCall>;
export type ArtistRecoveredMultipleGenerationHydrationReceiptOptions = Readonly<
...
const attestationAbi = new Interface([
  ...
  `function personhoodProofSummary(bytes32 recordHash) view returns(${multiple.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_PERSONHOOD_SUMMARY_TUPLE})`,
  "function personhoodProofSummaryHash(bytes32 recordHash) view returns(bytes32)",
  `function c2paCredentialRecord(bytes32 recordHash) view returns(${multiple.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_CREDENTIAL_HEAD_TUPLE})`,
  `function c2paCredentialHead(bytes32 artistId) view returns(${multiple.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_CREDENTIAL_HEAD_TUPLE})`,
...
async function attestationRows(reader: ArtistRecoveredHydrationReader, owner: Address,
  certificate: multiple.ArtistRecoveredMultipleGenerationHydrationPrepared, tag: number, requireLatest: boolean) {
  const { payload } = multiple.decodeArtistRecoveredMultipleGenerationHydrationOwnerPayload(certificate.data[4].typedState, 4);
  const state = multiple.decodeArtistRecoveredMultipleGenerationHydrationState(payload.semanticState, 4, payload.provenance);
  const bundles = state.rows.map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationAttribution(raw).records);
  const decoded = multiple.decodeArtistRecoveredMultipleGenerationHydrationAuxiliary(payload.semanticState, 4, payload.provenance);
  const inventory = multiple.decodeArtistRecoveredMultipleGenerationHydrationInventory(decoded.auxiliary);
  type Head = multiple.ArtistRecoveredMultipleGenerationHydrationCredentialHead;
  type Record = multiple.ArtistRecoveredMultipleGenerationHydrationAttestationBundle["records"][number]["attestation"]["record"];
  const zeroHead = emptyTuple<Head>(multiple.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_CREDENTIAL_HEAD_TUPLE);
  ...
  for (const entry of payload.provenance.journal) {
    if (entry.receipt.operation === 44n) continue;
    ...
    const summary = isPersonhood ? bundle.personhood.find(s => s.recordHash === r.record.recordHash) : {
      summary: emptyTuple<multiple.ArtistRecoveredMultipleGenerationHydrationPersonhoodSummary>(multiple.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_PERSONHOOD_SUMMARY_TUPLE), summaryHash: io.ZERO,
    };
  ...
  for (const { k, a, index, entry } of ordered) {
    ...
    if (terms.subjectKind === 10n && terms.schemaId === id("6529STREAM_ARTIST_C2PA_CREDENTIALS_V1")) {
      const credential = tupleValue<multiple.ArtistRecoveredMultipleGenerationHydrationCredentialPayload>(multiple.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_CREDENTIAL_PAYLOAD_TUPLE, r.statement);
...
async function archiveClocks(reader: ArtistRecoveredHydrationReader,
  certificate: multiple.ArtistRecoveredMultipleGenerationHydrationPrepared, tag: number) {
  const p = certificate.admission.provenance;
  const ownerPayload = (index: 0 | 3 | 4) => multiple.decodeArtistRecoveredMultipleGenerationHydrationOwnerPayload(certificate.data[index].typedState, index).payload;
  const payload = ownerPayload(4), decoded = multiple.decodeArtistRecoveredMultipleGenerationHydrationAuxiliary(payload.semanticState, 4, payload.provenance);
  const inventory = multiple.decodeArtistRecoveredMultipleGenerationHydrationInventory(decoded.auxiliary);
  const selected: multiple.ArtistRecoveredMultipleGenerationHydrationArchiveOperation[] = [], envelopes: Hex[] = [];
  ...
  for (let era = 0; era < p.origins.length; era++) {
    ...
    for (let index = 0n; index < count; index++) {
      ...
      const envelope = multiple.decodeArtistRecoveredMultipleGenerationHydrationArchiveEnvelope(raw);
  ...
  const bindings = multiple.decodeArtistRecoveredMultipleGenerationHydrationState(bindingPayload.semanticState, 0, bindingPayload.provenance).rows
    .map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationBindingBundle(raw));
  const acceptances = multiple.decodeArtistRecoveredMultipleGenerationHydrationState(acceptancePayload.semanticState, 3, acceptancePayload.provenance).rows
    .map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationAcceptanceBundle(raw));
  ...
  const clocks = multiple.validateArtistRecoveredMultipleGenerationHydrationClocks(decoded.state, p, inventory, { envelopes, bindings, acceptances });
  const attribution = decoded.state.rows.map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationAttribution(raw));
  multiple.validateArtistRecoveredMultipleGenerationHydrationRevocations(decoded.state, p, inventory, attribution.map(row => row.history), clocks);
  const { payload: identity } = multiple.decodeArtistRecoveredMultipleGenerationHydrationOwnerPayload(certificate.data[2].typedState, 2);
  const identities = multiple.decodeArtistRecoveredMultipleGenerationHydrationState(identity.semanticState, 2, identity.provenance).rows
    .map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationIdentity(raw));
  multiple.validateArtistRecoveredMultipleGenerationHydrationAttestations(identities, certificate.admission.collections, attribution.map(row => row.records), p, inventory, clocks);
}
...
async function generationRows(reader: ArtistRecoveredHydrationReader, suite: ArtistHydrationSuite,
  certificate: multiple.ArtistRecoveredMultipleGenerationHydrationPrepared, tag: number, currentBinding: boolean, currentAttribution: boolean) {
  const state = (index: 0 | 3 | 4) => {
    const { payload } = multiple.decodeArtistRecoveredMultipleGenerationHydrationOwnerPayload(certificate.data[index].typedState, index);
    return multiple.decodeArtistRecoveredMultipleGenerationHydrationState(payload.semanticState, index, payload.provenance);
  };
  const bindings = state(0).rows.map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationBindingBundle(raw));
  const acceptances = state(3).rows.map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationAcceptanceBundle(raw));
  const attribution = state(4).rows.map(raw => multiple.decodeArtistRecoveredMultipleGenerationHydrationAttribution(raw).history);
  for (let k = 0; k < bindings.length; k++) {
    ...
    for (let g = 0; g < b.bindings.rows.length; g++) {
      ...
      const revocation = row.item.accepted && g + 1 < b.bindings.rows.length ? history.revocations[revoked++] : null;
      if (currentAttribution) io.equal(await io.read(reader, suite.owners[4], generationAbi, "attributionDispute", [cid, generation], tag), revocation?.head ?? emptyTuple(ARTIST_ATTRIBUTION_HEAD_TUPLE), "Retained generation dispute head differs");
      if (revocation) {
        io.equal(await io.read(reader, suite.owners[4], generationAbi, "attributionDisputeRecord", [revocation.opening.recordHash], tag), revocation.opening, "Retained governed opening differs");
        io.equal(await io.read(reader, suite.owners[4], generationAbi, "attributionDisputeResolution", [revocation.resolution.actionId], tag), revocation.resolution, "Retained class-two revoke resolution differs");
      }
    }
...
async function sourceCounter(reader: ArtistRecoveredHydrationReader, source: ArtistHydrationSuite,
  _input: multiple.ArtistRecoveredMultipleGenerationHydrationInput, certificate: multiple.ArtistRecoveredMultipleGenerationHydrationPrepared, tag: number) {
...
async function destinationCounter(reader: ArtistRecoveredHydrationReader, capture: ArtistRecoveredMultipleGenerationHydrationCapture,
...
async function delegationRows(reader: ArtistRecoveredHydrationReader, owner: Address,
  certificate: multiple.ArtistRecoveredMultipleGenerationHydrationPrepared, tag: number) {
  const { payload } = multiple.decodeArtistRecoveredMultipleGenerationHydrationOwnerPayload(certificate.data[2].typedState, 2);
  const state = multiple.decodeArtistRecoveredMultipleGenerationHydrationState(payload.semanticState, 2, payload.provenance);
  for (const raw of state.rows) {
    const identity = multiple.decodeArtistRecoveredMultipleGenerationHydrationIdentity(raw);
...
async function consentRows(reader: ArtistRecoveredHydrationReader, owner: Address,
  certificate: multiple.ArtistRecoveredMultipleGenerationHydrationPrepared, tag: number, requireLatest: boolean) {
  const { payload } = multiple.decodeArtistRecoveredMultipleGenerationHydrationOwnerPayload(certificate.data[6].typedState, 6);
  const state = multiple.decodeArtistRecoveredMultipleGenerationHydrationState(payload.semanticState, 6, payload.provenance);
  for (const raw of state.rows) {
    const b = multiple.decodeArtistRecoveredMultipleGenerationHydrationConsents(raw).rows;
    const q = b.original;
...
const original = createRecoveredHydrationWorkflow<multiple.ArtistRecoveredMultipleGenerationHydrationInput, multiple.ArtistRecoveredMultipleGenerationHydrationCall>({
  normalizeInputDraft: multiple.normalizeArtistRecoveredMultipleGenerationHydrationInputDraft,
  request: input => input.request,
  finalizeInput: (input, inventory) => multiple.normalizeArtistRecoveredMultipleGenerationHydrationInput({ request: { ...input.request, expectedSemanticInventory: inventory }, royaltyFreezes: input.royaltyFreezes }),
  inputFromCall: call => ({ request: call.request, royaltyFreezes: call.royaltyFreezes }),
  prepareCall: multiple.prepareArtistRecoveredMultipleGenerationHydrationCall,
  normalizeCall: multiple.normalizeArtistRecoveredMultipleGenerationHydrationCall,
  preparationCalldata: multiple.artistRecoveredMultipleGenerationHydrationPreparationCalldata,
  normalizePrepared: multiple.normalizeArtistRecoveredMultipleGenerationHydrationPrepared,
  decodeOwnerPayload: multiple.decodeArtistRecoveredMultipleGenerationHydrationOwnerPayload,
  semanticInventory: multiple.artistRecoveredMultipleGenerationHydrationSemanticInventory,
  commitment: multiple.artistRecoveredMultipleGenerationHydrationCommitment,
  ownerAfter: multiple.artistRecoveredMultipleGenerationHydrationOwnerAfter,
  profileEvidence: multiple.encodeArtistRecoveredMultipleGenerationHydrationProfileEvidence,
  validateInput: multiple.validateArtistRecoveredMultipleGenerationHydrationInput,
  freshIdentity: input => {
...
export async function captureArtistRecoveredMultipleGenerationHydration(reader: ArtistRecoveredHydrationReader, deployment: ArtistRecoveredHydrationDeployment,
  caller: Address, input: multiple.ArtistRecoveredMultipleGenerationHydrationInput, options: { readonly blockTag: number; readonly gasLimit: bigint }) {
  const result = await original.capture(reader, deployment, caller, input, options);
  capturedHere.add(result); return result;
}
export async function simulateArtistRecoveredMultipleGenerationHydration(reader: ArtistRecoveredHydrationReader, capture: ArtistRecoveredMultipleGenerationHydrationCapture,
...
export async function observeArtistRecoveredMultipleGenerationHydrationRefusal(reader: ArtistRecoveredHydrationReader, input: ArtistRecoveredMultipleGenerationHydrationCapture,
  options: { readonly blockTag: number; readonly gasLimit: bigint }) {
...
function transportOptions(input: ArtistRecoveredMultipleGenerationHydrationReceiptOptions): ArtistRecoveredMultipleGenerationHydrationReceiptOptions {
...
async function retainedTransport(reader: ArtistRecoveredHydrationReceiptReader, capture: ArtistRecoveredMultipleGenerationHydrationCapture,
  hash: Hex, options: ArtistRecoveredMultipleGenerationHydrationReceiptOptions) {
...
export async function reconcileArtistRecoveredMultipleGenerationHydrationReceipt(reader: ArtistRecoveredHydrationReceiptReader,
  input: ArtistRecoveredMultipleGenerationHydrationCapture, transactionHash: Hex, inputOptions: ArtistRecoveredMultipleGenerationHydrationReceiptOptions) {
  const capture = io.freeze(structuredClone(input)), hash = io.hash(transactionHash), options = transportOptions(inputOptions);
  multiple.normalizeArtistRecoveredMultipleGenerationHydrationCall(capture.prepared);
...
export const inspectArtistRecoveredMultipleGenerationHydrationHistory = reconcileArtistRecoveredMultipleGenerationHydrationReceipt;
/** Checks eligibility for a fresh import. A completed import is not eligible for replay. */
export async function inspectArtistRecoveredMultipleGenerationHydrationCurrent(reader: ArtistRecoveredHydrationReader, deployment: ArtistRecoveredHydrationDeployment,
  caller: Address, input: multiple.ArtistRecoveredMultipleGenerationHydrationInput, options: { readonly blockTag: number; readonly gasLimit: bigint }) {
  const capture = await captureArtistRecoveredMultipleGenerationHydration(reader, deployment, caller, input, options);
  return simulateArtistRecoveredMultipleGenerationHydration(reader, capture, { blockTag: capture.observed.blockNumber, gasLimit: capture.preparationGasLimit });
}
