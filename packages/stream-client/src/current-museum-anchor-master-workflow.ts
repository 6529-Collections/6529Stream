...
export interface MuseumCodePin {
  readonly address: Address;
  readonly codeHash: Hex
}

export interface MuseumAnchorDeployment {
  readonly chainId: bigint;
  readonly core: MuseumCodePin;
  readonly executor: MuseumCodePin
}

export interface MuseumMasterDeployment extends MuseumAnchorDeployment {
  readonly metadata: MuseumCodePin;
  readonly masterSelection: MuseumCodePin;
  readonly schemaRegistry: MuseumCodePin;
  readonly externalCoverage: MuseumCodePin;
  readonly store: MuseumCodePin;
  readonly artist: {
    readonly registry: MuseumCodePin;
    readonly coordinator: MuseumCodePin;
    readonly identity: MuseumCodePin;
    readonly binding: MuseumCodePin;
    readonly attribution: MuseumCodePin
  };
}

export interface MuseumBlock {
  readonly blockNumber: number;
  readonly blockHash: Hex;
  readonly timestamp: bigint
}

export interface MuseumGovernanceCatalog {
...
function keys(v: unknown, required: readonly string[], optional: readonly string[] = []): void {
  if (!v || typeof v !== "object" || Array.isArray(v)
    || Reflect.ownKeys(v).some(k => typeof k !== "string" || ![...required, ...optional].includes(k))
...
function hash(v: unknown, zero = false): Hex {
  if (typeof v !== "string" || !isHexString(v, 32) || (!zero && v.toLowerCase() === ZeroHash))
...
function bytes(v: unknown, max = 32768): Hex {
  if (typeof v !== "string" || !isHexString(v, true) || (v.length - 2) / 2 > max)
    throw Error("Malformed or oversized bytes");
  return v.toLowerCase() as Hex;
}
...
function stable(v: unknown): string {
  const tagged = (x: unknown): unknown => {
    ...
    if (x && typeof x === "object")
      return ["object", Object.keys(x).sort().map(k => [k, tagged((x as Record<string, unknown>)[k])])];
...
async function header(p: Reader, tag: number) {
  ...
  if (!b || b.number !== tag || !Number.isSafeInteger(b.timestamp) || b.timestamp < 0)
...
async function unchanged(p: Reader, h: {
  ...
  equal(
    await header(p, h.blockNumber),
    { blockNumber: h.blockNumber, blockHash: h.blockHash, timestamp: h.timestamp },
...
async function runtime(p: Reader, v: MuseumCodePin, tag: number): Promise<void> {
  ...
  if (code === "0x" || (code.length === 48 && code.startsWith("0xef0100"))
    || !same(keccak256(code), v.codeHash))
    throw Error("Pinned runtime differs");
}

function plain(t: ParamType, value: any): any {
  if (t.baseType === "array") return Array.from(value, item => plain(t.arrayChildren!, item));
  if (t.baseType === "tuple")
    return Object.fromEntries(t.components!.map((part, i) => [part.name, plain(part, value[i])]));
  return value;
}
...
function digest(v: unknown): Hex {
  return keccak256(toUtf8Bytes(stable(v))) as Hex;
}
...
function coordinates(d: MuseumMasterDeployment): museum.MuseumAnchorMasterCoordinates {
  return {
    ...
    metadata: d.metadata.address,
    masterSelection: d.masterSelection.address,
    schemaRegistry: d.schemaRegistry.address,
    externalCoverage: d.externalCoverage.address
  };
}
...
async function governance(p: Reader, d: MuseumAnchorDeployment, tag: number) {
  ...
  if (b[0] !== true || b[1] !== true
    || (await read(p, d.executor.address, "minimumDelay", [1n], tag))[0] !== 172800n)
    throw Error("Only sealed ordinary class1 governance is supported");
  ...
  return {
    governanceNonce: uint((await read(p, d.executor.address, "governanceNonce", [], tag))[0]),
    catalog: {
      candidateProfileHash: hash(profile),
      catalogHash: hash(catalogHash),
      entryCount: uint(count),
      revision: uint(revision, 64),
...
export interface MuseumAnchorCapture extends MuseumBlock {
  readonly deployment: MuseumAnchorDeployment;
  readonly kind: "conditionSources" | "conservationFloor";
  readonly candidate: MuseumCodePin;
  readonly previous: museum.MuseumAnchorBindingState;
  readonly sourceHead: {
    readonly count: bigint;
    readonly head: Hex
  };
  readonly plan: museum.MuseumAnchorBindingPlan;
  readonly governanceNonce: bigint;
  readonly catalog: MuseumGovernanceCatalog;
  readonly captureHash: Hex;
}

/** Direct candidate observations do not prove Core's bounded nested STATICCALL admission. */
export async function captureMuseumAnchorBinding(
  p: Reader,
  input: MuseumAnchorDeployment,
  kind: MuseumAnchorCapture["kind"],
  candidateInput: MuseumCodePin,
  options: { readonly blockTag: number }
): Promise<MuseumAnchorCapture> {
  keys(options, ["blockTag"]);
  const d = anchorDeployment(input),
    candidate = pin(candidateInput),
    tag = number(options.blockTag);
  if (kind !== "conditionSources" && kind !== "conservationFloor")
  ...
  for (const [name, expected] of [
    ...
    ["governanceAuthority", d.executor.address],
    ["executorCodeHash", d.executor.codeHash]
  ] as const) {
    if (!same((await read(p, candidate.address, name, [], tag))[0], expected))
      throw Error(`Candidate ${name} differs`);
  }
  if ((await read(p, candidate.address, "deploymentChainId", [], tag))[0] !== d.chainId
    || (await read(
      ...
      candidate.address,
      "supportsInterface",
      [kind === "conditionSources" ? "0xa621c1b7" : "0xaf0adf33"],
      tag
    ))[0] !== true)
    throw Error("Candidate chain/interface differs");
  ...
  const plan = museum.prepareMuseumAnchorBinding(
    { chainId: d.chainId, core: d.core.address, executor: d.executor.address },
    {
      kind,
      candidate: candidate.address,
      runtimeCodeHash: candidate.codeHash,
      previous
    }
  );
  equal(
    await read(p, d.core.address, `${kind}Transition`, [candidate.address], tag),
    [plan.transition.scope, plan.transition.oldHash, plan.transition.newHash],
    "Original anchor transition differs"
  );
...
function savedAnchor(v: MuseumAnchorCapture): MuseumAnchorCapture {
  ...
  anchorDeployment(out.deployment);
  pin(out.candidate);
  museum.normalizeMuseumAnchorBindingPlan(out.plan);
...
async function originalAnchor(p: Reader, c: MuseumAnchorCapture): Promise<void> {
  equal(
    await captureMuseumAnchorBinding(p, c.deployment, c.kind, c.candidate, { blockTag: c.blockNumber }),
    c,
    "Historical anchor capture changed"
  );
}
...
export interface MuseumConservationTierRead extends MuseumBlock {
  ...
  readonly completedMints: bigint;
  readonly declared: Hex;
  readonly effective: Hex;
}

/** Raw zero for an unknown collection is kept distinct from an undeclared known collection. */
export async function readMuseumConservationTier(
  p: Reader,
  input: MuseumAnchorDeployment,
  collection: bigint,
  options: { readonly blockTag: number }
): Promise<MuseumConservationTierRead> {
  keys(options, ["blockTag"]);
  const d = anchorDeployment(input),
    collectionId = uint(collection),
    tag = number(options.blockTag),
    h = await base(p, d, tag);
...
async function selected(
  p: Reader,
  d: MuseumMasterDeployment,
  kind: string,
  expected: MuseumCodePin,
  tag: number
): Promise<void> {
  ...
  const v = {
    ...
    codeHash: raw[1],
    moduleType: raw[3],
    registryStatus: raw[6]
  };
  if (!same(v.target, expected.address) || !same(v.codeHash, expected.codeHash)
    || !same(v.moduleType, id(kind))
    || v.registryStatus !== 1n)
    throw Error(`Current ${kind} pointer differs`);
}

async function bindings(p: Reader, d: MuseumMasterDeployment, tag: number, artists: boolean): Promise<void> {
  await Promise.all(
    [d.metadata, d.masterSelection, d.schemaRegistry, d.store, d.externalCoverage].map(x => runtime(p, x, tag))
  );
  for (const [host, name, expected] of [
    [d.metadata, "core", d.core.address],
    [d.metadata, "coreCodeHash", d.core.codeHash],
    [d.metadata, "schemaRegistry", d.schemaRegistry.address],
    [d.metadata, "schemaRegistryCodeHash", d.schemaRegistry.codeHash],
    [d.metadata, "chunkStore", d.store.address],
    [d.metadata, "chunkStoreCodeHash", d.store.codeHash],
    [d.schemaRegistry, "chunkStore", d.store.address],
    [d.masterSelection, "core", d.core.address],
    [d.masterSelection, "metadata", d.metadata.address],
    [d.masterSelection, "schemaRegistry", d.schemaRegistry.address],
    [d.masterSelection, "chunkStore", d.store.address],
    [d.masterSelection, "externalCoverage", d.externalCoverage.address],
    [d.masterSelection, "profileHash", museum.MUSEUM_MASTER_PROFILE_HASH],
    [d.externalCoverage, "core", d.core.address]
  ] as const)
    if (!same((await read(p, host.address, name, [], tag))[0], expected))
      throw Error(`Pinned ${name} binding differs`);
  if ((await read(p, d.masterSelection.address, "deploymentChainId", [], tag))[0] !== d.chainId)
    throw Error("Master deployment chain differs");
  if (!artists) return;
  await Promise.all(Object.values(d.artist).map(x => runtime(p, x, tag)));
  for (const [host, name, expected] of [
    [d.metadata, "artistRegistry", d.artist.registry.address],
    [d.metadata, "artistRegistryCodeHash", d.artist.registry.codeHash],
    [d.artist.registry, "core", d.core.address],
    [d.artist.registry, "operationCoordinator", d.artist.coordinator.address]
  ] as const) {
    if (!same((await read(p, host.address, name, [], tag))[0], expected))
      throw Error(`Artist ${name} differs`);
  }
  if ((await read(p, d.artist.coordinator.address, "deploymentChainId", [], tag))[0] !== d.chainId)
    throw Error("Artist coordinator chain differs");
  ...
  if (!same(suite.registry, d.artist.registry.address) || !same(suite.core, d.core.address)
    || !same(suite.owners[0], d.artist.binding.address)
    || !same(suite.owners[2], d.artist.identity.address)
    || !same(suite.owners[4], d.artist.attribution.address))
    throw Error("Artist original suite differs");
  for (const owner of [d.artist.identity, d.artist.binding, d.artist.attribution]) {
    for (const [name, expected] of [
      ["core", d.core.address],
      ["artistRegistry", d.artist.registry.address],
      ["operationCoordinator", d.artist.coordinator.address]
    ] as const) {
      if (!same((await read(p, owner.address, name, [], tag))[0], expected))
        throw Error("Artist owner binding differs");
    }
    if ((await read(p, owner.address, "deploymentChainId", [], tag))[0] !== d.chainId)
      throw Error("Artist owner chain differs");
  }
}
...
async function gas(p: Reader, d: MuseumMasterDeployment, tag: number): Promise<void> {
  const cap = (await read(
    ...
    d.metadata.address,
    "gasParameter",
    [id("6529STREAM_GGP_METADATA_DEPENDENCY_READ_GAS")],
    tag
  ))[0];
  ...
  for (const name of ["6529STREAM_GGP_MEDIA_MASTER_MANIFEST_READ_GAS", "6529STREAM_GGP_MEDIA_MASTER_COVERAGE_READ_GAS"]) {
    ...
    if (value < floor || floor < 500000n || value > (1n << 64n) - 1n || failureClass !== 2n
      || revision === 0n)
      throw Error("Master read cap differs");
  }
}
...
async function chunk(p: Reader, d: MuseumMasterDeployment, contentHash: Hex, tag: number): Promise<Hex> {
  ...
  if (BigInt((data.length - 2) / 2) !== length || !same(keccak256(data), contentHash)
    || !same(bytes(await p.getCode(pointer, tag), 8193), `0x00${data.slice(2)}`))
    throw Error("Stored payload bytes/runtime differ");
  return data;
}
...
async function definitions(p: Reader, d: MuseumMasterDeployment, waiver: boolean, tag: number): Promise<void> {
  const rows = [
    [
      waiver ? museum.MUSEUM_MASTER_WAIVER_SCHEMA_ID : museum.MUSEUM_MASTER_SCHEMA_ID,
      0n,
      waiver ? museum.MUSEUM_MASTER_WAIVER_SCHEMA_HASH : museum.MUSEUM_MASTER_SCHEMA_HASH,
      waiver ? 2121n : 1042n
    ],
    [id("STREAM_MEDIA_MASTER_SELECTED_SLOTS_PROFILE_V1"), 2n, museum.MUSEUM_MASTER_PROFILE_HASH, 2039n],
    [museum.MUSEUM_MASTER_CANONICALIZATION_ID, 1n, museum.MUSEUM_MASTER_CANONICALIZATION_HASH, 362n]
  ] as const;
  for (const [documentId, kind, expectedHash, length] of rows) {
    ...
    if (!v.exists || v.kind !== kind || v.status !== 0n || !same(v.contentHash, expectedHash)
      || !same(v.canonicalizationId, id("RAW_BYTES"))
      || v.supersedesId !== ZeroHash
      || v.totalBytes !== length
      || v.chunkCount === 0n
      || v.chunkCount > 64n)
      throw Error("Original ACTIVE definition differs");
    hash(v.declarationHash);
    let raw = "0x";
    for (let index = 0n; index < v.chunkCount; ++index) {
      const part = await chunk(
        p,
        d,
        hash((await read(p, d.schemaRegistry.address, "documentChunkHashAt", [documentId, index], tag))[0]),
        tag
      );
      if (index + 1n < v.chunkCount && part.length !== 16386)
        throw Error("Short intermediate document chunk");
      raw += part.slice(2);
      if ((raw.length - 2) / 2 > Number(length)) throw Error("Oversized definition");
    }
    if (BigInt((raw.length - 2) / 2) !== length || !same(keccak256(raw), expectedHash))
      throw Error("Retained definition differs");
  }
}

export interface MuseumMasterMediaFacts {
  readonly context: museum.MuseumMasterMediaContext;
  readonly displayHashes: readonly [Hex, Hex, Hex];
...
async function association(p: Reader, d: MuseumMasterDeployment, cid: bigint, tag: number): Promise<museum.MuseumMasterAssociation> {
  ...
  if (b.artistId === ZeroHash) {
    if (Object.values(b).some(v => v !== ZeroHash && v !== ZeroAddress && v !== false && v !== 0n))
      throw Error("Nonempty zero-Artist binding");
    return {
      artistId: ZeroHash,
      bindingHash: ZeroHash,
      generation: 0n,
      identityRecordHash: ZeroHash
    };
  }
  ...
  if (!b.accepted || ![2n, 3n].includes(status) || generation === 0n || generation !== b.generation
    || !same(hash(identityRecordHash), hash(b.identityRecordHash)))
    throw Error("Current Artist association differs");
  address(b.artistAddress);
  return {
    artistId: hash(b.artistId),
    bindingHash: hash(b.bindingHash),
    generation,
    identityRecordHash: hash(identityRecordHash)
  };
}
...
async function media(p: Reader, d: MuseumMasterDeployment, cid: bigint, tag: number): Promise<MuseumMasterMediaFacts> {
  ...
  if (m.manifestURI !== "" || m.manifestHash !== ZeroHash || m.alternatesURI !== ""
    || m.alternatesHash !== ZeroHash)
    throw Error("Opaque media denominator is unsupported");
  ...
  if (serving.animationBaseURI !== "" || serving.imageURI !== m.imageURI)
    throw Error("Serving media denominator differs");
  let occupiedMask = 0n;
  const displayHashes = ["image", "animation", "content"].map((key, index) => {
    const source = m[`${key}SourceType`],
      uri = m[`${key}URI`],
      h = hash(m[`${key}Hash`], true);
    if (source > 8n || (source === 0n ? uri !== "" || h !== ZeroHash : uri === "" || h === ZeroHash))
      throw Error("Invalid native media slot");
    if (source !== 0n) occupiedMask |= 1n << BigInt(index);
    return h;
  }) as [Hex, Hex, Hex];
  const inventoryHash = keccak256(
    coder.encode(
      ["bytes32", abi.getFunction("mediaManifest")!.outputs[0]!],
      [id("6529STREAM_MEDIA_MASTER_INVENTORY_V1"), m]
    )
  ) as Hex;
  const context = {
    subjectId: museum.museumMasterCollectionSubject(d.chainId, d.core.address, cid),
    manifestHash,
    inventoryHash,
    occupiedMask
  };
  equal(
    await read(p, d.masterSelection.address, "collectionMediaContext", [cid], tag),
    [context.subjectId, context.manifestHash, context.inventoryHash, context.occupiedMask],
    "Original media context differs"
  );
...
function blankPublication(): museum.MuseumMasterPublicationEvidence {
  return {
    attestationRecordHash: ZeroHash,
    artistId: ZeroHash,
    bindingHash: ZeroHash,
    bindingGeneration: 0n,
    signer: ZeroAddress,
    authorityClass: 0n,
    requiredCapability: 0n,
    signedAt: 0n,
    publicationHash: ZeroHash
  };
}

async function publicationEvidence(
  p: Reader,
  d: MuseumMasterDeployment,
  cid: bigint,
  r: museum.MuseumMasterCollectionRecord,
  recorder: Address,
  authorization: Hex,
  tag: number,
  recordedAt?: bigint
): Promise<museum.MuseumMasterPublicationEvidence> {
  ...
  if (!same(saved.metadataHostCodeHash, d.metadata.codeHash))
    throw Error("Original publication host runtime differs");
  const e = museum.normalizeMuseumMasterPublicationEvidence(saved.evidence);
  if (!same(e.attestationRecordHash, authorization) || e.authorityClass !== 1n
    || e.requiredCapability !== 1n
    || !same(e.signer, recorder)
    || e.bindingGeneration === 0n
    || e.signedAt === 0n
    || (recordedAt !== undefined && e.signedAt > recordedAt)
    || !same(e.publicationHash, keccak256(museum.encodeMuseumMasterPublication(expected.publication))))
    throw Error("Waiver requires original principal class1 publication evidence");
  hash(e.artistId);
  hash(e.bindingHash);
  ...
  equal(
    a,
    {
      recordHash: authorization,
      subjectStateHash: ZeroHash,
      schemaId: id("6529STREAM_ARTIST_RECORD_PUBLICATION_V1"),
      statementHash: expected.statementHash,
      generation: e.bindingGeneration,
      signedAt: e.signedAt,
      signer: e.signer
    },
    "Original op24 record differs"
  );
  equal(
    (await read(
      p,
      d.artist.attribution.address,
      "statementBytes",
      [expected.statementHash],
      tag,
      undefined,
      480
    ))[0],
    expected.statement,
    "Original publication statement differs"
  );
  return e;
}
...
async function recordPolicy(p: Reader, d: MuseumMasterDeployment, waiver: boolean, tag: number) {
  const [policy] = await read(
    ...
    d.metadata.address,
    "recordPolicy",
    [id(waiver ? "ARTIST_STATEMENT" : "MEDIA_RELATIONSHIP")],
    tag
  );
  if (!policy.admitted
    || !same(
      policy.family,
      id(waiver ? "6529STREAM_RECORD_FAMILY_ARTIST_V1" : "6529STREAM_RECORD_FAMILY_MEDIA_RELATIONSHIP_V1")
    ))
    throw Error("Original record policy differs");
  return museum.normalizeMuseumMasterRecordPolicy(policy);
}

export interface MuseumMasterStoredRecord {
  readonly record: museum.MuseumMasterCollectionRecord;
  readonly receipt: museum.MuseumMasterRecordReceipt;
  readonly evidence: museum.MuseumMasterRecordEvidence;
...
async function storedRecord(
  ...
  cid: bigint,
  recordHash: Hex,
  original: museum.MuseumMasterCollectionRecord,
  waiver: boolean,
  tag: number,
  livePolicy: boolean
): Promise<MuseumMasterStoredRecord> {
  const [saved, receiptRaw] = await read(p, d.metadata.address, "collectionRecord", [recordHash], tag);
  const r = museum.normalizeMuseumMasterCollectionRecord(saved),
    receipt = museum.normalizeMuseumMasterRecordReceipt(receiptRaw);
  ...
  if (receipt.collectionId !== cid || receipt.recordedAt === 0n
    ...
    || (!waiver && receipt.artistAuthorization !== ZeroHash)
    || !same(receipt.schemaDefinitionHash, schemaHash)
    || !same(receipt.canonicalizationDefinitionHash, museum.MUSEUM_MASTER_CANONICALIZATION_HASH)
    || !same(museum.museumMasterRecordHash(coordinates(d), receipt.recorder, cid, r), recordHash))
    throw Error("Original record receipt differs");
  if (livePolicy) {
    const policy = await recordPolicy(p, d, waiver, tag);
    if ((policy.authorizationMask & (1n << receipt.authorizationClass)) === 0n)
      throw Error("Original record class no longer admitted");
  }
  if (!same(
    (await read(p, d.metadata.address, "recordHashAt", [cid, r.recordType, receipt.recordIndex], tag))[0],
    recordHash
  ))
    throw Error("Original record lane differs");
  let previous = ZeroHash as Hex;
  if (receipt.recordIndex > 0n) {
    const priorHash = hash(
      (await read(p, d.metadata.address, "recordHashAt", [cid, r.recordType, receipt.recordIndex - 1n], tag))[0]
    );
    const [prior] = await read(p, d.metadata.address, "collectionRecordReceipt", [priorHash], tag);
    if (prior.collectionId !== cid || prior.recordIndex + 1n !== receipt.recordIndex)
      throw Error("Prior native record differs");
    previous = hash(prior.recordChainHash);
  }
  if (!same(
    museum.museumMasterRecordChainHash(
      coordinates(d),
      cid,
      r.recordType,
      previous,
    ...
    throw Error("Original record chain differs");
  const payloadHash = hash(r.contentHash.digest),
    payload = await chunk(p, d, payloadHash, tag);
  let publication = blankPublication(),
    publicationEvidenceHash = ZeroHash as Hex;
  if (waiver) {
    hash(receipt.artistAuthorization);
    if ((await read(p, d.metadata.address, "consumedArtistAuthorization", [receipt.artistAuthorization], tag))[0] !== true)
      throw Error("Artist publication was not consumed");
    publication = await publicationEvidence(
      p,
      d,
      cid,
      r,
    ...
    publicationEvidenceHash = keccak256(
      coder.encode([abi.getFunction("publicationAttestation")!.outputs[0]!], [savedPublication])
    ) as Hex;
  }
  const evidence = {
    recordHash,
    ...
    authorizationClass: receipt.authorizationClass,
    recordedAt: receipt.recordedAt,
    recordIndex: receipt.recordIndex,
    recordChainHash: receipt.recordChainHash,
    receiptHash: keccak256(museum.encodeMuseumMasterRecordReceipt(receipt)) as Hex,
    publication,
...
async function coverage(
  p: Reader,
  d: MuseumMasterDeployment,
  s: museum.MuseumMasterSelection,
  tag: number
): Promise<Hex> {
  hash(s.association.artistId);
  hash(s.masterObjectHash);
  hash(s.coverageHash);
  const [o] = await read(p, d.externalCoverage.address, "objectIdentity", [s.masterObjectHash], tag);
  if (!same(o.artistId, s.association.artistId) || o.contentHash === ZeroHash
    || same(o.contentHash, s.displayHash)
    || o.sha256Digest === ZeroHash
    || o.arweaveDataRoot === ZeroHash
    || o.byteSize === 0n)
    throw Error("Original master object differs");
  const [c] = await read(
    p,
    d.externalCoverage.address,
    "requireCoverage",
    [s.coverageHash, s.association.artistId, s.masterObjectHash],
    tag
  );
  for (const name of ["artistId", "contentHash", "sha256Digest", "arweaveDataRoot", "byteSize"])
    equal(c[name], o[name], "Original coverage/object differs");
  if (!same(c.coverageHash, s.coverageHash) || !same(c.objectHash, s.masterObjectHash)
    || !same(c.profileHash, id("STREAM_EXTERNAL_ARTIFACT_COVERAGE_V1")))
    throw Error("Original coverage identity differs");
  return keccak256(
    coder.encode(
      [abi.getFunction("objectIdentity")!.outputs[0]!, abi.getFunction("requireCoverage")!.outputs[0]!],
      [o, c]
    )
  ) as Hex;
}
...
function selection(c: museum.MuseumAnchorMasterCoordinates, cid: bigint, value: unknown): museum.MuseumMasterSelection {
  ...
  if (s.status === 0n) {
    ...
    equal(
      museum.encodeMuseumMasterSelection(s),
      coder.encode([museum.MUSEUM_MASTER_SELECTION_TUPLE], [zero]),
      "Malformed absent selection"
    );
  } else if (s.revision === 0n || !same(museum.museumMasterSelectionHash(c, cid, s), s.selectionHash))
    throw Error("Selection hash/revision differs");
  return s;
}

export interface MuseumMasterCapture extends MuseumBlock {
  readonly deployment: MuseumMasterDeployment;
  readonly prepared: museum.MuseumAnchorMasterCall;
  readonly media: MuseumMasterMediaFacts | null;
  readonly original: MuseumMasterStoredRecord | null;
  readonly previous: museum.MuseumMasterSelection | null;
  readonly expectedSelection: museum.MuseumMasterSelection | null;
  readonly publicationEvidence: museum.MuseumMasterPublicationEvidence | null;
  readonly authorizationClass: bigint | null;
  readonly captureHash: Hex;
}

export async function captureMuseumAnchorMaster(
  p: Reader,
  deploymentInput: MuseumMasterDeployment,
  callInput: museum.MuseumAnchorMasterCall,
  options: { readonly blockTag: number }
): Promise<MuseumMasterCapture> {
  keys(options, ["blockTag"]);
  const d = masterDeployment(deploymentInput),
    prepared = museum.normalizeMuseumAnchorMasterCall(callInput),
    tag = number(options.blockTag);
  ...
  if ((await read(p, d.core.address, "collectionExists", [cid], tag))[0] !== true)
    throw Error("Unknown collection");
  let mediaFacts: MuseumMasterMediaFacts | null = null,
    original: MuseumMasterStoredRecord | null = null;
  let previous: museum.MuseumMasterSelection | null = null,
    expectedSelection: museum.MuseumMasterSelection | null = null;
  let publication: museum.MuseumMasterPublicationEvidence | null = null,
    authorizationClass: bigint | null = null;
  if (r.kind === "declareConservationTier") {
    await selected(p, d, "COLLECTION_METADATA", d.metadata, tag);
    const tier = await readMuseumConservationTier(
      p,
      { chainId: d.chainId, core: d.core, executor: d.executor },
      cid,
      { blockTag: tag }
    );
    if (tier.declared !== ZeroHash || tier.completedMints !== 0n)
      throw Error("Tier already declared or collection minted");
    const [local] = await read(
      p,
      d.metadata.address,
      "familyWriter",
      [cid, id("6529STREAM_RECORD_FAMILY_CONSERVATION_V1"), 7n, prepared.caller],
      tag
    );
    const [global] = await read(
      ...
      d.metadata.address,
      "familyWriter",
      [0n, id("6529STREAM_RECORD_FAMILY_CONSERVATION_V1"), 8n, prepared.caller],
      tag
    );
    if (!local && !global) throw Error("Conservation7/global8 writer required");
  } else if (r.kind === "recordCollectionRecordWithPayload"
    || r.kind === "recordArtistCollectionRecordWithPayload") {
    await selected(p, d, "COLLECTION_METADATA", d.metadata, tag);
    ...
    if (waiver) {
      await selected(p, d, "ARTIST_REGISTRY", d.artist.registry, tag);
      if ((await read(p, d.metadata.address, "consumedArtistAuthorization", [r.authorization], tag))[0] !== false)
        throw Error("Artist authorization already consumed");
      ...
      equal(
        (await read(
          p,
          d.artist.registry.address,
          "requireRecordPublication",
          [r.authorization, pub],
          tag,
          d.metadata.address
        ))[0],
        publication,
        "Live Artist publication admission differs"
      );
      ...
      for (const cl of [3n, 4n, 6n, 7n, 8n]) {
        ...
        if (local || global) {
          authorizationClass = cl;
          break;
        }
      }
      if (authorizationClass !== 6n && authorizationClass !== 7n)
        throw Error("Master publication needs original MEDIA6/7 writer");
    }
    if (!(policy.authorizationMask & (1n << authorizationClass)))
      throw Error("Record class is not admitted");
  } else {
    await currentContext(p, d, tag);
    const waiver = r.kind === "adoptWaiver";
    await definitions(p, d, waiver, tag);
    mediaFacts = await media(p, d, cid, tag);
    const slot = waiver ? r.slot : r.witness.mediaSlot,
      displayHash = mediaFacts.displayHashes[Number(slot) - 1]!;
    if (displayHash === ZeroHash || !(mediaFacts.context.occupiedMask & (1n << (slot - 1n))))
      throw Error("Unoccupied media slot");
    const manifestHash = waiver ? r.manifestHash : r.witness.selectedMediaManifestHash;
    if (!same(manifestHash, mediaFacts.context.manifestHash)
      || !same(r.original.subjectId, mediaFacts.context.subjectId))
      throw Error("Current media/subject differs");
    ...
    previous = selection(
      coordinates(d),
      cid,
      (await read(p, d.masterSelection.address, "currentMaster", [cid, mediaFacts.context.subjectId, slot], tag))[0]
    );
    if (previous.revision !== r.expectedRevision
      || !same(r.witness.predecessor, previous.original.recordHash)
      || same(r.recordHash, previous.original.recordHash)
      || r.expectedRevision === (1n << 64n) - 1n)
      throw Error("Master predecessor/revision differs");
    const status = waiver ? 2n : 1n;
    if (previous.status !== 0n
      && (original.evidence.recordedAt < previous.original.recordedAt
        || (previous.status === status && original.evidence.recordIndex <= previous.original.recordIndex)))
      throw Error("Original record lineage regressed");
    const objectId = museum.museumMasterObjectId(
      coordinates(d),
      cid,
      mediaFacts.context.subjectId,
      manifestHash,
      slot,
      displayHash
    );
    ...
    if (waiver) {
      ...
      equal(
        r.witness.artist,
        {
          artistId: mediaFacts.association.artistId,
          bindingGeneration: mediaFacts.association.generation,
          bindingHash: mediaFacts.association.bindingHash
        },
        "Waiver current association differs"
      );
      const e = original.evidence.publication;
      if (!same(e.artistId, mediaFacts.association.artistId)
        || !same(e.bindingHash, mediaFacts.association.bindingHash)
        || e.bindingGeneration !== mediaFacts.association.generation)
        throw Error("Waiver publication association differs");
    ...
    const next: museum.MuseumMasterSelection = {
      status,
      subjectId: mediaFacts.context.subjectId,
      manifestHash,
      mediaSlot: slot,
      displayHash,
      objectId,
      original: original.evidence,
      association: mediaFacts.association,
      masterObjectHash: waiver ? ZeroHash : r.witness.masterObjectHash,
      coverageHash: waiver ? ZeroHash : r.witness.coverageHash,
      masterRole,
      predecessor: r.witness.predecessor,
      revision: r.expectedRevision + 1n,
      selectionHash: ZeroHash
    };
...
function savedMaster(v: MuseumMasterCapture): MuseumMasterCapture {
  ...
  const out = structuredClone(v),
    { captureHash, ...facts } = out;
  masterDeployment(out.deployment);
  museum.normalizeMuseumAnchorMasterCall(out.prepared);
...
async function originalMaster(p: Reader, c: MuseumMasterCapture): Promise<void> {
  equal(
    await captureMuseumAnchorMaster(p, c.deployment, c.prepared, { blockTag: c.blockNumber }),
    c,
    "Historical Museum capture changed"
  );
}

export interface MuseumMasterSimulation extends MuseumBlock {
  readonly capture: MuseumMasterCapture;
  readonly observed: MuseumMasterCapture;
  readonly returnData: Hex;
  readonly admission: "original-call simulation; direct dependency reads do not establish nested gas equivalence";
}

export async function simulateMuseumAnchorMaster(
  p: Reader,
  input: MuseumMasterCapture,
  options: { readonly blockTag: number }
): Promise<MuseumMasterSimulation> {
  keys(options, ["blockTag"]);
  const c = savedMaster(input),
    tag = number(options.blockTag);
  ...
  const name = c.prepared.request.kind,
    decoded = abi.decodeFunctionResult(name, raw);
  if (!same(abi.encodeFunctionResult(name, decoded), raw))
    throw Error("Noncanonical original call return");
  if (name === "adoptMaster" || name === "adoptWaiver")
    equal(
      plain(abi.getFunction(name)!.outputs[0]!, decoded[0]),
      c.expectedSelection,
      "Simulated selection differs"
    );
  else if (name !== "declareConservationTier" && !same(decoded[0], c.prepared.recordHash))
    throw Error("Simulated record differs");
  ...
  return freeze({
    ...
    admission: "original-call simulation; direct dependency reads do not establish nested gas equivalence"
  });
}
...
export interface MuseumMasterSelectionRead extends MuseumBlock {
  ...
  readonly selection: museum.MuseumMasterSelection;
...
export async function readMuseumMasterSelection(
  p: Reader,
  input: MuseumMasterDeployment,
  request: {
    readonly collectionId: bigint;
    readonly subjectId: Hex;
    readonly slot: bigint;
  ...
  options: { readonly blockTag: number }
): Promise<MuseumMasterSelectionRead> {
  keys(request, ["collectionId", "subjectId", "slot"], ["revision"]);
  keys(options, ["blockTag"]);
  const d = masterDeployment(input),
    cid = uint(request.collectionId),
    subjectId = hash(request.subjectId, true),
    slot = uint(request.slot, 8);
  const revision = request.revision === undefined ? null : uint(request.revision, 64),
    tag = number(options.blockTag);
  if ((await p.getNetwork()).chainId !== d.chainId) throw Error("RPC chain differs");
  const h = await header(p, tag);
  ...
  const s = selection(
    ...
    (await read(
      p,
      d.masterSelection.address,
      revision === null ? "currentMaster" : "masterSelectionAt",
      revision === null ? [cid, subjectId, slot] : [cid, subjectId, slot, revision],
      tag
    ))[0]
  );
  if (s.status !== 0n
    && (!same(s.subjectId, subjectId) || s.mediaSlot !== slot
      || (revision !== null && s.revision !== revision)))
    throw Error("Selection coordinates differ");
  await unchanged(p, h);
  return freeze({
    deployment: d,
    collectionId: cid,
    ...
    selection: s,
    ...h,
    interpretation: "retained selection; no current closure claim"
  });
}
...
export interface MuseumCollectionMastersRead extends MuseumBlock {
  readonly deployment: MuseumMasterDeployment;
  readonly collectionId: bigint;
  readonly media: MuseumMasterMediaFacts;
  readonly selections: readonly museum.MuseumMasterSelection[];
  readonly factsHash: Hex;
}

export async function requireMuseumCollectionMasters(
  p: Reader,
  input: MuseumMasterDeployment,
  collection: bigint,
  options: { readonly blockTag: number }
): Promise<MuseumCollectionMastersRead> {
  keys(options, ["blockTag"]);
  const d = masterDeployment(input),
    cid = uint(collection),
    tag = number(options.blockTag),
    h = await base(p, d, tag);
  await currentContext(p, d, tag);
  const m = await media(p, d, cid, tag),
    selections: museum.MuseumMasterSelection[] = [];
  ...
  for (let slot = 1n; slot <= 3n; ++slot) {
    ...
    const s = selection(
      coordinates(d),
      cid,
      (await read(p, d.masterSelection.address, "currentMaster", [cid, m.context.subjectId, slot], tag))[0]
    );
    if (s.status === 0n || !same(s.subjectId, m.context.subjectId) || s.mediaSlot !== slot
      || !same(s.manifestHash, m.context.manifestHash)
      || !same(s.displayHash, m.displayHashes[Number(slot) - 1]))
      throw Error("Current master denominator is incomplete");
  ...
  equal(
    await read(p, d.masterSelection.address, "requireCollectionMasters", [cid, m.context.subjectId], tag),
    [factsHash],
    "Original closure facts differ"
  );
  await unchanged(p, h);
  return freeze({
    deployment: d,
...
export interface PreparedMuseumAnchorGovernance {
  readonly capture: MuseumAnchorCapture;
  readonly proposer: Address;
  readonly batch: museum.MuseumAnchorGovernanceBatch
}

export interface MuseumAnchorGovernanceOperation {
  readonly prepared: PreparedMuseumAnchorGovernance;
  readonly stage: "publish" | "schedule" | "execute";
  readonly caller: Address;
  readonly call: UnsignedCall;
}

export function prepareMuseumAnchorGovernance(
  input: MuseumAnchorCapture,
  proposer: Address,
  window: museum.MuseumAnchorGovernanceWindow
): PreparedMuseumAnchorGovernance {
  const c = savedAnchor(input),
    batch = museum.museumAnchorGovernanceBatch(c.plan, c.governanceNonce, window);
  museum.assertMuseumAnchorGovernanceWindow(batch.window, c.timestamp);
  if (toUtf8Bytes(batch.window.reasonURI).length > 2048)
    throw Error("Reason URI exceeds client bound");
...
function preparedGovernance(v: PreparedMuseumAnchorGovernance): PreparedMuseumAnchorGovernance {
  ...
  const b = museum.normalizeMuseumAnchorGovernanceBatch(v.batch),
    out = prepareMuseumAnchorGovernance(v.capture, v.proposer, b.window);
  equal(out, v);
  return out;
}

export function prepareMuseumAnchorGovernanceOperation(
  input: PreparedMuseumAnchorGovernance,
  stage: MuseumAnchorGovernanceOperation["stage"],
  caller: Address
): MuseumAnchorGovernanceOperation {
  const prepared = preparedGovernance(input),
    actor = address(caller);
  ...
  return freeze({
    ...
    caller: actor,
    call: stage === "publish"
      ? prepared.batch.publicationCall
...
async function publication(p: Reader, b: museum.MuseumAnchorGovernanceBatch, tag: number): Promise<Address | null> {
  ...
  const pointer = address(ptr),
    expected = coder.encode(["bytes[]"], [[b.plan.call.data]]);
  if (!same(bytes(await p.getCode(pointer, tag), 24576), `0x00${expected.slice(2)}`))
    throw Error("Published bytes differ");
  return pointer;
}

async function action(p: Reader, o: MuseumAnchorGovernanceOperation, tag: number) {
  const b = o.prepared.batch,
    [a] = await read(p, b.plan.coordinates.executor, "governanceAction", [b.actionId], tag);
  const expected = {
    actionClass: 1n,
    target: b.plan.call.to,
    value: 0n,
    selector: b.plan.governanceCall.selector,
    ...
    proposer: o.prepared.proposer,
    reasonHash: b.window.reasonHash,
    reasonURI: b.window.reasonURI,
    manifestHash: b.window.manifestHash
  };
  for (const [key, value] of Object.entries(expected)) {
    if (a[key] !== value && (key === "reasonURI" || !same(a[key], value)))
      throw Error(`Scheduled ${key} differs`);
  }
  equal(
    (await read(p, b.plan.coordinates.executor, "scheduledCallData", [b.actionId], tag))[0],
    [b.plan.call.data]
  );
  const ptr = await publication(p, b, tag);
  if (!ptr
    || !same(
      (await read(p, b.plan.coordinates.executor, "scheduledCallDataPointer", [b.actionId], tag))[0],
      ptr
    ))
    throw Error("Scheduled pointer differs");
  return a;
}

export interface MuseumAnchorGovernanceSimulation extends MuseumBlock {
  readonly operation: MuseumAnchorGovernanceOperation;
  readonly returnData: Hex;
}

export async function simulateMuseumAnchorGovernance(
  p: Reader,
  input: MuseumAnchorGovernanceOperation,
  options: { readonly blockTag: number }
): Promise<MuseumAnchorGovernanceSimulation> {
  keys(options, ["blockTag"]);
  const o = governanceOperation(input),
    c = o.prepared.capture,
    d = c.deployment,
    b = o.prepared.batch,
    tag = number(options.blockTag);
  if (tag < c.blockNumber) throw Error("Simulation predates capture");
  ...
  if (o.stage !== "publish") {
    ...
    if (o.stage === "schedule") {
      ...
      if (fresh.governanceNonce !== b.nonce || !await publication(p, b, tag))
        throw Error("Nonce changed or call data not published");
      const [owner] = await read(p, d.executor.address, "owner", [], tag);
      if (!same(owner, o.caller)
        && (await read(p, d.executor.address, "isProposer", [o.caller], tag))[0] !== true)
        throw Error("Unauthorized proposer");
    } else {
      const a = await action(p, o, tag);
      if (a.status !== 1n || h.timestamp < b.window.notBefore || h.timestamp > b.window.expiresAfter)
        throw Error("Action is not executable");
    }
  }
  ...
  if (o.stage === "execute") {
    if (raw !== "0x") throw Error("Noncanonical execution return");
  }
  else {
    const name = o.stage === "publish" ? "publishGovernanceCallData" : "scheduleGovernanceBatch",
      result = abi.decodeFunctionResult(name, raw);
    if (!same(abi.encodeFunctionResult(name, result), raw))
      throw Error("Noncanonical governance return");
    if (o.stage === "schedule" && !same(result[0], b.actionId)) throw Error("Simulated action differs");
    if (o.stage === "publish") {
      address(result[0]);
      if (previous && !same(previous, result[0])) throw Error("Publication pointer changed");
    }
  }
  await unchanged(p, h);
...
export type MuseumReceiptOptions = {
...
export interface MuseumEventReference {
  readonly address: Address;
...
async function receipt(
  ...
  if (!r || !tx || r.status !== 1 || !same(r.hash, transactionHash) || !same(tx.hash, transactionHash)
    || tx.chainId !== chainId
    || tx.value !== 0n
    || r.blockNumber <= captureBlock
    || tx.blockNumber !== r.blockNumber
    || !same(tx.blockHash, r.blockHash)
    || !same(tx.to, r.to)
    || !same(tx.from, r.from))
    throw Error("Receipt/transaction identity or chronology differs");
  const data = bytes(tx.data, 262144);
  if (execution === "direct") {
    if (!same(tx.from, caller) || !same(tx.to, call.to) || !same(data, call.data))
      throw Error("Direct call differs");
    ...
    if (!same(tx.to, caller) || !same(safeAbi.encodeFunctionData("execTransaction", v), data)
      ...
      || !same(v[2], call.data)
      || v[3] !== 0n)
      throw Error("Safe requires exact ordinary zero-value CALL");
  } else throw Error("Unknown execution transport");
  const tag = number(r.blockNumber),
    h = await header(p, tag);
  ...
  const logs: Log[] = r.logs.map(l => {
    if (l.removed || !same(l.transactionHash, transactionHash) || l.blockNumber !== tag
      || !same(l.blockHash, h.blockHash)
      || !Array.isArray(l.topics)
      || l.topics.length > 4)
      throw Error("Malformed log identity");
    return {
      address: address(l.address),
      topics: l.topics.map((x: string) => hash(x, true)),
      data: bytes(l.data, 32768),
      index: number(l.index)
    };
  });
  if (logs.some((l, i) => i > 0 && l.index <= logs[i - 1]!.index))
    throw Error("Duplicate/unordered receipt logs");
  ...
  function found(target: Address, name: string, iface = abi) {
    ...
    return logs.filter(l => same(l.address, target) && same(l.topics[0], f.topicHash)).map(log => {
      const args = iface.decodeEventLog(f, log.data, log.topics),
        encoded = iface.encodeEventLog(f, args);
  ...
  function finish() {
    if (execution === "safe") {
      requireSafeExecution(
        {
          status: 1,
          logs: logs.map(({ address, topics, data }) => ({ address, topics: [...topics], data }))
        },
        caller,
        expectedSafeTxHash!
      );
      ...
      if (successes.length !== 1
        || logs.some(l => same(l.address, caller) && same(l.topics[0], safePlain.getEvent("ExecutionFailure")!.topicHash)))
        throw Error("Safe requires one success and no failure");
      const log = successes[0]!,
        iface = log.topics.length === 2 ? safeIndexed : safePlain;
      const f = iface.getEvent("ExecutionSuccess")!,
        args = iface.decodeEventLog(f, log.data, log.topics),
        encoded = iface.encodeEventLog(f, args);
...
export interface MuseumAnchorMasterReceipt extends MuseumBlock {
  readonly capture: MuseumMasterCapture;
  readonly transactionHash: Hex;
  readonly events: readonly MuseumEventReference[];
  readonly selection: museum.MuseumMasterSelection | null;
  readonly original: MuseumMasterStoredRecord | null;
  readonly stateAttribution: "operation events and immutable records; no latest-state or collection-finality claim";
}

export async function inspectMuseumAnchorMasterReceipt(
  p: ReceiptReader,
  input: MuseumMasterCapture,
  options: MuseumReceiptOptions
): Promise<MuseumAnchorMasterReceipt> {
  keys(
    options,
    ["transactionHash", "execution"],
    options.execution === "safe" ? ["expectedSafeTxHash"] : []
  );
  const expectedSafeTxHash = options.execution === "safe" ? hash(options.expectedSafeTxHash) : undefined;
  const c = savedMaster(input),
    d = c.deployment,
    prepared = c.prepared,
    transactionHash = hash(options.transactionHash);
  ...
  const r = prepared.request,
    cid = r.collectionId;
  await runtime(p, d.metadata, tag);
  let retained: museum.MuseumMasterSelection | null = null,
    original: MuseumMasterStoredRecord | null = null;
  if (r.kind === "declareConservationTier") {
    ...
    if (!same((await read(p, d.core.address, "declaredConservationTier", [cid], tag))[0], r.tier))
      throw Error("Permanent tier readback differs");
    ...
    m.one(
      d.masterSelection.address,
      "MediaMasterSelected",
      [cid, expected.subjectId, expected.mediaSlot, expected]
    );
    retained = selection(
      coordinates(d),
      cid,
      (await read(
        ...
        d.masterSelection.address,
        "masterSelectionAt",
        [cid, expected.subjectId, expected.mediaSlot, expected.revision],
        tag
      ))[0]
    );
    ...
    const waiver = r.kind === "recordArtistCollectionRecordWithPayload",
      recorder = waiver ? r.recorder : prepared.caller;
    await runtime(p, d.store, tag);
    ...
    if (original.receipt.recordedAt !== m.timestamp
      || original.receipt.authorizationClass !== c.authorizationClass
      || !same(original.receipt.recorder, recorder))
      throw Error("Published record timestamp/writer differs");
    equal(original.payload, prepared.payload!.canonical);
    const cl = `0x${original.receipt.authorizationClass.toString(16).padStart(64, "0")}`;
    const publishedIndex = m.one(
      d.metadata.address,
      "CollectionRecordRecorded",
      [
        cid,
        r.record.recordType,
        r.record.subjectId,
        r.record,
        prepared.recordHash,
        original.receipt.recordChainHash,
        recorder,
        cl,
        1n
      ]
    ...
    if (waiver) {
      if (!same(original.receipt.artistAuthorization, r.authorization))
        throw Error("Consumed Artist authorization differs");
      const consumedIndex = m.one(
        d.metadata.address,
        "ArtistRecordAuthorizationConsumed",
        [r.authorization, prepared.recordHash, recorder, prepared.caller]
      );
  ...
  return freeze({
    ...
    selection: retained,
    original,
    stateAttribution: "operation events and immutable records; no latest-state or collection-finality claim"
  });
}
...
export async function inspectMuseumAnchorGovernanceReceipt(
  p: ReceiptReader,
  input: MuseumAnchorGovernanceOperation,
  options: MuseumReceiptOptions
): Promise<MuseumAnchorGovernanceReceipt> {
  keys(
    options,
    ["transactionHash", "execution"],
    options.execution === "safe" ? ["expectedSafeTxHash"] : []
  ...
  const o = governanceOperation(input),
    ...
    d = c.deployment,
    b = o.prepared.batch;
  const transactionHash = hash(options.transactionHash),
  ...
  if (o.stage === "publish") {
    ...
    const common = [
      ...
      d.core.address,
      0n,
      c.plan.governanceCall.selector,
      b.callsHash,
      b.scopeHash,
      b.oldValueHash,
      b.newValueHash
    ];
    const validated = m.one(
      d.executor.address,
      "GovernanceActionPolicyValidated",
      [
        1n,
        b.actionId,
        o.stage === "schedule" ? 1n : 2n,
        c.catalog.candidateProfileHash,
        c.catalog.catalogHash
      ]
    );
    if (o.stage === "schedule") {
      museum.assertMuseumAnchorGovernanceWindow(b.window, m.timestamp);
      if (![1n, 2n].includes(state.status) || g.governanceNonce < b.nonce + 1n)
        throw Error("Scheduled action/nonce differs");
      const scheduled = m.one(
        d.executor.address,
      ...
      if (m.found(d.core.address, "ConditionSourcesBound").length
        || m.found(d.core.address, "ConservationFloorBound").length)
        throw Error("Scheduling cannot bind an anchor");
    } else {
      if (state.status !== 3n || !same(state.executor, o.caller) || m.timestamp < b.window.notBefore
        || m.timestamp > b.window.expiresAfter)
        throw Error("Executed action/window differs");
      const bound = m.one(
        d.core.address,
        c.kind === "conditionSources" ? "ConditionSourcesBound" : "ConservationFloorBound",
      ...
      if (!(bound < executed && executed < validated))
        throw Error("Anchor/governance event order differs");
      ...
      equal(
        binding,
        { target: c.candidate.address, runtimeCodeHash: c.candidate.codeHash },
        "Permanent anchor differs"
      );
