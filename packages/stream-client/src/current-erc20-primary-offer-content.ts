...
const version = id("6529STREAM_ERC20_PRIMARY_OFFER_GATE_V1");
...
export function erc20PrimaryOfferGateConfigHash(manifest: CuratedManifest, managerCodeHash: Hex, adapterCodeHash: Hex): Hex {
 ...
 */
export async function inspectERC20PrimaryOfferManifest(
  ...
  if (await read("offerPurchaseVersion", 32) !== version) throw Error("Gate is not the ERC20 primary offer capability");
  ...
  const configHash = erc20PrimaryOfferGateConfigHash(saved, managerHash, adapterHash);
