...
export interface RevenuePlan {
  ...
  readonly ownerCall: RevenueActorCall | null; readonly approval: "current" | "fixed" | "template" | null;
...
function fact(v: { resolver: unknown; revenueClass: unknown; scope: unknown; scopeId: unknown; assignmentHash: unknown }): RevenueFact {
  return freeze({ resolver: addr(v.resolver), revenueClass: hash(v.revenueClass), scope: uint(v.scope, 8), scopeId: uint(v.scopeId), assignmentHash: hash(v.assignmentHash) });
}
...
export class CurrentRevenueClient {
  ...
  #intent(input: RevenueIntent): RevenueIntent {
    ...
    const i = { ...input };
    const keys = i.kind === "snapshot-current" ? ["kind", "collectionId"] : ["kind", "collectionId", "scope", "scopeId", i.kind === "primary-template" ? "templateId" : "profileId", ...(i.kind === "royalty-set" ? ["royaltyBps"] : [])];
  ...
  async quote(provider: RPC, callerInput: Address, input: RevenueIntent, options: { blockTag?: BlockTag } = {}): Promise<RevenuePlan> {
    ...
    const f = fact((await this.#read(provider, key, method, args, caller, o.tag))[0]);
    ...
    const terms = { existingStateHash, intent, caller, artistAccount, codeHashes: o.codes, fact: f, rawSource, sourcePolicyHash, mode, electionHash, previewCall: this.#call(key, method, args), ownerCall, approval };
  ...
  async assertInstalled(provider: RPC, plan: RevenuePlan, caller: Address, options: { blockTag?: BlockTag } = {}): Promise<void> {
    ...
    else actual = hash((await this.#read(provider, "primary", "primaryEconomicsFacts", [i.collectionId, i.scope, i.scopeId], caller, o.tag))[0].assignmentHash);
