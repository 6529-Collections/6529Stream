...
// ABI104 evm.methodIdentifiers: prepare(StreamArtistOnboardingTypes.SuiteConfiguration,StreamArtistRecoveredHydrationTypes.Request).
const PREPARATION_SELECTOR = "0x72c84763";
...
async function sourceInventories(
  reader: ArtistRecoveredHydrationReader,
  pins: ArtistRecoveredHydrationDeployment,
  destination: ArtistHydrationSuite,
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const { header: exportHeader, payload } = rh.decodeArtistRecoveredHydrationOwnerPayload(certificate.data[i]!.typedState, i as ArtistHydrationOwnerIndex);
...
async function collectCertificate(
  ...
  const tupleCalldata = preparationAbi.encodeFunctionData("prepare", [destination, request]);
  const data = bytes(`${PREPARATION_SELECTOR}${tupleCalldata.slice(10)}`, MAX_CALL_BYTES);
  const raw = bytes(await reader.call({
  ...
  return rh.normalizeArtistRecoveredHydrationPrepared(plain(preparationAbi.getFunction("prepare")!.outputs[0]!, decoded[0]));
}
...
function captured(input: ArtistRecoveredHydrationCapture): ArtistRecoveredHydrationCapture {
  keys(input, [
  ...
  rh.normalizeArtistRecoveredHydrationCall(copied.prepared);
  rh.normalizeArtistRecoveredHydrationPrepared(copied.certificate);
...
export async function captureArtistRecoveredHydration(
  reader: ArtistRecoveredHydrationReader,
  inputDeployment: ArtistRecoveredHydrationDeployment,
  inputCaller: Address,
  inputRequest: rh.ArtistRecoveredHydrationRequest,
  options: { readonly blockTag: number; readonly gasLimit: bigint },
): Promise<ArtistRecoveredHydrationCapture> {
  const pins = deployment(inputDeployment);
  const caller = address(inputCaller);
  const request = rh.normalizeArtistRecoveredHydrationRequestDraft(inputRequest);
  ...
  const certificate = await collectCertificate(reader, pins, ctx.destination, request, caller, tag, gasLimit);
  ...
  const semanticInventory = rh.artistRecoveredHydrationSemanticInventory(certificate);
  ...
  const finalRequest = rh.normalizeArtistRecoveredHydrationRequest({ ...request, expectedSemanticInventory: semanticInventory });
  const prepared = rh.prepareArtistRecoveredHydrationCall(pins.destination.registry.address, caller, finalRequest);
  ...
  const commitment = rh.artistRecoveredHydrationCommitment(coordinates(pins), finalRequest, certificate);
  ...
  const owners = await sourceInventories(reader, pins, ctx.destination, finalRequest, certificate, tag);
  ...
  const after = before.map((snapshot, i) => rh.artistRecoveredHydrationOwnerAfter(
  ...
  const profileEvidence = rh.encodeArtistRecoveredHydrationProfileEvidence(finalRequest, certificate);
  ...
  equal(await collectCertificate(reader, pins, ctx.destination, finalRequest, caller, tag, gasLimit),
    certificate, "Original preparation certificate changed during collection");
  await unchanged(reader, ctx.observed);
  const body: Omit<ArtistRecoveredHydrationCapture, "captureHash"> = {
    deployment: pins,
...
export interface ArtistRecoveredHydrationSimulation {
  readonly capture: ArtistRecoveredHydrationCapture;
  readonly observed: ArtistRecoveredHydrationObservation;
...
export async function simulateArtistRecoveredHydration(
  reader: ArtistRecoveredHydrationReader,
  input: ArtistRecoveredHydrationCapture,
  options: { readonly blockTag: number; readonly gasLimit: bigint },
): Promise<ArtistRecoveredHydrationSimulation> {
  const saved = captured(input);
  const tag = blockNumber(options.blockTag);
  const gasLimit = gas(options.gasLimit);
  if (tag < saved.observed.blockNumber) throw Error("Simulation precedes reviewed capture");
  const historical = await captureArtistRecoveredHydration(reader, saved.deployment, saved.prepared.caller,
    saved.prepared.request, { blockTag: saved.observed.blockNumber, gasLimit: saved.preparationGasLimit });
  ...
  const current = tag === saved.observed.blockNumber ? historical : await captureArtistRecoveredHydration(
    reader, saved.deployment, saved.prepared.caller, saved.prepared.request,
 ...
 */
export async function reconcileArtistRecoveredHydrationReceipt(
  ...
  const prior = await captureArtistRecoveredHydration(reader, saved.deployment, saved.prepared.caller,
    saved.prepared.request, { blockTag: tag - 1, gasLimit: saved.preparationGasLimit });
