// Compiler-encoded RPC consistency mock for the full dispute-history graph.
...
import * as rh from '../dist/current-artist-recovered-multiple-dispute-hydration.js';
import * as workflow from '../dist/current-artist-recovered-multiple-dispute-hydration-workflow.js';
...
import { fixture, compiledABI, compiledLibraryEvents, compiledLibraryValueInterface, libraryValueABI } from './current-artist-recovered-multiple-dispute-hydration-source-fixture.mjs';
...
const domains=Array.from({length:7},(_,i)=>rh.artistRecoveredMultipleDisputeHydrationOwnerDomain(i));
...
function attestationRead(material,method,args) {
  if(method==='c2paCredentialHead') {const value=material.credentialHeads.get(args[0]);if(value)return value;if(material.certificate.admission.artists.some(q=>q.artistId===args[0]))return zeroValue(rh.ARTIST_RECOVERED_MULTIPLE_DISPUTE_HYDRATION_CREDENTIAL_HEAD_TUPLE);}
  for(const b of material.attestations) {
    ...
    for(const saved of b.records) {
      ...
      if(method==='c2paCredentialRecord')return material.credentialRecords.get(args[0])??zeroValue(rh.ARTIST_RECOVERED_MULTIPLE_DISPUTE_HYDRATION_CREDENTIAL_HEAD_TUPLE);
...
function generationRead(material,method,args,fragment) {
  if(method==='repudiationCount') {
    const all=material.attribution.map(row=>row.history),known=all.some(b=>b.artistId===args[0]&&b.repudiations.some(r=>artistAttributionAuthorityHeadHash(r.record.authorityHead)===args[1]));
    if(!known)throw Error('Unknown retained authority cohort');
    return[all.filter(b=>b.artistId===args[0]).reduce((n,b)=>n+BigInt(b.repudiations.filter(r=>r.terminal.phase===1n&&artistAttributionAuthorityHeadHash(r.record.authorityHead)===args[1]).length),0n)];
  }
...
export function setup(options={}) {
  ...
  const semanticRows=i=>rh.decodeArtistRecoveredMultipleDisputeHydrationState(material.payloads[i].semanticState,i,material.payloads[i].provenance).rows;
  material.bindings=semanticRows(0).map(rh.decodeArtistRecoveredMultipleDisputeHydrationBindingBundle);
  material.acceptances=semanticRows(3).map(rh.decodeArtistRecoveredMultipleDisputeHydrationAcceptanceBundle);
  material.attribution=semanticRows(4).map(rh.decodeArtistRecoveredMultipleDisputeHydrationAttribution);
  material.attestations=material.attribution.map(row=>row.records);
  material.identities=semanticRows(2).map(rh.decodeArtistRecoveredMultipleDisputeHydrationIdentity);
  material.contents=semanticRows(6).map(raw=>rh.decodeArtistRecoveredMultipleDisputeHydrationConsents(raw).rows);
  material.clocks=rh.decodeArtistRecoveredMultipleDisputeHydrationInventory(rh.decodeArtistRecoveredMultipleDisputeHydrationAuxiliary(material.payloads[4].semanticState,4,material.payloads[4].provenance).auxiliary);
    ...
    async call(tx){
      ...
      switch(method){
        ...
        case 'replayCell':{
          ...
          const index=data.origins.findIndex(v=>rh.artistRecoveredMultipleDisputeHydrationReplayKey(target,owner,v)===args[0]);
...
export async function capture(s){const c=await workflow.captureArtistRecoveredMultipleDisputeHydration(s.provider,s.deployment,s.caller,s.input,{blockTag:10,gasLimit:10000000n});s.state.captured=c;return c;}
export function install(s,c,mode='direct'){
  ...
  for(let i=0;i<c.descriptor.pageHashes.length;i++)append(rh.artistRecoveredMultipleDisputeHydrationPageId(coordinates,c.commitment,c.descriptor,BigInt(i)),`0x${c.profileEvidence.slice(2+i*40960,2+(i+1)*40960)}`,i);
...
export const run=(s,c,mined)=>workflow.reconcileArtistRecoveredMultipleDisputeHydrationReceipt(s.provider,c,H('tx'),mined.options);
