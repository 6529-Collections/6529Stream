// Compiler-encoded RPC consistency mock for the full generation graph.
...
import * as rh from '../dist/current-artist-recovered-multiple-generation-hydration.js';
import * as workflow from '../dist/current-artist-recovered-multiple-generation-hydration-workflow.js';
import { ARTIST_HYDRATION_SUITE_TUPLE } from '../dist/current-artist-authority-hydration.js';
import { fixture, compiledABI, compiledLibraryEvents, compiledLibraryValueInterface, libraryValueABI } from './current-artist-recovered-multiple-generation-hydration-source-fixture.mjs';
...
const domains=Array.from({length:7},(_,i)=>rh.artistRecoveredMultipleGenerationHydrationOwnerDomain(i));
...
import { semanticFixture } from './current-artist-recovered-multiple-generation-hydration-semantic-fixture.mjs';

function attestationRead(material,method,args) {
  if(method==='c2paCredentialHead') {const value=material.credentialHeads.get(args[0]);if(value)return value;if(material.certificate.admission.artists.some(q=>q.artistId===args[0]))return zeroValue(rh.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_CREDENTIAL_HEAD_TUPLE);}
  for(const b of material.attestations) {
    ...
    if(method==='personhoodAttestation'&&b.collectionId===args[0]&&b.artistId===args[1]) {
      const value=b.records.findLast(v=>v.attestation.input.terms.subjectKind===10n&&[H('6529STREAM_ARTIST_PERSONHOOD_WAIVER_V1'),H('6529STREAM_ARTIST_PERSONHOOD_EVIDENCE_V1')].includes(v.attestation.input.terms.schemaId));
    ...
    for(const saved of b.records) {
      ...
      if(method==='c2paCredentialRecord')return material.credentialRecords.get(args[0])??zeroValue(rh.ARTIST_RECOVERED_MULTIPLE_GENERATION_HYDRATION_CREDENTIAL_HEAD_TUPLE);
...
function generationRead(material,method,args,fragment) {
  for(let k=0;k<material.bindings.length;k++) {
    ...
    if(method==='attributionDispute'&&cid===args[0]) {
      ...
      const r=history.revocations.find(r=>r.opening.terms.bindingGeneration===args[1]);
      return[r?.head??zeroValue(fragment.outputs[0])];
    }
    for(const row of history.revocations) {
      if(method==='attributionDisputeRecord'&&row.opening.recordHash===args[0])return[row.opening];
      if(method==='attributionDisputeResolution'&&row.resolution.actionId===args[0])return[row.resolution];
    }
  }
  ...
  for(const b of bundles) {
    ...
    for(const r of q.economics) {
      if(['economicsRecord','economicsRecordForBinding'].includes(method)&&equalTerms(r.item.terms,args[0])) {
        ...
        if(args[1]===q.artistId&&args[2]===r.item.association.bindingGeneration&&args[3]===r.item.association.bindingHash)return r.item.recordHash;
      }
    ...
    if(method==='royaltyFreezeRecord') {const r=b.royalties.find(r=>args[1]===q.artistId&&args[2]===r.item.bindingGeneration&&equalTerms(r.terms,args[0]));if(r)return r.item;}
...
export function setup(options={}) {
  ...
  const semanticRows=i=>rh.decodeArtistRecoveredMultipleGenerationHydrationState(material.payloads[i].semanticState,i,material.payloads[i].provenance).rows;
  material.bindings=semanticRows(0).map(rh.decodeArtistRecoveredMultipleGenerationHydrationBindingBundle);
  material.acceptances=semanticRows(3).map(rh.decodeArtistRecoveredMultipleGenerationHydrationAcceptanceBundle);
  material.attribution=semanticRows(4).map(rh.decodeArtistRecoveredMultipleGenerationHydrationAttribution);
  material.attestations=material.attribution.map(row=>row.records);
  material.contents=semanticRows(6).map(raw=>rh.decodeArtistRecoveredMultipleGenerationHydrationConsents(raw).rows);
  material.clocks=rh.decodeArtistRecoveredMultipleGenerationHydrationInventory(rh.decodeArtistRecoveredMultipleGenerationHydrationAuxiliary(material.payloads[4].semanticState,4,material.payloads[4].provenance).auxiliary);
  ...
  sourceCatalogs.set(source.archive,material.archiveRows.map(row=>({pointer:row.pointer,payloadType:row.kind,payloadHash:row.hash})));
  ...
  const provider={
    ...
    async call(tx){
      ...
      switch(method){
        ...
        case 'attributionDispute':case 'attributionDisputeRecord':case 'attributionDisputeResolution':case 'platformWorksState': {
        ...
        case 'configurationHash':return encode([isSource?material.clocks.catalogues[0].configurationHash:H('destination configuration')]);
        ...
        case 'replayCell':{
          ...
          const index=data.origins.findIndex(v=>rh.artistRecoveredMultipleGenerationHydrationReplayKey(target,owner,v)===args[0]);
        ...
        case 'artistArchiveBindingHashV2': {
          ...
          return encode([hash(['bytes32','uint256','address','address','bytes4','bytes32','uint16','uint256'],[H('6529STREAM_ARTIST_ARCHIVE_BINDING_V2'),1n,selected.registry,isSource?deployment.source.coordinator.address:deployment.destination.coordinator.address,interfaceId,H('6529STREAM_ARTIST_ARCHIVE_V2'),2n,24575n])]);
        }
        ...
        case 'artistEvidenceMetadataV2': {assert.equal(args[1],1n);if(isSource){const row=material.archiveRows.find(r=>r.evidenceId===args[0]);if(!row)throw Error('Unknown original Archive evidence');return encode([row.hash,row.pointer,BigInt((row.raw.length-2)/2),row.atBlock]);}return encode(state.metadata.get(args[0]));}
        case 'artistEvidenceBytesV2': {assert.equal(args[1],1n);if(isSource){const row=material.archiveRows.find(r=>r.evidenceId===args[0]);if(!row)throw Error('Unknown original Archive evidence');return encode([row.raw]);}return encode([state.evidence.get(args[0])]);}
...
export async function capture(s){const c=await workflow.captureArtistRecoveredMultipleGenerationHydration(s.provider,s.deployment,s.caller,s.input,{blockTag:10,gasLimit:10000000n});s.state.captured=c;return c;}
export function install(s,c,mode='direct'){
  ...
  for(let i=0;i<c.descriptor.pageHashes.length;i++)append(rh.artistRecoveredMultipleGenerationHydrationPageId(coordinates,c.commitment,c.descriptor,BigInt(i)),`0x${c.profileEvidence.slice(2+i*40960,2+(i+1)*40960)}`,i);
...
export const run=(s,c,mined)=>workflow.reconcileArtistRecoveredMultipleGenerationHydrationReceipt(s.provider,c,H('tx'),mined.options);
