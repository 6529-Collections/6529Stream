...
import type { ArtistRecoveredHydrationRequest } from "../src/current-artist-recovered-hydration.js";
import {
  captureArtistRecoveredHydration,
  simulateArtistRecoveredHydration,
  reconcileArtistRecoveredHydrationReceipt,
  type ArtistRecoveredHydrationCapture,
  type ArtistRecoveredHydrationDeployment,
} from "../src/current-artist-recovered-hydration-workflow.js";
import { createSafeCallPlan } from "../src/safe-plan.js";

declare const provider: Provider;
declare const deployment: ArtistRecoveredHydrationDeployment;
declare const request: ArtistRecoveredHydrationRequest;
declare const caller: Address;
declare const digest: Hex;
declare const captured: ArtistRecoveredHydrationCapture;

const capture = await captureArtistRecoveredHydration(provider, deployment, caller, request, {
  blockTag: 10,
...
const checked = await simulateArtistRecoveredHydration(provider, capture, { blockTag: 11, gasLimit: 10_000_000n });
const simulated: true = checked.registrySimulated;
const promised: false = checked.futureExecutionGuaranteed;
const direct = await reconcileArtistRecoveredHydrationReceipt(provider, checked.capture, digest, { execution: "direct" });
const safe = await reconcileArtistRecoveredHydrationReceipt(provider, checked.capture, digest, {
  execution: "safe",
...
captureArtistRecoveredHydration(provider, deployment, caller, request, { blockTag: "latest", gasLimit: 1n });
// @ts-expect-error Original preparation needs an explicit gas bound.
captureArtistRecoveredHydration(provider, deployment, caller, request, { blockTag: 10 });
// @ts-expect-error Gas quantities retain exact bigint semantics.
simulateArtistRecoveredHydration(provider, captured, { blockTag: 11, gasLimit: 1_000_000 });
// @ts-expect-error The library certificate alone is not a reviewed capture.
simulateArtistRecoveredHydration(provider, captured.certificate, { blockTag: 11, gasLimit: 1n });
// @ts-expect-error Safe verification requires an independently supplied transaction hash.
reconcileArtistRecoveredHydrationReceipt(provider, captured, digest, { execution: "safe" });
// @ts-expect-error Delegatecall is not an original recovered Registry transport.
reconcileArtistRecoveredHydrationReceipt(provider, captured, digest, { execution: "delegatecall" });
