...
import * as rh from '../dist/current-artist-recovered-multiple-attestation-hydration.js';
import * as workflow from '../dist/current-artist-recovered-multiple-attestation-hydration-workflow.js';
import { ARTIST_HYDRATION_SUITE_TUPLE } from '../dist/current-artist-authority-hydration.js';
import { fixture, compiledABI, compiledLibraryEvents, compiledLibraryValueInterface, libraryValueABI } from './current-artist-recovered-multiple-attestation-hydration-source-fixture.mjs';
...
const domains=Array.from({length:7},(_,i)=>rh.artistRecoveredMultipleAttestationHydrationOwnerDomain(i));
...
import { semanticFixture, T, zero } from './current-artist-recovered-multiple-attestation-hydration-semantic-fixture.mjs';

function attestationRead(material,method,args) {
  if(method==='c2paCredentialHead') {const value=material.credentialHeads.get(args[0]);if(value)return value;}
  for(const b of material.attestations) {
    ...
    if(method==='personhoodAttestation'&&b.collectionId===args[0]&&b.artistId===args[1]) {
      ...
      return value?.attestation.record??zero(T.attestation.components.find(c=>c.name==='records').arrayChildren.components.find(c=>c.name==='attestation').components.find(c=>c.name==='record'));
    }
    ...
    for(const saved of b.records) {
      ...
      if(method==='c2paCredentialRecord')return material.credentialRecords.get(args[0])??zero(T.credentialHead);
      if(method==='personhoodProofSummary'||method==='personhoodProofSummaryHash') {
        ...
        return p?.summary??zero(T.attestation.components.find(c=>c.name==='personhood').arrayChildren.components.find(c=>c.name==='summary'));
      }
    }
  }
  ...
  const provider={
    ...
    async call(tx){
      ...
      switch(method){
        ...
        case 'bindingAt': {assert.equal(owner,0);assert.equal(args[1],1n);const row=material.bindings.find(b=>b.scope.collectionId===args[0]);if(!row)throw Error('Unknown original binding');return encode([row.history]);}
        case 'acceptedAt':case 'acceptanceRecord': {assert.equal(owner,3);const row=material.acceptances.find(b=>b.scope.bindingHash===args[0]);if(!row)throw Error('Unknown original acceptance');return encode([method==='acceptedAt'?row.acceptedAt:row.record]);}
        ...
        case 'replayCell':{
          ...
          const index=data.origins.findIndex(v=>rh.artistRecoveredMultipleAttestationHydrationReplayKey(target,owner,v)===args[0]);
...
export async function capture(s){const c=await workflow.captureArtistRecoveredMultipleAttestationHydration(s.provider,s.deployment,s.caller,s.input,{blockTag:10,gasLimit:10000000n});s.state.captured=c;return c;}
export function install(s,c,mode='direct'){
  ...
  for(let i=0;i<c.descriptor.pageHashes.length;i++)append(rh.artistRecoveredMultipleAttestationHydrationPageId(coordinates,c.commitment,c.descriptor,BigInt(i)),`0x${c.profileEvidence.slice(2+i*40960,2+(i+1)*40960)}`,i);
...
export const run=(s,c,mined)=>workflow.reconcileArtistRecoveredMultipleAttestationHydrationReceipt(s.provider,c,H('tx'),mined.options);
