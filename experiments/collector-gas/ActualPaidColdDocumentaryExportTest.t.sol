...
contract ActualPaidColdDocumentaryExportTest is ActualCanonicalDutchDocumentaryFixture, ActualPaidColdPurchaseSupport {
    function _deployCurrentStack(address artist_, address platform) internal override {
        address artistSuiteDeployment = _artistArtifactCreate(
            "test/helpers/StreamCurrentTestPaidDocumentaryArtistSuiteDeployment.sol:StreamCurrentTestPaidDocumentaryArtistSuiteDeployment",
            abi.encode()
        );
        address deployment = _artistArtifactCreate(
            "test/helpers/StreamCurrentTestPaidDocumentaryStackDeployment.sol:StreamCurrentTestPaidDocumentaryStackDeployment",
    ...
    function testExportPaidColdDocumentaryEmpty() public { _exportCold("paid-documentary-empty"); }
    function testExportPaidColdDocumentary128() public { paidArtworkLength = 128; _exportCold("paid-documentary-128"); }
    ...
    function _coldReleaseKey() internal view override returns (bytes32) {
        return _documentaryReleaseKey(_documentaryContext());
    }
    ...
    function _coldRoles() internal view override returns (address[13] memory) {
        return [BUYER, address(companionProducts.dutch), address(core), address(manager), address(ledger),
            address(companionRecorder), address(artists), address(entropy), address(documentaryFloor),
