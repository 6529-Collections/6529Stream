...
| Slither first-party High/Medium | Blocked | Pinned Slither 0.11.5 analysis records 32 retained production rows (2 High, 30 Medium): 30 remain Open (zero confirmed gaps, six design-review rows, and 24 pending dispositions), while two Medium `StreamSplitWallet` `incorrect-equality` rows have focused False Positive dispositions; issue #658 owns the remaining remediation and reviewed disposition work |
