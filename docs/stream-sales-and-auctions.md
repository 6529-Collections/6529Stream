...
9. Core-native identity (ADR 0016). Every launch collection uses Core's
   standard ERC-721 mutation entries, so sale, auction, custody, airdrop,
   private-sale, offer, burn-to-mint, and redemption configuration contains no
   identity-mode branch or facade-specific rejection. Each flow still passes
   its ordinary custody, authorization, settlement, burn-block, and receiver-
   safety gates. A future wrapper or successor asset model must define new
   adapter profiles in its own accepted ADR and cannot reuse this launch
   conformance claim.
...
2. Shape: a dedicated phase ID; one `MintBatch` per distribution slice
   ...
   - Failure-isolated (ADR 0013 decision U6): the executor is a
     ...
     must use this shape. The failure-isolated shape uses the same Core-native
     ERC-721 custody and delivery path for every launch collection (ADR 0016).
...
1. Custody: an auctioned pre-minted token must be held by the auction
   ...
   are never custody. English auctions use the same Core-native custody and
   delivery path for every launch collection (ADR 0016).
...
2. Custody entry is a same-transaction
   ...
   for every launch collection through Core's sole ERC-721 path (ADR 0016).
...
3. The source-collection set, conversion ratio (sources burned per token
   ...
   frozen mint policy. Every launch source uses Core's native burn entry and
   remains subject to the collection burn block (ADR 0016; [CMC-BURN]).
...
22. Core-native sale-flow coverage (ADR 0016): the [SSA-IDENTITY]
    rule 9 suite proves every launch collection uses the same standard
    ERC-721 configuration and settlement path for English auctions,
    custody inventory and consignment, custody-held private sales and
    offers, failure-isolated airdrops, and burn programs. The suite
    also proves the target Core ABI contains no identity-mode branch,
    facade-specific error, transfer-controller callback, or alternate
    ownership-event path.
