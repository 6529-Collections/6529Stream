...
The gate authenticates enabled price fields, including `true/0`, and leaves
charging to the sale consumer. The Manager and Ledger consume only the proven
cap. An inconsistent `false/nonzero` price presentation still reverts with
`MintAllowlistPriceOverrideUnsupported`. A valid price-bearing gate result does
not itself collect payment or declare a free tier; those are sale policies.
See [native allowlist price programs](native-allowlist-price-programs.md) for the
independent sale-side proof check. That single-token consumer uses inline
Manager caps; it does not implement this gate's distinct authorization envelope.
...
proof's cap and price values. It intentionally excludes proof sibling
