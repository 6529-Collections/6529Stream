...
C2PA products from frozen Artist source `3f1a0680`. Its integration input is
`a65145ea`; local dependency commit `0aa64e4f` combines those inputs. The separate
...
The helpers and eleven current-graph tests are source/type checked. Native
execution, gas measurement, complete STATIC admission, actual current-graph
...
3. `deploy` constructs the original attribution companion, original
   ...
   includes the C2PA display fields. Reusing an older Encoding selector/runtime
   is invalid.
...
`delta` supplies the following fifteen edges at the frozen source. Runtime
...
The last bound covers the maximum full-view string plus its raw ABI envelope.
...
stable-artwork/live-annotation output profile. The pending Artist standing-conflict
API and exact original-op46 resolution behavior are outside frozen `3f1a0680`.
After that successor is frozen, refresh constructors, selectors, runtime pins,
the complete read closure and affected tests before claiming its composition.
