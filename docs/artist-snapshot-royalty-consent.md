...
actual split wallet for that profile. The Resolver owner elects the mode before
...
This first mode does not admit default fallback, disabled zero-rate snapshots,
generic token economics consent, collection freezing or clearing. Snapshot
...
scope-2 approval for a future token.
