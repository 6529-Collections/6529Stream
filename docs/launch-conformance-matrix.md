...
| Citation-format checker | every decision citation matches `(ADR <number> decision <id>)` with the owning ADR's id shape — `D`/`R`/`T`/`U`/`V` prefixes, with ADR 0009's plain numerals exempt by construction — and the cited id exists in the cited ADR | spec-policy Decision Citation Format (ADR 0014 decision V9) | to build |
