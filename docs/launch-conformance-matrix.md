...
|---|---|---|---|
| Markdown link checker (`tools/docs/check_markdown_links.py`) | every relative link and heading anchor across the docs tree resolves | spec-policy inventory hygiene | exists |
| Mint-manager domain-constants checker (`tools/protocol/check_mint_manager_domain_constants.py`) | the checked manager table row-for-row against `StreamMintManager.sol` — constants, string preimages, hash values, and the pinned historical input aliases | protocol v1, StreamMintManager Domain Constants | exists |
...
| GGP/GTP identifier completeness checker (`tools/protocol/check_governed_parameter_identifiers.py`) | the unique closed-world 22-GGP/3-GTP section and every exact mirror/LTA row, including order, labels, preimages, recomputed hashes, identifier schema, target Owner/Inputs cells, LTA host/normative-home cells, GTP coordinator ownership/home, and comment-resistant live generic-host `6529STREAM_GGP_`/`6529STREAM_GTP_` prefix derivations; malformed or extra rows fail and no per-ID Solidity constants are claimed | protocol v1 governed-parameter identifier mirror (ADR 0011 decision R12; ADR 0013 decision U9; ADR 0017) | exists |
...
| Core-native ERC-2981 and shared bounded-read completion buffer | `StreamCore.royaltyInfo`, `StreamCore.tokenURI`, `StreamCore.contractURI`, revenue resolver, metadata router | canonical `0x54f77a09` resolver selector; malformed/OOG/external-call resolver fallback; all-cold gas; precheck and staticcall read current `ROYALTY_RESOLVER_GAS_LIMIT` GGP value through the EIP-150 multiplicative precheck shape with the host-coupled `ROYALTY_RETURN_GAS_BUFFER` ([RSR-2981-GAS]); the shared buffer's just-below/at/above threshold suite covers maximum bounded returndata, malformed/oversized data, fallback construction, and full call-budget consumption for all three Core reads, replayed across simulated delayed 2x raise chains with reproducible offchain/fork measurement parity and no onchain probe authority ([RSR-2981-GAS].6; [MRR-ROUTER-GGP]; issue #671; ADR 0013 decision U7; ADR 0017) | Core bytecode size, resolver/router gas report, shared-buffer measurement evidence | mandatory |
