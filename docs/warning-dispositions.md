...
| `LINT-BLOCK-TIMESTAMP-FINALITY-REGISTRY` | `block-timestamp` | [`smart-contracts/domains/finality/StreamFinalityRecordState.sol`](../smart-contracts/domains/finality/StreamFinalityRecordState.sol) | Records the observation time of admitted finality; the retired local scheduling clock is no longer authoritative | `accepted-protocol-time-window` for recorded observation time; canonical governance owns execution timing | Keep current stored-record checks in `test/unit/finality/StreamFinalityStoredState.t.sol`; historical clock coverage is mapped in `test/regression/legacy/finality/README.md` |
| `LINT-BLOCK-TIMESTAMP-FINALITY-PREVIEW` | `block-timestamp` | [`test/regression/legacy/finality/contracts/LegacyLocalClockArtworkFinalityPreview.sol`](../test/regression/legacy/finality/contracts/LegacyLocalClockArtworkFinalityPreview.sol) | Retained historical preview mirrors the retired local scheduling clock | `accepted-test-only`; current Preview has no local timestamp path and rejects retired readiness calls | Keep historical parity tests in `test/regression/legacy/finality/StreamArtworkFinalityRegistry.t.sol`; current replacement preview remains required |
