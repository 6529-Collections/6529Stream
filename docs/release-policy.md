...
Mutable `StreamCore` runtime size and EIP-170 headroom are owned by the
committed
[`release-artifacts/latest/bytecode-release-proof.json`](../release-artifacts/latest/bytecode-release-proof.json).
The proof is below the approved 22,184-byte objective and passes the normative
2,000-byte production deployment headroom rule in the
...
This satisfies
[issue #654](https://github.com/6529-Collections/6529Stream/issues/654)'s
artifact-backed Core headroom requirement. It does not make the protocol
production-ready or make Core the default home for future marketplace,
provenance, permanence, royalty-policy, frontend, indexer, or
collector-evidence features. The unimplemented #670 satellites still require
concrete source/security acceptance: ADR 0021's revenue architecture is
Accepted but source-blocked, while ADR 0022's artist architecture remains
Proposed. Future Core work continues to follow [PV1-CORE-CUTOVER]: it is
satellite-only with zero Core delta, or pairs a mandatory Core addition with
real removal and reports a net-negative production-profile runtime delta. The
approved
bytecode-spend ceiling remains 22,184 bytes, with its baseline margin derived
from that ceiling and the EIP-170 limit,
so future Core growth above that ceiling still requires an accepted exception
record during development. Accepted headroom-recovery records use
`measured_delta_bytes` as `runtime_size_bytes - baseline_runtime_size_bytes`,
so bytecode reductions are negative deltas and bytecode spend remains positive.
The final proof includes every mandatory hook at its production call shape;
isolated scratch deltas and a partial Core below the ceiling are not release
evidence.
