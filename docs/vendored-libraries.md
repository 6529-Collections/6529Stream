...
The local Base64 repair additionally has literal dirty-tail vectors, a retained
40-byte inline-image counterexample, adjacent-object preservation and a
randomized independent byte-wise encoder oracle in
`test/unit/libraries/StreamBase64MemoryTail.t.sol`. The old encoder could include
bytes after the logical input in unused padding bits. The repair saves and
zeros exactly the following 32-byte word after output/table allocation, then
restores it before returning. Existing clean-tail encodings remain unchanged.
Compiler products embedding this helper change and require matching build and
release evidence; earlier snapshots retain their historical source identity.
