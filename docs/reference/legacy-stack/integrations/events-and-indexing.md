# Historical reference: events and indexing

> This is the retained legacy-stack documentation from commit `330ac1d40b1a0d385399ca68423503c892ea0c04`.
> Its references to “current” describe that older stack. Do not use its
> contracts, signing payloads, or addresses with the permanent current Core.
> Use the [current guide](../../../integrations/events-and-indexing.md) for new integrations.
...
Use
[`docs/reference/legacy-stack/integrations/examples/typescript-event-decoding-and-indexer-ingestion.md`](examples/typescript-event-decoding-and-indexer-ingestion.md)
...
| Marketplace/indexer evidence | [`docs/integrations/marketplace-indexer-evidence.md`](../../../integrations/marketplace-indexer-evidence.md) | `ONE-005` retained marketplace/indexer evidence requirements for event replay, metadata refresh, cache invalidation, royalty display, and platform coverage |
...
| TypeScript event decoding snippets | [`docs/reference/legacy-stack/integrations/examples/typescript-event-decoding-and-indexer-ingestion.md`](examples/typescript-event-decoding-and-indexer-ingestion.md) | `INT-015` checked event topic catalog loading, topic0 dispatch, normalized log identity, decode boundary, idempotent ingestion, read-after-event queue, confirmation depth, and reorg rollback examples |
| Integration conformance fixtures | [`docs/reference/legacy-stack/integrations/integration-conformance-fixtures.md`](integration-conformance-fixtures.md), [`docs/reference/legacy-stack/integrations/fixtures/integration-conformance-fixtures.json`](fixtures/integration-conformance-fixtures.json) | `INT-016` checked local fixture bundle for event topic dispatch, normalized log identity, read-after-event queues, duplicate log idempotency, unknown emitter, unknown topic, confirmation depth, reorg rollback, and no-secret redaction diagnostics |
...
| Drops contract | [`smart-contracts/domains/mint/legacy/StreamDrops.sol`](../../../../smart-contracts/domains/mint/legacy/StreamDrops.sol) | Drop authorization, signer, auction contract, and fixed-price credit events |
| Auction contract | [`smart-contracts/domains/auctions/legacy/AuctionContract.sol`](../../../../smart-contracts/domains/auctions/legacy/AuctionContract.sol) | Auction lifecycle, bid, settlement, and proceeds events |
...
| Minter bridge | [`smart-contracts/domains/mint/legacy/StreamMinter.sol`](../../../../smart-contracts/domains/mint/legacy/StreamMinter.sol) | Phase, fixed-price batch mint, auction mint, auction end-time, contract-reference events, and bridge reads such as original auction end-time values |
| Randomizer lifecycle | [`smart-contracts/integrations/randomizers/legacy/StreamRandomizerLifecycle.sol`](../../../../smart-contracts/integrations/randomizers/legacy/StreamRandomizerLifecycle.sol) | Request, fulfillment, stale, failure, retry, and burned-token randomness events |
...
| Metadata event tests | [`test/regression/legacy/metadata/StreamMetadataEvents.t.sol`](../../../../test/regression/legacy/metadata/StreamMetadataEvents.t.sol) | Current metadata event behavior |
| Contract metadata tests | [`test/regression/legacy/metadata/StreamContractMetadata.t.sol`](../../../../test/regression/legacy/metadata/StreamContractMetadata.t.sol) | Contract URI, URI hash, pause, event, and admin behavior |
| EIP-712 tests | [`test/unit/mint/StreamDropsEIP712.t.sol`](../../../../test/unit/mint/StreamDropsEIP712.t.sol) | Drop consumed/cancelled/signer event behavior |
| ERC-1271 tests | [`test/unit/mint/StreamDropsERC1271.t.sol`](../../../../test/unit/mint/StreamDropsERC1271.t.sol) | Contract signer behavior |
| Auction custody tests | [`test/regression/legacy/auctions/StreamAuctionCustody.t.sol`](../../../../test/regression/legacy/auctions/StreamAuctionCustody.t.sol) | Auction custody and no-bid state |
| Auction payment tests | [`test/regression/legacy/auctions/StreamAuctionPayments.t.sol`](../../../../test/regression/legacy/auctions/StreamAuctionPayments.t.sol) | Bidder/proceeds credit behavior |
| Minter event tests | [`test/regression/legacy/mint/StreamMinterEvents.t.sol`](../../../../test/regression/legacy/mint/StreamMinterEvents.t.sol) | Minter bridge event fields and read-after-event behavior |
...
| `PrimaryRevenueAssignment` | `chainId:resolverAddress:revenueClass:scope:scopeId` | `PrimaryTemplateCreated`, `PrimaryTemplateEntryRecorded`, `PrimaryAssignmentSet`, `PrimaryAssignmentCleared`, `PrimaryAssignmentFrozenEvent`, `PrimaryTemplateMaterialized`, resolver reads |
| `PrimarySettlement` | `chainId:settlementAddress:settlementKey` | `PrimaryRevenueSettled`, settlement reads |
...
| Primary revenue | `PrimaryTemplateCreated`, `PrimaryTemplateEntryRecorded`, `PrimaryAssignmentSet`, `PrimaryAssignmentCleared`, `PrimaryAssignmentFrozenEvent`, `PrimaryTemplateMaterialized`, `SettlementCallerUpdated`, `PrimaryRevenueSettled`, `PrimaryRevenueSettlementContext`, `PrimaryRevenueSettlementPolicy` | Resolver assignment/template state, authorized settlement caller state, official primary revenue distinct from passive split-wallet receipts, raw sale context fields, resolved policy hashes, assignment-hash drift, and native/ERC-20 asset totals |
...
| Primary revenue | `PrimaryTemplateCreated`, `PrimaryTemplateEntryRecorded`, `PrimaryAssignmentSet`, `PrimaryAssignmentCleared`, `PrimaryAssignmentFrozenEvent`, `PrimaryTemplateMaterialized`, `SettlementCallerUpdated`, `PrimaryRevenueSettled`, `PrimaryRevenueSettlementContext`, `PrimaryRevenueSettlementPolicy` | resolver `primaryTemplate(templateId)`, `primaryTemplateEntry(templateId,index)`, `resolvePrimaryAssignment(collectionId,tokenId,revenueClass)`, `primaryAssignmentHash(...)`; settlement `settlementCaller(caller)`, `settlementConsumed(settlementKey)`, `resolvedPrimaryPolicyHash(...)`, `officialSettled(revenueClass,profileId,wallet,asset)`, `totalOfficialSettled(asset)`. Use `address(0)` as the native ETH asset sentinel. Treat `PrimaryRevenueSettled` as official primary-sale evidence only when emitted by the selected settlement adapter; pair it with the same-key context and policy events for raw sale fields, `settlementId`, `policyMode`, settlement caller, expected policy hash, resolved policy hash, and assignment hash. `PrimaryTemplateMaterialized` is public deterministic resolver-cache state, not official settlement evidence by itself. Direct ETH or ERC-20 receipts at a split wallet remain passive receipt state until a settlement event records them as official. |
...
forge test --match-path test/regression/legacy/protocol/StreamEventReconstructability.t.sol
forge test --match-path test/regression/legacy/mint/StreamMinterEvents.t.sol
```
