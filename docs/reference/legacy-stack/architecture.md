...
> Use the [current guide](../../architecture.md) for new integrations.
...
| --- | --- | --- |
| `StreamAdmins` | Root admin, global/function admin, pause guardian, unpause admin, signer manager, and emergency-recipient registry | [`docs/adr/0004-admin-governance.md`](../../adr/0004-admin-governance.md), `test/unit/access/StreamAdmins.t.sol` |
| `DependencyRegistry` | Versioned generative dependency source, content hashes, provenance, and deprecation state | [`docs/dependency-operations.md`](../../dependency-operations.md), `test/regression/legacy/dependencies/StreamDependencyRegistry.t.sol` |
| `StreamCore` | ERC-721 ownership, collection configuration, metadata generation, dependency pins, burns, freeze manifests, and randomizer registration | [`docs/metadata.md`](../../metadata.md), `test/regression/legacy/metadata/StreamMetadataFreeze.t.sol` |
| `StreamMinter` | Mint phase gating and mint authority bridge into `StreamCore` | [`docs/adr/0001-drop-authorization.md`](../../adr/0001-drop-authorization.md), `test/StreamCoreMinting.t.sol`, `test/unit/protocol/StreamEmergencyWithdraw.t.sol` |
...
| `StreamAuctions` | Auction escrow custody, bidding, settlement, cancellation, bidder/proceeds credits, and auction-local accounting | [`docs/auction-custody.md`](../../auction-custody.md), `test/regression/legacy/auctions/StreamAuctionInvariant.t.sol` |
| `StreamCuratorsPool` | Curator Merkle reward claims, delegation checks, reward credits, and curator-pool surplus boundaries | [`docs/adr/0003-payment-accounting.md`](../../adr/0003-payment-accounting.md), `test/unit/revenue/StreamCuratorsPool.t.sol` |
| `NextGenRandomizerVRF` | VRF request lifecycle adapter, callback validation, raw-output hash storage, retry, stale, and failed post-processing state | [`docs/randomizer-operations.md`](../../randomizer-operations.md), `test/regression/legacy/entropy/StreamRandomizerLifecycle.t.sol` |
| `NextGenRandomizerRNG` | arRNG request lifecycle adapter, reserve accounting boundary, callback validation, retry, stale, and failed post-processing state | [`docs/randomizer-operations.md`](../../randomizer-operations.md), `test/unit/entropy/StreamRandomizerPayments.t.sol` |
...
[`tools/build/check_contract_size_budget.py`](../../../tools/build/check_contract_size_budget.py).
Run `python -m tools.build.build_release_artifacts` and then
`python -m tools.build.check_contract_size_budget` before relying on any
...
[`tools/build/check_core_bytecode_spend_policy.py`](../../../tools/build/check_core_bytecode_spend_policy.py):
...
- canonical command: `python -m tools.build.build_release_artifacts`;
...
`test/StreamDropAuthorization.t.sol`, `test/regression/legacy/mint/StreamFixedPricePayments.t.sol`,
and `test/regression/legacy/protocol/StreamPaymentsInvariant.t.sol`.
...
`test/regression/legacy/auctions/StreamAuctionPayments.t.sol`, and `test/regression/legacy/auctions/StreamAuctionInvariant.t.sol`.
...
`test/regression/legacy/entropy/StreamRandomizerLifecycle.t.sol`, `test/regression/legacy/metadata/StreamMetadataGolden.t.sol`, and
...
- payment owed/reserved/surplus coherence in `test/regression/legacy/protocol/StreamPaymentsInvariant.t.sol`;
...
- metadata golden fixtures and browser checks in
  `test/regression/legacy/metadata/StreamMetadataGolden.t.sol`, `tools/protocol/check_metadata_fixtures.py`, and
