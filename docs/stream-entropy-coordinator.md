...
1. The as-built permanent Core threshold is
   `ENTROPY_REGISTRATION_GAS_LIMIT +
   ceil(ENTROPY_REGISTRATION_GAS_LIMIT / 63) + R + C`, where the gas limit
   is the live governed value, `R` is the post-entropy parent reserve, and
   `C = 3,300` is the conservative 3,300-gas cold `CALL` upfront reserve.
   The ceiling form is mandatory: integer truncation must not understate
   EIP-150's one-64th parent retention at a non-divisible gas limit. Core
   encodes the coordinator calldata before the admission check, so ABI
   encoding and its memory expansion precede the threshold. No source-level
   work that consumes unaccounted gas occurs between the check and the
   low-level call.
...
3. The as-built permanent-Core boundary test
   ...
   all-zero-to-nonzero EOA-recipient tail. Separate regressions prove
   coordinator failure or nonempty return data rolls back every mint write and
   event.
...
5. The canonical as-built source artifact is
   `release-artifacts/post-entropy-mint-completion-gas.json`, generated by
   `tools/protocol/generate_post_entropy_completion_gas.py` and validated by
   `tools/protocol/check_post_entropy_completion_gas.py`. It binds the dedicated
   via-IR snapshot, permanent Core and shared-buffer sources, these normative
   homes, the linked permanent-Core runtime receipt, the measured runtime
   delta, and the exact reserve calculation. It is as-built source evidence,
   not a candidate-instance measurement and not an authorization input for
   #684.
6. The permanent Core implements the entropy-coordinator registration seam,
   encodes the coordinator calldata before the admission check, reserves the
   3,300-gas cold `CALL` upfront reserve, forwards the exact live governed
   limit, and preserves whole-transaction rollback. The exact deployment
   candidate remains blocked by #656 and the two concrete #670 pointer rows;
   this source evidence makes no deployment or production-readiness claim.
