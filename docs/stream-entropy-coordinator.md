...
Post-registration completion reserve [EC-REGGAS-COMPLETION]:

1. The #672 planning lower bound is
   `ceil(ENTROPY_REGISTRATION_GAS_LIMIT * 64 / 63) + R`, where the gas limit
   is the live governed value and `R` is the post-entropy parent reserve.
   The ceiling form is mandatory: integer truncation must not understate
   EIP-150's one-64th parent retention at a non-divisible gas limit. This
   formula binds only the forwarding and measured EOA-tail terms. It is a
   planning lower bound, not an as-built exact `gasleft()` threshold: it
   excludes ABI encoding and memory expansion, low-level `CALL` upfront and
   dynamic costs, and source-level work between the guard and the `CALL`
   opcode.
2. `R` is derived from the production-profile, via-IR measurement of the
   first-mint, all-zero-to-nonzero EOA-recipient tail beginning immediately
   after a successful coordinator return. The target fixture includes
   prepared-state cleanup, collection and global live-supply writes,
   token/collection live-metadata accounting, ERC-721 balance and owner
   writes, the `Transfer` event, and `_safeMint`'s EOA path. The measured
   upper bound receives a 25% margin and is rounded up to the next 1,000 gas.
   The current checksum-bound planning measurement is 128,886 gas, so
   `R = 162,000`.
3. Just below, exactly at, and just above the planning formula result are
   pure policy-predicate tests. A separate high-parent-gas scenario proves
   that a coordinator can consume the full permitted registration stipend and
   still leave enough parent gas for the qualifying EOA mint to complete.
   These two fixture checks do not prove that an actual call made with exactly
   the planning lower bound forwards the full stipend or preserves `R`.
4. Contract-recipient callback gas is outside this fixed EOA guarantee.
   `_safeMint` still invokes the receiver, and the transaction caller must
   supply any callback gas in addition to the fixed admission requirement.
   Callback failure or exhaustion reverts the whole mint atomically; this
   section makes no promise that an unbounded receiver callback can complete.
5. The canonical planning artifact is
   `release-artifacts/post-entropy-mint-completion-gas.json`, generated by
   `scripts/generate_post_entropy_completion_gas.py` and validated by
   `scripts/check_post_entropy_completion_gas.py`. It binds the dedicated
   via-IR snapshot, target-fixture sources, these normative homes, current
   transitional Core source/runtime facts, the zero-Core-delta disposition,
   and the exact reserve calculation. It is planning evidence, not a
   candidate-instance measurement and not an authorization input for #684.
6. Current `StreamCore` has no entropy-coordinator registration seam and this
   #672 slice changes no production Core bytecode. Issue #654 must evaluate
   and measure the complete low-level-call admission boundary at the actual
   `CALL`, including ABI encoding, memory expansion, `CALL` upfront and dynamic
   costs, and all intervening source-level work. It must enforce the live
   governed limit, prove the exact forwarded stipend and post-return reserve
   at the candidate-instance threshold, preserve whole-transaction rollback,
   remeasure the linked Core, and compile the complete target at or below the
   production size ceiling before this target evidence can become as-built
   evidence.
