...
| --- | ---: | --- |
| `missing_natspec` | 345 | First-party function, event, or custom error declaration lacks nearby NatSpec |
| `public_variable_getter_missing_natspec` | 121 | Compiler-generated public getter lacks NatSpec on the state variable |
| `declaration_not_in_source` | 446 | ABI entry is inherited or otherwise not declared in the first-party source body |

The checker reports 108 documented release-surface entries and 912 explicit
