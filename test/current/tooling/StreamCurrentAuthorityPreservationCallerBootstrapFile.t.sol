...
import {
    StreamCurrentAuthorityPreservationCallerBaseline as Baseline
...
contract StreamCurrentAuthorityPreservationCallerBootstrapFileTest {
    ...
    function testRejectsTraversalBeforeAttemptingFileRead() public {
        vm.expectRevert(abi.encodeWithSelector(Baseline.InvalidArtifactPrefix.selector));
    ...
    function testBinaryInputRetainsLiveAccountValidation() public {
        ...
        bytes memory reason =
            abi.encodeWithSelector(Baseline.PrestateAccountMismatch.selector, address(bootstrap));
    ...
    function testExistingOutputIsPreservedWithFileInput() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Baseline.ExistingArtifact.selector, output));
        bootstrap.exportPreparationFile(prefix);
