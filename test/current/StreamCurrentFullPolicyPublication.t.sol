...
import "../helpers/StreamCurrentFullPolicyFinalityFixture.sol";
import "../helpers/StreamCurrentFullPolicyPublicationBase.sol";
import {
    StreamPolicySnapshotTypesV2 as PublicationSnapshot
...
contract StreamCurrentFullPolicyPublicationTest is StreamCurrentFullPolicyFinalityFixture {
    ...
    function testActualFullOutputsRequireOriginalArtistConsentAndPreserveRetry() public {
        ...
        vm.expectRevert();
        assemblyRouter.publishVerifiedPolicyContentRoot(publication);
        require(
    ...
    /// Current live-sanction attribution changes checkpoint bytes: the final unchanged-manifest
    /// assertions expose that unresolved source dependency. This is not an accepted success path.
