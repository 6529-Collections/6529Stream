...
import "../interfaces/ILegacyRC1StreamRevenueResolver.sol";
import "../interfaces/ILegacyRC1StreamAssetPolicyRegistry.sol";
...
contract LegacyRC1StreamRevenueResolver is ILegacyRC1StreamRevenueResolver, Ownable {
    ...
    ILegacyRC1StreamSplitFactory public immutable splitFactoryContract;
    ILegacyRC1StreamAssetPolicyRegistry private immutable _assetPolicyRegistry;
    ...
    constructor(ILegacyRC1StreamSplitFactory splitFactory_) {
        ...
        ILegacyRC1StreamAssetPolicyRegistry registry;
        try splitFactory_.assetPolicyRegistry() returns (ILegacyRC1StreamAssetPolicyRegistry registry_) {
            registry = registry_;
        ...
        returns (bytes32 profileId, address wallet, bytes32 entriesHash)
    {
        ...
        ILegacyRC1StreamSplitWallet.SplitEntry[] memory concreteEntries =
            new ILegacyRC1StreamSplitWallet.SplitEntry[](template.entries.length);
        for (uint256 i = 0; i < template.entries.length; i++) {
            ...
            concreteEntries[i] = ILegacyRC1StreamSplitWallet.SplitEntry({
    ...
    function _canonicalizeConcreteEntries(ILegacyRC1StreamSplitWallet.SplitEntry[] memory entries)
        private
        pure
        returns (ILegacyRC1StreamSplitWallet.SplitEntry[] memory canonicalEntries)
    {
        ...
        canonicalEntries = new ILegacyRC1StreamSplitWallet.SplitEntry[](uniqueCount);
        uint256 cursor = 0;
        for (uint256 i = 0; i < entries.length; i++) {
            if (
                i == 0 || entries[i].account != entries[i - 1].account
    ...
    function _sortSplitEntries(ILegacyRC1StreamSplitWallet.SplitEntry[] memory entries) private pure {
        for (uint256 i = 1; i < entries.length; i++) {
