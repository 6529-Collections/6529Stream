...
import "../interfaces/ILegacyRC1StreamSplitWallet.sol";
...
contract LegacyRC1StreamSplitWallet is ILegacyRC1StreamSplitWallet, ReentrancyGuard {
    ...
    function _assetPolicyRegistryOrRevert(address asset) private view returns (address registry) {
        (bool success, bytes memory data) = factory.staticcall(
            abi.encodeWithSelector(ILegacyRC1StreamSplitFactory.assetPolicyRegistry.selector)
        );
