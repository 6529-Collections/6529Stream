...
import "../interfaces/ILegacyRC1StreamSplitFactory.sol";
import "./LegacyRC1StreamSplitWallet.sol";

/// @notice Creates immutable split profiles and their deterministic native split wallets.
contract LegacyRC1StreamSplitFactory is ILegacyRC1StreamSplitFactory {
    ...
    ILegacyRC1StreamAssetPolicyRegistry public immutable override assetPolicyRegistry;
    ...
    struct Profile {
        ...
        address wallet;
        ILegacyRC1StreamSplitWallet.SplitEntry[] entries;
        address[] accounts;
    ...
    constructor(ILegacyRC1StreamAssetPolicyRegistry assetPolicyRegistry_) {
    ...
    function splitWalletInitCodeHash() public pure override returns (bytes32) {
        return keccak256(type(LegacyRC1StreamSplitWallet).creationCode);
    }
    ...
    function splitWalletRuntimeCodeHash() public pure override returns (bytes32) {
        return keccak256(type(LegacyRC1StreamSplitWallet).runtimeCode);
    }

    /// @notice Creates or reuses a canonical split profile and deploys its deterministic wallet.
    function createProfile(
        ILegacyRC1StreamSplitWallet.SplitEntry[] calldata entries,
        bytes32 metadataURIHash
    ) external override returns (bytes32 profileId, address wallet) {
        (
            ILegacyRC1StreamSplitWallet.SplitEntry[] memory canonicalEntries,
        ...
        if (created) {
            ...
            for (uint256 i = 0; i < canonicalEntries.length; i++) {
                ILegacyRC1StreamSplitWallet.SplitEntry memory splitEntry = canonicalEntries[i];
    ...
    function profileIdFor(ILegacyRC1StreamSplitWallet.SplitEntry[] calldata entries, bytes32 metadataURIHash)
        external
        view
        override
        returns (bytes32)
    ...
    {
        ILegacyRC1StreamSplitWallet.SplitEntry storage splitEntry = _profiles[profileId].entries[index];
    ...
    function _deployWallet(bytes32 profileId) private returns (address wallet) {
        ...
        LegacyRC1StreamSplitWallet splitWallet = new LegacyRC1StreamSplitWallet{ salt: profileId }();
        ...
        returns (bool)
    {
        ...
        ILegacyRC1StreamSplitWallet splitWallet = ILegacyRC1StreamSplitWallet(wallet);
    ...
    function _canonicalize(ILegacyRC1StreamSplitWallet.SplitEntry[] calldata entries)
        private
        pure
        returns (
            ILegacyRC1StreamSplitWallet.SplitEntry[] memory canonicalEntries,
        ...
        )
    {
        ...
        canonicalEntries = new ILegacyRC1StreamSplitWallet.SplitEntry[](length);
    ...
    function _sortEntries(ILegacyRC1StreamSplitWallet.SplitEntry[] memory entries) private pure {
        for (uint256 i = 1; i < entries.length; i++) {
            ILegacyRC1StreamSplitWallet.SplitEntry memory current = entries[i];
            uint256 j = i;
    ...
    function _entryLess(
        ILegacyRC1StreamSplitWallet.SplitEntry memory left,
        ILegacyRC1StreamSplitWallet.SplitEntry memory right
    ...
    function _copyEntries(Profile storage profile)
        private
        view
        returns (ILegacyRC1StreamSplitWallet.SplitEntry[] memory entries)
    {
        entries = new ILegacyRC1StreamSplitWallet.SplitEntry[](profile.entries.length);
