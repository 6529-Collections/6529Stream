...
import "../../../../smart-contracts/interfaces/standards/IERC20.sol";

import "../interfaces/ILegacyRC1StreamAssetPolicyRegistry.sol";
import "../interfaces/ILegacyRC1StreamPrimarySaleSettlement.sol";
...
contract LegacyRC1StreamPrimarySaleSettlement is ILegacyRC1StreamPrimarySaleSettlement, Ownable, ReentrancyGuard {
    ...
    ILegacyRC1StreamRevenueResolver public immutable override revenueResolver;
    ILegacyRC1StreamSplitFactory public immutable splitFactory;
    ILegacyRC1StreamAssetPolicyRegistry public immutable assetPolicyRegistry;
    ...
    constructor(ILegacyRC1StreamRevenueResolver revenueResolver_) {
        ...
        ILegacyRC1StreamSplitFactory splitFactory_ = ILegacyRC1StreamSplitFactory(revenueResolver_.splitFactory());
        if (address(splitFactory_).code.length == 0) {
            revert UnverifiedSplitWallet(bytes32(0), address(0));
        }
        ILegacyRC1StreamAssetPolicyRegistry assetPolicyRegistry_ = splitFactory_.assetPolicyRegistry();
    ...
    {
        ILegacyRC1StreamRevenueResolver.ResolvedPrimaryAssignment memory assignment =
