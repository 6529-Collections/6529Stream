...
import {
    StreamCurrentAuthorityScopedPolicyBytesFixture
} from "./StreamCurrentAuthorityScopedPolicyBytesFixture.sol";
import {
    StreamCurrentAuthorityScopedPolicyExportFixture
} from "./StreamCurrentAuthorityScopedPolicyExportFixture.sol";
import {
    StreamFinalityScope,
    StreamFinalityScopeType
} from "../../smart-contracts/interfaces/stream/finality/StreamArtworkFinalityTypes.sol";

/// @notice Explicit local simulation entrypoints for fresh scoped capture and whole-byte coverage.
...
contract StreamCurrentAuthorityScopedPolicyLocalRecipe is
    StreamCurrentAuthorityScopedPolicyBytesFixture,
    StreamCurrentAuthorityScopedPolicyExportFixture
{
    ...
    function exportScopedSources(
        ...
        string memory label
    ) external returns (AuthorityScopedExport memory) {
        AuthorityScopedPublication memory publication = _setupScopedRecipe(kind);
        AuthorityScopedExport memory result =
            _authorityExportScopedPolicySources(publication, directory, label);
    ...
    function coverExportedScopedSources(
        ...
        (
            bytes32 domain,
            ScopedExportContext memory context,
            AuthorityScopedPublication memory publication,
            ScopedExportToken[] memory tokens
        ) = abi.decode(
            sourceABI,
            (bytes32, ScopedExportContext, AuthorityScopedPublication, ScopedExportToken[])
        );
        require(
            domain == keccak256("6529STREAM_CURRENT_AUTHORITY_SCOPED_STATIC_EXPORT_V1")
    ...
    function _coverScopedRecipe(
        ...
        AuthorityScopedReference memory referenceResult = _authorityPublishScopedPolicyReference(
            publication,
        ...
        returns (AuthorityScopedPublication memory publication)
    {
        ...
        publication = _authorityPublishScopedPolicy(kind);
