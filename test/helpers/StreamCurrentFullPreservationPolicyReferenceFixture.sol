...
import {
    StreamCurrentFullPreservationPolicyPublicationFixture
} from "./StreamCurrentFullPreservationPolicyPublicationFixture.sol";
import {
    StreamPreservationPolicyReferencePublicationV1
} from "../../smart-contracts/domains/preservation/StreamPreservationPolicyReferencePublicationV1.sol";
import {
    StreamPreservationPolicyReferenceTypesV1 as PolicyReference
...
import {
    StreamPreservationPolicySnapshotTypesV1 as ReferenceSnapshot
} from "../../smart-contracts/interfaces/stream/metadata/StreamPreservationPolicySnapshotTypesV1.sol";
import {
    IStreamPreservationPolicyContentCheckpointV1 as ReferenceContent
...
/// @notice Actual preservation V1 reference publication and locking after the current preservation-policy snapshot.
...
abstract contract StreamCurrentFullPreservationPolicyReferenceFixture is
    StreamCurrentFullPreservationPolicyPublicationFixture
{
    StreamPreservationPolicyReferencePublicationV1 internal publicationReference;
    ...
    function _publicationPrepareReferenceDefinitions() internal {
        ...
        publicationReference = StreamPreservationPolicyReferencePublicationV1(host);
        require(
        ...
        for (uint256 i; i < names.length; ++i) {
            string memory path = i < 3
                ? string.concat(
                    "docs/schemas/preservation/preservation-policy-collection-reference-v1.",
    ...
    function _publicationPublishReference(
        ...
        p.observation.referenceId = keccak256(
            abi.encode(
                "preservation-policy supplied first and last reference observations",
        ...
        "urn:fixture:preservation-policy:supplied-reference-observations";
        ...
        require(
            keccak256(abi.encode(original)) == keccak256(abi.encode(p))
                ...
                && keccak256(publicationReference.referencePayload(assemblyReferenceRecord))
                    == keccak256(canonical),
            "complete original preservation reference and payload retained"
        );
