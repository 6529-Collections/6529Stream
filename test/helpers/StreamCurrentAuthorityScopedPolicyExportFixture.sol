...
import {
    StreamCurrentAuthorityScopedPolicyPublicationFactoryV2 as SPExportFactory
} from "../../smart-contracts/domains/finality/StreamCurrentAuthorityScopedPolicyPublicationFactoryV2.sol";
import {
    IStreamScopedPolicyContentCheckpointV2 as SPExportCheckpoint
...
interface CurrentAuthorityScopedExportVm {
    function exists(string calldata path) external view returns (bool);
}

/// @notice Exports authenticated current scoped STATIC bytes for a separate fresh browser run.
/// @dev No historical/native output or browser result is substituted. The exporter uses actual
/// current Router endpoints and complete onchain source joins. The supplied run directory must
...
abstract contract StreamCurrentAuthorityScopedPolicyExportFixture is
    StreamCurrentAuthorityScopedPolicyPublicationFixture
{
    bytes32 private constant SP_EXPORT_DOMAIN =
        keccak256("6529STREAM_CURRENT_AUTHORITY_SCOPED_STATIC_EXPORT_V1");

    struct AuthorityScopedExport {
        string directory;
    ...
    function _authorityExportScopedPolicySources(
        AuthorityScopedPublication memory source,
        string memory outputDirectory,
        string memory runLabel
    ) internal returns (AuthorityScopedExport memory result) {
        ...
        require(
            !CurrentAuthorityScopedExportVm(address(assemblyVm)).exists(result.directory),
            "fresh export directory required"
        );
        _scopedExportRequireSource(source);
        ScopedExportContext memory context = _scopedExportContext();
        ScopedExportToken[] memory tokens = _scopedExportTokens(source);
        bytes memory canonical = abi.encode(SP_EXPORT_DOMAIN, context, source, tokens);
        ...
        for (uint256 i; i < tokens.length; ++i) {
            ScopedExportToken memory token = tokens[i];
    ...
    function _scopedExportRequireSource(AuthorityScopedPublication memory p) private view {
        ...
        require(p.snapshot.manifestHash == keccak256(p.snapshotPayload));
        require(
        ...
        require(
            keccak256(abi.encode(assemblyRouter.scopedPolicyContentRootBinding(p.rootHash)))
    ...
    function _scopedExportContext() private view returns (ScopedExportContext memory c) {
    ...
    function _scopedExportTokens(AuthorityScopedPublication memory p)
        private
        view
        returns (ScopedExportToken[] memory tokens)
    {
        ...
        tokens = new ScopedExportToken[](count);
        for (uint256 i; i < count; ++i) {
            ScopedExportToken memory t;
            t.membershipIndex = i;
            ...
            t.html = bytes(assemblyRouter.tokenHTML(t.tokenId));
            t.metadataJSON = bytes(assemblyRouter.tokenJSON(t.tokenId));
            ...
            require(
                ...
                "fresh current HTML equals original checkpoint leaf"
            );
            require(
                t.metadataKeccak == t.output.leaf.metadataHash,
                "fresh current full JSON equals original checkpoint leaf"
            );
            require(
    ...
    function _scopedExportIdentity(
        ...
        ScopedExportContext memory c,
        ScopedExportToken[] memory tokens,
        string memory label,
        bytes32 hash,
        bytes32 sha
    ) private pure returns (string memory json) {
        json = string.concat(
            '{"schema":"6529STREAM_CURRENT_AUTHORITY_SCOPED_STATIC_EXPORT_V1","captureExecuted":false,"runLabel":"',
            label,
            '","sourceKeccak256":',
            _scopedExportHash(hash),
            ',"sourceSha256":',
            _scopedExportHash(sha),
            ',"sourceABIFile":"source-identity.abi.hex","sourceABIEncoding":"abi.encode(domain,ScopedExportContext,AuthorityScopedPublication,ScopedExportToken[])","contextABI":',
    ...
    function _scopedExportTokenJSON(ScopedExportToken memory t)
        private
        pure
    ...
    function _scopedExportCaptureInputs(ScopedExportToken[] memory tokens)
        private
        pure
        returns (string memory json)
    {
        json = string.concat(
            '{"captureExecuted":false,"sourceFieldsOnly":true,"capture1":',
    ...
    function _scopedExportCaptureToken(ScopedExportToken memory t)
        private
        pure
        returns (string memory)
    {
