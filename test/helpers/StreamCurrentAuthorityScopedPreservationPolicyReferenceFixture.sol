...
import {
    StreamPreservationTokenProducerProfilesV1 as ScopedReferenceProducerProfiles
} from "../../smart-contracts/interfaces/stream/finality/StreamPreservationTokenProducerProfilesV1.sol";
import {
    IStreamPreservationRendererV1 as ScopedPreservationCapture
...
import {
    StreamScopedPreservationPolicySnapshotTypesV1 as ScopedReferenceSnapshot
} from "../../smart-contracts/interfaces/stream/metadata/StreamScopedPreservationPolicySnapshotTypesV1.sol";
import {
    IStreamScopedPreservationPolicySnapshotPublicationV1 as ScopedReferenceSnapshots
} from "../../smart-contracts/interfaces/stream/metadata/IStreamScopedPreservationPolicySnapshotPublicationV1.sol";
import {
    IStreamPreservationPolicyContentCheckpointV1 as ScopedReferenceCheckpoint
...
import {
    StreamScopedPreservationPolicyReferenceDefinitionsV2 as ScopedReferenceDefinitions
} from "../../smart-contracts/domains/records/StreamScopedPreservationPolicyReferenceDefinitionsV2.sol";
import {
...
/// @notice Genuine scoped preservation reference publication from explicitly supplied observations.
...
abstract contract StreamCurrentAuthorityScopedPreservationPolicyReferenceFixture is
    StreamCurrentAuthorityScopedPreservationPolicyPublicationFixture
{
    ...
    function _authorityPublishScopedPreservationPolicyReference(
        ...
        p.observation.referenceId = keccak256(
            abi.encode(
                "current-authority supplied scoped preservation reference observations",
        ...
        returns (ScopedReferenceHost host)
    {
        ...
        require(
            keccak256(abi.encode(snapshot)) == keccak256(abi.encode(publication.snapshot))
                ...
                && checkpoint.preservationProfile == ScopedReferenceProducerProfiles.FAMILY_PROFILE
    ...
    function _scopedReferenceDefinitions() private {
        string[7] memory names = [
            ...
            "STREAM_SCOPED_PRESERVATION_POLICY_REFERENCE_PROFILE_V2",
        ...
        for (uint256 i; i < names.length; ++i) {
            string memory path = i < 3
                ? string.concat(
                    "docs/schemas/preservation/scoped-preservation-policy-reference-v2.",
        ...
        )
    {
        ...
        address producer = output.preservation.producer;
        require(
            producer != address(0) && producer.codehash == output.preservation.producerCodeHash
                && ScopedReferenceProducerProfiles.isSupported(output.preservation.profile),
            "exact retained preservation producer"
        );
        ...
        require(
            capture.animationHTML.length != 0 && capture.animationHTML.length <= 40960
                ...
                && keccak256(capture.animationHTML)
                    == keccak256(
                        bytes(ScopedPreservationCapture(producer).preservationTokenHTML(token))
                    )
                && keccak256(json)
                    == keccak256(
                        bytes(ScopedPreservationCapture(producer).preservationTokenJSON(token))
            ...
            "supplied capture is exact admitted preservation HTML/JSON and accepted checkpoint"
        );
        require(
    ...
    function _scopedReferenceRequireSaved(
        ...
        ScopedReference.Dependencies memory dependencies =
            ScopedReferenceHost(saved.host).dependencies();
        require(
            receipt.sourcesHash
                == keccak256(
                    abi.encode(
            ...
            "exact fixed V2 reference source domain and captured original dependencies"
        );
        ScopedReference.Receipt memory committed =
            abi.decode(abi.encode(saved.receipt), (ScopedReference.Receipt));
        committed.observation.recordHash = 0;
        committed.observation.recordChainHash = 0;
        require(
            saved.recordHash
                == keccak256(
                    abi.encode(
            ...
            "exact fixed V2 reference record without mutating retained receipt"
        );
        require(
        ...
        for (uint256 i; i < saved.source.samples.length; ++i) {
            uint256 ordinal = i == 0 ? 0 : original.checkpoint.tokenCount - 1;
            ScopedReferenceCheckpoint.Output memory output = original.outputRows[ordinal];
            require(
                abi.encode(saved.source.samples[i].preservation).length == 288
                    && abi.encode(saved.source.samples[i].preservationAdmission).length == 224
                    && keccak256(abi.encode(saved.source.samples[i].preservation))
                        == keccak256(abi.encode(output.preservation))
                    && keccak256(abi.encode(saved.source.samples[i].preservationAdmission))
                        == keccak256(abi.encode(output.preservationAdmission))
                    && keccak256(abi.encode(saved.source.samples[i].entropy))
                        == keccak256(abi.encode(output.entropy))
                    && saved.source.samples[i].terminalAdmissionHash == output.terminalAdmissionHash
                    && saved.source.samples[i].selection.tokenId == output.leaf.tokenId,
                "reference retains each exact full preservation output binding and admission"
            );
            require(
