...
import {
    StreamCurrentAuthorityScopedPreservationPolicyRenderCriticalInventoryV1 as BytesInventory
} from "../../smart-contracts/domains/preservation/StreamCurrentAuthorityScopedPreservationPolicyRenderCriticalInventoryV1.sol";
import {
    StreamScopedPreservationPolicyRenderCriticalTypesV1 as BytesScoped
...
import {
    StreamScopedPreservationPolicyRenderCriticalNativeReadsV1 as BytesNative
...
import {
    StreamScopedPreservationPolicyReferenceInventoryReadsV1 as BytesReferences
...
import {
    StreamScopedPreservationPolicyRenderCriticalDefinitionsV1 as BytesDefinitions
} from "../../smart-contracts/domains/preservation/StreamScopedPreservationPolicyRenderCriticalDefinitionsV1.sol";
import {
    StreamScopedPreservationPolicyRenderCriticalTokenReadsV1 as BytesTokens
} from "../../smart-contracts/domains/preservation/StreamScopedPreservationPolicyRenderCriticalTokenReadsV1.sol";
import {
    StreamScopedPreservationPolicyRenderCriticalScriptReadsV1 as BytesScripts
} from "../../smart-contracts/domains/preservation/StreamScopedPreservationPolicyRenderCriticalScriptReadsV1.sol";
import {
    StreamScopedPreservationPolicyRenderCriticalRendererReadsV1 as BytesRenderers
} from "../../smart-contracts/domains/preservation/StreamScopedPreservationPolicyRenderCriticalRendererReadsV1.sol";
import {
    StreamScopedPreservationPolicyRenderCriticalCitationReadsV1 as BytesCitations
} from "../../smart-contracts/domains/preservation/StreamScopedPreservationPolicyRenderCriticalCitationReadsV1.sol";
import {
    StreamScopedPreservationPolicySnapshotTypesV1 as BytesSnapshot
} from "../../smart-contracts/interfaces/stream/metadata/StreamScopedPreservationPolicySnapshotTypesV1.sol";
import {
    IStreamScopedPreservationPolicySnapshotPublicationV1 as BytesSnapshots
} from "../../smart-contracts/interfaces/stream/metadata/IStreamScopedPreservationPolicySnapshotPublicationV1.sol";
import {
    StreamScopedPreservationPolicyReferenceTypesV1 as BytesReference
} from "../../smart-contracts/interfaces/stream/preservation/StreamScopedPreservationPolicyReferenceTypesV1.sol";
import {
    IStreamScopedPreservationPolicyReferencePublicationV1 as BytesReferenceHost
...
import {
    StreamPreservationPolicyAdmissionInventoryV1 as BytesPreservation
} from "../../smart-contracts/domains/preservation/StreamPreservationPolicyAdmissionInventoryV1.sol";
import {
    IStreamPreservationPolicyContentCheckpointV1 as BytesContent
} from "../../smart-contracts/interfaces/stream/finality/IStreamPreservationPolicyContentCheckpointV1.sol";
import {
    IStreamPreservationRegistryV1 as BytesPreservationRegistry
} from "../../smart-contracts/interfaces/stream/metadata/IStreamPreservationRegistryV1.sol";

/// @notice Reads all original byte sources needed by the actual scoped preservation bundle.
...
/// dependencies before source dispatch, including the complete sixth preservation-admission stage.
/// Full producer bindings come from authenticated output/admission (nine words), never the
/// narrower six-word producer getter. Documentary bytes were retained before op24 signing;
...
abstract contract StreamCurrentAuthorityScopedPreservationPolicyBytesFixture is
    StreamCurrentAuthorityScopedPreservationPolicySealingFixture
{
    ...
    function _scopedBytesFrame(
        ...
        require(
            f.context.snapshotSource.content.preservationProfile
        ...
        f.payloads = publication.payloads;
    ...
    function _scopedBytesExpected(
        ...
        (BytesItems.Item[] memory rows, uint64 total) = BytesNative.items(
            f.dependencies, f.context, 0, 64, f.context.snapshotSource.content.preservationProfile
        );
        ...
        uint64 cursor;
        do {
            (rows, total) = BytesReferences.items(
                f.dependencies,
                referenceContext,
                cursor,
                64,
            ...
            require(rows.length != 0 && rows.length <= total - cursor, "advancing reference page");
            _scopedBytesRememberAll(f, rows);
            cursor += uint64(rows.length);
        } while (cursor < total);
        _scopedBytesRecordCandidates(f, records, rightsResult);
        for (uint64 i; i < 31; ++i) {
            (bytes32 document, bytes32 expected) =
                BytesDefinitions.definition(i, f.context.snapshotSource.content.preservationProfile);
        ...
        for (uint64 i; i < f.context.tokenCount; ++i) {
            ...
            // Phase five is mandatory and is the only phase that completes a token ordinal.
            (, BytesTokens.Original memory original) =
                BytesTokens.sourceAt(f.dependencies, f.context, i);
            at = 0;
            do {
                (BytesItems.Item memory row, uint64 size) = BytesPreservation.itemForPlan(
                    f.dependencies,
                    f.context.snapshotSource.content,
                    original.selection,
                    original.output,
                    at++
                );
    ...
    function _scopedSourceBytes(
        ...
        if (role == keccak256("ORIGINAL_SCOPED_PRESERVATION_POLICY_SNAPSHOT_RECORD_V1")) {
            return _scopedRead(
        ...
        if (role == keccak256("ORIGINAL_SCOPED_PRESERVATION_POLICY_CONTENT_ROOT_V1")) {
        ...
        if (role == keccak256("SCOPED_PRESERVATION_POLICY_REFERENCE_MANIFEST")) {
            return _scopedDynamic(
        ...
        returns (bytes memory)
    {
        ...
        if (role == keccak256("PRESERVATION_TOKEN_METADATA_JSON")) {
            return _scopedDynamic(
                item.source, abi.encodeWithSignature("preservationTokenJSON(uint256)", token)
            );
        }
        if (role == keccak256("PRESERVATION_TOKEN_ANIMATION_HTML")) {
            return _scopedDynamic(
                item.source, abi.encodeWithSignature("preservationTokenHTML(uint256)", token)
            );
        }
        if (role == keccak256("PRESERVATION_TOKEN_IMAGE")) {
            uint64 ordinal = _scopedTokenOrdinal(f, token);
            (, BytesTokens.Original memory original) =
                BytesTokens.sourceAt(f.dependencies, f.context, ordinal);
            BytesContent.Payload memory supplied = f.payloads[ordinal];
            // The producer has JSON/HTML getters, but no separate image getter. Retain the exact
            // checkpoint-authoring bytes already authenticated by the actual tokenItems call.
            require(
                supplied.tokenId == token && supplied.producer == item.source
                    && supplied.producer == original.output.preservation.producer
                    && item.sourceRecord == f.context.checkpointHash && supplied.image.length != 0
                    && supplied.image.length <= 2048
                    && keccak256(supplied.image) == original.output.leaf.imageHash,
                "actual retained preservation image preimage"
            );
        ...
        if (
            role == keccak256("PRESERVATION_PRODUCER_BINDING")
                || role == keccak256("PRESERVATION_ADMISSION")
                || role == keccak256("PRESERVATION_REGISTRATION")
                || role == keccak256("PRESERVATION_DECLARED_READS")
                || role == keccak256("PRESERVATION_COMPLETE_TARGETS")
        ) return _scopedPreservationBytes(f, item);
        return _scopedScriptOrRendererBytes(f, item);
    }

    function _scopedPreservationBytes(ScopedBytesFrame memory f, BytesItems.Item memory item)
        private
        view
        returns (bytes memory)
    {
        ...
        bytes32 key = keccak256(
            abi.encode(
                ...
                original.selection.selection.versionKey,
                original.output.preservation.producer,
                original.output.preservation.profile
            )
        );
        require(item.sourceRecord == key, "exact original preservation registration coordinate");
        // sourceAt already authenticates the complete canonical 512-byte admission against
        // current producer/registry/runtime pins. Preserve its full nominal output encoding.
        if (item.role == keccak256("PRESERVATION_PRODUCER_BINDING")) {
            require(item.source == original.output.preservation.producer);
            bytes memory binding = abi.encode(original.output.preservation);
            require(binding.length == 288, "complete nine-word producer binding");
            return binding;
        }
        require(item.source == registry);
        if (item.role == keccak256("PRESERVATION_ADMISSION")) {
            bytes memory admission = abi.encode(original.output.preservationAdmission);
            require(admission.length == 224, "complete seven-word admission");
            return admission;
        }
        if (item.role == keccak256("PRESERVATION_REGISTRATION")) {
            bytes memory raw = _scopedRead(
                registry, abi.encodeCall(BytesPreservationRegistry.preservationRecord, (key))
            );
            require(raw.length == 576, "complete original preservation registration");
            return raw;
        }
        if (item.role == keccak256("PRESERVATION_DECLARED_READS")) {
            return _scopedRead(
                registry, abi.encodeCall(BytesPreservationRegistry.preservationReads, (key))
            );
        }
        require(item.role == keccak256("PRESERVATION_COMPLETE_TARGETS"));
        uint256 count = BytesRegistry(registry).targetCount();
        require(count != 0 && count <= 64);
        BytesRegistry.Target[] memory targets = new BytesRegistry.Target[](count);
        for (uint256 i; i < count; ++i) {
            targets[i] = BytesRegistry(registry).targetAt(i);
        }
