...
import "./StreamCurrentFullPreservationPolicyArchiveFixture.sol";
...
abstract contract StreamCurrentFullPreservationPolicyPreparationFixture is
    StreamCurrentFullPreservationPolicyArchiveFixture
{
    ...
    function _prepareFullPolicyArtwork() internal {
        ...
        _assemblyLockPresentation();
        _prepareFullPolicyViewBeforeFreeze();
        _freezeFullPolicyStatic();
        _assemblyLockContent();
        _assemblyCloseAndFreezeCore();
        _admitFullPolicyCurrentCitation();
        _admitFullPolicyPreservation();
        _assertFullPolicyPrepared();
    }

    /// @dev Artist presentation is already locked for authentic snapshot reads; a genuine
    /// VIEW recipe must declare/adopt and publish its root before the STATIC family lock
    /// and Core freeze. The base COLLECTION recipe has no such adoption.
    function _prepareFullPolicyViewBeforeFreeze() internal virtual { }
    ...
    function _assemblyLockPresentation() internal {
        ...
        require(
            assemblyRouter.collectionServingFacts(1).artistIdentityLocked
                && assemblyRouter.collectionServingFacts(1).displayMetadataLocked,
            "original presentation locks precede VIEW snapshot source"
        );
    }
