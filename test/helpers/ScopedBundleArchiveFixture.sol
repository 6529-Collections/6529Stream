...
import "../unit/preservation/StreamExternalArtifactCoverage.t.sol";

/// @dev Exact existing actual archive/Safe preparation, reused without a mock coverage receipt.
abstract contract ScopedBundleArchiveFixture is OfficialSafeFixture {
    event log_named_uint(string key, uint256 value);
    ExternalArtifactVm internal constant vm =
        ExternalArtifactVm(address(uint160(uint256(keccak256("hevm cheat code")))));
    StreamExternalArtifactCoverage internal host;
    StreamArweaveObjectCheckpointVerifier internal verifier;
    StreamRoleRegistry internal roles;
    ExternalArchiveGovernanceBoundary internal governance;
    ExternalArchiveCoreBoundary internal core;
    OfficialSafe internal agentSafe;
    OfficialSafe internal fixitySafe;
    uint256[] internal agentKeys;
    uint256[] internal fixityKeys;
    ...
    E.ObjectIdentity internal object;
    bytes32 internal objectHash;
    bytes32 internal checkpointHash;
    bytes32 internal transactionId;
    bytes32 internal firstFamily;
    bytes32 internal secondFamily;
    bytes internal firstPath;
    bytes internal lastPath;

    function _setupArchive() internal {
    ...
    function _checkpointTerms() internal view returns (A.Checkpoint memory c) {
    ...
    function _certificate(A.Checkpoint memory c) internal returns (A.ObserverProof[] memory p) {
    ...
    function _admit(string memory name, A.Family memory f) internal returns (bytes32 hash) {
        bytes32 scope;
        bytes32 oldHash;
        bytes32 newHash;
    ...
    function _status(bytes32 hash, uint8 status) internal {
    ...
    function _recordReceipt(bool first, uint256 nonce) internal returns (bytes32 hash) {
    ...
    function _fixity(bytes32 receiptHash, uint8 outcome) internal view returns (E.Fixity memory f) {
    ...
    function _covered() internal returns (bytes32 first, bytes32 second, bytes32 hash) {
    ...
    function _fails(address target, bytes memory data) internal {
        (bool ok,) = target.call(data);
        require(!ok, "expected rejection");
    }

    function _role(address holder, bool granted) internal {
