...
import {
    StreamCurrentArtistPreservationAttributionV1 as PreservationProjection
} from "../../smart-contracts/domains/metadata/StreamCurrentArtistPreservationAttributionV1.sol";
import {
    StreamCurrentArtistPreservationRendererV1 as PreservationProducer
} from "../../smart-contracts/domains/metadata/StreamCurrentArtistPreservationRendererV1.sol";
import {
    IStreamPreservationRendererV1 as PreservationProducerAPI
} from "../../smart-contracts/interfaces/stream/metadata/IStreamPreservationRendererV1.sol";
import {
    IStreamPreservationAttributionV1 as PreservationProjectionAPI
...
import {
    StreamFinalityScope as PreservationScope,
    StreamFinalityScopeType as PreservationScopeType
} from "../../smart-contracts/interfaces/stream/finality/StreamArtworkFinalityTypes.sol";

import {
    StreamStaticAttributionCompanion as StaticOriginalAttribution
} from "../../smart-contracts/domains/metadata/StreamStaticAttributionCompanion.sol";
import {
    StreamStaticArtistLineageSource as StaticArtistSource
} from "../../smart-contracts/domains/metadata/StreamStaticArtistLineageSource.sol";
import {
    StreamStaticArtistLineageCompanion as StaticArtistAttribution
} from "../../smart-contracts/domains/metadata/StreamStaticArtistLineageCompanion.sol";
import {
    StreamRendererV1 as StaticLiveRenderer
} from "../../smart-contracts/domains/metadata/StreamRendererV1.sol";

import {
    StreamCurrentAuthorityPreservationRecordPublicationFixture
} from "./StreamCurrentAuthorityPreservationRecordPublicationFixture.sol";
import {
...
/// insertion, before the original Core serving assertion. Separate preservation admission uses
/// the real lineage/Artist and two freshly rendered paid-token JSON/HTML goldens. Its retained
/// analysis is still synthetic and partial; target-side equality is not independent conformance.
/// Runtime/gas/size remain unexecuted.
/// This helper does not change the already-pinned native reference catalog or claim scoped roots.
abstract contract StreamCurrentAuthorityPreservationStaticPrefixFixture is
    StreamCurrentAuthorityPreservationRecordPublicationFixture
{
    ...
    address internal assemblyPreservationProducer;
    PreservationProjection internal assemblyPreservationProjection;
    PreservationRegistry.PreservationRegistration internal assemblyPreservationRegistration;
    StaticVersions.Read[] internal assemblyPreservationReads;
    bytes32 internal assemblyPreservationKey;
    bytes32 internal assemblyPreservationAdmissionHash;
    bytes32 internal assemblyPreservationRecordHash;
    bytes32 internal assemblyPreservationGoldenJSON;
    bytes32 internal assemblyPreservationGoldenHTML;
    StaticOriginalAttribution internal assemblyOriginalStaticAttribution;
    StaticArtistSource internal assemblyStaticArtistSource;
    StaticArtistAttribution internal assemblyStaticArtistAttribution;

    /// @dev Rejoins the actual token's original frozen selector and immutable Registry admission.
    /// Fresh output is still produced by the current-Artist profile at the consumer's read.
    function _authorityPreservationProducer(uint256 tokenId) internal view returns (address) {
        require(
            assemblyPreservationKey != 0 && assemblyPreservationAdmissionHash != 0,
            "actual admitted preservation producer is required"
        );
        _assemblyStaticRequest(tokenId);
        StaticRouterAPI.ConfigRecord memory configured =
            assemblyRouter.resolvedMetadataConfig(tokenId);
        require(
            configured.selection.registry == address(assemblyStaticProducts.versions)
                && configured.selection.registryCodeHash == assemblyStaticProducts.registryCodeHash
                && configured.selection.versionKey == assemblyStaticVersion
                && configured.selection.renderer == address(assemblyStaticProducts.renderer)
                && configured.selection.rendererCodeHash == assemblyStaticProducts.rendererCodeHash,
            "exact original token renderer selection"
        );
        PreservationRegistry registry =
            PreservationRegistry(address(assemblyStaticProducts.versions));
        (
            PreservationRegistry.ProducerBinding memory binding,
            PreservationRegistry.Admission memory admission
        ) = registry.requirePreservation(
            assemblyStaticVersion,
            assemblyPreservationProducer,
            keccak256("6529STREAM_CURRENT_ARTIST_PRESERVATION_RENDER_V1")
        );
        require(
            keccak256(abi.encode(binding)) == keccak256(abi.encode(_staticPreservationBinding()))
                && keccak256(abi.encode(admission)) == assemblyPreservationAdmissionHash
                && keccak256(abi.encode(registry.preservationRecord(assemblyPreservationKey)))
                    == assemblyPreservationRecordHash,
            "retained full current-Artist admission"
        );
        require(
            assemblyStaticArtistSource.currentSuite().registry == assemblySuite.registry,
            "actual admitted current Artist suite"
        );
        return assemblyPreservationProducer;
    }
    ...
    function _beforeAssemblyArtistOnboarding() internal virtual override {
        ...
        _predeployPreservationSuccessors();
        _staticPrefixDeploy();
    ...
    function _afterAssemblyTokenFinalized(uint256 tokenId) internal virtual override {
        ...
        if (assemblyPreservationKey == 0) _staticPrefixAdmitPreservation(tokenId);
        _authorityPreservationProducer(tokenId);
    ...
    function _staticPrefixDeploy() private {
        ...
        assemblyOriginalStaticAttribution =
            new StaticOriginalAttribution(c.core, c.router, c.artist, c.finality, c.executor);
        assemblyStaticArtistSource =
            new StaticArtistSource(c.core, c.router, _preservationArtistCoordinators());
        assemblyStaticArtistAttribution = new StaticArtistAttribution(
            address(assemblyOriginalStaticAttribution),
            address(assemblyStaticArtistSource),
            c.executor
        );
        assemblyStaticProducts.configurationHash = keccak256(abi.encode(c));
        // Same exact public getters/attribution ABI, with the distinct actual lineage companion.
        // StaticPlan validates every source and runtime pin; this cast does not relabel its profile.
        assemblyStaticProducts.attribution =
            StaticOriginalAttribution(address(assemblyStaticArtistAttribution));
        assemblyStaticProducts.attributionCodeHash =
        address(assemblyStaticArtistAttribution).codehash;
        assemblyStaticProducts.renderer = new StaticLiveRenderer(
            StaticLiveRenderer.Deployment(
                StaticLiveRenderer.Sources(
                    c.core,
                    c.router,
                    c.metadata,
                    c.entropy,
                    c.dependencyRegistry,
                    address(assemblyStaticArtistAttribution)
                ),
                c.executor,
                c.readGas,
                c.attributionGas,
        ...
        assemblyStaticProducts.rendererCodeHash = address(assemblyStaticProducts.renderer).codehash;
        // Both concrete products exist before the Registry freezes its immutable target roster.
        assemblyPreservationProjection = new PreservationProjection(
            address(assemblyStaticArtistAttribution),
            address(assemblyStaticArtistAttribution),
            c.executor
        );
        assemblyPreservationProducer = address(
            new PreservationProducer(
                address(assemblyStaticProducts.renderer),
                address(assemblyPreservationProjection),
                c.executor,
                c.readGas,
                c.attributionGas
            )
    ...
    function _staticPrefixTargets() internal view returns (StaticVersions.Target[] memory targets) {
        ...
        targets = new StaticVersions.Target[](8);
        ...
        targets[6] = StaticVersions.Target(
            assemblyPreservationProducer,
            assemblyPreservationProducer.codehash,
            keccak256("PRESERVATION_RENDERER")
        );
        targets[7] = StaticVersions.Target(
            address(assemblyPreservationProjection),
            address(assemblyPreservationProjection).codehash,
    ...
    function _staticPrefixDirectReads() internal {
        ...
        for (uint16 i; i < targets.length; ++i) {
            ...
            if (target == address(assemblyCore)) {
                ...
                require(
                    target == assemblyPreservationProducer
                        || target == address(assemblyPreservationProjection),
                    "new preservation-only targets have no invented original read"
                );
            }
        }
    ...
    function _assemblyStaticRequireServing(uint256 tokenId) internal view {
        ...
        require(
            assemblyStaticArtistSource.currentSuite().registry == assemblySuite.registry,
            "live attribution follows the actual admitted current Artist suite"
        );
        require(
            assemblyStaticArtistAttribution.attributionProfile()
                == keccak256("6529STREAM_STATIC_CURRENT_ARTIST_ATTRIBUTION_V1"),
            "explicit current Artist live profile"
        );
    ...
    function _staticPreservationBinding()
        private
        view
        returns (PreservationRegistry.ProducerBinding memory)
    {
        return PreservationRegistry.ProducerBinding(
            assemblyPreservationProducer,
            assemblyPreservationProducer.codehash,
            keccak256("6529STREAM_CURRENT_ARTIST_PRESERVATION_RENDER_V1"),
            address(assemblyCore),
    ...
    /// @dev Retains every original declared read and adds only concrete typed selectors.
    /// This finite fixture roster is deliberately not a transitive/opcode analysis claim.
    function _staticPreservationReadRoster()
        private
        view
        returns (StaticVersions.Read[] memory reads)
    {
        reads = new StaticVersions.Read[](assemblyStaticReads.length + 7);
        for (uint256 i; i < assemblyStaticReads.length; ++i) {
            reads[i] = assemblyStaticReads[i];
        }
        uint16 producer = _staticPreservationTarget(assemblyPreservationProducer);
        uint16 projection = _staticPreservationTarget(address(assemblyPreservationProjection));
        uint16 lineage = _staticPreservationTarget(address(assemblyStaticArtistAttribution));
        (address encoding,) = assemblyStaticProducts.renderer.encodingBinding();
        uint16 encoder = _staticPreservationTarget(encoding);
        uint256 next = assemblyStaticReads.length;
        reads[next++] = StaticVersions.Read(
            producer, PreservationProducerAPI.preservationProfile.selector, 32, true
        );
        reads[next++] = StaticVersions.Read(
            producer, PreservationProducerAPI.preservationBinding.selector, 192, true
        );
        reads[next++] = StaticVersions.Read(
            producer, PreservationProducerAPI.preservationTokenJSON.selector, 16777216, false
        );
        reads[next++] = StaticVersions.Read(
            producer, PreservationProducerAPI.preservationTokenHTML.selector, 16777216, false
        );
        reads[next++] = StaticVersions.Read(
            projection, PreservationProjectionAPI.preservationAttribution.selector, 32832, false
        );
        reads[next++] = StaticVersions.Read(
            lineage, StaticArtistAttribution.preservationAttribution.selector, 32832, false
        );
        reads[next++] =
            StaticVersions.Read(encoder, StaticEncoding.renderCurrent.selector, 16777216, false);
        require(next == reads.length, "complete typed preservation read roster");
        ...
        for (uint256 i = 1; i < reads.length; ++i) {
            require(
                _staticPrefixReadOrder(reads[i - 1]) < _staticPrefixReadOrder(reads[i]),
                "unique sorted preservation reads"
            );
        }
    }

    function _staticPreservationTarget(address target) private view returns (uint16 index) {
        uint256 count = assemblyStaticProducts.versions.targetCount();
        bool found;
        for (uint256 i; i < count; ++i) {
            StaticVersions.Target memory t = assemblyStaticProducts.versions.targetAt(i);
            if (t.target == target) {
                require(
                    !found && i <= type(uint16).max && t.codeHash == target.codehash,
                    "exact immutable preservation target"
                );
                found = true;
                index = uint16(i);
            }
        }
        require(found, "preservation target exists before original admission");
    }

    /// @dev Real original Registry/Safe/Executor admission on the first actual paid token.
    /// The two observed output goldens establish target-side repeatability only. They do not
    /// establish independently expected artwork bytes or transitive STATIC conformance.
    function _staticPrefixAdmitPreservation(uint256 tokenId) private {
        require(
            tokenId == 1 && assemblyCore.totalSupply() == 1
                && assemblyCore.ownerOf(tokenId) == ASSEMBLY_BUYER
                && assemblyCore.coordinatorAtMint(tokenId) == address(assemblyEntropy)
                && assemblyStaticCitation != 0 && assemblyPreservationKey == 0,
            "preservation admission follows first paid token and current citation"
        );
        _assemblyStaticRequest(tokenId);
        require(
            assemblyStaticArtistSource.currentSuite().registry
                == assemblyStaticConfiguration.artist,
            "original actual Artist at initial preservation admission"
        );
        require(
            assemblyPreservationProjection.preservationAttributionProfile()
                    == keccak256("6529STREAM_CURRENT_ARTIST_NON_SANCTION_ATTRIBUTION_V1")
                && assemblyPreservationProjection.originalAttribution()
                    == address(assemblyStaticArtistAttribution)
                && assemblyPreservationProjection.liveAttribution()
                    == address(assemblyStaticArtistAttribution)
                && assemblyPreservationProjection.lineageSource()
                    == address(assemblyStaticArtistSource)
                && assemblyPreservationProjection.catalogueHash()
                    == assemblyStaticArtistSource.catalogueHash(),
            "actual current-Artist non-sanction projection"
        );
        bytes32 originalEvidence = _staticPreservationOriginalEvidence(tokenId);
        StaticVersions.Read[] memory reads = _staticPreservationReadRoster();
        PreservationRegistry.PreservationRegistration memory r =
            _staticPreservationDocuments(tokenId, reads);
        PreservationRegistry registry =
            PreservationRegistry(address(assemblyStaticProducts.versions));
        (bytes32 scope, bytes32 previous, bytes32 next) = registry.preservationTransition(r, reads);
        bytes32 action = _assemblyGovernanceCall(
            1,
            address(registry),
            abi.encodeCall(PreservationRegistry.registerPreservation, (r, reads)),
            scope,
            previous,
            next
        );
        _staticPreservationReadback(r, reads, action);
        require(
            _staticPreservationOriginalEvidence(tokenId) == originalEvidence,
            "separate preservation admission leaves original evidence and serving bytes unchanged"
        );
        require(
            keccak256(
                    bytes(
                        PreservationProducerAPI(assemblyPreservationProducer)
                            .preservationTokenJSON(tokenId)
                    )
                ) == assemblyPreservationGoldenJSON
                && keccak256(
                    bytes(
                        PreservationProducerAPI(assemblyPreservationProducer)
                            .preservationTokenHTML(tokenId)
                    )
                ) == assemblyPreservationGoldenHTML,
            "fresh producer bytes retain the two actual admitted goldens"
        );
    }

    function _staticPreservationDocuments(uint256 tokenId, StaticVersions.Read[] memory reads)
        private
        returns (PreservationRegistry.PreservationRegistration memory r)
    {
        r.versionKey = assemblyStaticVersion;
        r.binding = _staticPreservationBinding();
        bytes memory schema = bytes(
            '{"fixture":true,"type":"object","profile":"6529STREAM_CURRENT_ARTIST_PRESERVATION_RENDER_V1","projection":"current Artist non-sanction","conformanceReviewed":false}'
        );
        r.schemaDocument = _staticPrefixDocument(
            "CURRENT_AUTHORITY_PRESERVATION_SCHEMA_FIXTURE_V1",
            StaticSchema.DocumentKind.SCHEMA,
            schema
        );
        PreservationRegistry.PreservationAnalysis memory analysis =
            PreservationRegistry.PreservationAnalysis(
                ...
                keccak256(schema),
                _staticPreservationReadHash(reads),
                keccak256("SYNTHETIC FIXTURE: no transitive opcode analysis executed"),
                keccak256(
                    "SYNTHETIC FIXTURE: partial original reads plus typed preservation, lineage and current encoding"
                ),
                true
            );
        r.analysisDocument = _staticPrefixDocument(
            "CURRENT_AUTHORITY_PRESERVATION_ANALYSIS_FIXTURE_V1",
            StaticSchema.DocumentKind.CATALOG,
            abi.encode(analysis)
        );
        bytes memory json = bytes(
            PreservationProducerAPI(assemblyPreservationProducer).preservationTokenJSON(tokenId)
        );
        bytes memory html = bytes(
            PreservationProducerAPI(assemblyPreservationProducer).preservationTokenHTML(tokenId)
        );
        require(
            json.length != 0 && html.length != 0 && StaticContentBytes.matchesAnimation(json, html),
            "fresh complete producer JSON and exact executable HTML"
        );
        assemblyPreservationGoldenJSON = keccak256(json);
        assemblyPreservationGoldenHTML = keccak256(html);
        PreservationRegistry.PreservationGoldenVector[] memory vectors =
            new PreservationRegistry.PreservationGoldenVector[](2);
        PreservationScope memory scope =
            PreservationScope(PreservationScopeType.COLLECTION, 1, 0, 0);
        vectors[0] =
            PreservationRegistry.PreservationGoldenVector(scope, tokenId, 0, 2, keccak256(json));
        vectors[1] =
            PreservationRegistry.PreservationGoldenVector(scope, tokenId, 0, 3, keccak256(html));
        r.goldenDocument = _staticPrefixDocument(
            "CURRENT_AUTHORITY_PAID_PRESERVATION_GOLDEN_FIXTURE_V1",
            StaticSchema.DocumentKind.CATALOG,
            abi.encode(vectors)
        );
    }

    function _staticPreservationReadHash(StaticVersions.Read[] memory reads)
        private
        view
        returns (bytes32)
    {
    ...
    function _staticPreservationReadback(
        PreservationRegistry.PreservationRegistration memory r,
        StaticVersions.Read[] memory reads,
        bytes32 action
    ) private {
        PreservationRegistry registry = PreservationRegistry(
            address(assemblyStaticProducts.versions)
        );
        ...
        require(
            registry.preservationKey(r.versionKey, r.binding.producer, r.binding.profile) == key,
            "literal four-word preservation key"
        );
        bytes32 declaration = keccak256(
            abi.encode(
                ...
                assemblyStaticProducts.versions.targetSetHash(),
                assemblyStaticProducts.versions.version(r.versionKey).registrationHash,
                r,
                reads
            )
        );
        PreservationRegistry.PreservationRecord memory expected =
            PreservationRegistry.PreservationRecord(
                r,
                declaration,
                _staticPreservationReadHash(reads),
                keccak256(assemblySchemas.documentBytes(r.analysisDocument)),
                keccak256(assemblySchemas.documentBytes(r.goldenDocument)),
                action
            );
        require(
            action != 0
                && keccak256(abi.encode(registry.preservationRecord(key)))
                    == keccak256(abi.encode(expected))
                && keccak256(abi.encode(registry.preservationReads(key)))
                    == keccak256(abi.encode(reads)),
            "complete original Registry preservation record, action and reads"
        );
        (
            PreservationRegistry.ProducerBinding memory binding,
            PreservationRegistry.Admission memory admission
        ) = registry.requirePreservation(r.versionKey, r.binding.producer, r.binding.profile);
        PreservationRegistry.Admission memory expectedAdmission = PreservationRegistry.Admission(
            address(registry),
            address(registry).codehash,
            r.versionKey,
            declaration,
            expected.readSetHash,
            expected.analysisHash,
            expected.goldenHash
        );
        require(
            abi.encode(binding, admission).length == 512
                && keccak256(abi.encode(binding)) == keccak256(abi.encode(r.binding))
                && keccak256(abi.encode(admission)) == keccak256(abi.encode(expectedAdmission)),
            "all nine producer and seven admission words"
        );
        assemblyPreservationRegistration = r;
        for (uint256 i; i < reads.length; ++i) {
            assemblyPreservationReads.push(reads[i]);
        }
        assemblyPreservationKey = key;
        assemblyPreservationAdmissionHash = keccak256(abi.encode(admission));
        assemblyPreservationRecordHash = keccak256(abi.encode(expected));
    }

    function _staticPreservationOriginalEvidence(uint256 tokenId) private view returns (bytes32) {
        bytes32 records = keccak256(
            abi.encode(
                assemblyStaticProducts.versions.version(assemblyStaticVersion),
                assemblyStaticProducts.versions.registration(assemblyStaticVersion),
                assemblyStaticProducts.versions.reads(assemblyStaticVersion),
                ...
                keccak256(assemblySchemas.documentBytes(assemblyStaticRegistration.goldenDocument))
            )
        );
        return keccak256(
            abi.encode(
