...
import "./StreamCurrentFullPreservationPolicyReferenceFixture.sol";
...
import {
    StreamPreservationPolicyRenderCriticalInventoryV1
} from "../../smart-contracts/domains/preservation/StreamPreservationPolicyRenderCriticalInventoryV1.sol";
import {
    StreamPreservationPolicyRenderCriticalTypesV1 as PolicyInventory
} from "../../smart-contracts/interfaces/stream/preservation/StreamPreservationPolicyRenderCriticalTypesV1.sol";
import {
    StreamPreservationPolicyRenderCriticalTokenReadsV1 as PolicyTokens
...
/// @notice Materializes and covers every genuine preservation source occurrence, including all STATIC stages.
/// @dev Browser observations and package endpoint witnesses are caller-supplied fixture inputs.
abstract contract StreamCurrentFullPreservationPolicyInventoryFixture is
    StreamCurrentFullPreservationPolicyReferenceFixture
{
    StreamPreservationPolicyRenderCriticalInventoryV1 internal publicationInventory;
        ...
        returns (AssemblyInventory.Item[][] memory rows)
    {
        ...
        publicationInventory =
            StreamPreservationPolicyRenderCriticalInventoryV1(publicationGraph.children[5]);
        ...
        for (uint256 i; i < fullPolicyTokens.length; ++i) {
            ...
            publicationInventory.appendToken(
                id,
                IStreamPreservationPolicyContentCheckpointV1.Payload(
                    tokenId,
                    address(assemblyPreservationRenderer),
                    bytes(""),
                    publicationHTML[tokenId]
                )
            );
            ...
            stages = 0;
            while (publicationInventory.tokenProgress(id).phase == 4) {
                require(
            ...
            while (publicationInventory.plan(id).nextToken == i) {
                require(
                    publicationInventory.tokenProgress(id).phase == 5 && ++stages <= 74,
                    "bounded complete preservation declaration and targets"
                );
                publicationInventory.appendTokenPreservation(id);
            }
            require(
                publicationInventory.plan(id).nextToken == i + 1, "all six token stages complete"
            );
        }
        ...
        require(index == count && count > 40, "complete preservation and original STATIC stages");
        ...
        private
    {
        ...
        for (uint64 i; i < 2; ++i) {
            ...
            _publicationRemember(abi.encode(original.output.preservation));
            _publicationRemember(abi.encode(original.output.preservationAdmission));
            bytes32 preservationKey = PreservationRegistry(registry)
                .preservationKey(
                    key, original.output.preservation.producer, original.output.preservation.profile
                );
            _publicationRemember(
                _publicationRawRead(
                    registry,
                    abi.encodeCall(PreservationRegistry.preservationRecord, (preservationKey))
                )
            );
            _publicationRemember(
                _publicationRawRead(
                    registry,
                    abi.encodeCall(PreservationRegistry.preservationReads, (preservationKey))
                )
            );
            _publicationRemember(
                _publicationRawRead(
