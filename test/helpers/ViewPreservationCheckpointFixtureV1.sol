...
contract PreservationOutputViewRecordProbe is CheckpointViewRecordProbe {
...
/// @dev Actual checkpoint/current reader/Documents/State/Store/Renderer/preservation adapter.
/// Preservation attribution and the independent Registry admission are explicit typed replies.
...
abstract contract ViewPreservationCheckpointFixtureV1 is ViewCheckpointFixtureV2 {
    ...
    PolicyViewWire internal preservation;
    Admission.ProducerBinding internal producerBinding;
    Admission.Admission internal admission;

    function _build(uint256 count, uint256 burnedToken) internal {
        records = new PreservationOutputViewRecordProbe(address(core), store);
        ...
        for (uint256 i; i < count; ++i) {
            _preserved(11 * (i + 1), bytes('{"state":"typed_live"}'));
        }
        serving = new Serving(
            ServingAPI.Configuration(
                address(core),
        ...
        producerBinding = Admission.ProducerBinding(
            address(serving),
        ...
        admission = Admission.Admission(
            ...
            keccak256("actual typed preservation registration"),
            keccak256("actual typed complete preservation reads"),
            keccak256("actual typed preservation analysis"),
            keccak256("actual typed preservation goldens")
        );
        _answer(
            core,
    ...
    function _installAdmission(Admission.ProducerBinding memory b, Admission.Admission memory a)
        internal
    {
        _answer(
            core,
    ...
    function _preserved(uint256 token, bytes memory value) internal {
        _answer(
            preservation,
        ...
        returns (bytes memory json, bytes memory html)
    {
        ...
        string memory h;
        if (burned) {
            (, j) = serving.historicalPreservationViewJSON(adopted, token);
            (, h) = serving.historicalPreservationViewHTML(adopted, token);
        } else {
            (, j) = serving.preservationViewJSON(scope, token);
            (, h) = serving.preservationViewHTML(scope, token);
        }
