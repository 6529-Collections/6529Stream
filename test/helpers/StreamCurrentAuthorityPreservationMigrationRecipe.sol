...
import { CurrentAuthoritySuccessorGraph } from "./StreamCurrentAuthorityMigrationRecipe.sol";
...
abstract contract StreamCurrentAuthorityPreservationMigrationRecipe is
    StreamCurrentAuthorityRecoveryRecipe
{
    ...
    T.SuiteConfiguration[2] internal preservationSuccessorSuites;
    StreamArtistOnboardingCoordinator[2] internal preservationSuccessorCoordinators;
    bool internal preservationSuccessorsPrepared;

    /// @dev This distinct finite-catalogue fixture predeploys both actual suites from the selected
    /// original foundation. Construction grants no lineage/current authority. B is imported from
    /// A and C is later imported from the then-current B through the unchanged real migration flow.
    /// The original unpredicted-successor fixture remains a separate unchanged test surface.
    function _predeployPreservationSuccessors() internal {
        require(
            !preservationSuccessorsPrepared && authorityEra == 0, "one original finite catalogue"
        );
        T.SuiteConfiguration memory original = assemblySuite;
        bytes32 selected = keccak256(
            abi.encode(StreamCurrentStackPlan.readPointer(assemblyCore, AUTHORITY_POINTER))
        );
        for (uint256 i; i < 2; ++i) {
            ...
            require(
                destination.registry != original.registry && destination.registry.code.length != 0
                    && coordinator.configurationHash() != 0,
                "genuine unselected future suite"
            );
            preservationSuccessorSuites[i] = destination;
            preservationSuccessorCoordinators[i] = coordinator;
        }
        require(
            preservationSuccessorSuites[0].registry != preservationSuccessorSuites[1].registry,
            "two distinct actual future suites"
        );
        require(
            selected
                == keccak256(
            ...
            "catalogue construction does not select authority"
        );
        preservationSuccessorsPrepared = true;
    }

    function _preservationArtistCoordinators() internal view returns (address[] memory values) {
        require(preservationSuccessorsPrepared && authorityEra == 0, "original catalogue capture");
        values = new address[](3);
        values[0] = address(assemblyCoordinator);
        values[1] = address(preservationSuccessorCoordinators[0]);
        values[2] = address(preservationSuccessorCoordinators[1]);
    }
    ...
    function _authorityMigrateNext() internal {
        require(
            authorityEra < 2 && preservationSuccessorsPrepared, "two actual catalogue successors"
        );
        T.SuiteConfiguration memory source = assemblySuite;
        T.SuiteConfiguration memory destination = preservationSuccessorSuites[authorityEra];
        StreamArtistOnboardingCoordinator coordinator =
            preservationSuccessorCoordinators[authorityEra];
        require(
            keccak256(abi.encode(coordinator.suiteConfiguration()))
                == keccak256(abi.encode(destination)),
            "exact admitted future suite before cutover"
        );
        require(
            destination.registry != source.registry && destination.registry.code.length != 0
                && StreamCurrentStackPlan.readPointer(assemblyCore, AUTHORITY_POINTER).target
                    == source.registry,
            "actual catalogue successor remains unselected before cutover"
        );
