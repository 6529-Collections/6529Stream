...
import "../../../../smart-contracts/interfaces/stream/access/IStreamAdmins.sol";
import "../../../../smart-contracts/interfaces/stream/legacy/mint/IStreamMinter.sol";
...
import "../../../../smart-contracts/interfaces/stream/dependencies/IDependencyRegistry.sol";
...
enum LegacyStreamTokenLifecycle {
    UNKNOWN,
    PREPARED_INCOMPLETE,
    MINTED,
    BURNED
...
contract LegacyStreamCore is ERC721, Ownable, IERC4906, IERC2981 {
    ...
    /// @dev Test-only compatibility overload for the Permanent Core manager boundary.
    function mintFromManager(
        uint256 collectionId,
        address initialRecipient,
        ...
        _requireNoPreparedMint();
        _requireCollectionNotFrozen(collectionId);
        string memory legacyTokenData = string(_tokenData);
        _requireTokenDataWithHash(legacyTokenData, tokenDataHash);
        collectionAdditonalDataStructure storage collectionData =
            collectionAdditionalData[collectionId];
        (tokenId, collectionSerial) = _allocateTokenIdentity(collectionId, collectionData);
        _mintProcessing(
    ...
    function prepareMintFromManager(
        ...
        collectionAdditonalDataStructure storage collectionData =
            collectionAdditionalData[collectionId];
        (tokenId, collectionSerial) = _allocateTokenIdentity(collectionId, collectionData);
        tokenData[tokenId] = legacyTokenData;
        preparedMintRecords[tokenId] = PreparedMintRecord({
            exists: true, operationId: operationId, collectionId: collectionId
        });
        pendingPreparedMintManager = msg.sender;
    ...
    function completePreparedMintFromManager(
        ...
        _requireMintManager();
        _completePreparedMint(tokenId, initialRecipient, operationId, _saltfun_o);
    }
    ...
    function completePreparedMintFromManager(
        ...
        _requireMintManager();
        _completePreparedMint(tokenId, initialRecipient, operationId, uint256(mintCommitment));
    }

    function _completePreparedMint(
        uint256 tokenId,
        address initialRecipient,
        bytes32 operationId,
        uint256 _saltfun_o
    ...
    function tokenLifecycle(uint256 tokenId) external view returns (uint8 lifecycle) {
        if (burnedTokenAuditRecords[tokenId].burned) {
            return uint8(LegacyStreamTokenLifecycle.BURNED);
        }
        if (preparedMintRecords[tokenId].exists) {
            return uint8(LegacyStreamTokenLifecycle.PREPARED_INCOMPLETE);
        }
        if (_exists(tokenId)) {
            return uint8(LegacyStreamTokenLifecycle.MINTED);
        }
        return uint8(LegacyStreamTokenLifecycle.UNKNOWN);
    }
