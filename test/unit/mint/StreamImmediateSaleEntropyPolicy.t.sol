...
contract TerminalSaleCoordinatorFixture {
    ...
    function configure(uint8 kind, uint256 fee) public {
        // kind 0 disabled, 1 async not required, 2 original required async.
        _p = P.PolicyRecord(
            true,
            kind != 2,
            true,
            kind == 0 ? P.Mode.DISABLED : P.Mode.ASYNC,
            P.SecurityClass.HIGH_ASSURANCE,
            kind == 2 ? P.RenderRequirement.REQUIRED : P.RenderRequirement.NOT_REQUIRED,
            kind == 2 ? 0 : 1,
            3,
            bytes32(uint256(91)),
            0,
    ...
    function supportsInterface(bytes4 id) external view returns (bool) {
        if (fault == 10) assembly ("memory-safe") {
            mstore(0, 2)
            return(0, 32)
        }
    ...
    function tokenEntropy(uint256) external view returns (uint256[8] memory a) {
        a[0] = _p.mode == P.Mode.DISABLED ? 1 : 2;
    ...
    function tokenEntropyStatus(uint256) external view returns (uint256) {
        return fault == 7 ? 5 : (_p.mode == P.Mode.DISABLED ? 1 : 2);
    }
...
contract StreamImmediateSaleEntropyPolicyTest is CharacterizationTestBase {
    ...
    function testReceiverBurnKeepsOriginalTerminalIdentityAndDoesNotRequest() public {
        core.setLifecycle(3);
        sale.execute(100000);
