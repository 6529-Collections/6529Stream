...
import {
    StreamNativePrimaryOfferAuthorization as OfferAuth
...
contract PrimaryOfferAuthorizationHarness {
    function validate(
...
contract StreamNativePrimaryOfferAuthorizationTest {
    PrimaryOfferVm private constant vm =
        PrimaryOfferVm(address(uint160(uint256(keccak256("hevm cheat code")))));
    ...
    PrimaryOfferAuthorizationHarness private host;
    ...
    function setUp() public {
        vm.warp(1000);
        host = new PrimaryOfferAuthorizationHarness();
    }
    ...
    function testCollectionLevelOfferAllowsZeroSelectedLeafAndTokenIdRemainsZero() public {
        ...
        q.batch.contextHash = keccak256("host-validated prepared mint execution context");
        _signBoth(q);
    ...
    function testBatchAuthorityRecipientsBytesAndCommitmentRemainExact() public {
        for (uint256 field; field < 12; ++field) {
            ...
            else if (field == 6) q.batch.initialRecipients[0] = q.terms.buyer;
    ...
    function testSellerAndBuyerERC1271ValidateExactlyClaimedAccounts() public {
        Request memory q = _request();
        PrimaryOffer1271 seller = new PrimaryOffer1271();
        PrimaryOffer1271 buyer = new PrimaryOffer1271();
    ...
    function testSellerMembershipCheckedBeforeAnyOutsiderERC1271Magic() public {
        Request memory q = _request();
        PrimaryOffer1271 outsider = new PrimaryOffer1271();
    ...
    function _request() private returns (Request memory q) {
        q.context = OfferAuth.Context(address(0xC0DE), address(0x6000), 400000);
        ...
        q.batch.contextHash = keccak256("host-validated prepared mint context");
