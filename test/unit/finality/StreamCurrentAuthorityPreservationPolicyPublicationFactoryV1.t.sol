...
import {
    PreservationPolicyGraphExternalBoundaryV1
} from "./StreamPreservationPolicyPublicationFactoryV1.t.sol";
import {
...
import {
    StreamCurrentAuthorityPreservationPolicyPublicationFactoryV1 as GraphFactory
...
import {
    IStreamCurrentAuthorityPreservationPolicyPublicationFactoryV1 as AuthorityFactory
...
import {
    StreamCurrentAuthorityPreservationPolicyGraphSelectionV1 as Selection
...
import {
    StreamCurrentAuthorityPreservationPolicyRenderCriticalInventoryV1 as InventoryChild
} from "../../../smart-contracts/domains/preservation/StreamCurrentAuthorityPreservationPolicyRenderCriticalInventoryV1.sol";
import {
    StreamCurrentAuthorityPreservationPolicyBundleArchiveCoverageV1 as BundleChild
...
contract AuthorityCollectionPreservationGraphLateBoundary {
...
contract AuthorityCollectionPreservationGraphResolverBoundary {
...
contract AuthorityCollectionPreservationGraphSelectionHarness {
    function initialize(Native.Config memory c, Binding.CollectionFactoryBinding memory b)
        external
        view
    ...
    function current(Selection.Context memory c, StreamFinalityScope memory scope)
        external
        view
        returns (Native.Config memory, GraphTypes.Graph memory)
    {
...
contract StreamCurrentAuthorityPreservationPolicyPublicationFactoryV1Test is
    ScopedPreservationSnapshotFixtureV1
{
    ...
    AuthorityCollectionPreservationGraphResolverBoundary private resolver;
        ...
        public
    {
        ...
        AuthorityCollectionPreservationGraphResolverBoundary template =
            new AuthorityCollectionPreservationGraphResolverBoundary(a);
        address predicted =
        ...
        require(
            graphFactory.preservationPolicyPublicationFactoryProfile() == Domains.FACTORY_PROFILE
        );
        resolver = new AuthorityCollectionPreservationGraphResolverBoundary(a);
        require(
    ...
    function testSevenRealChildrenSelectFixedV2ProductsAndFullAuthorityProfiles() public {
        ...
        require(inventory.originProfile() == D.PRESERVATION_POLICY_INVENTORY_PROFILE);
        require(
        ...
        require(bundle.INVENTORY_PROFILE() == D.PRESERVATION_POLICY_INVENTORY_PROFILE);
        require(
    ...
    function testRetainedGraphRejectsLegacyOrUnknownChildCapabilities() public {
        ...
        bytes[4] memory inputs = [
            ...
            abi.encodeCall(GraphSnapshot.preservationPolicySnapshotProfile, ()),
            abi.encodeCall(GraphReference.preservationPolicyReferenceProfile, ())
        ];
    ...
    function testMissingResolverRejectsOperativeUseWithoutCreatingAChild() public {
        ...
        require(
            createVm.getNonce(address(f)) == beforeNonce
                && f.graphForPlan(collectionFactory.currentInventoryPlan(scope)).graphId == 0
        );
    }
    ...
    function testSelectionUsesNewRecipeGraphAndInventoryDomains() public {
        StreamFinalityScope memory scope = _setup();
        GraphTypes.Graph memory g = graphFactory.prepareGraph(scope, 7);
        AuthorityCollectionPreservationGraphSelectionHarness h =
            new AuthorityCollectionPreservationGraphSelectionHarness();
        Native.Config memory original = _original(graphRecipe);
        Binding.CollectionFactoryBinding memory b = _binding();
        ...
        (Native.Config memory selected, GraphTypes.Graph memory actual) = h.current(c, scope);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Selection.PolicyGraphSource.selector, address(graphFactory))
        );
        h.initialize(original, b);
        b = _binding();
        snapshotVm.mockCall(
            address(graphFactory),
            abi.encodeCall(GraphInterface.preservationPolicyPublicationFactoryProfile, ()),
    ...
    function testOldSixthStageProfileAndChangedFullDependencyHashRejectRetainedGraph() public {
        ...
        snapshotVm.mockCall(
            ...
            abi.encode(keccak256("6529STREAM_PRESERVATION_POLICY_COLLECTION_RENDER_CRITICAL_V1"))
        );
        vm.expectRevert(
        ...
        snapshotVm.mockCall(
            g.children[5],
            abi.encodeWithSignature("preservationPolicyInventoryProfile()"),
            abi.encode(D.PRESERVATION_POLICY_INVENTORY_PROFILE)
        );
        O.Dependencies memory changedOrigin = origin;
        changedOrigin.originGas += 1;
        snapshotVm.mockCall(
    ...
    function _setup() private returns (StreamFinalityScope memory scope) {
        ...
        resolver = new AuthorityCollectionPreservationGraphResolverBoundary(_anchors(graphRecipe));
    ...
    function _binding() private view returns (Binding.CollectionFactoryBinding memory b) {
    ...
    function _recipeFixture() private returns (GraphTypes.Recipe memory r) {
        address late = address(new AuthorityCollectionPreservationGraphLateBoundary());
