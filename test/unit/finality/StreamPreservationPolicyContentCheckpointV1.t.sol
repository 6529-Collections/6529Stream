...
abstract contract PreservationPolicyContentFixtureV1 is ScopedPolicyContentFixtureV2 {
        ...
        returns (Capture memory c)
    {
        ...
        if (scope.scopeType == StreamFinalityScopeType.COLLECTION) {
            CollectionFactory factory = new CollectionFactory(_scopedDependencies());
        ...
        for (uint256 i; i < count; ++i) {
            ...
            c.producers[i] = new PreservationOutputBoundary(
        ...
        StreamTerminalEntropyReadiness ready = new StreamTerminalEntropyReadiness(
            address(core), address(router), source, 2000000, 6000000
        );
        c.host = _deploy(
