...
contract StreamFinalityMultiOriginPresentationTest is FinalityMultiOriginFixture {
    ...
    function _selection(StreamFinalityScope memory actualScope) private {
        ...
        table.set(
            abi.encodeCall(
                IStreamConservationRecordSelection.intentLock, (actualScope.collectionId, subject)
            ),
            abi.encode(unlocked)
        );
        ...
        current.environment.registry = c.targets[11];
        current.registryCodeHash = c.codeHashes[11];
    ...
    function _configurationReads() private {
        ...
        for (uint256 i; i < 4; ++i) {
            table.set(abi.encodeWithSignature(selectors[i]), abi.encode(address(table)));
            table.set(
                abi.encodeWithSignature(hashSelectors[i]), abi.encode(address(table).codehash)
            );
        }
        table.set(abi.encodeWithSignature("deploymentChainId()"), abi.encode(block.chainid));
        table.set(
            abi.encodeCall(
    ...
    function _worker(
        ...
        worker.set(
            abi.encodeCall(
                Worker.lineage, (sd, actualScope.collectionId, presented, selected.association)
            ),
    ...
    function testOriginalPresentationCanDifferFromCurrentArtistAfterAuthenticatedLineage() public {
        ...
        require(harness.artist(c, O.INVENTORY_PROFILE, scope, inputs) == ARTIST);
    }
    ...
    function testStatementMustNameExactCurrentIntentAndInterviewCommitments() public {
        ...
        _reject(
            abi.encodeCall(harness.artist, (c, O.INVENTORY_PROFILE, scope, wrong)),
            Presentation.FinalityPresentationChanged.selector
        );
        wrong = inputs;
    ...
    function testCurrentSelectionCannotBeReplacedByRetainedReceiptAlone() public {
        ...
        _reject(
            abi.encodeCall(harness.artist, (c, O.INVENTORY_PROFILE, scope, inputs)),
    ...
    function testMalformedWorkerCannotReturnPartialLineageCertificate() public {
        worker.set(
            abi.encodeCall(
                Worker.lineage, (sd, scope.collectionId, presented, selected.association)
            ),
            abi.encode(current)
        );
        _reject(
    ...
    function testScopedProjectionUsesActualScopedConservationSubject() public {
        _publish(O.SCOPED_INVENTORY_PROFILE);
        scope = StreamFinalityScope(StreamFinalityScopeType.TOKEN, 7, 9, 0);
        _selection(scope);
        require(harness.artist(c, O.SCOPED_INVENTORY_PROFILE, scope, inputs) == ARTIST);
        scope.tokenId = 10;
        _reject(
            abi.encodeCall(harness.artist, (c, O.SCOPED_INVENTORY_PROFILE, scope, inputs)),
