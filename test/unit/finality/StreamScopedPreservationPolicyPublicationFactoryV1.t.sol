...
import {
    IStreamScopedPolicyPublicationFactoryV2 as OriginalGraphInterface
} from "../../../smart-contracts/interfaces/stream/finality/IStreamScopedPolicyPublicationFactoryV2.sol";
import {
    IStreamScopedPolicySnapshotPublicationV2 as OriginalGraphSnapshot
} from "../../../smart-contracts/interfaces/stream/metadata/IStreamScopedPolicySnapshotPublicationV2.sol";
import {
    IStreamScopedPolicyReferencePublicationV2 as OriginalGraphReference
...
contract ScopedPreservationGraphLateBoundaryV1 {
    function marker() external pure returns (uint256) {
        return 1;
    }
}

contract StreamScopedPreservationPolicyPublicationFactoryV1Test is
    ScopedPreservationReferenceFixtureV1
{
    ...
    function testRecipeConstructionNeverRequiresMintedScopeOrPreparedChildren() public {
        ...
        require(f.scopedPreservationPolicyPublicationFactoryProfile() == GraphTypes.PROFILE);
        ...
        public
    {
        ...
        GraphContent content = GraphContent(g.children[1]);
        GraphOutput output = GraphOutput(g.children[2]);
        bytes32 profile = keccak256("6529STREAM_SCOPED_PRESERVATION_POLICY_CONTENT_V1");
        require(content.preservationPolicyProfile() == profile && output.outputProfile() == profile);
        require(content.sourceFactory() == address(scopedFactory));
        require(content.entropySourceSet() == g.sourceSet);
        require(content.terminalReadiness() == g.children[0]);
        require(output.contentCheckpoint() == g.children[1]);
        require(output.artifactCoverage() == address(snapshotCoverage));
        require(
            !GraphSnapshot(g.children[3]).supportsInterface(type(OriginalGraphSnapshot).interfaceId)
        );
        require(
            !GraphReference(g.children[4])
                .supportsInterface(type(OriginalGraphReference).interfaceId)
        );
        require(
    ...
    function _recipeFixture() private returns (GraphTypes.Recipe memory r) {
        address late = address(new ScopedPreservationGraphLateBoundaryV1());
