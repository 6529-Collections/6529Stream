...
import {
    StreamViewPolicyOutputManifestV2 as Manifest
} from "../../../smart-contracts/domains/finality/StreamViewPolicyOutputManifestV2.sol";
import {
    StreamViewPolicyManifestTypesV2 as M
} from "../../../smart-contracts/interfaces/stream/finality/StreamViewPolicyManifestTypesV2.sol";
import {
...
import {
    IStreamViewPolicyContentCheckpointV2 as CP
} from "../../../smart-contracts/interfaces/stream/finality/IStreamViewPolicyContentCheckpointV2.sol";
import {
    StreamViewPolicyManifestEncodingV2 as Encoding
} from "../../../smart-contracts/domains/finality/StreamViewPolicyManifestEncodingV2.sol";
import {
    StreamViewPolicyOutputSchemasV2 as Definitions
} from "../../../smart-contracts/domains/finality/StreamViewPolicyOutputSchemasV2.sol";

/// @dev Explicit current checkpoint boundary. Actual observation/source tests are in the sibling suite.
contract ViewManifestCheckpointBoundary {
...
contract StreamViewPolicyOutputManifestV2Test is CharacterizationTestBase {
    ...
    ViewManifestCheckpointBoundary private checkpoint;
    ...
    function setUp() public {
        ...
        checkpoint = new ViewManifestCheckpointBoundary(address(core));
        ...
        _register(
            "STREAM_VIEW_POLICY_OUTPUT_PART_V2",
            IStreamSchemaRegistry.DocumentKind.SCHEMA,
            Definitions.document(Definitions.PART)
        );
        _register(
            "STREAM_ABI_VIEW_POLICY_OUTPUT_PART_V2",
            IStreamSchemaRegistry.DocumentKind.CANONICALIZATION,
            Definitions.document(Definitions.PART_CANON)
        );
        _register(
            "STREAM_VIEW_POLICY_OUTPUT_MANIFEST_V2",
            IStreamSchemaRegistry.DocumentKind.SCHEMA,
            Definitions.document(Definitions.INDEX)
        );
        _register(
            "STREAM_ABI_VIEW_POLICY_OUTPUT_MANIFEST_V2",
            IStreamSchemaRegistry.DocumentKind.CANONICALIZATION,
    ...
    function _part(uint64 first) private view returns (bytes memory raw) {
        ...
        raw = abi.encode(
            keccak256("STREAM_VIEW_POLICY_OUTPUT_PART_V2"),
            config.chainId,
            address(core),
            address(checkpoint),
            checkpoint.configurationHash(),
    ...
    function _index(bytes32[] memory keys) private view returns (bytes memory raw) {
        ...
        raw = abi.encode(
            keccak256("STREAM_VIEW_POLICY_OUTPUT_MANIFEST_V2"),
            config.chainId,
            address(core),
            address(checkpoint),
            checkpoint.configurationHash(),
        ...
        require(
            raw.length == 640 + 288 * keys.length
    ...
    function testLiteralCompletePartIndexAndRecordWithBurnedDiscriminant() public {
        ...
        require(
            part
                == keccak256(
                    abi.encode(
                        keccak256("6529STREAM_VIEW_OUTPUT_PART_VERIFIED_V2"),
                        block.chainid,
                        address(verifier),
        ...
        bytes32 chain =
            keccak256(
            abi.encode(keccak256("6529STREAM_VIEW_OUTPUT_PART_CHAIN_V2"), plan, _header())
        );
        chain = keccak256(
            abi.encode(keccak256("6529STREAM_VIEW_OUTPUT_PART_CHAIN_V2"), chain, uint16(0), d)
        );
        require(
        ...
        require(
            record
                == keccak256(
                    abi.encode(
                        keccak256("6529STREAM_VIEW_OUTPUT_MANIFEST_VERIFIED_V2"),
                        block.chainid,
                        address(verifier),
                        verifier.configurationHash(),
                        plan,
    ...
    function testWrongFullHeaderTerminalFieldsAndTrailingBytesAreRejected() public {
        ...
        raw = _part(0);
        raw[640 + 32 * 26 + 31] ^= bytes1(uint8(1));
    ...
    function testPreparationHasNoCurrentRecordAndLateIndexFailureRollsBackRetry() public {
        ...
        raw[640 + 32 * 8 + 31] ^= bytes1(uint8(1));
