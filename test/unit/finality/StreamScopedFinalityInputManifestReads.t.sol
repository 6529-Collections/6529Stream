...
import "../../../smart-contracts/domains/finality/StreamScopedFinalityInputManifestReads.sol";
...
contract ScopedInputManifestGovernanceBoundary {
    bool private active;
...
contract ScopedInputManifestGraphBoundary {
    address public core;
...
contract ScopedInputManifestConsumerBoundary {
    StreamScopedFinalityInputManifestReads.Dependencies private d;
    ...
    function encoded(StreamScopedFinalityInputManifestTypes.Statement memory s)
        external
        view
        returns (bytes memory)
    {
        return StreamScopedFinalityInputManifestReads.encode(d, s);
    }

    function checked(StreamScopedFinalityInputManifestTypes.Statement memory s, bytes32 hash)
        external
        view
        returns (bytes32, bytes32)
    {
        return StreamScopedFinalityInputManifestReads.requireCurrent(d, s, hash);
    }

    function inputsHash(StreamScopedFinalityInputManifestTypes.Statement memory s)
        external
        view
        returns (bytes32)
    {
        return StreamScopedFinalityInputManifestReads.scopeInputsHash(d, s);
    }
}

contract StreamScopedFinalityInputManifestReadsTest is
    CharacterizationTestBase,
    OfficialSafeFixture
{
    StreamSchemaRegistry private schemas;
    StreamSchemaDocumentStore private store;
    ScopedInputManifestGovernanceBoundary private governance;
    ScopedInputManifestGraphBoundary private core;
    ScopedInputManifestGraphBoundary private metadata;
    ScopedInputManifestGraphBoundary private registry;
    ScopedInputManifestConsumerBoundary private consumer;
    StreamScopedFinalityInputManifestTypes.Statement private statement;
    bytes32 private manifestHash;
    uint256 private originalChainId;
    bytes32 private constant SCHEMA = keccak256("6529STREAM_SCOPED_FINALITY_INPUT_MANIFEST_V1");
    bytes32 private constant CANON = keccak256("6529STREAM_SCOPED_FINALITY_INPUT_MANIFEST_ABI_V1");

    function setUp() public {
        originalChainId = block.chainid;
        governance = new ScopedInputManifestGovernanceBoundary();
        schemas = new StreamSchemaRegistry(address(governance));
        store = StreamSchemaDocumentStore(schemas.chunkStore());
        core = new ScopedInputManifestGraphBoundary();
        metadata = new ScopedInputManifestGraphBoundary();
        registry = new ScopedInputManifestGraphBoundary();
        ...
        consumer = new ScopedInputManifestConsumerBoundary(
        ...
        _register(
            "6529STREAM_SCOPED_FINALITY_INPUT_MANIFEST_V1",
            IStreamSchemaRegistry.DocumentKind.SCHEMA,
            StreamScopedFinalityInputManifestSchemas.document(SCHEMA)
        );
        _register(
            "6529STREAM_SCOPED_FINALITY_INPUT_MANIFEST_ABI_V1",
            IStreamSchemaRegistry.DocumentKind.CANONICALIZATION,
            StreamScopedFinalityInputManifestSchemas.document(CANON)
        );
        statement.scope =
    ...
    function _checkFails(StreamScopedFinalityInputManifestTypes.Statement memory s, bytes32 hash)
        private
        view
    {
    ...
    function testStoreRetentionDoesNotSubstituteForOriginalRegistryStaging() public {
        StreamScopedFinalityInputManifestTypes.Statement memory s = statement;
    ...
    function testPortableDefinitionsAreExactRegisteredBytes() public view {
        require(
            keccak256(
                bytes(vm.readFile("docs/schemas/finality/scoped-input-manifest-v1.definition.json"))
    ...
    function testFullPrecisionNumbersAndCanonicalScopedIdentity() public {
        StreamScopedFinalityInputManifestTypes.Statement memory s = statement;
    ...
    function testReleaseAndSeasonSameIdCannotSubstituteManifestOrInputs() public {
        StreamScopedFinalityInputManifestTypes.Statement memory s = statement;
        bytes32 originalInputs = consumer.inputsHash(statement);
        s.scope.scopeType = StreamFinalityScopeType.SEASON;
        require(consumer.inputsHash(s) != originalInputs, "full scope in permanent inputs");
        _checkFails(s, manifestHash);
        bytes memory raw = consumer.encoded(s);
        (bytes32 hash,) = store.publishChunk(raw);
        registry.stageFinalityManifest(raw);
        require(hash != manifestHash, "full scope in independent schema envelope");
        consumer.checked(s, hash);
    }

    function testTokenRequiresExactlyOneLeafAndItsOwnRootSnapshotReference() public {
        StreamScopedFinalityInputManifestTypes.Statement memory s = statement;
        s.scope = StreamFinalityScope(StreamFinalityScopeType.TOKEN, 1, 99, 0);
        s.leafCount = 1;
        bytes memory raw = consumer.encoded(s);
        (bytes32 hash,) = store.publishChunk(raw);
        registry.stageFinalityManifest(raw);
        consumer.checked(s, hash);
        s.contentRoot ^= bytes32(uint256(1));
        _checkFails(s, hash);
        s.contentRoot ^= bytes32(uint256(1));
        s.snapshotManifestHash ^= bytes32(uint256(1));
        _checkFails(s, hash);
        s.snapshotManifestHash ^= bytes32(uint256(1));
        s.referenceRenderManifestHash ^= bytes32(uint256(1));
        _checkFails(s, hash);
        s.referenceRenderManifestHash ^= bytes32(uint256(1));
        s.leafCount = 2;
        _checkFails(s, hash);
    ...
    function testCollectionAndViewNeverMasqueradeAsThisScopedProfile() public {
        StreamScopedFinalityInputManifestTypes.Statement memory s = statement;
        s.scope = StreamFinalityScope(StreamFinalityScopeType.COLLECTION, 1, 0, 0);
        _checkFails(s, manifestHash);
        s.scope = StreamFinalityScope(StreamFinalityScopeType.VIEW, 1, 0, keccak256("view"));
        _checkFails(s, manifestHash);
        require(SCHEMA != keccak256("6529STREAM_FINALITY_INPUT_MANIFEST_V1"));
        require(CANON != keccak256("6529STREAM_FINALITY_INPUT_MANIFEST_ABI_V1"));
    }
