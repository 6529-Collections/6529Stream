...
} from "../../../smart-contracts/domains/finality/StreamFinalityScopedPolicyMetadataPayloadV2.sol";
import {
    StreamFinalityScopedPolicyMetadataRootV2 as RootRead
} from "../../../smart-contracts/domains/finality/StreamFinalityScopedPolicyMetadataRootV2.sol";
import {
...
} from "../../../smart-contracts/domains/finality/StreamFinalityScopedPolicyProviderPinsV2.sol";
import {
...
import {
    IStreamScopedPolicyContentRootPublicationV2 as PR
...
import {
    StreamScopedPolicyOutputSchemasV2 as Output1
} from "../../../smart-contracts/domains/finality/StreamScopedPolicyOutputSchemasV2.sol";
import {
    StreamScopedPolicyContentRootSchemasV2 as Root1
...
contract ScopedPolicyReadReply {
...
contract ScopedPolicyPinsFrame {
    function check(R.Config memory c) external view {
        R.requirePins(c);
    }
}

contract StreamScopedPolicyReadFramesTest {
    ScopedPolicyReadVm private constant vm =
        ScopedPolicyReadVm(address(uint160(uint256(keccak256("hevm cheat code")))));
    ...
    ScopedPolicyReadReply private snapshot;
    ScopedPolicyReadReply private router;
    ScopedPolicyReadReply private outputs;

    function setUp() public {
        vm.warp(1000);
        snapshot = new ScopedPolicyReadReply();
        router = new ScopedPolicyReadReply();
        outputs = new ScopedPolicyReadReply();
        ...
        source.entropy.planId = keccak256("complete coordinator plan");
        source.entropy.inventoryHash = keccak256("all indexed coordinators");
        source.entropy.policyChainHash = keccak256("all policy rows");
        source.entropy.policyCount = 2;
        source.entropy.allFrozen = true;
        for (uint256 i; i < 2; ++i) {
            StreamFinalityCoordinatorPolicyV2 memory row;
            row.coordinator = address(uint160(0x700 + i));
            row.indexedCodeHash = keccak256(abi.encode("coordinator code", i));
            row.firstTokenIndex = i * 8;
            row.frozen = true;
            row.moduleVersion = keccak256(abi.encode("module version", i));
            row.moduleManifestHash = keccak256(abi.encode("module manifest", i));
            row.moduleSchemaHash = keccak256(abi.encode("module schema", i));
            row.deploymentManifestHash = keccak256(abi.encode("deployment", i));
            row.policyHash = keccak256(abi.encode("policy", i));
            row.provider = address(uint160(0x800 + i));
            row.epoch = uint32(i + 1);
            row.salt = keccak256(abi.encode("salt", i));
            row.componentDataHash = keccak256(abi.encode("component", i));
            row.explicitPolicy = true;
            row.collectionPolicy.configured = true;
            row.collectionPolicy.frozen = true;
            row.collectionPolicy.revision = uint64(i + 1);
            row.collectionPolicy.policyHash = row.policyHash;
            source.entropy.policies.push(row);
        }
    ...
    function _payload() private returns (bytes memory raw) {
        S.Source memory value = source;
        S.Publication memory p = publication;
        S.Receipt memory fields = receipt;
        bytes32 domain = keccak256("6529STREAM_SCOPED_POLICY_SNAPSHOT_PAYLOAD_V2");
        bytes32 sourceDomain = keccak256("6529STREAM_SCOPED_POLICY_SNAPSHOT_SOURCES_V2");
    ...
    function readPayload() external view returns (S.Source memory) {
        return Payload.read(config, dependencies, publication, receipt);
    }

    function testFullPayloadAndCallerMemoryRemainExact() public {
        {
            _payload();
            S.Publication memory p = publication;
            S.Receipt memory r = receipt;
            bytes32 before = keccak256(abi.encode(p, r));
            S.Source memory actual = Payload.read(config, dependencies, p, r);
    ...
    function _binding() private view returns (PR.Binding memory b) {
        b.profileId = Root1.PROFILE;
    ...
    function _root() private returns (Root.Record memory r, PR.Binding memory b) {
        _payload();
        b = _binding();
        ...
        bytes32 domain = keccak256("6529STREAM_SCOPED_POLICY_CONTENT_ROOT_STATE_V2");
        ...
        router.reply(
            abi.encodeCall(PR.scopedPolicyContentRootBinding, (keccak256("root record"))),
            abi.encode(b)
        );
        router.reply(
    ...
    function readRoot() external view returns (bytes32, Root.Record memory, PR.Binding memory) {
        ...
        return RootRead.read(config, publication.scope, publication.outputManifestRecord, f);
    }

    function testFullRootBindingAndIndependentStateHash() public {
        {
            (Root.Record memory r, PR.Binding memory b) = _root();
    ...
    function testRootBindingAllWordsAndFutureTimeRefuseThenRestore() public {
        ...
        for (uint256 i; i < 23; ++i) {
            ...
            router.reply(
                abi.encodeCall(PR.scopedPolicyContentRootBinding, (keccak256("root record"))), bad
            );
            vm.expectRevert(M.InvalidScopedProviderMetadata.selector);
            this.readRoot();
        }
        router.reply(
            abi.encodeCall(PR.scopedPolicyContentRootBinding, (keccak256("root record"))), raw
        );
        r.publishedAt = 1001;
        router.reply(
            abi.encodeCall(Root.scopedContentRootRecord, (keccak256("root record"))), abi.encode(r)
    ...
    function _reply(R.Config memory c, uint256 i, string memory selector, bytes memory value)
        private
    {
        ScopedPolicyReadReply(c.targets[i]).reply(abi.encodeWithSignature(selector), value);
    }
    ...
    function _pins(address caller) private returns (R.Config memory c) {
        ...
        for (uint256 i; i < 22; ++i) {
            ScopedPolicyReadReply t = new ScopedPolicyReadReply();
            t.expect(caller);
        ...
        bytes32 content = keccak256("6529STREAM_SCOPED_POLICY_CURRENT_FULL_CONTENT_V2");
        ...
        for (uint256 i; i < 7; ++i) {
            ScopedPolicyReadReply(c.targets[ids[i]])
        ...
        ScopedPolicyReadReply selection = new ScopedPolicyReadReply();
        ScopedPolicyReadReply set = new ScopedPolicyReadReply();
    ...
    function testPinsCompleteTypedGraphAndDelegateHostIdentity() public {
        ...
        ScopedPolicyPinsFrame other = new ScopedPolicyPinsFrame();
    ...
    function testPinsConfigurationRuntimeProfileAndReserveOrder() public {
        ...
        _reply(c, 6, "scopedOutputProfile()", abi.encode(bytes32(0)));
    ...
    function _graph() private returns (M.RootFacts memory expected) {
        for (uint256 i; i < 11; ++i) {
            ScopedPolicyReadReply t = new ScopedPolicyReadReply();
        ...
        ScopedPolicyReadReply membership = ScopedPolicyReadReply(config.membership);
        ...
        ScopedPolicyReadReply factory = new ScopedPolicyReadReply();
        ...
        ScopedPolicyReadReply(dependencies.targets[10])
            .reply(abi.encodeWithSignature("factory()"), abi.encode(address(factory)));
        ScopedPolicyReadReply(dependencies.targets[7])
        ...
        bytes32[3] memory hashes =
            [Definitions.SCHEMA_HASH, Definitions.PROFILE_HASH, Definitions.CANON_HASH];
        ...
        bytes32 domain = keccak256("6529STREAM_SCOPED_POLICY_SNAPSHOT_RECORD_V2");
        ...
        (Root.Record memory record_, PR.Binding memory binding_) = _root();
        snapshot.reply(
    ...
    function testOriginalMetadataEntryReturnsAllFactsAndKeepsCurrentBranch() public {
        {
            M.RootFacts memory expected = _graph();
            M.RootFacts memory historical = M.rootFacts(config, publication.scope, false);
            ...
            M.rootFacts(config, publication.scope, true);
            snapshot.reply(
                abi.encodeCall(
            ...
            M.RootFacts memory current = M.rootFacts(config, publication.scope, true);
            assertEq(abi.encode(current), abi.encode(expected));
            (bytes32 value, uint64 count, bytes32 schema) = M.root(config, publication.scope);
            assertEq(
