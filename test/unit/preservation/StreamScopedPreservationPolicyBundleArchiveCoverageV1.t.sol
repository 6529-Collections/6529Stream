...
import {
    StreamScopedPreservationPolicyBundleArchiveCoverageV1
} from "../../../smart-contracts/domains/preservation/StreamScopedPreservationPolicyBundleArchiveCoverageV1.sol";
import {
...
/// @dev Explicit complete preservation-inventory boundary; archive observations, original byte proofs and Safes are real.
/// No complete inventory materialization or finality acceptance is claimed by this component host.
contract ScopedPreservationBundleInventoryBoundaryV1 {
    address public core;
    ...
    function supportsInterface(bytes4 id) external pure returns (bool) {
        return id == 0x01ffc9a7 || id == type(InventoryInterface).interfaceId;
    }

    function scopedPreservationPolicyInventoryProfile() external view returns (bytes32) {
        return
            admittedProfile
                ? Scoped.PROFILE
...
contract ScopedPreservationBundleEnvironmentBoundaryV1 {
...
contract StreamScopedPreservationPolicyBundleArchiveCoverageV1Test is ScopedBundleArchiveFixture {
    StreamScopedPreservationPolicyBundleArchiveCoverageV1 private bundle;
    ScopedPreservationBundleInventoryBoundaryV1 private inventory;
    ScopedPreservationBundleEnvironmentBoundaryV1 private onchain;
    T.Item[] private rows;
    ...
    function setUp() public {
        ...
        onchain = new ScopedPreservationBundleEnvironmentBoundaryV1();
        inventory = new ScopedPreservationBundleInventoryBoundaryV1(
        ...
        bundle = new StreamScopedPreservationPolicyBundleArchiveCoverageV1(d);
    ...
    function testFullScopedCoveragePreservesActualOriginalBytesAndSeparateDomain() public {
        require(bundle.supportsInterface(type(BundleInterface).interfaceId));
        require(!bundle.supportsInterface(type(OriginalBundleInterface).interfaceId));
        ...
        require(
            saved
                == keccak256(
                    abi.encode(
                        keccak256(
                            "6529STREAM_SCOPED_PRESERVATION_POLICY_BUNDLE_ARCHIVE_COVERAGE_V1"
                        ),
                        block.chainid,
                        address(bundle),
    ...
    function testNewBundleRejectsOriginalPolicyProfileAndEvidenceDomainBeforeProgress() public {
        ...
        require(
            abi.encode(result).length == 288
                && bundle.scopedPreservationPolicyBundleArchiveProfile() == PB.PROFILE
        );
    }
}
