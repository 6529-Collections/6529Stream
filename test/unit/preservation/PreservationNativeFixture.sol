...
abstract contract PreservationNativeFixture is
    ContentRootPublicationFixture,
    EntropyTimeAuthorityFixture
{
    event log_named_uint(string key, uint256 value);
    PreservationSourceVm internal constant cheat =
        PreservationSourceVm(address(uint160(uint256(keccak256("hevm cheat code")))));
    ...
    PreservationSourceArchiveBoundary internal archive;
    address internal artifactTarget;
    ...
    function setUp() public virtual override {
        ...
        artifactTarget = _createArtifactCoverage();
        leaves = new StreamContentLeafManifest(
            address(core),
            address(checkpoint),
            artifactTarget,
            address(executor),
            _gas("CONTENT_LEAF_MANIFEST_READ_GAS", _leafReadGas(), 2)
        );
        provider =
    ...
    function _createArtifactCoverage() internal virtual returns (address) {
        ...
        return address(archive);
    }

    function _leafReadGas() internal pure virtual returns (uint256) {
        return 500000;
    }
    ...
    function _root() internal virtual {
    ...
    function _consumer() internal returns (PreservationSourceSnapshotConsumer) {
        return new PreservationSourceSnapshotConsumer(
