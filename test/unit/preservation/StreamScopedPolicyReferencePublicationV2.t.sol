...
import {
    StreamScopedPolicyPublicationReferenceDeploymentV2 as CapacityReferenceDeploy454
} from "../../../smart-contracts/domains/finality/StreamScopedPolicyPublicationReferenceDeploymentV2.sol";
import {
...
contract StreamScopedPolicyReferencePublicationV2Test is ScopedPolicyReferenceFixtureV2 {
    function testCapacityReferenceDeploymentKeepsCreatorArgumentsAndIndependentHistory() public {
        ...
        Ref child = Ref(CapacityReferenceDeploy454.deploy(r, g));
        require(
            address(child)
    ...
    function testCapacityReferenceOperationsRetainCallerEventAndAtomicRetry() public {
        ...
        RefT.Receipt memory expected;
        {
            ...
            require(
                payloadDomain == keccak256("6529STREAM_SCOPED_POLICY_REFERENCE_PAYLOAD_V2")
        ...
        // Ordered Metadata-read boundary forces the original post-retention grant check.
        // It models a late inconsistent read, not a state-changing callback from the fixed Store.
        {
            bytes memory query = abi.encodeWithSignature(
                "familyWriter(uint256,bytes32,uint8,address)",
                referenceInput.scope.collectionId,
                Families.CURATOR,
                uint8(3),
                recorder
            );
            bytes[] memory replies = new bytes[](2);
            replies[0] = abi.encode(true, uint64(3));
            replies[1] = abi.encode(true, uint64(4));
            ReferenceAuthoritySequenceVm454(address(vm))
                .mockCalls(address(metadata), query, replies);
            vm.expectRevert(
                abi.encodeWithSelector(RefT.ScopedPolicyReferenceAuthority.selector, recorder)
            );
            vm.prank(recorder);
            ...
            require(
                referenceHost.referenceCount(referenceInput.scope) == 0,
                "late grant mismatch rolls back append"
            );
            vm.expectRevert(
                abi.encodeWithSelector(RefT.ScopedPolicyReferenceUnknown.selector, literalRecord)
            );
            referenceHost.referenceRecord(literalRecord);
            replies = new bytes[](1);
            replies[0] = abi.encode(true, uint64(3));
            ReferenceAuthoritySequenceVm454(address(vm))
                .mockCalls(address(metadata), query, replies);
