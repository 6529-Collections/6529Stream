...
import {
    ArtistGuardianSupersessionFixture
} from "./ArtistGuardianSupersessionFixture.sol";
...
abstract contract ArtistRecoveredAuthorityFixture is ArtistGuardianSupersessionFixture {
    ...
    function _rhCompromise(bytes32 subject) internal {
    ...
    function _rhRecoverV2(bytes32 executed, bytes32 retained) internal {
    ...
    function _rhFreshGuardian(Successor memory next) internal returns (bytes32 record) {
    ...
    function _rhSourceHash() internal view returns (bytes32 hash) {
        for (uint8 i; i < 7; ++i) {
            hash = keccak256(
                abi.encode(
                    hash,
    ...
    function _rhTwoImports() internal returns (Successor memory last, bytes32 fresh) {
        _rhBaseline();
